...
class Harrison_Rent(scrapy.Spider):
    name = 'Harrison_Rent'
    prefix = 'https://www.harrisonagents.com.au'
    url = "https://www.harrisonagents.com.au/rent/properties-for-lease/"
    headers = {
    ...
    def parse(self, response):
        for property in response.css("a.listing-url"):
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'Harrison Agents'

            property_add = response.css('.full-address ::text').getall()  # full_adddress
            ...
            item['Field6'] = response.css(".bedroom .capsule ::text").get('').strip()
            item['Field7'] = response.css(".bathroom .capsule ::text").get('').strip()
            item['Field8'] = response.css(".carspace .capsule ::text").get('').strip()

            # sold_price = response.css(".price ::text").get('').strip()
