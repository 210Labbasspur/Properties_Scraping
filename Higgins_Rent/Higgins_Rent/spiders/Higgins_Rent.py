...
class Higgins_Rent(scrapy.Spider):
    name = "Higgins_Rent"
    url = ('https://higginsrealestatelismore.com.au/wp-admin/admin-ajax.php?action=get_posts&query%5Bpost_type%5D%5Bvalue%5D=listings&query%5Bcount%5D%5Bvalue%5D=20&query%5Borderby%5D%5Bvalue%5D=meta_value&query%5Bmeta_key%5D%5Bvalue%5D=dateListed&query%5Bsold%5D%5Bvalue%5D=0&query%5BsaleOrRental%5D%5Bvalue%5D=Rental&query%5BsaleOrRental%5D%5Btype%5D=equal&query%5Btype%5D%5Bvalue%5D=Residential&query%5Btype%5D%5Btype%5D=equal&query%5Border%5D%5Bvalue%5D=dateListed-desc&query%5Border%5D%5Btype%5D=equal&query'
           '%5Bpaged%5D%5Bvalue%5D={}')
    prefix = "https://higginsrealestatelismore.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Higgins_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(1655)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Higgins Real Estate Lismore'
            ...
            if response.css(".col-md-right .property-agent-details-wrapper .agent-mb"):
                ...
                if agent1_name:
                    ...
                    item['Field20'] = agent1.xpath(".//*[contains(@data-phonelabel,'agent_phone')]/a/text()").get('').strip().replace('mailto:','')
            ...
            if len(response.css(".col-md-right .property-agent-details-wrapper .agent-mb").getall()) > 1:
                ...
                if agent2:
                    ...
                    item['Field26'] = agent2.xpath(".//*[contains(@data-phonelabel,'agent_phone')]/a/text()").get('').strip().replace('mailto:','')
            ...
            if len(response.css(".col-md-right .property-agent-details-wrapper .agent-mb").getall()) > 2:
                ...
                if agent3:
                    ...
                    item['Field31'] = agent3.xpath(".//*[contains(@data-phonelabel,'agent_phone')]/a/text()").get('').strip().replace('mailto:','')
