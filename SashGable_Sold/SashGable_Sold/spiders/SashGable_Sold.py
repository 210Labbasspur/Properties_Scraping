...
class SashGable_Sold(scrapy.Spider):
    name = "SashGable_Sold"

    url = "https://www.sashandgable.com.au/our-sales?page=1"
    prefix = "https://www.sashandgable.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(SashGable_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".title a ::attr(href)").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://www.sashandgable.com.au{each_url}'
        ...
        for property_url in all_links:
            property_url = f'https://www.sashandgable.com.au{property_url}'
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3251)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Sash & Gable Property'

            property_street = response.css('.page-title ::text').get('').strip()  # property_street
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'flaticon-person1')]/parent::li[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'flaticon-shower5')]/parent::li[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'flaticon-car95')]/parent::li[1]/text()").get('').strip()

            price = response.css('.price ::text').get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.title strong ::text').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.xpath("//*[contains(@class,'title')]/following-sibling::p/text()").getall())  # description
            ...
            '''          AGENTS          '''
            if response.css(".widget .agent-card"):
                agent1 = response.css(".widget .agent-card")[0]
            ...
            if len(response.css(".widget .agent-card").getall()) > 1:
                agent2 = response.css(".widget .agent-card")[1]
            ...
            if len(response.css(".widget .agent-card").getall()) > 2:
                agent3 = response.css("widget .agent-card")[2]
            ...
            land_area = response.xpath("//*[contains(@class,'flaticon-display6')]/parent::li[1]/text()").get('').strip()
            if land_area:
            ...
            #
            feature_count = 58
            for feature in response.css("#tab-features li.active"):   ##  first 4 features are ID, bed, bath etc.
