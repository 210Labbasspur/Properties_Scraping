...
class TheBeaches_Rent(scrapy.Spider):
    name = "TheBeaches_Rent"
    url = 'https://www.thebeachesrealestate.com.au/rent/'
    api_url = 'https://www.thebeachesrealestate.com.au/wp-admin/admin-ajax.php'
    ...
    def __init__(self, *args, **kwargs):
        super(TheBeaches_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def start_requests(self):
        yield scrapy.Request(self.url, callback=self.parse, headers=self.headers)


    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".listing-item a ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            ...
            item['Field6'] = ''.join(e.strip() for e in response.xpath("//*[contains(@class,'room bed')]/text()").getall())
            item['Field7'] = ''.join(e.strip() for e in response.xpath("//*[contains(@class,'room bath')]/text()").getall())
            item['Field8'] = ''.join(e.strip() for e in response.xpath("//*[contains(@class,'room car')]/text()").getall())

            price = response.css('.property-price ::text').get('').strip().replace('For Sale asking','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price
            ...
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".property-desc ::text").getall())  # description
            ...
            land_area = response.xpath('//*[contains(@class,"property-land-size")]/text()').get('').strip().replace('Land size:','').replace('is','').replace('m²','')
            if land_area:
