...
class BrianMark_Sale(scrapy.Spider):
    name = "BrianMark_Sale"
    url = 'https://www.brianmark.com.au/wp-json/api/listings/all?priceRange=&category=&type=property&status=current&paged={}&limit=19&address=&bed=&bath=&car=&sort='
    prefix = "https://www.brianmark.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(BrianMark_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, property):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3013)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Brian Mark Real Estate'

            item['Field5'] = property.get('address',{}).get('fullAddress','')

            item['Field6'] = property.get('bedrooms','')
            item['Field7'] = property.get('bathrooms','')
            item['Field8'] = property.get('parkingSpaces','')

            item['Field9'] = property.get('price',{}).get('value','').replace('$','').replace('Sold','').replace(' ','')

            item['Field12'] = property.get('heading','')
            # item['Field12'] = f' '.join(e.strip() for e in response.css('.listing-item ::text').getall())

            '''         Uploading Images on Wasabi S3 Bucket            '''
            images = []
            images.append(property.get('propertyImage',{}).get('featured',''))
            for img in property.get('propertyImage',{}).get('list',[]):
            ...
            item['Field36'] = property.get('landDetails',{}).get('value','') if 'm' in property.get('landDetails',{}).get('unit','') else ''
            item['Field37'] = property.get('buildingDetails',{}).get('value','') if 'm' in property.get('buildingDetails',{}).get('unit','') else ''
