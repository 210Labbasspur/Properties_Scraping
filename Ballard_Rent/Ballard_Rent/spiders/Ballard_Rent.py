...
class Ballard_Rent(scrapy.Spider):
    name = 'Ballard_Rent'
    prefix = 'https://ballardproperty.com.au'
    url = "https://ballardproperty.com.au/properties-for-lease"
    headers = {
    ...
    def parse(self, response):
        for property in response.xpath("//*[contains(@class,'fourthirds-med')]/parent::a[1]"):
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'Ballard Property'

            property_add = response.xpath("//*[contains(@itemprop,'address')]/span/text()").getall()  # full_adddress
            ...
            # print('Bed Bath Car is : ', bed_bath_car)
            if response.xpath("//*[contains(@class,'icon-bed')]"):
                item['Field6'] = bed_bath_car[1].strip()
            if response.xpath("//*[contains(@class,'icon-bath')]"):
                item['Field7'] = bed_bath_car[2].strip()
            if response.xpath("//*[contains(@class,'icon-car')]"):
                item['Field8'] = bed_bath_car[3].strip()

            # sold_price = response.css('.muted ::text').get('').strip().replace('Sold','').replace(' ','')
            # if re.search(r'\d', sold_price):
            #     item['Field9'] = (sold_price.strip()).replace('SOLD','').replace('$','')   # sold_price

            description = response.css('.med-7 ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            # Agent_Imagess = response.xpath("//*[contains(@class,'popup-gallery slider-image')]/div/picture/source/img/@src").getall()
            # new_img_urls = []
