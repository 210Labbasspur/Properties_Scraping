...
class AmirPrestige_Rent(scrapy.Spider):
    name = 'AmirPrestige_Rent'
    prefix = 'https://www.amirprestige.com.au'
    url = "https://www.amirprestige.com.au/lease/residential/"
    headers = {
        'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        # 'cookie': '_ga=GA1.1.1303911921.1715928222; arp_scroll_position=633; _ga_6CV9CGYL8F=GS1.1.1715928221.1.1.1715928403.0.0.0',
        # 'priority': 'u=0, i',
        # 'referer': 'https://www.amirprestige.com.au/sold/residential/',
    ...
    def parse(self, response):
        for property in response.css("a.listing-info__link"):
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field6'] = ''.join(e.strip() for e in response.css("#quickinfo .bedroom ::text").getall())  # description
            item['Field7'] = ''.join(e.strip() for e in response.css("#quickinfo .bathroom ::text").getall())  # description
            item['Field8'] = ''.join(e.strip() for e in response.css("#quickinfo .carspace ::text").getall())  # description

            # sold_price = response.css('.price-container .price__label ::text').get('').strip().replace(' ', '')
            # if re.search(r'\d', sold_price):
            #     item['Field9'] = (sold_price.strip()).replace('$', '')  # sold_price

            description = response.css('.detail-description ::text').getall()  # description
            ...
            if response.css(".listing-agent"):
                ...
                if agent1:
                    ...
                    agent1_mobile = agent1.xpath(".//*[contains(@href,'tel:')]")[1]
                    item['Field21'] = agent1_mobile.css("::attr(href)").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                ...
                if len(response.css(".listing-agent").getall()) > 1:
                    ...
                    if agent2:
                        ...
                        agent2_mobile = agent2.xpath(".//*[contains(@href,'tel:')]")[1]
                        item['Field26A'] = agent2_mobile.css("::attr(href)").get('').strip().replace('tel:','')
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                ...
                if len(response.css(".listing-agent").getall()) > 2:
                    ...
                    if agent3:
                        ...
                        agent3_mobile = agent3.xpath(".//*[contains(@href,'tel:')]")[1]
                        item['Field31A'] = agent3_mobile.css("::attr(href)").get('').strip().replace('tel:','')
                        '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            elif response.xpath("//*[contains(@class,'building-size')]/text()").get('').strip():
                item['Field36'] = response.xpath("//*[contains(@class,'building-size')]/text()").get('').strip().replace('sqm', '').replace(' ', '')  # land_area
