...
class CreateVIC_Rent(scrapy.Spider):
    name = "CreateVIC_Rent"

    url = "https://createvic.com.au/for-rent/"
    prefix = "https://createvic.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(CreateVIC_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3035)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Create Real Estate'

            property_street = response.css('.elementor-widget-heading .elementor-heading-title ::text').get('').strip()  # property_street
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'e-font-icon-svg e-fas-bed')]/parent::span/parent::div/following-sibling::div//span/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'e-font-icon-svg e-fas-bath')]/parent::span/parent::div/following-sibling::div//span/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'e-font-icon-svg e-fas-car')]/parent::span/parent::div/following-sibling::div//span/text()").get('').strip()

            price = response.css('#listingprice span ::text').get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price
            ...
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".elementor-widget-container p ::text").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            images = []
            images_js = response.xpath("//*[contains(@id,'custom-slider-script-js-extra')]/text()").get('').strip().replace('/* <![CDATA[ */','').replace('};','}').replace('var sliderData = ','').replace('/* ]]> */','')
            if json.loads(images_js):
                data = json.loads(images_js)
            ...
            '''          AGENTS          '''
            if response.xpath("//*[contains(@class,'elementor-element elementor-element-ca16566 e-con-full e-flex e-con e-child')]"):
                agent1 = response.xpath("//*[contains(@class,'elementor-element elementor-element-ca16566 e-con-full e-flex e-con e-child')]")[0]
                agent1_name = agent1.css('h2.elementor-heading-title ::text').get('').strip()
                if agent1_name:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".agentprofile img ::attr(data-lazy-src)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.xpath("//*[contains(@class,'elementor-element elementor-element-ca16566 e-con-full e-flex e-con e-child')]").getall()) > 1:
                agent2 = response.xpath("//*[contains(@class,'elementor-element elementor-element-ca16566 e-con-full e-flex e-con e-child')]")[1]
                agent2_name = agent2.css('h2.elementor-heading-title ::text').get('').strip()
                if agent2:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".agentprofile img ::attr(data-lazy-src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.xpath("//*[contains(@class,'elementor-element elementor-element-ca16566 e-con-full e-flex e-con e-child')]").getall()) > 2:
                agent3 = response.xpath("//*[contains(@class,'elementor-element elementor-element-ca16566 e-con-full e-flex e-con e-child')]")[2]
                agent3_name = agent3.css('h2.elementor-heading-title ::text').get('').strip()
                if agent3:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".agentprofile img ::attr(data-lazy-src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
