...
class Clark_Sale(scrapy.Spider):
    name = 'Clark_Sale'
    prefix = 'https://clarkea.com.au'
    url = "https://clarkea.com.au/wp-json/api/listings/all?priceRange=&type=property&status=current&paged={}&limit=16&address=&bed=&bath=&car=&sort="
    headers = {
        'accept': '*/*',
        'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        # 'cookie': 'cf_chl_3=e0b926f5ed922e8; cf_chl_rc_m=24; cfz_facebook-pixel=%7B%22tbUA_fb-pixel%22%3A%7B%22v%22%3A%22fb.2.1714349853665.1794596625%22%2C%22e%22%3A1745885853665%7D%2C%22uldA_fb-pixel%22%3A%7B%22v%22%3A%22fb.2.1714349853665.140424342%22%2C%22e%22%3A1745885853665%7D%7D; _gid=GA1.3.2062530597.1714349858; arp_scroll_position=2200; _ga=GA1.1.1136355789.1714349851; _ga_RTMSCNZ7C0=GS1.1.1714352326.2.1.1714352329.0.0.0',
        'pagefrom': 'archive',
        'priority': 'u=1, i',
        'referer': 'https://clarkea.com.au/recent-sales/',
    ...
    def parse(self, response):
        ...
        for property in data['results']:
            ...
            mini_data['sold_price'] = property.get('propertyPricing').get('value').replace('Sold','').replace('$','')
            mini_data['property_type'] = property.get('propertyCategory')
            mini_data['land_area'] = property.get('landDetails').get('value')
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            description = response.css('.single-listing__content--wrapper ::text').getall()  # description
            ...
            '''          AGENTS          '''
            if response.css(".single-listing__agent .single-listing__agent-item"):
                agent1 = response.css(".single-listing__agent .single-listing__agent-item")[0]
                agent1_name = agent1.css('.single-listing__agent-name ::text').get('').strip()
                if agent1:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field18'] = agent1.css('.mb-1 ::text').get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images = agent1.css(".mb-0 ::attr(src)").get('').strip()
                ...
                if len(response.css(".single-listing__agent .single-listing__agent-item").getall()) > 1:
                    agent2 = response.css(".single-listing__agent .single-listing__agent-item")[1]
                    agent2_name = agent2.css('.single-listing__agent-name ::text').get('').strip()
                    if agent2:
                        item['Field23'] = agent2_name.strip()
                        item['Field24'] = agent2.css('.mb-1 ::text').get('').strip()
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent2.css(".mb-0 ::attr(src)").get('').strip()
                ...
                if len(response.css(".single-listing__agent .single-listing__agent-item").getall()) > 2:
                    agent3 = response.css(".single-listing__agent .single-listing__agent-item")[2]
                    agent3_name = agent3.css('.single-listing__agent-name ::text').get('').strip()
                    if agent3:
                        item['Field28'] = agent3_name.strip()
                        item['Field29'] = agent3.css('.mb-1 ::text').get('').strip()
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent3.css(".mb-0 ::attr(src)").get('').strip()
            ...
            feature_count = 58
            for feature in response.css(".single-listing__content--features ul li"):
