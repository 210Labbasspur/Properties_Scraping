...
class LouisCarr_Sold(scrapy.Spider):
    name = 'LouisCarr_Sold'
    prefix = 'https://www.louiscarr.com.au'
    url = "https://www.louiscarr.com.au/sold-properties?page=1"
    headers = {
    ...
    def parse(self, response):
        for property in response.css("a.propertyTile-address"):
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field6'] = response.xpath("//*[contains(text(),'bedroom')]/span[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(text(),'bathroom')]/span[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(text(),'parking')]/span[1]/text()").get('').strip()

            sold_price = response.css('.propertyPage-property-contracttext ::text').get('').strip().replace('Sold','').replace(' ','')
            if re.search(r'\d', sold_price):
                item['Field9'] = (sold_price.strip()).replace('Sold','').replace('$','')   # sold_price

            description = response.css('.user-content p ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess = response.xpath("//img[contains(@class,'block')]/@src").getall()
            new_img_urls = []
            ...
            '''          AGENTS          '''
            if response.css(".agentAvatar"):
                agent1 = response.css(".agentAvatar")[0]
                agent1_name = agent1.css('.agentAvatar-name ::text').get('').strip()
                if agent1_name:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field18'] = agent1.css(".agentAvatar-title ::text").get('').strip()  # agent_full_name_1
                ...
                if len(response.css(".agentAvatar").getall()) > 1:
                    agent2 = response.css(".agentAvatar")[1]
                    agent2_name = agent2.css('.agentAvatar-name ::text').get('').strip()
                    if agent2:
                        item['Field23'] = agent2_name.strip()
                        item['Field24'] = agent2.css(".agentAvatar-title ::text").get('').strip()  # agent_full_name_1
            ...
            land_area = response.xpath("//*[contains(text(),'land area')]/span[1]/text()").get('').strip()
            if land_area:
