...
class TheAgency_Rent(scrapy.Spider):
    name = 'TheAgency_Rent'
    prefix = 'https://theagency.com.au'
    url = "https://theagency.com.au/properties-for-lease"
    headers = {
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'The Agency'

            property_add = response.xpath("//*[contains(@property,'og:street-address')]/@content").getall()  # full_adddress
            ...
            if response.xpath("//*[contains(@class,'icon-bed')]"):
                item['Field6'] = bbc[1].strip()
            if response.xpath("//*[contains(@class,'icon-bath')]"):
                item['Field7'] = bbc[2].strip()
            if response.xpath("//*[contains(@class,'icon-car')]"):
                input_string = bbc[3].strip()
                numbers = ''.join([char for char in input_string if char.isdigit()])
                item['Field8'] = numbers

            # sold_price = response.xpath("//h4[contains(text(),'Sold')]/following-sibling::div[1]/text()").get('').strip().replace('Sold','').replace(' ','')
