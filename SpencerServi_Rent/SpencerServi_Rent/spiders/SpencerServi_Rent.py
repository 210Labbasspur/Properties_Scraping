...
class SpencerServi_Rent(scrapy.Spider):
    name = 'SpencerServi_Rent'
    prefix = 'https://www.spencerservi.com.au'
    url = "https://www.spencerservi.com.au/properties-for-lease"
    headers = {
    ...
    def parse(self, response):
        ...
        all_details_links = []
        for url in response.css(".searchResults a.block"):
        ...
        '''  #######   Original Code   #######  '''
        for property in response.css(".searchResults a.block"):
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'Spencer & Servi'

            property_add = response.css('.text-left span ::text').getall()  # full_adddress
            ...
            bbc = response.css('.text-right-sml ::text').getall()
            if response.xpath("//*[contains(@class,'icon-bed')]"):
                item['Field6'] = bbc[0].strip()
            if response.xpath("//*[contains(@class,'icon-bath')]"):
                item['Field7'] = bbc[1].strip()
            if response.xpath("//*[contains(@class,'icon-car')]"):
                input_string = bbc[2].strip()
                numbers = ''.join([char for char in input_string if char.isdigit()])
                item['Field8'] = numbers

            sold_price = response.css('.pricetext ::text').get('').strip().replace('$','').replace(' ','')
            ...
            if response.css(".agentTile"):
                ...
                if agent1_name:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images = self.prefix + agent1.css("img ::attr(src)").get('').strip()
                ...
                if len(response.css(".agentTile").getall()) > 1:
                    ...
                    if agent2:
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = self.prefix + agent2.css("img ::attr(src)").get('').strip()
                ...
                if len(response.css(".agentTile").getall()) > 2:
                    ...
                    if agent3:
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = self.prefix + agent3.css("img ::attr(src)").get('').strip()
