...
class Wilson_Sold(scrapy.Spider):
    name = "Wilson_Sold"
    url = 'https://www.wilsonagents.com.au/wp-admin/admin-ajax.php'
    prefix = "https://www.wilsonagents.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Wilson_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3188)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Wilson Agents'
            ...
            item['Field8'] = response.xpath("//i[contains(@class,'las la-car')]/following-sibling::span[1]/text()").get('').strip()
            ...
            prop_images = response.css(".embed-responsive-item .img-default-used-size-large ::attr(src)").getall()
            images = []
