...
class TracyYap_Rent(scrapy.Spider):
    name = 'TracyYap_Rent'
    prefix = 'https://tracyyap.com.au'
    url = "https://tracyyap.com.au/properties-for-lease"
    headers = {
    ...
    def parse(self, response):
        ...
        all_details_links = []
        for url in response.xpath("//a[contains(@ga-event-key,'propertyTile')]"):
        ...
        '''  #######   Original Code   #######  '''
        for property in response.xpath("//a[contains(@ga-event-key,'propertyTile')]"):
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'Tracy Yap Realty'

            property_add = response.css('h1.m0 ::text').getall()  # full_adddress
            ...
            bbc = response.css('.text-left-med .push-lrg-1 div ::text').getall()
            bbc_code = response.css('.text-left-med .push-lrg-1 div')
            if bbc_code.xpath(".//*[contains(@class,'icon-bed')]"):
                item['Field6'] = bbc[0].strip()
            if bbc_code.xpath(".//*[contains(@class,'icon-bath')]"):
                item['Field7'] = bbc[1].strip()
            if bbc_code.xpath(".//*[contains(@class,'icon-car')]"):
                input_string = bbc[2].strip()
                numbers = ''.join([char for char in input_string if char.isdigit()])
                item['Field8'] = numbers

            sold_price = response.css('#contentContainer .highlight .text-left-med .block ::text').get('').strip().replace('SOLD OFF-MARKET','').replace(' ','')
            if re.search(r'\d', sold_price):
                item['Field9'] = (sold_price.strip()).replace('SOLD','').replace('$','')   # sold_price

            description = response.css('#contentContainer p ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess = response.xpath("//*[contains(@property,'og:image')]/@content").getall()
            new_img_urls = []
            ...
            '''          AGENTS          '''
            if response.css(".agentRow"):
                agent1 = response.css(".agentRow")[0]
                agent1_name = agent1.css('.agentName .oneline ::text').get('').strip()
                if agent1_name:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images_style = agent1.css(".tcell a ::attr(style)").get('').strip()
                ...
                if len(response.css(".agentRow").getall()) > 1:
                    agent2 = response.css(".agentRow")[1]
                    agent2_name = agent2.css('.agentName .oneline ::text').get('').strip()
                    if agent2:
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images_style = agent2.css(".tcell a ::attr(style)").get('').strip()
                        Agent_Images = self.prefix + re.search(r'url\((.*?)\)', Agent_Images_style).group(1)
                ...
                if len(response.css(".agentRow").getall()) > 2:
                    agent3 = response.css(".agentRow")[2]
                    agent3_name = agent3.css('.agentName .oneline ::text').get('').strip()
                    if agent3:
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images_style = agent3.css(".tcell a ::attr(style)").get('').strip()
                        Agent_Images = self.prefix + re.search(r'url\((.*?)\)', Agent_Images_style).group(1)
