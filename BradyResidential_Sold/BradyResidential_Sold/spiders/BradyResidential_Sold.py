...
class BradyResidential_Sold(scrapy.Spider):
    name = "BradyResidential_Sold"

    url = "https://www.bradyresidential.com.au/buy/search-properties/?keywords=&list=sold&min_price=&max_price=&bedrooms=&bathrooms=&carspaces="
    prefix = "https://www.bradyresidential.com.au"
    headers = {
    ...
    count = 1
    db = 'BradyResidential'
    collection = 'BradyResidential_Sold'
    bucket_prefix = f'BR_{collection}'

    local_file_path = '/img'

    def __init__(self, *args, **kwargs):
        super(BradyResidential_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".img-container ::attr(href)").getall()
        links = []
        ...
        next_page = response.xpath("//*[contains(@class,'next page-numbers')]/@href").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2966)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Brady Residential'

            property_street = response.css('.xxxxxxxxxxxxxx ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.property-address-title ::text').getall() if e.strip())  # property_suburb
            property_address = f"{property_street} {property_suburb}"
            item['Field5'] = property_address  # full_adddress

            item['Field6'] = ''.join(e.strip() for e in response.css("#quickinfo li.bedroom ::text").getall())
            item['Field7'] = ''.join(e.strip() for e in response.css("#quickinfo li.bathroom ::text").getall())
            item['Field8'] = ''.join(e.strip() for e in response.css("#quickinfo li.carspace ::text").getall())

            price = response.css('#quickinfo .price__label ::text').get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('h5.sub-title ::text').getall())  # description
            ...
            prop_images = response.xpath("//*[contains(@class,'item-image')]/div/picture/source/img/@src").getall()
            images = []
            ...
            if response.css(".listing-agent"):
                ...
                if agent1_name:
                    ...
                    item['Field20'] = agent1.css(".agent-info-item-mobile a ::attr(href)").get('').strip().replace('tel:','')
                    item['Field21'] = agent1.css(".agent-info-item-phone a ::attr(href)").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css(".listing-agent").getall()) > 1:
                ...
                if agent2:
                    ...
                    item['Field26'] = agent2.css(".agent-info-item-mobile a ::attr(href)").get('').strip().replace('tel:','')
                    item['Field26A'] = agent2.css(".agent-info-item-phone a ::attr(href)").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css(".listing-agent").getall()) > 2:
                ...
                if agent3:
                    ...
                    item['Field31'] = agent3.css(".agent-info-item-mobile a ::attr(href)").get('').strip().replace('tel:','')
                    item['Field31A'] = agent3.css(".agent-info-item-phone a ::attr(href)").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            #
            # feature_count = 58
            # for feature in response.css(".col-sm-4"):   ##  first 4 features are ID, bed, bath etc.
