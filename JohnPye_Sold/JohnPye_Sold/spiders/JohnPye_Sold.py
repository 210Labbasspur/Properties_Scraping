...
class JohnPye_Sold(scrapy.Spider):
    name = 'JohnPye_Sold'
    prefix = 'https://www.johnpye.com.au'
    url = "https://www.johnpye.com.au/sell/recent-sales/"
    headers = {
        'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        'cache-control': 'max-age=0',
        # 'cookie': '_ga=GA1.3.471218687.1712940093; calltrk_referrer=direct; calltrk_landing=https%3A//www.johnpye.com.au/sell/recent-sales/; calltrk_session_id=c3d64e98-66cb-4c6b-af43-ff5cca37d28e; arp_scroll_position=200; _gid=GA1.3.1076674012.1713295349; _gat=1; _ga_T2NJ2WGT6J=GS1.3.1713295351.3.0.1713295351.0.0.0',
    ...
    def parse(self, response):
        for property in response.xpath("//*[contains(@class,'listing column')]"):
            property_url = property.css('.image a::attr(href)').get('').strip()
            print(self.count, property_url)
            self.count += 1
            sold_price = property.xpath(".//*[contains(text(),'SOLD')]/span[1]/text()").get('').strip()
            yield scrapy.Request(url=property_url, headers=self.headers, callback=self.Detail_parse, meta={'sold_price':sold_price})
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'JohnPye Your Property'

            property_add = response.css('.street_address .value::text , .suburb .value::text').getall()  # full_adddress
            ...
            item['Field6'] = response.css('.bedrooms ::text').get('').strip()
            item['Field7'] = response.css('.bathrooms ::text').get('').strip()
            item['Field8'] = response.css('.carspaces ::text').get('').strip()

            # sold_price = response.css(".price__label ::text").get('').strip().replace(' ','')
            # if sold_price:
            #     item['Field9'] = (sold_price.strip()).replace('$','')   # sold_price
            item['Field9'] = response.meta['sold_price'].replace('$','')
            ...
            Agent_Imagess = response.xpath("//*[contains(@class,'images cycle-slideshow')]/figure/div[1]/@data-bg").getall()
            new_img_urls = []
            ...
            if agent1:
                ...
                agent1_phone = ''.join(element.strip() for element in agent1.css(".name+ .contact ::text").getall())  # description
                item['Field20'] = agent1_phone.strip()  # description
                item['Field21'] = ''.join(element.strip() for element in agent1.css(".contact+ .contact ::text").getall())  # description
                '''         Uploading Images on Wasabi S3 Bucket            '''
                Agent_Images = agent1.css(".image img ::attr(data-lazy-src)").get('').strip()
            ...
            if len(response.css("div.agent").getall()) > 1:
                ...
                if agent2:
                    ...
                    agent2_phone = ''.join(element.strip() for element in agent2.css(".name+ .contact ::text").getall())  # description
                    item['Field26'] = agent2_phone.strip()  # description
                    item['Field26A'] = ''.join(element.strip() for element in agent2.css(".contact+ .contact ::text").getall())  # description
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images = agent2.css(".image img ::attr(data-lazy-src)").get('').strip()
            ...
            if len(response.css("div.agent").getall()) == 3:
                agent3 = response.css("div.agent")[2]
                agent3_name = agent3.css('.name strong ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    agent3_phone = ''.join(element.strip() for element in agent3.css(".name+ .contact ::text").getall())  # description
                    item['Field31'] = agent3_phone.strip()  # description
                    item['Field31A'] = ''.join(element.strip() for element in agent3.css(".contact+ .contact ::text").getall())  # description
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images = agent3.css(".image img ::attr(data-lazy-src)").get('').strip()
            ...
            if response.xpath("//*[contains(text(),'Land Size')]/following-sibling::span[1]/text()"):
            ...
            elif response.xpath("//*[contains(text(),'Building Size')]/following-sibling::span[1]/text()"):
