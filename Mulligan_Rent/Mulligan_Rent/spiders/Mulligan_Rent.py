...
class Mulligan_Rent(scrapy.Spider):
    name = 'Mulligan_Rent'
    prefix = 'https://www.mulliganproperty.com.au'
    url = ("https://www.mulliganproperty.com.au/data/results/?listing_sale_method=Rent&listing_status=lease&gclid=Cj0KCQjwiMmwBhDmARIsABeQ7xSHmz9TEjo6I5hwxwVWJoFJNviqbvX7q7ITv6IxGamgogIPeTTGOEgaAiiEEALw_wcB&"
           "status=lease&gad_source=1&gallery=1&pg={}")
    headers = {
        'accept': '*/*',
        'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        # 'cookie': '_WHEELS_AUTHENTICITY=ULaO7W4MM5kryr%2BzTPlA19xvlf3OSaJKJqEaCEFPHtJ5PyYCP9%2BGlKAULbUcW6zkyYeIdFABo3XtkBN9fjhjMiJxtuz6feKUiS%2FBDtTIzM%2FWqPrM6fwUft%2Fz5FfzOEuQluh4XmPzOTxTPsAv6cYLcA%3D%3D; FLASH=%7B%7D; arp_scroll_position=2245; FLASH=%7B%7D',
        'priority': 'u=0, i',
        # 'referer': 'https://www.mulliganproperty.com.au/sold?gad_source=1&gclid=Cj0KCQjwiMmwBhDmARIsABeQ7xSHmz9TEjo6I5hwxwVWJoFJNviqbvX7q7ITv6IxGamgogIPeTTGOEgaAiiEEALw_wcB',
    ...
    count = 1
    db = 'Mulligan'
    collection = 'Mulligan_Rent'
    bucket_prefix = f'D_{collection}'

    def start_requests(self):
        yield scrapy.Request(url='https://www.mulliganproperty.com.au/residential/lease', callback=self.parse, headers=self.headers)


    def parse(self, response):
        total_entries = int(response.css('.search-container em::text').get('').strip())
        page_no = 1
        entry = 0
    ...
    def parse_listing(self, response):
        ...
        for property in selector.css('a.image'):
            entry += 1
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'Mulligan Property'

            property_add = response.css('#listing-show-foxtrot h1 ::text').getall()  # full_adddress
            ...
            bed_bath_car = response.css('.description h3 ::text').get('').strip()
            item['Field6'] = re.search(r'(\d+)\s*Bed', bed_bath_car).group(1)
            item['Field7'] = re.search(r'(\d+)\s*Bath', bed_bath_car).group(1)
            item['Field8'] = re.search(r'(\d+)\s*Car', bed_bath_car).group(1)

            # sold_price = response.xpath("//*[contains(text(),'Sold for')]/text()").get('').strip()
            # if sold_price:
            #     item['Field9'] = (re.search(r'\$\d+(?:,\d+)*', sold_price).group()).replace('$','').replace(' ','')   # sold_price

            description = response.css('h4 ::text, .description .left ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess_style = response.css("#allImagesButton li ::attr(style)").getall()
            Agent_Imagess = []
            for style in Agent_Imagess_style:
                url_match = re.search(r'url\((.*?)\)', style)
                if url_match:
                    Agent_Imagess.append(url_match.group(1))
            new_img_urls = []
            ...
            '''          AGENTS          '''
            if response.css(".staff-member"):
                agent1 = response.css(".staff-member")[0]
                ...
                if agent1:
                    ...
                    agent1_phone = agent1.css(".name+ a ::text").get('').strip().replace("Call","")
                    item['Field20'] = agent1_phone.strip()
                    agent1_mobile = agent1.css(".name~ a+ a ::text").get('').strip().replace("Call","")
                    item['Field21'] = agent1_mobile.strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images = agent1.css(".staff-image img ::attr(src)").get('').strip()
                ...
                if len(response.css(".staff-member").getall()) > 1:
                    agent2 = response.css(".staff-member")[1]
                    agent2_name = agent2.css('.name ::text').get('').strip()
                    if agent2:
                        item['Field23'] = agent2_name.strip()
                        agent2_phone = agent2.css(".name+ a ::text").get('').strip().replace("Call", "")
                        item['Field26'] = agent2_phone.strip()
                        agent2_mobile = agent2.css(".name~ a+ a ::text").get('').strip().replace("Call", "")
                        item['Field26A'] = agent2_mobile.strip()
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent2.css(".staff-image img ::attr(src)").get('').strip()
                ...
                if len(response.css(".staff-member").getall()) == 3:
                    agent3 = response.css(".staff-member")[2]
                    agent3_name = agent3.css('.name ::text').get('').strip()
                    if agent3:
                        item['Field28'] = agent3_name.strip()
                        agent3_phone = agent3.css(".name+ a ::text").get('').strip().replace("Call", "")
                        item['Field31'] = agent3_phone.strip()
                        agent3_mobile = agent3.css(".name~ a+ a ::text").get('').strip().replace("Call", "")
                        item['Field31A'] = agent3_mobile.strip()
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent3.css(".staff-image img ::attr(src)").get('').strip()
