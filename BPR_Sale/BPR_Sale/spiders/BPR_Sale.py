...
class BPR_Sale(scrapy.Spider):
    name = "BPR_Sale"
    url = 'https://www.breakfastpointrealty.com.au/wp-admin/admin-ajax.php'
    prefix = "https://www.breakfastpointrealty.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(BPR_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2895)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Breakfast Point Realty'
            ...
            # item['Field8'] = property.get('garage', '').strip() if property.get('garage','') != '0' else property.get('carport', '')
            ...
            feature_count = 58
            for feature in response.css(".description-container .col-lg-4"):   ##  first 4 features are ID, bed, bath etc.
