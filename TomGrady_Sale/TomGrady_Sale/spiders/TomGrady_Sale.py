...
class TomGrady_Sale(scrapy.Spider):
    name = "TomGrady_Sale"

    url = "https://www.tomgrady.com.au/real-estate/residential-for-sale"
    prefix = "https://www.tomgrady.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(TomGrady_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".thumb-label-item-title a ::attr(href)").getall()
        links = []
        for each_url in all_links:
            each_url = f'https:{each_url}'
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2324)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Tom Grady Real Estate'

            property_street = response.css('.propertyAddress .textLight ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.propertyAddress .textBold ::text').getall() if e.strip())  # property_suburb
            ...
            price = response.css('.propertyBBC .textUppercase ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            ...
            '''          AGENTS          '''
            if response.css("#AgentSlider .item"):
                agent1 = response.css("#AgentSlider .item")[0]
                agent1_name = ' '.join(e.strip() for e in agent1.css('.staffName ::text').getall())
                if agent1_name:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field18'] = agent1.css('.staffPosition small ::text').get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".imgWrapper ::attr(style)").get('').strip().replace('overflow:hidden;','').replace('background-image: url(','').replace(');','').replace('background-size: cover;','').replace('background-repeat: no-repeat;','').replace('background-position:center top;','').replace(' ','').replace('\n','').replace('\t','').replace('\r','')
            ...
            if len(response.css("#AgentSlider .item").getall()) > 1:
                agent2 = response.css("#AgentSlider .item")[1]
                agent2_name = ' '.join(e.strip() for e in agent2.css('.staffName ::text').getall())
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.css('.staffPosition small ::text').get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".imgWrapper ::attr(style)").get('').strip().replace('overflow:hidden;','').replace('background-image: url(','').replace(');','').replace('background-size: cover;','').replace('background-repeat: no-repeat;','').replace('background-position:center top;','').replace(' ','').replace('\n','').replace('\t','').replace('\r','')
                    images_string = f'https:{agent_image}' if agent_image else ""
            ...
            if len(response.css("#AgentSlider .item").getall()) > 2:
                agent3 = response.css("#AgentSlider .item")[2]
                agent3_name = ' '.join(e.strip() for e in agent3.css('.staffName ::text').getall())
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field29'] = agent3.css('.staffPosition small ::text').get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".imgWrapper ::attr(style)").get('').strip().replace('overflow:hidden;','').replace('background-image: url(','').replace(');','').replace('background-size: cover;','').replace('background-repeat: no-repeat;','').replace('background-position:center top;','').replace(' ','').replace('\n','').replace('\t','').replace('\r','')
                    images_string = f'https:{agent_image}' if agent_image else ""
            ...
            item['Field35'] = response.xpath('//*[contains(text(),"Land Content")]/following-sibling::*[1]/text()').get('').strip()

            land_area = response.xpath('//*[contains(text(),"Land Size")]/following-sibling::*[1]/text()').get('').strip().replace('Land','').replace('is','').replace('m²','')
            if land_area:
            ...
            #
            feature_count = 58
            for feature in response.css(".showFeatureList")[4:]:   ##  first 4 features are ID, bed, bath etc.
                if 'condition' in feature.css('b::text').get('').strip().lower():
                    item['Field52'] = feature.css('b::text').get('').strip()
                else:
                    item[f'Field{feature_count}'] = ' '.join(e.strip() for e in feature.css('::text').getall())
                    feature_count += 1
