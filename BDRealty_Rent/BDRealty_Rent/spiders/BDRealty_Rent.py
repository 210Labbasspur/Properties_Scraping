...
class BDRealty_Rent(scrapy.Spider):
    name = "BDRealty_Rent"

    url = "https://www.bdrealty.com.au/for-lease/"
    prefix = "https://www.bdrealty.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(BDRealty_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    # def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".card--property ::attr(href)").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://www.bdrealty.com.au{each_url}'
        ...
        for property_url in all_links:
            property_url = f'https://www.bdrealty.com.au{property_url}'
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(9000)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'BD Realty'

            property_street = response.css('.xxxxxxxxxxx ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.property__inner h2 span ::text').getall() if e.strip())  # property_suburb
            property_address = f"{property_street} {property_suburb}"
            item['Field5'] = property_address  # full_adddress

            bbc = response.css('.property__inner .property__stats')
            item['Field6'] = bbc.xpath(".//*[contains(text(),' Bed')]/text()").get('').strip().replace('Bed','').replace(' ','')
            item['Field7'] = bbc.xpath(".//*[contains(text(),' Bath')]/text()").get('').strip().replace('Bath','').replace(' ','')
            item['Field8'] = bbc.xpath(".//*[contains(text(),' Car')]/text()").get('').strip().replace('Car','').replace(' ','')

            price = response.xpath("//*[contains(text(),'Price:')]/following-sibling::span[1]/text()").get('').strip().replace('Sold','').replace('$','').replace('for','').replace(',','').replace('Offers','').replace('Over','')
            if price:
                item['Field9'] = price.replace(' ','')
            ...
            '''          AGENTS          '''
            if response.css(".property__details-agents .card--property-agent"):
                agent1 = response.css(".property__details-agents .card--property-agent")[0]
                agent1_name = agent1.css('h4 span ::text').get('').strip()
                if agent1_name:
                    ...
                    item['Field20'] = agent1.css(".card__phone span ::text").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.xpath(".//*[contains(@class,'gatsby-image-wrapper ')]/img/@data-src").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'https://www.bdrealty.com.au/{agent_image}' if agent_image else ""
                    # item['Field22'] = images_string
                    # print('Agent1 image is :',images_string)
                    random_number = random.randint(1, 10000000000)
                    item['Field22'] = ",".join(self.uploaded(images_string, random_number))

            if len(response.css(".property__details-agents .card--property-agent").getall()) > 1:
                agent2 = response.css(".property__details-agents .card--property-agent")[1]
                agent2_name = agent2.css('h4 span ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field25'] = agent2.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:', '')
                    item['Field26'] = agent2.css(".card__phone span ::text").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.xpath(".//*[contains(@class,'gatsby-image-wrapper')]/img/@data-src").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'https://www.bdrealty.com.au/{agent_image}' if agent_image else ""
            ...
            if len(response.css(".property__details-agents .card--property-agent").getall()) > 2:
                agent3 = response.css(".property__details-agents .card--property-agent")[2]
                agent3_name = agent3.css('h4 span ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field30'] = agent3.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:', '')
                    item['Field31'] = agent3.css(".card__phone span ::text").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.xpath(".//*[contains(@class,'gatsby-image-wrapper ')]/img/@data-src").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'https://www.bdrealty.com.au/{agent_image}' if agent_image else ""
            ...
            land_area = response.xpath('//*[contains(text(),"Land Size")]/following-sibling::span/text()').get('').strip().replace('Land','').replace('is','').replace('m²','')
            if land_area:
