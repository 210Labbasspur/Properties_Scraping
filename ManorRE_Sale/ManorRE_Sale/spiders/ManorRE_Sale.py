...
class ManorRE_Sale(scrapy.Spider):
    name = 'ManorRE_Sale'
    prefix = 'https://www.manorrealestate.com.au'
    url = "https://www.manorrealestate.com.au/properties-for-sale"
    headers = {
    ...
    def parse(self, response):
        for property in response.css("a.propertyPhotoTile-anchor"):
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'Manor Real Estate'

            property_add = response.css('h2.responsive-margin-bot ::text').getall()  # full_adddress
            ...
            # bbc = response.xpath("//*[contains(text(),'Bath')]/text()").get('').strip()
            item['Field6'] = response.xpath("//span[contains(text(),'Bed')]/text()").get('').strip().replace('Bed','').replace('s','').replace('\xa0','').replace('/','')
            item['Field7'] = response.xpath("//span[contains(text(),'Bath')]/text()").get('').strip().replace('Bath','').replace('s','').replace('\xa0','').replace('/','')
            item['Field8'] = response.xpath("//span[contains(text(),'Car')]/text()").get('').strip().replace('Car','').replace('s','').replace('\xa0','').replace('/','')

            # sold_price = response.css('span.allcaps ::text').get('').strip().replace('Sold','').replace(' ','')
            # if re.search(r'\d', sold_price):
            #     item['Field9'] = (sold_price.strip()).replace(' ','').replace('$','')   # sold_price

            description = response.css('.row p ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess = response.xpath("//link[contains(@itemprop,'url')]/@href").getall()
            new_img_urls = []
