...
class ThePropertyCollective_Sale(scrapy.Spider):
    name = "ThePropertyCollective_Sale"

    url = "https://www.thepropertycollective.com.au/buy?12c45c01_page=1"
    prefix = "https://www.thepropertycollective.com.au"
    headers = {
    ...
    count = 1
    db = 'ThePropertyCollective'
    collection = 'ThePropertyCollective_Sale'
    ...
    def __init__(self, *args, **kwargs):
        super(ThePropertyCollective_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".propertylistings a ::attr(href)").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://www.thepropertycollective.com.au{each_url}'
        ...
        for property_url in all_links:
            property_url = f'https://www.thepropertycollective.com.au{property_url}'
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3256)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'The Property Collective'

            property_street = response.css('.property-address ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('xxxxxxxxxxxxxxxxx ::text').getall() if e.strip())  # property_suburb
            property_address = f"{property_street} {property_suburb}"
            item['Field5'] = property_address  # full_adddress

            item['Field6'] = response.xpath("//*[contains(@alt,'Bed icon charcoal')]/preceding-sibling::div[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@alt,'Bath icon charcoal')]/preceding-sibling::div[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@alt,'Car icon charcoal')]/preceding-sibling::div[1]/text()").get('').strip()

            price = response.css('.property-price ::text').get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price
            ...
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".listing-wrapper .property-description ::text").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.css(".collective-auction-wrapper .w-dyn-repeater-item ::attr(style)").getall()
            images = []
            for img in prop_images:
                img = img.replace('background-image:url(','').replace("&quot;","").replace(")","")
            ...
            '''          AGENTS          '''
            if response.xpath("//*[@class='listing-wrapper']//*[@class='team-member']"):
                agent1 = response.xpath("//*[@class='listing-wrapper']//*[@class='team-member']")[0]
                agent1_name = agent1.css('.agent-name-copy ::text').get('').strip()
                if agent1_name:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".team-member img ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.xpath("//*[@class='listing-wrapper']//*[@class='team-member']").getall()) > 1:
                agent2 = response.xpath("//*[@class='listing-wrapper']//*[@class='team-member']")[0]
                agent2_name = agent2.css('.agent-name-copy ::text').get('').strip()
                if agent2:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".team-member img ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.xpath("//*[@class='listing-wrapper']//*[@class='team-member']").getall()) > 2:
                agent3 = response.xpath("//*[@class='listing-wrapper']//*[@class='team-member']")[2]
                agent3_name = agent3.css('.agent-name-copy ::text').get('').strip()
                if agent3:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".team-member img ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
