...
class Marando_Sale(scrapy.Spider):
    name = 'Marando_Sale'
    prefix = 'http://www.marandorealestatesw.com.au'
    url = "https://www.marandorealestatesw.com.au/page/forsalelistings"
    headers = {
        'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'Accept-Language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        'Cache-Control': 'max-age=0',
        'Connection': 'keep-alive',
        # 'Cookie': 'arp_scroll_position=235',
        # 'Referer': 'https://www.marandorealestatesw.com.au/page/recent-local-sales',
    ...
    def parse(self, response):
        for property in response.xpath("//*[contains(@class,'address')]"):#[:2]:
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            property_address = response.css('.propertyContainer h2 ::text').get('').strip()   # full_adddress
            item['Field5'] = property_address.strip()   # full_adddress
            
            item['Field6'] = response.xpath("//*[contains(@alt,'Bedrooms')]/preceding-sibling::h6[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@alt,'Bathrooms')]/preceding-sibling::h6[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@alt,'Car Spaces')]/preceding-sibling::h6[1]/text()").get('').strip()
            ...
            description = response.css('.property_description p:nth-child(3) ::text').getall()   # description
            item['Field12'] = ' '.join(element.strip().replace('\t',' ') for element in description)   # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            new_img_urls = response.css('#gallerychange img ::attr(src)').getall()
            ...
            agent1 = response.css(".agent_details:nth-child(3)")
            agent1_name = agent1.css('span ::text').get('').strip()
            if agent1_name:
                first_name1, last_name1 = agent1_name.split(maxsplit=1)
                item['Field15'] = first_name1.strip()   # agent_first_name_1
                item['Field16'] = last_name1.strip()   # agent_surname_name_1
                item['Field17'] = agent1_name.strip()   # agent_full_name_1
                agent_details = agent1.css('::text').getall()
                item['Field20'] = agent_details[-2].strip()
                '''         Uploading Images on Wasabi S3 Bucket            '''
                Agent_Imagess = response.xpath("//*[contains(@class,'agent_details')][1]/preceding-sibling::img[1]/@src").getall()
                Agent_Imagesss = []
                ...
                agent1_id = agent_details[-1].strip().replace(' ','')
                id = agent_details[-2].strip().replace(' ','')
            ...
            agent2 = response.css(".agent_details~ .agent_details")
            agent2_name = agent2.css('span ::text').get('').strip()
            if agent2:
                item['Field23'] = agent2.css('span ::text').get('').strip()
                agent_details = agent2.css('::text').getall()
                item['Field26'] = agent_details[-2].strip()
                '''         Uploading Images on Wasabi S3 Bucket            '''
                Agent_Imagess = response.xpath("//*[contains(@class,'agent_details')][2]/preceding-sibling::img[1]/@src").getall()
                Agent_Imagesss = []
                for url in Agent_Imagess:
                    Agent_Imagesss.append(self.prefix + url)
                Agent_Images = ','.join(Agent_Imagesss[:1])
            ...
            item['Field33'] = response.xpath("//*[contains(text(),'Property ID:')]/text()").get('').strip().replace('Property ID:','')   # external_property_id
