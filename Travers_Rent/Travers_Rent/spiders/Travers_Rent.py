...
class Travers_Rent(scrapy.Spider):
    name = 'Travers_Rent'
    prefix = 'https://www.traversgray.com.au'
    url = "https://www.traversgray.com.au/for-lease"
    headers = {
        'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'Accept-Language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        'Cache-Control': 'max-age=0',
        'Connection': 'keep-alive',
        # 'Cookie': 'PHPSESSID=jiltt9as04nj2ql9hdht8h54j1',
    ...
    def parse(self, response):
        for property in response.css(".propertyListItemWrapper a"):
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'Travers Gray Real Estate'

            property_add = response.css('.addressHeading ::text , .suburbHeading span ::text').getall()  # full_adddress
            ...
            item['Field6'] = response.css("article:nth-child(1) h5 ::text").get('').strip()
            item['Field7'] = response.css("article:nth-child(2) h5 ::text").get('').strip()
            item['Field8'] = response.css("article:nth-child(3) h5 ::text").get('').strip()

            # sold_price = response.css('.leftContentWrapper h2 ::text').get('').strip().replace('Sold for','').replace(' ','')
            # if sold_price:
            ...
            Agent_Imagess = response.css(".fancybox img ::attr(src)").getall()
            new_img_urls = []
            ...
            '''          AGENTS          '''
            if response.css("#agentSlider .carousel-inner div.item"):
                agent1 = response.css("#agentSlider .carousel-inner div.item")[0]
                ...
                if agent1:
                    ...
                    agent1_phone = agent1.css(".infoWrapper a:nth-child(2) h4 ::text").get('').strip()
                    item['Field20'] = agent1_phone.strip()
                    agent1_mobile = agent1.css(".infoWrapper a:nth-child(3) h4::text").get('').strip()
                    item['Field21'] = agent1_mobile.strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images = 'https:' + agent1.css(".imgWrapper img ::attr(src)").get('').strip()
                ...
                if len(response.css("#agentSlider .carousel-inner div.item").getall()) > 1:
                    agent2 = response.css("#agentSlider .carousel-inner div.item")[1]
                    agent2_name = agent2.css('h4 span ::text').get('').strip()
                    if agent2:
                        item['Field23'] = agent2_name.strip()
                        agent2_phone = agent2.css(".infoWrapper a:nth-child(2) h4 ::text").get('').strip()
                        item['Field26'] = agent2_phone.strip()
                        agent2_mobile = agent2.css(".infoWrapper a:nth-child(3) h4::text").get('').strip()
                        item['Field26A'] = agent2_mobile.strip()
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = 'https:' + agent2.css(".imgWrapper img ::attr(src)").get('').strip()
                ...
                if len(response.css("#agentSlider .carousel-inner div.item").getall()) == 3:
                    agent3 = response.css("#agentSlider .carousel-inner div.item")[2]
                    agent3_name = agent3.css('h4 span ::text').get('').strip()
                    if agent3:
                        item['Field28'] = agent3_name.strip()
                        agent3_phone = agent3.css(".infoWrapper a:nth-child(2) h4 ::text").get('').strip()
                        item['Field31'] = agent3_phone.strip()
                        agent3_mobile = agent3.css(".infoWrapper a:nth-child(3) h4::text").get('').strip()
                        item['Field31A'] = agent3_mobile.strip()
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = 'https:' + agent3.css(".imgWrapper img ::attr(src)").get('').strip()
            ...
            item['Field33'] = response.xpath("//*[contains(text(),'Property ID')]/parent::section[1]/parent::span[1]/following-sibling::span[1]/section[1]/text()").get('').strip().replace('Property ID:','')
            # item['Field35'] = response.xpath("//*[contains(text(),'Land Size')]/parent::section[1]/parent::span[1]/following-sibling::span[1]/section[1]/text()").get('').strip().replace('Property ID:','')
            land_area = response.xpath("//*[contains(text(),'Land Size')]/parent::section[1]/parent::span[1]/following-sibling::span[1]/section[1]/text()").get('').strip().replace('Property ID:','')
            if land_area:
