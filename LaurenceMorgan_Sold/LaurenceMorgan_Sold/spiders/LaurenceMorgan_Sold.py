...
class LaurenceMorgan_Sold(scrapy.Spider):
    name = "LaurenceMorgan_Sold"

    url = 'https://www.laurencemorgan.com.au/wp-admin/admin-ajax.php'
    ...
    data = {
        ...
        'slf_suburb': '',
        'slf_maxprice': '0',
        'slf_rescom': 'all_property_category',
        'slf_propertytype': '',
        'slf_agent': '',
        'slf_sort': 'sold-date-desc',
        'slf_paged': '2',
        'slf_property_status': 'sold',
        'slf_loadmore': 'true',
    }
    count = 1
    db = 'LaurenceMorgan'
    collection = 'LaurenceMorgan_Sold'
    ...
    def __init__(self, *args, **kwargs):
        super(LaurenceMorgan_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def start_requests(self):
        ...
        payload['slf_paged'] = str(page_no)
        yield scrapy.FormRequest(self.url, method='POST',formdata=payload, headers=self.headers, callback=self.parse,
                             meta={'page_no':page_no})

    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.xpath("//*[@class='property']/@data-url").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2948)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Laurence Morgan'

            property_street = response.xpath("//*[contains(@class,'xxxxxxxxxxx')]/text()").get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.xpath("//*[contains(@class,'et_pb_column et_pb_column_3_4 et_pb_column_2_tb_body')]/div/div/h1/text()").getall() if e.strip())  # property_suburb
            property_address = f"{property_street} {property_suburb}"
            item['Field5'] = property_address  # full_adddress

            item['Field6'] = response.css(".bedroom h2 ::text").get('').strip()
            item['Field7'] = response.css(".bathroom h2 ::text").get('').strip()
            item['Field8'] = response.css(".parking h2 ::text").get('').strip()

            price = response.xpath("//*[contains(text(),' Price')]/parent::div/text()").get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','').replace('for','')
            if price:
                item['Field9'] = price

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.xxxxxxx ::text').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".et_pb_text_28_tb_body p ::text").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.xpath("//*[contains(@alt,'Gallery Image')]/@src").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.css(".author-block"):
                agent1 = response.css(".author-block")[0]
                agent1_name = agent1.css('.author-details p ::text').get('').strip()
                if agent1_name:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field18'] = agent1.css('.xxxxxxxxxx ::text').get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".author-image img ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.css(".author-block").getall()) > 1:
                agent2 = response.css(".author-block")[1]
                agent2_name = agent2.css('.author-details p ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.css('.xxxxxxxxxx ::text').get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".author-image img ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css(".author-block").getall()) > 2:
                agent3 = response.css(".author-block")[2]
                agent3_name = agent3.css('.author-details p ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field29'] = agent3.css('.xxxxxxxxxx ::text').get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".author-image img ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            item['Field35'] = response.xpath("//*[contains(text(),'Property Type')]/parent::div[1]/text()").get('').strip()
