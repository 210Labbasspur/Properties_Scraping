...
class ImageProperty_Rent(scrapy.Spider):
    name = 'ImageProperty_Rent'
    prefix = 'https://imageproperty.com.au'
    url = "https://imageproperty.com.au/homes-for-rent/"
    headers = {
    ...
    def parse(self, response):
        ...
        all_details_links = []
        for url in response.css(".card-text a"):
        ...
        '''  #######   Original Code   #######  '''
        for property in response.css(".card-text a"):
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'Image Property'

            property_add = response.css('.item-street ::text, .entry-title-sub ::text').getall()  # full_adddress
            ...
            item['Field6'] = ''.join(e.strip() for e in response.css("#property_bedrooms span ::text").getall())
            item['Field7'] = ''.join(e.strip() for e in response.css("#property_bathrooms span ::text").getall())
            item['Field8'] = ''.join(e.strip() for e in response.css("#property_parking span ::text").getall())

            sold_price = response.xpath("//*[contains(@class,'text-primary d-inline-block mt-1 font-weight-light')]/text()").get('').strip().replace('Sold','').replace(' ','')
            if re.search(r'\d', sold_price):
                item['Field9'] = (sold_price.strip()).replace('SOLD','').replace('$','')   # sold_price

            description = response.css('.property-description-readmore p ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess = response.xpath("//*[contains(@class,'img-link open-gallery')]/@href").getall()
            new_img_urls = []
            ...
            '''          AGENTS          '''
            if response.css(".agent-property-card"):
                agent1 = response.css(".agent-property-card")[0]
                agent1_name = agent1.css('.text-uppercase ::text').get('').strip()
                if agent1_name:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field18'] = agent1.css('.small ::text').get('').strip()  # agent_full_name_1
                    agent = response.css('.event-body')
                    agent1_phone = agent.xpath(".//a[contains(@href,'tel:')]/@href").get('').strip().replace('tel:','')
                    item['Field20'] = agent1_phone.strip()
                    phone_nos =  agent.xpath(".//a[contains(@href,'tel:')]/@href").getall()
                    if len(phone_nos) > 1:
                ...
                if len(response.css(".agent-property-card").getall()) > 1:
                    agent2 = response.css(".agent-property-card")[1]
                    agent2_name = agent2.css('.text-uppercase ::text').get('').strip()
                    if agent2:
                        item['Field23'] = agent2_name.strip()
                        item['Field24'] = agent2.css('.small ::text').get('').strip()  # agent_full_name_1
                ...
                if len(response.css(".agent-property-card").getall()) > 2:
                    agent3 = response.css(".agent-property-card")[2]
                    agent3_name = agent3.css('.text-uppercase ::text').get('').strip()
                    if agent3:
                        item['Field28'] = agent3_name.strip()
                        item['Field29'] = agent3.css('.small ::text').get('').strip()  # agent_full_name_1
            ...
            item['Field35'] = response.css("#property_catagory-icon span ::text").get('').strip()
            land_area = response.css("#property_land_area span ::text").get('').strip()
            if land_area:
