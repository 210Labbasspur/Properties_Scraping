...
class FreeMans_Sold(scrapy.Spider):
    name = "FreeMans_Sold"

    url = "https://www.freemanestates.com.au/sold-listings/"
    prefix = "https://www.freemanestates.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(FreeMans_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        links = []
        for each_url in response.css(".listing-bottom"):
        ...
        for property_url in response.css(".listing-bottom"):
            price = property_url.css(".listing-price-value ::text").get('').strip().replace('Offers Over','')
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2073)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Free Mans'

            property_street = response.css('.xxxxxxxxxxxxxx ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.address ::text').getall() if e.strip())  # property_suburb
            ...
            item['Field8'] = response.xpath("//*[contains(text(),'Car Park:')]/following-sibling::*[1]/text()").get('').strip()

            price = response.meta['price'].strip().replace('Sold','').replace('$','').replace(' ','').replace(',','')
            if price:
            ...
            '''          AGENTS          '''
            if response.css("div.wpsight-listing-agent"):
                agent1 = response.css("div.wpsight-listing-agent")[0]
                ...
                if agent1_name:
                    ...
                    item['Field20'] = agent1.xpath(".//*[contains(text(),'P.')]/text()").get('').strip().replace('P.','')
                    item['Field21'] = agent1.xpath(".//*[contains(text(),'M.')]/text()").get('').strip().replace('M.','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css("div.wpsight-listing-agent").getall()) > 1:
                agent2 = response.css("div.wpsight-listing-agent")[1]
                ...
                if agent2:
                    ...
                    item['Field26'] = agent2.xpath(".//*[contains(text(),'P.')]/text()").get('').strip().replace('P.','')
                    item['Field26A'] = agent2.xpath(".//*[contains(text(),'M.')]/text()").get('').strip().replace('M.','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css("div.wpsight-listing-agent").getall()) > 2:
                agent3 = response.css("div.wpsight-listing-agent")[2]
                ...
                if agent3:
                    ...
                    item['Field31'] = agent3.xpath(".//*[contains(text(),'P.')]/text()").get('').strip().replace('P.','')
                    item['Field31A'] = agent3.xpath(".//*[contains(text(),'M.')]/text()").get('').strip().replace('M.','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
