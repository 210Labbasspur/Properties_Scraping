...
class Villager_Sale(scrapy.Spider):
    name = 'Villager_Sale'
    prefix = 'https://villagerproperty.com.au'
    url = 'https://villagerproperty.com.au/buy/'
    headers = {
    ...
    count = 1
    db = 'Villager'
    collection = 'Villager_Sale'
    bucket_prefix = f'D_{collection}'

    def start_requests(self):
        yield scrapy.FormRequest(url=self.url, callback=self.parse, headers=self.headers)

    def parse(self, response):
        for property in response.css('.property-grid-image a'):
        ...
        if next_page:
            yield scrapy.FormRequest(url=next_page, callback=self.parse, headers=self.headers)
