...
class Creer_Sale(scrapy.Spider):
    name = 'Creer_Sale'
    prefix = 'https://creer.com.au'
    url = "https://creer.com.au/buy?p.2.offset={}&_data=catchall-route"
    headers = {
        'accept': '*/*',
        ...
        for check_property in data['pageData']['body']:
            if check_property['component'] == "wsPropertyListings":
                for property in check_property['data']['properties']['collection']:
                    prop_no += 1
                    property_url = 'https://creer.com.au/buy/' + property.get('uuid')
    ...
    def Detail_parse(self, mini_data, full_data):
        DB_already_exists = self.read_data_base('https://creer.com.au/buy/' + full_data.get('uuid'))
        if not DB_already_exists:
            ...
            item['Field14'] = 'https://creer.com.au/buy/' + full_data.get('uuid')  # external_link
            '''          AGENTS          '''
