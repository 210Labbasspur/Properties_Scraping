...
class Grants_Sold(scrapy.Spider):
    name = "Grants_Sold"

    url = "https://www.grantsea.com.au/page/1/?action=epl_search&post_type=property&property_status=sold#038;post_type=property&property_status=sold"
    prefix = "https://www.grantsea.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Grants_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3161)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = "Grant's"

            property_street = response.css('.entry-title ::text').get('').strip().replace('\t','').replace('\r','').replace('\n','').replace('  ','')  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.xxxxxxxxx ::text').getall() if e.strip())  # property_suburb
            property_address = f"{property_street} {property_suburb}".replace('  ',' ')
            item['Field5'] = property_address  # full_adddress

            item['Field6'] = response.css(".beds span ::text").get('').strip()
            item['Field7'] = response.css(".bath span ::text").get('').strip()
            item['Field8'] = response.css(".parking span ::text").get('').strip()
            ...
            '''          AGENTS          '''
            if response.xpath("//*[contains(text(),'Agent Details')]/following-sibling::div[1]/ul"):
                agent1 = response.xpath("//*[contains(text(),'Agent Details')]/following-sibling::div[1]/ul")[0]
                agent1_name = agent1.xpath(".//*[contains(text(),'Name:')]/parent::*/text()").get('').strip()
                if agent1_name:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field19'] =  agent1.xpath("//*[contains(@type,'email')]/@value").get('').strip().replace('mailto:','')
                    item['Field20'] = agent1.xpath(".//*[contains(text(),'Mobile:')]/parent::*/text()").get('').strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    # agent_image = agent1.css(".xxxxxxxxxxxxxxx ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.xpath("//*[contains(text(),'Agent Details')]/following-sibling::div[1]/ul").getall()) > 1:
                agent2 = response.xpath("//*[contains(text(),'Agent Details')]/following-sibling::div[1]/ul")[1]
                agent2_name = agent2.xpath(".//*[contains(text(),'Name:')]/parent::*/text()").get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field25'] = agent2.xpath("//*[contains(@type,'email')]/@value").get('').strip().replace('mailto:','')
                    item['Field26'] = agent2.xpath(".//*[contains(text(),'Mobile:')]/parent::*/text()").get('').strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    # agent_image = agent2.css(".agentTile-image ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    # images_string = f'https://pacepa.com.au{agent_image}' if agent_image else ""
                    # # item['Field27'] = images_string
                    # # print('Agent2 image is :',images_string)
                    # random_number = random.randint(1, 10000000000)
                    # item['Field27'] = ",".join(self.uploaded(images_string, random_number))

            if len(response.xpath("//*[contains(text(),'Agent Details')]/following-sibling::div[1]/ul").getall()) > 2:
                agent3 = response.xpath("//*[contains(text(),'Agent Details')]/following-sibling::div[1]/ul")[2]
                agent3_name = agent3.xpath(".//*[contains(text(),'Name:')]/parent::*/text()").get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field30'] = agent3.xpath("//*[contains(@type,'email')]/@value").get('').strip().replace('mailto:','')
                    item['Field31'] = agent3.xpath(".//*[contains(text(),'Mobile:')]/parent::*/text()").get('').strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    # agent_image = agent3.css(".agentTile-image ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    # images_string = f'https://pacepa.com.au{agent_image}' if agent_image else ""
            ...
            item['Field33'] = response.xpath("//*[contains(text(),'Property ID:')]/text()").get('').strip().replace('Property ID','').replace(':','')
            ...
            land_area = response.xpath("//*[contains(text(),'Land Size:')]/text()").get('').strip().replace('Land Size','').replace(':','')
            if land_area:
                if 'Acres' in land_area:
                    pass
                else:
                    item['Field36'] = land_area.replace('squareMeter','').replace(' ','')   # land_area
            floor_area = response.xpath("//*[contains(text(),'Build Size:')]/text()").get('').strip().replace('Land Size','').replace(':','')
            if floor_area:
