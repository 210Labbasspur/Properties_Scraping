...
class OnePercent_Rent(scrapy.Spider):
    name = "OnePercent_Rent"

    url = "https://onepercent.com.au/our-rentals/"
    prefix = "https://onepercent.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(OnePercent_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2201)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'One Percent Property Sales'

            property_street = response.css('.xxxxxxxxxxxxxx ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('h3.address ::text').getall() if e.strip())  # property_suburb
            ...
            price = response.css('.listing-price-value ::text').get('').strip().replace('Sold','').replace('$','').replace('Offers over','').replace(',','')
            ...
            '''          AGENTS          '''
            if response.css(".ctm_agent_card"):
                agent1 = response.css(".ctm_agent_card")[0]
                agent1_name = agent1.css('.teal-highlight ::text').get('').strip()
                if agent1_name:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".cutout ::attr(style)").get('').strip().replace("background-image:url('","").replace("');","")
            ...
            if len(response.css(".ctm_agent_card").getall()) > 1:
                agent2 = response.css(".ctm_agent_card")[1]
                agent2_name = agent2.css('.teal-highlight ::text').get('').strip()
                if agent2:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".cutout ::attr(style)").get('').strip().replace("background-image:url('","").replace("');","")
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css(".ctm_agent_card").getall()) > 2:
                agent3 = response.css(".ctm_agent_card")[2]
                agent3_name = agent3.css('.teal-highlight ::text').get('').strip()
                if agent3:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".cutout ::attr(style)").get('').strip().replace("background-image:url('","").replace("');","")
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            # item['Field35'] = response.css('.land-size ::text').get('').strip()
            ...
            feature_count = 58
            for feature in response.css(".listing-terms-blocks a"):   ##  first 4 features are ID, bed, bath etc.
