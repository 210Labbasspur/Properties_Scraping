...
class Taylor_Sold(scrapy.Spider):
    name = 'Taylor_Sold'
    prefix = 'https://taylorhedleyproperty.com'
    url = "https://taylorhedleyproperty.com/sold/"
    headers = {
        'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        'cache-control': 'max-age=0',
        # 'cookie': '_gcl_au=1.1.1128300193.1713706592; _ga_DFTXC8T89Z=GS1.1.1713874663.2.0.1713874663.60.0.0; _ga=GA1.2.1796878486.1713706592; _gid=GA1.2.806598656.1713874664; _gat_UA-237576665-1=1; arp_scroll_position=400',
        'priority': 'u=0, i',
    ...
    def parse(self, response):
        for property in response.css(".listing-bottom a"):
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'Taylor Hedley'

            property_add = response.css('h3.address ::text').getall()  # full_adddress
            ...
            item['Field6'] = response.xpath("//*[contains(text(),'Beds:')]/following-sibling::span[1]/text()").get('').strip()  # description
            item['Field7'] = response.xpath("//*[contains(text(),'Baths:')]/following-sibling::span[1]/text()").get('').strip()  # description
            item['Field8'] = response.xpath("//*[contains(text(),'Car Park:')]/following-sibling::span[1]/text()").get('').strip()  # description

            sold_price = response.css(".listing-price-value ::text").get('').strip().replace(' ', '')
            if sold_price:
                item['Field9'] = (sold_price.strip()).replace('$', '')  # sold_price

            description = response.css('#wpsight_elviria_listing_description-3 p ::text').getall()  # description
            item['Field12'] = ' '.join(
                element.strip().replace('\t', ' ').replace('Read More', '') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess = response.css("#wpsight_elviria_listing_image_slider-2 .image img ::attr(src)").getall()
            new_img_urls = []
            ...
            if response.xpath("//*[contains(@class,'wpsight-listing-agent clearfix')]"):
                ...
                agent1_name = agent1.xpath(".//*[contains(@itemprop,'name')]/text()").get('').strip()
                if agent1:
                    if agent1_name:
                    ...
                    agent1_phone = agent1.xpath(".//*[contains(text(),'M.')]/text()").get('').strip().replace('M.','')  # description
                    item['Field20'] = agent1_phone.strip()  # description
                    item['Field21'] = agent1.xpath(".//*[contains(text(),'P.')]/text()").get('').strip().replace('P.','')  # description
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                ...
                if len(response.xpath("//*[contains(@class,'wpsight-listing-agent clearfix')]").getall()) > 1:
                    ...
                    agent2_name = agent2.xpath(".//*[contains(@itemprop,'name')]/text()").get('').strip()
                    if agent2:
                        if agent1_name:
                            item['Field23'] = agent2_name.strip()
                        agent2_phone = agent2.xpath(".//*[contains(text(),'M.')]/text()").get('').strip().replace('M.','')  # description
                        item['Field26'] = agent2_phone.strip()  # description
                        item['Field26A'] = agent2.xpath(".//*[contains(text(),'P.')]/text()").get('').strip().replace('P.','')  # description
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                ...
                if len(response.xpath("//*[contains(@class,'wpsight-listing-agent clearfix')]").getall()) == 3:
                    ...
                    agent3_name = agent3.xpath(".//*[contains(@itemprop,'name')]/text()").get('').strip()
                    if agent3:
                        if agent3_name:
                            item['Field28'] = agent3_name.strip()
                        agent3_phone = agent3.xpath(".//*[contains(text(),'M.')]/text()").get('').strip().replace('M.','')  # description
                        item['Field31'] = agent3_phone.strip()  # description
                        item['Field31A'] = agent3.xpath(".//*[contains(text(),'P.')]/text()").get('').strip().replace('P.','')  # description
                        '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            feature_count = 58
            for feature in response.css(".listing-term"):
