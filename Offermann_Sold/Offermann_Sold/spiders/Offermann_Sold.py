...
class Offermann_Sold(scrapy.Spider):
    name = "Offermann_Sold"
    url = 'https://www.offermann.com.au/propertiesJson?callback=angular.callbacks._1&currentPage={}&perPage=12&sort=d_sold%20desc&listing_cat0=residential&listing_cat1=commercial&listing_cat2=business&web_stage=sold'
    prefix = "https://www.offermann.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Offermann_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        data = json.loads(response.text.replace('/**/','').replace('angular.callbacks._1(','').replace(');',''))
        links = []
        for each_url in data.get('rows',[]):
            each_url = f'https://offermann.com.au{each_url.get("fields",{}).get("link","")}'
        ...
        for property in data.get('rows',[]):
            property_url = f'https://offermann.com.au{property.get("fields",{}).get("link","")}' if property.get("fields",{}).get("link","") else ''
    ...
    def detail_parse(self, property):
        detail_page_url = f'https://offermann.com.au{property.get("link", "")}'
        property_url = f'https://offermann.com.au{property.get("link", "")}'
        database_sale_matching_url = self.read_data_base()
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3307)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Tom Offermann Real Estate'
