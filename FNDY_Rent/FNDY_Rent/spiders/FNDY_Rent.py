...
class FNDY_Rent(scrapy.Spider):
    name = 'FNDY_Rent'
    prefix = 'https://www.fndy.com.au'
    url = "https://www.fndy.com.au/renting/properties-for-lease/"
    headers = {
        'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        'cache-control': 'max-age=0',
        # 'cookie': '_ga=GA1.1.77714342.1712712898; _ga_DVGVMWZBKQ=GS1.1.1713125771.3.1.1713127015.0.0.0; arp_scroll_position=400',
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'First National Dee Why Real Estate'
            ...
            description = response.css('.listing-page-description .col-md-7 ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess = response.xpath("//*[contains(@class,'carousel')]/a[contains(@class,'slider-image')]/div/picture/source/img/@src").getall()
            new_img_urls = []
            ...
            land_area = response.xpath("//label[contains(text(),'Building Size')]/following-sibling::div[1]/text()").get('').strip()
            if land_area:
