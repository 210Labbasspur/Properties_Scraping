...
class DegreesRE_Sale(scrapy.Spider):
    name = "DegreesRE_Sale"

    url = "https://www.361realestate.com.au/properties/?category=&property_status=current&bedrooms=0&bathrooms=0&cars=&min_price=&max_price=&suburb="
    prefix = "https://www.361realestate.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(DegreesRE_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        # all_links = response.css(".more-details-btn ::attr(href)").getall()
        all_links = response.css('div.property-item')
        links = []
        for each_url in all_links:
            if 'month' in each_url.css(".property--price ::text").get('').strip().lower():
                pass
            else:
                each_url = each_url.css('.more-details-btn ::attr(href)').get('').strip()
        ...
        for property_url in all_links:
            if 'month' in property_url.css(".property--price ::text").get('').strip().lower():
                pass
            else:
                property_url = property_url.css('.more-details-btn ::attr(href)').get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3009)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = '361 Real Estate'

            property_street = response.css('.title--heading h1 ::text').get('').strip()  # property_street
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'fa-bed')]/parent::*[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'fa-bath')]/parent::*[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'fa-car')]/parent::*[1]/text()").get('').strip()

            price = response.css('.pricetag ::text').get('').strip().replace('Sold','').replace('$','').replace('AT','').replace(',','')
            if price:
                item['Field9'] = price

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.property_title ::text').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".entry--bio p ::text").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.css(".slider-item img ::attr(src)").getall()
            images = []
            ...
            if response.css("div.agent"):
                ...
                agent1_name = agent1.css('.agent--title ::text').get('').strip()
                if agent1_name:
            ...
            if len(response.css(".agent").getall()) > 1:
                ...
                agent2_name = agent2.css('.agent--title ::text').get('').strip()
                if agent2:
            ...
            if len(response.css(".agent").getall()) > 2:
                ...
                agent3_name = agent3.css('.agent--title ::text').get('').strip()
                if agent3:
            ...
            la = response.xpath('//*[contains(text(),"Land Size")]/parent::*/text()').getall()
            land_area = ''.join(la).strip().replace('Land','').replace('is','').replace('m²','')
            if land_area:
