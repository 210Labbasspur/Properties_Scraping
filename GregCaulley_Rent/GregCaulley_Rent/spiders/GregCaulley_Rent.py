...
class GregCaulley_Rent(scrapy.Spider):
    name = "GregCaulley_Rent"

    url = "https://gregcaulleyrealty.com.au/rent"
    prefix = "https://gregcaulleyrealty.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(GregCaulley_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.xpath("//*[contains(@class,'propertyCard')]/a/@href").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://gregcaulleyrealty.com.au{each_url}'
        ...
        for property_url in all_links:
            property_url = f'https://gregcaulleyrealty.com.au{property_url}'
        ...
        next_page = response.xpath("//*[contains(text(),'>')]/parent::a/@href").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3265)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Greg Caulley Realty'

            property_street = response.css('.ws_header_sub ::text').get('').strip()  # property_street
            ...
            item['Field6'] = ''.join(e.strip() for e in response.xpath("//*[contains(@class,'fa-bed-alt')]/parent::div[1]/text()").getall())
            item['Field7'] = ''.join(e.strip() for e in response.xpath("//*[contains(@class,'fa-bath')]/parent::div[1]/text()").getall())
            item['Field8'] = ''.join(e.strip() for e in response.xpath("//*[contains(@class,'fa-car-side')]/parent::div[1]/text()").getall())

            price = response.css('.pp_info_header_label_blck ::text').get('').strip().replace('Contact Agent','').replace('SOLD','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.pp_info_header_sum_header ::text').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".pp_info_summary ::text").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            js_text = response.xpath("//*[contains(text(),'window.__remixContext')]/text()").get('').strip().replace('window.__remixContext = ','').replace('};','}').replace('=','')
            if json.loads(js_text):
                data = json.loads(js_text)
                for img in data.get('routeData',{}).get('catchall-route',{}).get('pageData',{}).get('body'):
                    if img.get("component",'') == 'ppGallery':
                        prop_images = []
                        for image in img.get('data',{}).get('property',{}).get('marketing',{}).get('images',[]):
                            prop_images.append(image.get('url',''))

                        # prop_images = response.xpath("//img[contains(@class,'block')]/@src").getall()
                        images = []
            ...
            '''          AGENTS          '''
            if response.css(".pp_info_agent_blck"):
                agent1 = response.css(".pp_info_agent_blck")[0]
                agent1_name = agent1.css('div div ::text').get('').strip()
                if agent1_name:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    # item['Field18'] = agent1.css('div:nth-child(2) div:nth-child(2) ::text').get('').strip()
                    item['Field19'] = agent1.xpath(".//*[contains(@class,'fal fa-fw fa-at')]/parent::div[1]/text()").get('').strip().replace('mailto:','')
                    item['Field20'] = agent1.xpath(".//*[contains(@class,'fa-mobile')]/parent::div[1]/text()").get('').strip().replace('mailto:','')
                    item['Field21'] = agent1.xpath(".//*[contains(@class,'fa-phone')]/parent::div[1]/text()").get('').strip().replace('mailto:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".pp_info_agent_img ::attr(style)").get('').strip()
                    match = re.search(r'background-image:url\((.*?)\)', agent_image)        # Regular expression to extract URL inside `url(...)`
                    if match:
                        agent_image = match.group(1)
                        images_string = f'https://gregcaulleyrealty.com.au{agent_image}' if agent_image else ""
                        # item['Field22'] = images_string
                        # print('Agent1 image is :',images_string)
                        random_number = random.randint(1, 10000000000)
                        item['Field22'] = ",".join(self.uploaded(images_string, random_number))

            if len(response.css(".pp_info_agent_blck").getall()) > 1:
                agent2 = response.css(".pp_info_agent_blck")[1]
                agent2_name = agent2.css('div div ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    # item['Field24'] = agent2.css('div div:nth-child(2) ::text').get('').strip()
                    item['Field25'] = agent2.xpath(".//*[contains(@class,'fal fa-fw fa-at')]/parent::div[1]/text()").get('').strip().replace('mailto:','')
                    item['Field26'] = agent2.xpath(".//*[contains(@class,'fa-mobile')]/parent::div[1]/text()").get('').strip().replace('mailto:','')
                    item['Field26A'] = agent2.xpath(".//*[contains(@class,'fa-phone')]/parent::div[1]/text()").get('').strip().replace('mailto:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".pp_info_agent_img ::attr(style)").get('').strip()
                    match = re.search(r'background-image:url\((.*?)\)', agent_image)        # Regular expression to extract URL inside `url(...)`
                    if match:
                        agent_image = match.group(1)
                        images_string = f'https://gregcaulleyrealty.com.au{agent_image}' if agent_image else ""
            ...
            if len(response.css(".pp_info_agent_blck").getall()) > 2:
                agent3 = response.css(".pp_info_agent_blck")[2]
                agent3_name = agent3.css('div div ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    # item['Field29'] = agent3.css('div div:nth-child(2) ::text').get('').strip()
                    item['Field30'] = agent3.xpath(".//*[contains(@class,'fal fa-fw fa-at')]/parent::div[1]/text()").get('').strip().replace('mailto:','')
                    item['Field31'] = agent3.xpath(".//*[contains(@class,'fa-mobile')]/parent::div[1]/text()").get('').strip().replace('mailto:','')
                    item['Field31A'] = agent3.xpath(".//*[contains(@class,'fa-phone')]/parent::div[1]/text()").get('').strip().replace('mailto:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".pp_info_agent_img ::attr(style)").get('').strip()
                    match = re.search(r'background-image:url\((.*?)\)', agent_image)        # Regular expression to extract URL inside `url(...)`
                    if match:
                        agent_image = match.group(1)
                        images_string = f'https://gregcaulleyrealty.com.au{agent_image}' if agent_image else ""
            ...
            item['Field35'] = response.xpath('//*[contains(text(),"Property Type")]/following-sibling::td[1]/text()').get('').strip()

            land_area = response.xpath('//*[contains(text(),"Land")]/following-sibling::td[1]/text()').get('').strip().replace('Land','').replace('is','').replace('m²','')
            if land_area:
