...
class TheStudio_Rent(scrapy.Spider):
    name = 'TheStudio_Rent'
    prefix = 'https://thestudioea.com.au'
    url = "https://thestudioea.com.au/properties-for-lease"
    headers = {
        'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'Accept-Language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        'Connection': 'keep-alive',
        # 'Cookie': '_gid=GA1.3.2017767759.1714603461; VID=28853; SKYROCKET_CID=t6q2mi95b7eo828k5ou6vhmdp2; _ga_PK5Q5SRZNQ=GS1.1.1714858032.12.0.1714858032.60.0.0; _ga_CFHHBWJRWB=GS1.1.1714858032.12.0.1714858032.0.0.0; _ga_FJRYJ3024G=GS1.1.1714858033.12.0.1714858033.0.0.0; _ga=GA1.1.1264604441.1714476003; arp_scroll_position=4400; _ga_KPSTW9KSEV=GS1.1.1714858031.12.1.1714858232.0.0.0',
        # 'Referer': 'https://thestudioea.com.au/sold-properties',
    ...
    def parse(self, response):
        for property in response.css(".propertyTile-anchor"):
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'The Studio Estate Agents'

            property_add = response.css('.container h1 span ::text').getall()  # full_adddress
            ...
            type = ''
            bed_bath_car = response.xpath("//*[contains(@class,'icon-bed')]/parent::div[1]/text()").getall()
            if len(bed_bath_car) > 1:
                item['Field6'] = ''.join(re.findall(r'\d+', bed_bath_car[1].strip()))
                type = re.findall(r'[a-zA-Z]+', bed_bath_car[1]) if re.findall(r'[a-zA-Z]+', bed_bath_car[1]) else ''
            if len(bed_bath_car) > 2:
                item['Field7'] = ''.join(re.findall(r'\d+', bed_bath_car[2].strip()))
                type = re.findall(r'[a-zA-Z]+', bed_bath_car[2]) if re.findall(r'[a-zA-Z]+', bed_bath_car[2]) else ''
            if len(bed_bath_car) > 3:
                item['Field8'] = ''.join(re.findall(r'\d+', bed_bath_car[3].strip()))
                type = re.findall(r'[a-zA-Z]+', bed_bath_car[3]) if re.findall(r'[a-zA-Z]+', bed_bath_car[3]) else ''
            ...
            Agent_Imagess = response.css(".twothirds link ::attr(href)").getall()
            new_img_urls = []
            ...
            feature_count = 58
            for feature in response.css(".featureList li span"):
