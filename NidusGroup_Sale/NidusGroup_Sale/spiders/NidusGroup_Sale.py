...
class NidusGroup_Sale(scrapy.Spider):
    name = 'NidusGroup_Sale'
    prefix = 'https://nidusgroupre.com.au'
    url = "https://nidusgroupre.com.au/properties-for-sale/"
    headers = {
        'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        'cache-control': 'max-age=0',
        # 'cookie': 'epl_wp_session=0f48c13d86f1c562388255805dc47d3e%7C%7C1712782874%7C%7C1712781074; _ga=GA1.1.219669108.1712703675; arp_scroll_position=405; _ga_3QP9WM9SRK=GS1.1.1712703675.1.1.1712704912.0.0.0; epl_wp_session=0f48c13d86f1c562388255805dc47d3e%7C%7C1712782874%7C%7C1712781074',
    ...
    def parse(self, response):
        for property in response.css(".find-property__prop-link")[:2]:
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'NidusGroup'

            property_add = response.css('.property-detail__title ::text').getall()  # full_adddress
            ...
            item['Field6'] = response.css(".fa-bed~ .property-detail__aminities-text ::text").get('').strip().replace(' Bedroom','')
            item['Field7'] = response.css(".fa-bath~ .property-detail__aminities-text ::text").get('').strip().replace(' Bathroom','')
            item['Field8'] = response.css(".fa-car~ .property-detail__aminities-text ::text").get('').strip().replace(' Garage','')

            # sold_price = response.css('.sold-status ::text').get('').strip().replace('Sold ','')
            # if sold_price:
            #     item['Field9'] = (sold_price.strip()).replace('$','')   # sold_price

            description = response.css('.property-detail__content > p ::text').getall()  # description
            ...
            land_area = response.xpath("//*[contains(@class,'land-size')]/text()").get('').strip()
            if land_area:
                item['Field36'] = land_area.replace('m²','').replace('Land is','').replace(' ','')   # land_area
