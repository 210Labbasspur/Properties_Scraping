...
class Lara_Sale(scrapy.Spider):
    name = "Lara_Sale"

    url = "https://larare.com.au/properties-for-sale/?action=epl_search&post_type=property&property_status=current"
    prefix = "https://larare.com.au"
    headers = {
    ...
    count = 1
    db = 'Lara'
    collection = 'Lara_Sale'
    bucket_prefix = f'LRE_{collection}'

    local_file_path = '/img'

    def __init__(self, *args, **kwargs):
        super(Lara_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".polaroid-property a ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2993)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Lara Real Estate'
            ...
            item['Field6'] = response.xpath("//*[contains(@alt,'Bedrooms')]/parent::span/following-sibling::div[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@alt,'Bathrooms')]/parent::span/following-sibling::div[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@alt,'Carports')]/parent::span/following-sibling::div[1]/text()").get('').strip()
            ...
            prop_images = response.css(".single-image a ::attr(href)").getall()
            images = []
            ...
            land_area = response.xpath("//*[contains(@alt,'Land')]/parent::span/following-sibling::div[1]/text()").get('').strip()
            if land_area:
