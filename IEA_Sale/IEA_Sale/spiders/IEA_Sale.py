...
class IEA_Sale(scrapy.Spider):
    name = 'IEA_Sale'
    prefix = 'https://ieagroup.com.au'
    url = "https://ieagroup.com.au/residential/?paged={}"
    headers = {
        'sec-ch-ua': '"Chromium";v="124", "Google Chrome";v="124", "Not-A.Brand";v="99"',
        'Accept': 'text/plain, */*; q=0.01',
        # 'Referer': 'https://ieagroup.com.au/sold/',
    ...
    def parse(self, response):
        ...
        next_page = response.xpath("//*[contains(text(),'LOAD MORE')]").get('').strip()
        if next_page:
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            # sold_price = response.css('.suburb-price ::text').get('').strip().replace(' ','')
            # if sold_price:
            ...
            Agent_Imagess = response.css(".wpsight-image-slider-item a ::attr(href)").getall()
            new_img_urls = []
            ...
            '''          AGENTS          '''
            if response.css(".wpsight-listing-section-agent .wpsight-list-agent"):
                agent1 = response.css(".wpsight-listing-section-agent .wpsight-list-agent")[0]
                agent1_name = agent1.css('.wpsight-list-agent-name ::text').get('').strip()
                if agent1:
                ...
                if len(response.css(".wpsight-listing-section-agent .wpsight-list-agent").getall()) > 1:
                    agent2 = response.css(".wpsight-listing-section-agent .wpsight-list-agent")[1]
                    agent2_name = agent2.css('.wpsight-list-agent-name ::text').get('').strip()
                    if agent2:
                ...
                if len(response.css(".wpsight-listing-section-agent .wpsight-list-agent").getall()) > 2:
                    agent3 = response.css(".wpsight-listing-section-agent .wpsight-list-agent")[2]
                    agent3_name = agent3.css('.wpsight-list-agent-name ::text').get('').strip()
                    if agent3:
