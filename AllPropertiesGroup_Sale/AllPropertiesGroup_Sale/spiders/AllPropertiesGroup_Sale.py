...
class AllPropertiesGroup_Sale(scrapy.Spider):
    name = "AllPropertiesGroup_Sale"

    url = "https://allpropertiesgroup.com.au/for-sale/"
    prefix = "https://allpropertiesgroup.com.au"
    headers = {
    ...
    count = 1
    db = 'AllPropertiesGroup'
    collection = 'AllPropertiesGroup_Sale'
    ...
    def __init__(self, *args, **kwargs):
        super(AllPropertiesGroup_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".lp-box-link ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3344)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'All Properties Group'

            property_street = response.css('.rep-property-address h1 ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.propertyPage-property-address ::text').getall() if e.strip())  # property_suburb
            property_address = f"{property_street} {property_suburb}"
            item['Field5'] = property_address  # full_adddress

            item['Field6'] = response.css(".rep-property-single-content .rep-bedroom ::text").get('').strip()
            item['Field7'] = response.css(".rep-property-single-content .rep-bathroom ::text").get('').strip()
            item['Field8'] = response.css(".rep-property-single-content .rep-carpark ::text").get('').strip()

            price = response.css('.rep-property-single-content .rep-property-price ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            ...
            '''          AGENTS          '''
            if response.css(".rep-employee"):
                agent1 = response.css(".rep-employee")[0]
                agent1_name = agent1.css('.rep-employee-name a ::text').get('').strip()
                if agent1_name:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field18'] = agent1.css('.rep-employee-title ::text').get('').strip()
                    ...
                    if len(phone_nos) > 1:
                        item['Field21'] = phone_nos[-1].strip().replace('tel:','').replace('\u202d','').replace('\u202c','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".lp-image ::attr(style)").get('').strip().replace('background-image: url(','').replace(');','')
            ...
            if len(response.css(".rep-employee").getall()) > 1:
                agent2 = response.css(".rep-employee")[1]
                agent2_name = agent2.css('.rep-employee-name a ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.css('.rep-employee-title ::text').get('').strip()
                    ...
                    if len(phone_nos) > 1:
                        item['Field26A'] = phone_nos[-1].strip().replace('tel:','').replace('\u202d','').replace('\u202c','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".lp-image ::attr(style)").get('').strip().replace('background-image: url(','').replace(');','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css(".rep-employee").getall()) > 2:
                agent3 = response.css(".rep-employee")[2]
                agent3_name = agent3.css('.rep-employee-name a ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field29'] = agent3.css('.rep-employee-title ::text').get('').strip()
                    ...
                    if len(phone_nos) > 1:
                        item['Field31A'] = phone_nos[-1].strip().replace('tel:','').replace('\u202d','').replace('\u202c','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".lp-image ::attr(style)").get('').strip().replace('background-image: url(','').replace(');','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            item['Field35'] = response.css(".rep-property-single-content .rep-type ::text").get('').strip()

            land_area = response.css(".rep-property-single-content .rep-landarea ::text").get('').strip().replace('Land','').replace('is','').replace('m2','')
            if land_area:
