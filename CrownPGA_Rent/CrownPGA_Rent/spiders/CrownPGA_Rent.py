...
class CrownPGA_Rent(scrapy.Spider):
    name = "CrownPGA_Rent"

    url = "https://home.crownpga.com.au/leasing/residential-properties-for-lease"
    prefix = "https://home.crownpga.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(CrownPGA_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.xpath("//*[contains(@class,'renet_listing_item_parent')]/a/@href").getall()
        links = []
        ...
        next_page = response.xpath("//*[contains(@class,'fas fa-chevron-right')]/parent::a/@href").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2904)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Crown Property Group Australia'

            property_street = response.css('.renet_txt_listing_address ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.xxxxxxx ::text').getall() if e.strip())  # property_suburb
            property_address = f"{property_street} {property_suburb}"
            item['Field5'] = property_address  # full_adddress

            item['Field6'] = response.xpath("//*[contains(@class,'fas fa-bed')]/parent::*[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'fas fa-bath')]/parent::*[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'fas fa-car')]/parent::*[1]/text()").get('').strip()

            price = response.css('.renet_txt_listing_price ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.renet_txt_listing_heading ::text').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".renet_property_profile ::text").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.xpath("//*[contains(@class,'swiper-slide renet_image_wrapper')]/@style").getall()
            images = []
            for img in prop_images:
                img = img.strip().replace('background-image:url(','').replace(");","").replace('"','').replace("background-size:;","").replace("\n","").replace("\r","").replace("\t","").replace("  ","").replace(" ","")
            ...
            '''          AGENTS          '''
            item['Field15'] = 'Zoran'
            item['Field16'] = 'Veleski'
            item['Field17'] = 'Zoran Veleski'
            item['Field20'] = '0411 350 999'
            item['Field21'] = '1800 70 70 88'
            images_string = 'https://s3.ap-southeast-1.wasabisys.com/Images8_1/3045588674_1.jpg'
            ...
            item['Field33'] = response.xpath("//*[contains(text(),'Property ID')]/preceding-sibling::div[1]/text()").get('').strip()
            # bbc = response.css(".propertyPage-property-icons ::text").getall()
            # item['Field35'] = bbc[-1].strip() if bbc else ''

            land_area = response.xpath("//*[contains(text(),'Land Size')]/preceding-sibling::div[1]/text()").get('').strip().replace('approx.','').replace('','')
            if land_area:
            ...
            #
            feature_count = 58
            for feature in response.css(".renet_property_features .item .content:nth-child(2)")[3:]:   ##  first 4 features are ID, bed, bath etc.
