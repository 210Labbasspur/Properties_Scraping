...
class GriffithRE_Sold(scrapy.Spider):
    name = "GriffithRE_Sold"

    url = ("https://www.griffithre.com/wp-admin/admin-ajax.php?action=get_posts&query%5Bpost_type%5D%5Bvalue%5D=listings&query%5Bpost_type%5D%5Btype%5D=equal&query%5Bcount%5D%5Bvalue%5D=20&query%5Bcount%5D%5Btype%5D=equal&query%5Borderby%5D%5Bvalue%5D=meta_value&query%5Borderby%5D%5Btype%5D=equal&query%5Bmeta_key%5D%5Bvalue%5D=saleDate&query%5Bmeta_key%5D%5Btype%5D=equal&query%5Bsold%5D%5Bvalue%5D=1&query%5Bsold%5D%5Btype%5D=equal&query%5BsaleOrRental%5D%5Bvalue%5D=Sale&query%5BsaleOrRental%5D%5Btype%5D=equal&query"
           "%5Bpaged%5D%5Bvalue%5D={}&query%5Bpaged%5D%5Btype%5D=equal&query%5Bminprice%5D%5Bvalue%5D=0&query%5Bminprice%5D%5Btype%5D=equal&query%5Bmaxprice%5D%5Bvalue%5D=5000001&query%5Bmaxprice%5D%5Btype%5D=equal&query%5Border%5D%5Bvalue%5D=saleDate-desc&query%5Border%5D%5Btype%5D=equal&query%5Bdoing_wp_cron%5D%5Bvalue%5D=1623289688.2098410129547119140625&query%5Bdoing_wp_cron%5D%5Btype%5D=equal")
    prefix = "https://www.griffithre.com"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(GriffithRE_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(1653)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Griffith Real Estate'
            ...
            # item['Field9'] = property.get('priceMatch')
            item['Field9'] = response.css('.listing-info-price ::text').get('').strip().replace('$','')
            item['Field12'] = property.get('post_content')
            ...
            # item['Field33'] = response.xpath("//*[contains(text(),'property ID')]/parent::div[1]/text()")
