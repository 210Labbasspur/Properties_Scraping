...
class Wilsons_Rent(scrapy.Spider):
    name = 'Wilsons_Rent'
    prefix = 'https://www.wilsonsproperty.com.au'
    url = "https://www.wilsonsproperty.com.au/renting/properties-for-lease/"
    headers = {
    ...
    def parse(self, response):
        for property in response.css(".listing-headline a"):
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            if response.xpath("//*[contains(text(),'Garage')]/following-sibling::div[1]/text()").get('').strip():
                ...
                item['Field8'] = response.xpath("//*[contains(text(),'Car Spaces')]/following-sibling::div[1]/text()").get('').strip()
            elif response.xpath("//*[contains(text(),'Parking')]/following-sibling::div[1]/text()").get('').strip():
                item['Field8'] = response.xpath("//*[contains(text(),'Parking')]/following-sibling::div[1]/text()").get('').strip()

            # sold_price = response.xpath("//*[contains(text(),'Price')]/following-sibling::div[1]/text()").get('').strip()
            ...
            Agent_Imagess = response.xpath("//a[contains(@class,'image-gallery-item')]/@href").getall()
            new_img_urls = []
            ...
            if response.css(".mb-20"):
                ...
                if agent1_name:
                    ...
                    agent1_phone = ' '.join(e.strip() for e in agent1.css(".mobile a ::text").getall())  # description
                    item['Field20'] = agent1_phone.strip()
                    agent1_mobile = ' '.join(e.strip() for e in agent1.css(".phone a ::text").getall())  # description
                ...
                if len(response.css(".mb-20").getall()) > 1:
                    ...
                    if agent2:
                        ...
                        agent2_phone = ' '.join(e.strip() for e in agent2.css(".mobile a ::text").getall())  # description
                        item['Field26'] = agent2_phone.strip()
                        agent2_mobile = ' '.join(e.strip() for e in agent2.css(".phone a ::text").getall())  # description
                ...
                if len(response.css(".mb-20").getall()) == 3:
                    ...
                    if agent3:
                        ...
                        agent3_phone = ' '.join(e.strip() for e in agent3.css(".mobile a ::text").getall())  # description
                        item['Field31'] = agent3_phone.strip()
                        agent3_mobile = ' '.join(e.strip() for e in agent3.css(".phone a ::text").getall())  # description
