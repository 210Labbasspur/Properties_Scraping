...
class RemaxLM_Sale(scrapy.Spider):
    name = 'RemaxLM_Sale'
    prefix = 'https://www.remax-lifestylemarketing.com.au'
    url = "http://www.remax-lifestylemarketing.com.au/for-sale/properties-for-sale"
    headers = {
    ...
    def parse(self, response):
        ...
        all_details_links = []
        for url in response.css(".thumb-label-item-title a"):
            url = 'http:' + url.css('::attr(href)').get('').strip()
            all_details_links.append(url)
        ...
        '''  #######   Original Code   #######  '''
        for property in response.css(".thumb-label-item-title a"):
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'REMAX Lifestyle Marketing'

            property_add = response.css('.propertyDetails .col-xs-12 ::text').getall()  # full_adddress
            ...
            sold_price = response.css('.col-sm-6 .textBold.textUppercase ::text').get('').strip().replace('Sold','').replace('for','')
            if re.search(r'\d', sold_price):
                item['Field9'] = (sold_price.strip()).replace(',','').replace('$','')   # sold_price

            description = response.css('.propertyDescription p ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess = response.xpath("//*[contains(@class,'defaultimg rev-slidebg')]/@src").getall()
            new_img_urls = []
            ...
            '''          AGENTS          '''
            if response.css(".carousel-inner"):
                agent1 = response.css(".carousel-inner")[0]
                agent1_name = ' '.join(e.strip() for e in agent1.css("h2.staffName ::text").getall())
                if agent1_name:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field18'] = agent1.css('.staffPosition small ::text').get().strip()  # agent_full_name_1
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images_style = agent1.css(".imgWrapper  ::attr(style)").get('').strip()
                ...
                if len(response.css(".carousel-inner").getall()) > 1:
                    agent2 = response.css(".carousel-inner")[1]
                    agent2_name = ' '.join(e.strip() for e in agent1.css("h2.staffName ::text").getall())
                    if agent2:
                        item['Field23'] = agent2_name.strip()
                        item['Field24'] = agent2.css('.staffPosition small ::text').get().strip()  # agent_full_name_1
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images_style = agent2.css(".imgWrapper  ::attr(style)").get('').strip()
                        Agent_Images = 'https:'+ re.search(r'url\((.*?)\)', Agent_Images_style).group(1)
                ...
                if len(response.css(".carousel-inner").getall()) > 2:
                    agent3 = response.css(".carousel-inner")[2]
                    agent3_name = ' '.join(e.strip() for e in agent1.css("h2.staffName ::text").getall())
                    if agent3:
                        item['Field28'] = agent3_name.strip()
                        item['Field29'] = agent3.css('.staffPosition small ::text').get().strip()  # agent_full_name_1
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images_style = agent3.css(".imgWrapper  ::attr(style)").get('').strip()
                        Agent_Images = 'https:'+ re.search(r'url\((.*?)\)', Agent_Images_style).group(1)
            ...
            feature_count = 58
            for feature in response.xpath("//*[contains(@class,'fa fa-check')]/parent::span[1]/preceding-sibling::b[1]"):
