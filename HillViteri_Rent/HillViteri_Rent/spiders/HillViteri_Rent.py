...
class HillViteri_Rent(scrapy.Spider):
    name = 'HillViteri_Rent'
    prefix = 'https://www.hillandviteriproperty.com.au'
    url = "https://www.hillandviteriproperty.com.au/tenants/residential-for-lease/"
    headers = {
        'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        'cache-control': 'max-age=0',
        # 'cookie': 'PHPSESSID=ajljgkavd50llf7u5lmc6atkfm; _ga=GA1.1.769319870.1712910608; _gcl_au=1.1.1264112991.1712910614; WG_WEB=%7B%22W_I%22%3A0%7D; PHPSESSID=ajljgkavd50llf7u5lmc6atkfm; _ga_0TG8K7T7FL=GS1.1.1713080130.3.1.1713080231.60.0.0; timeCookie=Sun%20Apr%2014%202024%2012%3A44%3A55%20GMT%2B0500%20(Pakistan%20Standard%20Time); PHPSESSID=ajljgkavd50llf7u5lmc6atkfm',
    ...
    def parse(self, response):
        ...
        if 'page' in response.css(".page_next a ::attr(href)").get('').strip():
            next_page = response.css('.page_next a ::attr(href)').get('').strip()
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'Hill & Viteri Property'

            property_add = response.css('.address-suburb ::text').getall()  # full_adddress
            ...
            item['Field6'] = response.css(".bedrooms span:nth-child(1) ::text").get('').strip()
            item['Field7'] = response.css(".bathrooms span:nth-child(1) ::text").get('').strip()
            item['Field8'] = response.css(".carspaces span:nth-child(1) ::text").get('').strip()

            # sold_price = response.xpath("//*[contains(text(),'SOLD')]/parent::div[1]/following-sibling::div[1]/p/text()").get('').strip().replace(' ','')
            # if sold_price:
            #     item['Field9'] = (sold_price.strip()).replace('$','')   # sold_price

            description = response.css('.copy ::text, .address .address ::text').getall()  # description
            ...
            if agent1:
                ...
                agent1_mobile = agent1.css(".agent_mobile span ::text").get('').strip()
            ...
            if len(response.css(".agent_info").getall()) > 1:
                ...
                if agent2:
                    ...
                    agent2_mobile = agent2.css(".agent_mobile span ::text").get('').strip()
            ...
            if len(response.css(".agent_info").getall()) == 3:
                ...
                if agent3:
                    ...
                    agent3_mobile = agent3.css(".agent_mobile span ::text").get('').strip()
