...
class CoreRealty_Rent(scrapy.Spider):
    name = "CoreRealty_Rent"

    url = "https://corerealty.com.au/lease/"
    prefix = "https://corerealty.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(CoreRealty_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def start_requests(self):
        ...
        for each_url in all_links:
            each_url = f'https://corerealty.com.au{each_url}'
        ...
        for property_url in all_links:
            property_url = f'https://corerealty.com.au{property_url}'
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2968)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Core Realty'

            property_street = response.css('.property-header__left h1 ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.property__inner h1 ::text').getall() if e.strip())  # property_suburb
            property_address = f"{property_street} {property_suburb}".replace('\n','').replace('\r','').replace('\t','')
            item['Field5'] = property_address.replace('  ','')  # full_adddress

            bbc_html = response.xpath("//*[@class='property__main']/preceding-sibling::section[1]//div[@class='card__stats']")
            for bbc in bbc_html.css('span'):
                bbc = ''.join(e.strip() for e in bbc.css("::text").getall())
                if 'bed' in bbc.lower():
                    item['Field6'] = bbc.strip().lower().replace("bed","").replace("s","")
                if 'bath' in bbc.lower():
                    item['Field7'] = bbc.strip().lower().replace("bath","").replace("s","")
                if 'car' in bbc.lower():
                    item['Field8'] = bbc.strip().lower().replace("car","").replace("s","")
                if '㎡' in bbc.lower():
                    item['Field36'] = bbc.strip().lower().replace("㎡","")

            price = ''.join(e.strip() for e in response.css('.property__price ::text').getall())
            price = price.replace('Sold','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.property__content h2 ::text').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".property__description-wrap ::text").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.xpath("//*[@class='card__image']/img/@src").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.css(".card--agent"):
                agent1 = response.css(".card--agent")[0]
                ...
                if agent1_name:
                    ...
                    item['Field18'] = agent1.css('p ::text').get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css("picture img ::attr(data-src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'https://corerealty.com.au{agent_image}' if agent_image else ""
                    # item['Field22'] = images_string
                    # print('Agent1 image is :',images_string)
                    random_number = random.randint(1, 10000000000)
                    item['Field22'] = ",".join(self.uploaded(images_string, random_number))

            if len(response.css(".card--agent").getall()) > 1:
                agent2 = response.css(".card--agent")[1]
                agent2_name = agent2.css('h4 ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.css('p ::text').get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css("picture img ::attr(data-src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'https://corerealty.com.au{agent_image}' if agent_image else ""
            ...
            if len(response.css(".card--agent").getall()) > 2:
                agent3 = response.css(".card--agent")[2]
                agent3_name = agent3.css('h4 ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field29'] = agent3.css('p ::text').get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css("picture img ::attr(data-src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'https://corerealty.com.au{agent_image}' if agent_image else ""
