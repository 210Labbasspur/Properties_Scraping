...
class SN_Sale(scrapy.Spider):
    name = 'SN_Sale'
    prefix = 'https://www.steannicholls.com.au'
    url = "https://www.steannicholls.com.au/ajax-search-listings?direction=desc&order=date_listed&page={}&property_type%5B%5D=Residential+Land&property_type%5B%5D=Residential&type=Buy"
    headers = {
        'accept': '*/*',
        'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        # 'cookie': '__utmz=157150969.1713705674.1.1.utmcsr=(direct)|utmccn=(direct)|utmcmd=(none); _gs=0a15e7f00af7e4f84ff040a8da98f54d7ac9154e; __utmc=157150969; __utma=157150969.353293252.1713705674.1714311452.1714335630.7; __utmb=157150969.1.10.1714335630; arp_scroll_position=1200',
        'priority': 'u=1, i',
        # 'referer': 'https://www.steannicholls.com.au/listings?type=Sold',
    ...
    def parse(self, response):
        for property in response.css('a.listing-long'):
        ...
        '''       Pagination     '''
        next_page = response.css('a.load-search-more ::attr(href)').get('').strip()
        if next_page:
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'Stean Nicholls'

            property_add = response.css('.col-sm-12 h1 ::text').getall()  # full_adddress
            ...
            item['Field6'] = response.xpath("//*[contains(@src,'bed')]/following-sibling::span[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@src,'bath')]/following-sibling::span[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@src,'car')]/following-sibling::span[1]/text()").get('').strip()

            # sold_price = response.css('.ld-price ::text').get('').strip().replace('Sold for','').replace(' ','')
            # if re.search(r'\d', sold_price):
            #     item['Field9'] = (sold_price.strip()).replace('$','')   # sold_price

            description = response.css('.col-md-9 p ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess = response.css(".pgwslideshow li img ::attr(data-large-src)").getall()
            new_img_urls = []
            ...
            '''          AGENTS          '''
            if response.css(".widget-agent"):
                agent1 = response.css(".widget-agent")[0]
                agent1_name = agent1.css('.widget-agent-heading a ::text').get('').strip()
                if agent1:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field18'] = agent1.css('.widget-agent-subheading ::text').get('').strip()
                    item['Field19'] =  agent1.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:','')
                    agent1_phone = agent1.css(".widget-agent-phone ::text").get('').strip().replace('tel:','')
                ...
                if len(response.css(".widget-agent").getall()) > 1:
                    agent2 = response.css(".widget-agent")[1]
                    agent2_name = agent2.css('.widget-agent-heading a ::text').get('').strip()
                    if agent2:
                        item['Field23'] = agent2_name.strip()
                        item['Field24'] = agent2.css('.widget-agent-subheading ::text').get('').strip()
                        item['Field25'] = agent2.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:', '')
                        agent2_phone = agent2.css(".widget-agent-phone ::text").get('').strip().replace('tel:', '')
                ...
                if len(response.css(".widget-agent").getall()) > 2:
                    agent3 = response.css(".widget-agent")[2]
                    agent3_name = agent3.css('.widget-agent-heading a ::text').get('').strip()
                    if agent3:
                        item['Field28'] = agent3_name.strip()
                        item['Field29'] = agent3.css('.widget-agent-subheading ::text').get('').strip()
                        item['Field30'] = agent3.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:', '')
                        agent3_phone = agent3.css(".widget-agent-phone ::text").get('').strip().replace('tel:', '')
            ...
            #
            # land_area = response.xpath("//span[contains(text(),'sqm')]/text()").get('').strip()
            # if land_area:
            #     item['Field36'] = land_area.replace('sqm','').replace(' ','')   # land_area
            #
            # feature_count = 58
            # for feature in response.css(".highlights li"):
            #     if 'condition' in feature.css('::text').get('').strip():
