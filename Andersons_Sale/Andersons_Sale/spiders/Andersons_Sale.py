...
class Andersons_Sale(scrapy.Spider):
    name = "Andersons_Sale"

    url = "https://www.andersonsrealestate.com.au/properties-for-sale/?property_type&suburb&price_from&price_to&bedrooms&bathrooms&parking&status"
    prefix = "https://www.andersonsrealestate.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Andersons_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".listing-item-img a ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3311)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Andersons Real Estate'

            property_street = response.css('h1.uppercase ::text').get('').strip()  # property_street
            ...
            item['Field6'] = ' '.join(e.strip() for e in response.xpath("//*[contains(@src,'bed.png')]/parent::div[1]/text()").getall())
            item['Field7'] = ' '.join(e.strip() for e in response.xpath("//*[contains(@src,'bath.png')]/parent::div[1]/text()").getall())
            item['Field8'] = ' '.join(e.strip() for e in response.xpath("//*[contains(@src,'car.png')]/parent::div[1]/text()").getall())

            price = response.css('.single-price ::text').get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price
            ...
            '''          AGENTS          '''
            item['Field15'] = 'Grahame'
            item['Field16'] = 'Anderson'
            item['Field17'] = 'Grahame Anderson'
            item['Field19'] = 'grahame@andersonsrealestate.com.au'
            item['Field20'] = '0418230617'
            images_string = 'https://www.andersonsrealestate.com.au/images/agents/grahame-anderson.jpg'
            item['Field22'] = images_string
            print('Agent1 image is :',images_string)
            # random_number = random.randint(1, 10000000000)
            # item['Field22'] = ",".join(self.uploaded(images_string, random_number))

            item['Field23'] = 'Lyn Anderson'
            item['Field25'] = 'lyn@andersonsrealestate.com.au'
            item['Field26'] = '0409271350'
            images_string = 'https://www.andersonsrealestate.com.au/images/agents/lyn-anderson.JPG'
            ...
            id_area = response.css(".listing-infor-unit ::text").get('').strip()
            parts = id_area.split()
            area = next((part for part in parts if 'm²' in part), None)                         # Extract area (ends with 'm²')
            id_part = next((part for part in parts if part.startswith('ID:')), None)            # Extract ID (starts with 'ID:' prefix)
            id_value = id_part.split(':')[1] if id_part else None                               # Clean and return the extracted values
            # return area, id_value
            item['Field33'] = id_value
            item['Field36'] = area.replace('m²','')
