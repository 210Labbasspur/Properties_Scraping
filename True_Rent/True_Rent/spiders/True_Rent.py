...
class True_Rent(scrapy.Spider):
    name = 'True_Rent'
    prefix = 'https://www.trueproperty.com.au'
    url = "https://www.trueproperty.com.au/renting/find-a-rental-property/"
    headers = {
        'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        # 'cookie': '_ga=GA1.3.526708828.1713705597; _gcl_au=1.1.726834011.1713705598; _gid=GA1.3.556702349.1714069775; _gat=1; _ga_KY1R6TH87B=GS1.3.1714069776.2.0.1714069776.0.0.0; arp_scroll_position=3600',
        'priority': 'u=0, i',
        # 'referer': 'https://www.trueproperty.com.au/buying/recent-sales/',
    ...
    def parse(self, response):
        for property in response.css("a.img-container"):
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field6'] = ''.join(element.strip().replace(' ', '') for element in response.css("#quickinfo .bedroom ::text").getall())  # description
            item['Field7'] = ''.join(element.strip().replace(' ', '') for element in response.css("#quickinfo .bathroom ::text").getall())  # description
            item['Field8'] = ''.join(element.strip().replace(' ', '') for element in response.css("#quickinfo .carspace ::text").getall())  # description

            # sold_price = response.css('.price__label ::text').get('').strip().replace('$','')
            # if sold_price:
            #     item['Field9'] = (sold_price.strip()).replace('$','').replace(' ','')   # sold_price

            description = response.css('h5.sub-title ::text, .detail-description ::text').getall()  # description
            ...
            if response.css(".listing-agent"):
                ...
                if agent1:
                    ...
                    agent1_mobile = agent1.css(".agent-info-item-mobile a ::attr(href)").get('').strip().replace('tel:','')
                    item['Field20'] = agent1_mobile.strip()
                    agent1_phone = agent1.css(".agent-info-item-phone a ::attr(href)").get('').strip().replace('tel:','')
                ...
                if len(response.css(".listing-agent").getall()) > 1:
                    ...
                    if agent2:
                        ...
                        agent2_mobile = agent2.css(".agent-info-item-mobile a ::attr(href)").get('').strip().replace('tel:','')
                        item['Field26'] = agent2_mobile.strip()
                        agent2_phone = agent2.css(".agent-info-item-phone a ::attr(href)").get('').strip().replace('tel:','')
                ...
                if len(response.css(".listing-agent").getall())== 3:
                    ...
                    if agent3:
                        ...
                        agent3_mobile = agent3.css(".agent-info-item-mobile a ::attr(href)").get('').strip().replace('tel:','')
                        item['Field31'] = agent3_mobile.strip()
                        agent3_phone = agent3.css(".agent-info-item-phone a ::attr(href)").get('').strip().replace('tel:','')
            ...
            # land_area = response.css(".land-size ::text").getall('').strip()
            land_area = ''.join(element.strip().replace(' ', '') for element in response.css(".land-size ::text").getall())  # description
            if land_area:
