...
class RussellIsland_Rent(scrapy.Spider):
    name = 'RussellIsland_Rent'
    prefix = 'https://www.russellislandrealestate.com.au'
    url = 'https://www.russellislandrealestate.com.au/propertiesJson?callback=angular.callbacks._0&currentPage={}&perPage=12&sort=d_listing%20desc&listing_cat=rental'
    headers = {
        'accept': '*/*',
    ...
    def __init__(self, *args, **kwargs):
        super(RussellIsland_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        ...
        for property in data['rows']:
            ...
            print(self.count, ' # Property URL is : ', property_url)
            self.count += 1
    ...
    def Detail_parse(self, mini_data, full_data):
        ...
        if detail_page_url not in database_sale_matching_url:
            ...
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2279)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Russell Island Real Estate'
            ...
            item['Field9'] = mini_data.get('sold_price').replace('$','').replace('-','').replace(' ','').replace('SOLD','').replace(',','')
            ...
            item['Field37'] = full_data.get('fields',{}).get('floor_area_m2')
