...
class Asset_Sold(scrapy.Spider):
    name = 'Asset_Sold'
    prefix = 'https://www.mypropertyepping.com.au'
    url = "https://www.assetadvantage.com.au/buy-and-sell/recent-sales?&status=current&disposalMethod=sold&page=1"
    headers = {
        'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'Accept-Language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        'Connection': 'keep-alive',
        # 'Cookie': '_gcl_au=1.1.111739895.1714476087; PHPSESSID=g9rvqik01pfie9ofud5ft5v2g7; _gid=GA1.3.1107514214.1714594176; _gat=1; _gat_UA-149165228-34=1; _ga_36GHWDZFK0=GS1.1.1714594179.3.0.1714594179.0.0.0; _ga=GA1.1.1857049685.1714476084; _ga_9HC5W8CLXN=GS1.3.1714594179.2.0.1714594179.60.0.0; _ga_YJZLD2BD5K=GS1.3.1714594180.2.0.1714594180.0.0.0; arp_scroll_position=2725',
        # 'Referer': 'https://www.assetadvantage.com.au/buy-and-sell/recent-sales',
    ...
    def parse(self, response):
        for property in response.xpath("//*[contains(text(),'View Property')]"):
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field8'] = response.xpath("//*[contains(text(),'Car')]/following-sibling::span[1]/text()").get('').strip()
            ...
            Agent_Imagess = response.xpath("//*[contains(@alt,'Image')]/@src").getall()
            new_img_urls = []
            ...
            if response.css(".carousel-inner .item"):
                ...
                agent1_name = ' '.join(element.strip() for element in agent1.css('.textLight ::text').getall())
                if agent1:
                    ...
                    agent1_phone = agent1.xpath(".//*[contains(@class,'fa-mobile')]/following-sibling::a[1]/text()").get('').strip()
                    item['Field20'] = agent1_phone.strip()
                    agent1_mobile = agent1.xpath(".//*[contains(@class,'fa-phone')]/following-sibling::a[1]/text()").get('').strip()
                    item['Field21'] = agent1_mobile.strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images_style = agent1.css(".img-Circle ::attr(style)").get('').strip()
                    Agent_Images = 'https:' + re.search(r'url\((.*?)\)', Agent_Images_style).group(1)
                ...
                if len(response.css(".carousel-inner .item").getall()) > 1:
                    ...
                    agent2_name = ' '.join(element.strip() for element in agent2.css('.textLight ::text').getall())
                    if agent2:
                        item['Field23'] = agent2_name.strip()
                        item['Field24'] = agent2.css('.staffPosition small ::text').get('').strip()
                        agent2_phone = agent2.xpath(".//*[contains(@class,'fa-mobile')]/following-sibling::a[1]/text()").get('').strip()
                        item['Field26'] = agent2_phone.strip()
                        agent2_mobile = agent2.xpath(".//*[contains(@class,'fa-phone')]/following-sibling::a[1]/text()").get('').strip()
                        item['Field26A'] = agent2_mobile.strip()
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images_style = agent2.css(".img-Circle ::attr(style)").get('').strip()
                        Agent_Images = 'https:' + re.search(r'url\((.*?)\)', Agent_Images_style).group(1)
                ...
                if len(response.css(".carousel-inner .item").getall()) > 2:
                    ...
                    agent3_name = ' '.join(element.strip() for element in agent3.css('.textLight ::text').getall())
                    if agent3:
                        item['Field28'] = agent3_name.strip()
                        item['Field29'] = agent3.css('.staffPosition small ::text').get('').strip()
                        agent3_phone = agent3.xpath(".//*[contains(@class,'fa-mobile')]/following-sibling::a[1]/text()").get('').strip()
                        item['Field31'] = agent3_phone.strip()
                        agent3_mobile = agent3.xpath(".//*[contains(@class,'fa-phone')]/following-sibling::a[1]/text()").get('').strip()
                        item['Field31A'] = agent3_mobile.strip()
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images_style = agent3.css(".img-Circle ::attr(style)").get('').strip()
                        Agent_Images = 'https:' + re.search(r'url\((.*?)\)', Agent_Images_style).group(1)
            ...
            item['Field33'] = response.xpath("//*[contains(text(),'Property ID')]/following-sibling::span[1]/text()").get('').strip().replace('Property ID:','')
            ...
            feature_count = 58
            for feature in response.xpath("//*[contains(@class,'fa-check')]/parent::span[1]/preceding-sibling::b[1]"):
