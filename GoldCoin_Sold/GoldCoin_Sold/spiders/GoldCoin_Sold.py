...
class GoldCoin_Sold(scrapy.Spider):
    name = "GoldCoin_Sold"

    url = "https://www.goldcoinrealestate.com.au/pages/real-estate/results?widgetKey=134300&context=listings&returnAs=markup&listing_sale_method=sale&display_sale_method=SOLD&status=sold&pg=1"
    prefix = "https://www.goldcoinrealestate.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(GoldCoin_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".listing-card a ::attr(href)").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://www.goldcoinrealestate.com.au{each_url}'
        ...
        for property_url in all_links:
            property_url = f'https://www.goldcoinrealestate.com.au{property_url}'
        ...
        next_page = response.css(".load-listings button ::attr(hx-get)").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3160)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Gold Coin Real Estate'
            ...
            item['Field6'] = response.xpath("//*[contains(text(),'Bedroom')]/text()").get('').strip().replace('Bedroom','').replace(' ','').replace('s','').replace('\n','').replace('\xa0','')
            item['Field7'] = response.xpath("//*[contains(text(),'Bathroom')]/text()").get('').strip().replace('Bathroom','').replace(' ','').replace('s','').replace('\n','').replace('\xa0','')
            item['Field8'] = response.xpath("//*[contains(text(),'Carspace')]/text()").get('').strip().replace('Carspace','').replace(' ','').replace('s','').replace('\n','').replace('\xa0','')
            ...
            prop_images = response.xpath("//img[contains(@class,'lightbox-image')]/@src").getall()
            images = []
            ...
            item['Field35'] = response.css(".property-details-heading ::text").get('').strip()

            land_area = response.xpath('//*[contains(text(),"Land Size")]/following-sibling::span[1]/text()').get('').strip().replace('Land','').replace('is','').replace('m²','')
            if land_area:
