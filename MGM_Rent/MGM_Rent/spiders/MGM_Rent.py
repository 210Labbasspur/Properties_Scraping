...
class MGM_Rent(scrapy.Spider):
    name = 'MGM_Rent'
    prefix = 'https://mgmmartin.com'
    url = "https://mgmmartin.com/properties-for-lease"
    headers = {
        'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        'cache-control': 'max-age=0',
        # 'cookie': '_vwo_uuid_v2=D47026AA36A8670BFA3F5FF8EF28D1252|4b161f3f2ed370bd0c440447adaadf00; _ga=GA1.1.1422634545.1713705651; __zlcmid=1LOmf4dw2b64Ejb; VID=23884; PHPSESSID=fab3b7ff07cea991d61fb212a298de8f; arp_scroll_position=7700; _ga_VEZX6JZR4M=GS1.1.1714067179.3.1.1714067194.0.0.0',
        'priority': 'u=0, i',
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'MGM Martin'

            property_add = response.css('.propertyMapTile-address ::text').getall()  # full_adddress
            ...
            item['Field6'] = response.xpath("//span[contains(text(),'Bed')]/text()").get('').strip().replace('Bed','').replace('s','').replace(' ','')
            item['Field7'] = response.xpath("//span[contains(text(),'Bath')]/text()").get('').strip().replace('Bath','').replace('s','').replace(' ','')
            item['Field8'] = response.xpath("//span[contains(text(),'Car')]/text()").get('').strip().replace('Car','').replace('s','').replace(' ','')

            # sold_price = response.css('div.text-right ::text').get('').strip().replace(' ','')
            # if sold_price:
            ...
            if response.css(".agentTile"):
                ...
                if len(response.css(".agentTile").getall()) == 3:
                    ...
                    if agent3:
                        ...
                        agent3_phone = agent3.css(".agentTile-contact-mobile a ::text").get('').strip()
