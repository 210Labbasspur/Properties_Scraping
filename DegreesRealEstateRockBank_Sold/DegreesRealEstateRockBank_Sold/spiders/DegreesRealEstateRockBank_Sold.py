###############         DegreesRealEstateRockBank_Sold
...
class DegreesRealEstateRockBank_Sold(scrapy.Spider):
    name = "DegreesRealEstateRockBank_Sold"

    url = "https://www.361degreesrealestaterockbank.com.au/sold/?page=1"
    prefix = "https://www.361degreesrealestaterockbank.com.au"
    headers = {
    ...
    count = 1
    db = 'DegreesRealEstateRockBank'
    collection = 'DegreesRealEstateRockBank_Sold'
    ...
    def __init__(self, *args, **kwargs):
        super(DegreesRealEstateRockBank_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".btn-wrap a ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3010)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = '361 Degrees Real Estate Rock Bank'

            property_street = response.css('.page-content .col-xxl-12 h4 ::text').get('').strip()  # property_street
            ...
            bbc = response.xpath("//*[contains(@src,'icon1-4.svg')]/parent::*[1]/text()").getall()
            item['Field6'] = bbc[0].strip() if response.xpath("//*[contains(@src,'icon1-4.svg')]/parent::*[1]/text()").get('').strip() else ''
            item['Field7'] = bbc[1].strip() if response.xpath("//*[contains(@src,'icon1-5.svg')]/parent::*[1]/text()").get('').strip() else ''
            item['Field8'] = bbc[-1].strip() if response.xpath("//*[contains(@src,'icon1-8.svg')]/parent::*[1]/text()").get('').strip() else ''

            price = response.xpath("//*[contains(@src,'icon1-8.svg')]/parent::*[1]/preceding-sibling::*[1]/text()").get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.property-page-single h6 ::text').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.xpath("//*[contains(@class,'mb-30')]/following-sibling::p/text()").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.xpath("//img[contains(@class,'imgsizel')]/@src").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.xpath("//*[contains(text(),'Agent')]/following-sibling::div[1]/div"):
                agent1 = response.xpath("//*[contains(text(),'Agent')]/following-sibling::div[1]/div")[0]
                agent1_name = agent1.css('.text-inherit ::text').get('').strip()
                if agent1_name:
                    ...
                    item['Field20'] = agent1.xpath(".//*[contains(@class,'fa-phone')]/parent::*/text()").get('').strip().replace('mailto:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".media-img img ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.xpath("//*[contains(text(),'Agent')]/following-sibling::div[1]/div").getall()) > 1:
                agent2 = response.xpath("//*[contains(text(),'Agent')]/following-sibling::div[1]/div")[1]
                agent2_name = agent2.css('.text-inherit ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field25'] = agent2.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:', '')
                    item['Field26'] = agent2.xpath(".//*[contains(@class,'fa-phone')]/parent::*/text()").get('').strip().replace('mailto:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".media-img img ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.xpath("//*[contains(text(),'Agent')]/following-sibling::div[1]/div").getall()) > 2:
                agent3 = response.xpath("//*[contains(text(),'Agent')]/following-sibling::div[1]/div")[2]
                agent3_name = agent3.css('.text-inherit ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field30'] = agent3.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:', '')
                    item['Field31'] = agent3.xpath(".//*[contains(@class,'fa-phone')]/parent::*/text()").get('').strip().replace('mailto:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".media-img img ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
