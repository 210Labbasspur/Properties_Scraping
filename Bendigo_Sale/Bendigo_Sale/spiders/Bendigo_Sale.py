...
class Bendigo_Sale(scrapy.Spider):
    name = "Bendigo_Sale"
    url = 'https://www.bendigorealestate.com.au/wp-admin/admin-ajax.php'
    prefix = "https://www.bendigorealestate.com.au"
    headers = {
    ...
    'atts[property_type]': 'House,Townhouse,Unit,Block of Units',
    ...
    def __init__(self, *args, **kwargs):
        super(Bendigo_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2663)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Bendigo Real Estate'
            ...
            item['Field9'] = response.xpath("//*[contains(text(),'Price')]/following-sibling::*[1]/text()").get('').strip().replace('$','').replace(',','').replace(' ','')
            item['Field12'] = f"{property.get('headline','')} {property.get('description','')}".replace('\n','').replace('\t','').replace('\r','')

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.css(".slider-image ::attr(href)").getall()
            images = []
            ...
            if response.css(".listing-agent"):
                ...
                agent1_name = agent1.css('.agent-name p ::text').get('').strip()
                if agent1_name:
            ...
            if len(response.css(".listing-agent").getall()) > 1:
                ...
                agent2_name = agent2.css('.agent-name p ::text').get('').strip()
                if agent2:
            ...
            if len(response.css(".listing-agent").getall()) > 2:
                ...
                agent3_name = agent3.css('.agent-name p ::text').get('').strip()
                if agent3:
            ...
            feature_count = 58
            for feature in response.css(".list-unstyled li"):   ##  first 4 features are ID, bed, bath etc.
