...
class Meeks_Sold(scrapy.Spider):
    name = 'Meeks_Sold'
    prefix = 'https://www.meeksrealestate.com.au'
    url = "https://www.meeksrealestate.com.au/recently-sold"
    headers = {
        'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        # 'cookie': 'eagle_popup=Lead_generator; _eagle_session=%3D%3D--9cefc69eb147eb9ada00573dd98184abd97512df; _eagle_session=--bcea73a925df8464de7c8ecd8f4b72072a86cab0',
        'priority': 'u=0, i',
    ...
    def parse(self, response):
        for property in response.css(".content-wrap .meta-list a"):
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field6'] = response.xpath("//*[contains(text(),'Bedrooms')]/parent::li[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(text(),'Bathroom')]/parent::li[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(text(),'Garages')]/parent::li[1]/text()").get('').strip()

            sold_price = response.css('div.price ::text').get('').strip().replace(' ','')
            if re.search(r'\d', sold_price):
                item['Field9'] = (sold_price.strip()).replace('$','')   # sold_price

            description = response.css('.large-8 .section p ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ').replace('What is ?', '') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess = response.css(".swipebox ::attr(href)").getall()
            new_img_urls = []
            ...
            '''          AGENTS          '''
            if response.css(".padding30 .card-head"):
                agent1 = response.css(".padding30 .card-head")[0]
                ...
                if agent1:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images = agent1.css("img.photo ::attr(src)").get('').strip()
                ...
                if len(response.css(".padding30 .card-head").getall()) > 1:
                    agent2 = response.css(".padding30 .card-head")[1]
                    ...
                    if agent2:
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent2.css("img.photo ::attr(src)").get('').strip()
                ...
                if len(response.css(".padding30 .card-head").getall()) > 2:
                    agent3 = response.css(".padding30 .card-head")[2]
                    agent3_name = agent3.css('.title ::text').get('').strip()
                    if agent3:
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent3.css("img.photo ::attr(src)").get('').strip()
