...
class NashVille_Rent(scrapy.Spider):
    name = "NashVille_Rent"
    url = "https://www.nashvilleproperty.com.au/ajax/residential-proprties.php?data%5Bpage%5D={}&data%5Bstatus%5D=current&data%5BsaleOrLease%5D=LEASE&data%5BeaglePropertyId%5D="
    prefix = "https://www.nashvilleproperty.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(NashVille_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        ...
        if json.loads(response.text):
            ...
            for each_url in data:
                url_id = each_url.get('propertyId','')
                each_url = f'https://www.nashvilleproperty.com.au/residential-article.php?p={url_id}'
            ...
            for each_url in data:
                ...
                self.count += 1
                yield response.follow(each_url, headers=self.headers, callback=self.detail_parse, meta={'url_id':url_id})
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3316)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'NASHVILLE Property'

            property_street = response.css('.headline ::text').get('').strip()  # property_street
            ...
            item['Field6'] = response.xpath("//*[contains(text(),'BED')]/text()").get('').strip().replace('BED','').replace(' ','')
            item['Field7'] = response.xpath("//*[contains(text(),'BATH')]/text()").get('').strip().replace('BED','').replace(' ','')
            item['Field8'] = response.xpath("//*[contains(text(),'CAR')]/text()").get('').strip().replace('BED','').replace(' ','')

            # price = response.css('.xxxxxxxxxxxxxxxxxxx ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            ...
            item['Field33'] = response.meta['url_id']
