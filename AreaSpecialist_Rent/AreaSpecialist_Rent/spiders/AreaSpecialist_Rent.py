...
class AreaSpecialist_Rent(scrapy.Spider):
    name = 'AreaSpecialist_Rent'
    prefix = 'https://www.areaspecialist.com.au'
    url = ("https://www.areaspecialist.com.au/pages/real-estate/results?widgetKey=224993&context=listings&returnAs=markup&"
           "listing_sale_method=lease&display_sale_method=RENT&pg={}")
    headers = {
    ...
    count = 1
    db = 'AreaSpecialist'
    collection = 'AreaSpecialist_Rent'
    ...
    def parse(self, response):
        for property in response.xpath("//a[contains(@class,'card-wrapper')]"):
        ...
        if response.xpath("//*[contains(text(),'Load More')]"):
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'Area Specialist'

            property_add = response.css('.content .address ::text').getall()  # full_adddress
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'amenities')]/div[contains(text(),'Rest')]/text()").get('').strip().replace('Rest','').replace('s','')
            item['Field7'] = response.xpath("//*[contains(@class,'amenities')]/div[contains(text(),'Shower')]/text()").get('').strip().replace('Shower','').replace('s','')
            item['Field8'] = response.xpath("//*[contains(@class,'amenities')]/div[contains(text(),'Park')]/text()").get('').strip().replace('Park','').replace('s','')

            # sold_price = response.css('.content .price ::text').get('').strip().replace('Sold for','').replace('-','')
            # if re.search(r'\d', sold_price):
            #     item['Field9'] = (sold_price.strip()).replace('$','')   # sold_price

            description = response.css('.description .listing-description ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess = response.css(".lightbox-image ::attr(src)").getall()
            new_img_urls = []
            ...
            '''          AGENTS          '''
            if response.css(".agent-list"):
                agent1 = response.css(".agent-list")[0]
                if agent1.css('.agent-logo-name-image ::attr(alt)').get('').strip():
                    agent1_name = agent1.css('.agent-logo-name-image ::attr(alt)').get('').strip()
                elif agent1.css('.agent-name ::text').get('').strip():
                ...
                if agent1_name:
                    ...
                    item['Field19'] = agent1.xpath(".//*[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:','')
                    agent1_phone = agent1.css(".agent-mobile ::text").get('').strip()
                    item['Field20'] = agent1_phone.strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images = agent1.css(".agent-img ::attr(src)").get('').strip()
                ...
                if len(response.css(".agent-list").getall()) > 1:
                    agent2 = response.css(".agent-list")[1]
                    if agent2.css('.agent-logo-name-image ::attr(alt)').get('').strip():
                        agent2_name = agent2.css('.agent-logo-name-image ::attr(alt)').get('').strip()
                    elif agent2.css('.agent-name ::text').get('').strip():
                    ...
                    if agent2:
                        ...
                        item['Field25'] = agent2.xpath(".//*[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:', '')
                        agent2_phone = agent2.css(".agent-mobile ::text").get('').strip()
                        item['Field26'] = agent2_phone.strip()
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent2.css(".agent-img ::attr(src)").get('').strip()
                ...
                if len(response.css(".agent-list").getall()) > 2:
                    agent3 = response.css(".agent-list")[2]
                    if agent3.css('.agent-logo-name-image ::attr(alt)').get('').strip():
                        agent3_name = agent3.css('.agent-logo-name-image ::attr(alt)').get('').strip()
                    elif agent3.css('.agent-name ::text').get('').strip():
                    ...
                    if agent3:
                        ...
                        item['Field30'] = agent3.xpath(".//*[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:', '')
                        agent3_phone = agent3.css(".agent-mobile ::text").get('').strip()
                        item['Field31'] = agent3_phone.strip()
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent3.css(".agent-img ::attr(src)").get('').strip()
            ...
            item['Field35'] = response.css(".property-item:nth-child(1) .white-body-15px-11477-1357 ::text").get('').strip()
            ...
            if land_area:
                ...
                item['Field36'] = response.xpath("//*[contains(text(),'Building Size')]/following-sibling::span[1]/text()").get('').replace('m²','').replace(' ','')   # land_area
