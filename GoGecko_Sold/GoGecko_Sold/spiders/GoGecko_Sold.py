...
class GoGecko_Sold(scrapy.Spider):
    name = "GoGecko_Sold"

    url = "https://www.gogecko.com.au/sell/recently-sold/"
    prefix = "https://www.gogecko.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(GoGecko_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2083)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Go Gecko'

            property_street = response.css('.elementor-heading-title ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.xxxxxxxx ::text').getall() if e.strip())  # property_suburb
            ...
            price = response.css('span.page-price ::text').get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','')
            if price:
            ...
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".elementor-element-bb7f98a p ::text").getall())  # description
