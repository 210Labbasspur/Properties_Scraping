...
class DCCO_Sold(scrapy.Spider):
    name = 'DCCO_Sold'
    prefix = 'https://www.dcco.com.au'
    url = "https://www.dcco.com.au/search/?show_in_residential=true&show_in_sale=true&sold=1&order_by=listing_date&order=desc&radius=50&suburb-box=&min_price=&max_price=&min_bedrooms=&min_bathrooms=&min_carparks=&pageno=1"
    headers = {
        'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        # 'cookie': '_gcl_au=1.1.217550858.1713706639; __Secure-PHPSESSID=65tqs1lacevq4i554eat1f281u; _ga=GA1.3.1140201821.1713706640; _gid=GA1.3.1017646233.1714045692; _gat_gtag_UA_19296238_44=1; rl_visitor_history=6a6ac688-8b01-4b19-a3e1-7d3b91a8e007; sifi_user_id=undefined; _ga_T72KKSM93N=GS1.1.1714045689.4.0.1714045705.0.0.0; arp_scroll_position=5683',
        'priority': 'u=0, i',
        # 'referer': 'https://www.dcco.com.au/',
    ...
    def parse(self, response):
        for property in response.css(".card-image a"):
        ...
        if 'page' in response.xpath("//*[contains(@class,'fa-chevron-right')]/parent::a[1]/@href").get('').strip():
            next_page = response.xpath("//*[contains(@class,'fa-chevron-right')]/parent::a[1]/@href").get('').strip()
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'Davidson Cameron & Co'

            property_add = response.css('.text-blue ::text').getall()  # full_adddress
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'bed')]/parent::a[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'bath')]/parent::a[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'parking')]/parent::a[1]/text()").get('').strip()

            sold_price = response.css('.text-orange ::text').get('').strip().replace('Sold','').replace('for','')
            if sold_price:
                item['Field9'] = (sold_price.strip()).replace('$','').replace(' ','')   # sold_price

            description = response.css('div.description-text ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            property_Imagess = response.css(".gallery-image ::attr(href)").getall()
            new_img_urls = []
            for url in property_Imagess:
            ...
            '''          AGENTS          '''
            if response.css(".agents-profile"):
                agent1 = response.css(".agents-profile")[0]
                agent1_name = agent1.css('.agents-name ::text').get('').strip()
                if agent1:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images = agent1.css(".image ::attr(data-src)").get('').strip()
                ...
                if len(response.css(".agents-profile").getall()) > 1:
                    agent2 = response.css(".agents-profile")[1]
                    agent2_name = agent2.css('.agents-name ::text').get('').strip()
                    if agent2:
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent2.css(".image ::attr(data-src)").get('').strip()
                ...
                if len(response.css(".agents-profile").getall()) == 3:
                    agent3 = response.css(".agents-profile")[2]
                    agent3_name = agent3.css('.agents-name ::text').get('').strip()
                    if agent3:
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent3.css(".image ::attr(data-src)").get('').strip()
            ...
            item['Field33'] = response.css("strong ::text").get('').strip().replace('ID','').replace(':','')
            item['Field35'] = response.css(".header-subtitel ::text").get('').strip().replace('Property','').replace('-','')
            land_area = response.xpath("//*[contains(@class,'land-size')]/parent::a[1]/text()").get('').strip()
            if land_area:
