...
class Hodges_Rent(scrapy.Spider):
    name = "Hodges_Rent"

    url = "https://www.hodges.com.au/properties-for-lease/"
    prefix = "https://www.hodges.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Hodges_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    # def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css("a.card__link ::attr(href)").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://www.hodges.com.au{each_url}'
        ...
        for property_url in all_links:
            property_url = f'https://www.hodges.com.au{property_url}'
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2990)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Hodges'

            property_street = response.css('.property-header__left h1 ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.xxxxxxxxxxxxxx ::text').getall() if e.strip())  # property_suburb
            property_address = f"{property_street} {property_suburb}"
            item['Field5'] = property_address  # full_adddress

            item['Field6'] = response.xpath("//*[contains(@alt,'Beds')]/preceding-sibling::span[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@alt,'Baths')]/preceding-sibling::span[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@alt,'Cars')]/preceding-sibling::span[1]/text()").get('').strip()

            price = response.css('.property-header__price ::text').get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price
            ...
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".property-content__right ::text").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.xpath("//*[@class='slick-slide']/div/img/@src").getall()
            images = []
            images.append(response.xpath("//*[@class='slick-slide slick-active slick-current']/div/img/@src").get('').strip())
