...
class SophieCarter_Sale(scrapy.Spider):
    name = "SophieCarter_Sale"
    url = 'https://www.sophiecarter.com/wp-admin/admin-ajax.php'
    prefix = "https://www.sophiecarter.com"
    headers = {
    'accept': '*/*',
    'accept-language': 'en-US,en;q=0.9',
    'content-type': 'application/x-www-form-urlencoded; charset=UTF-8',
    'cookie': '_ga=GA1.1.624384058.1737715487; _ga_JQKDNYR68S=GS1.1.1738299433.3.1.1738299443.0.0.0',
    # 'origin': 'https://www.sophiecarter.com',
    'priority': 'u=1, i',
    'referer': 'https://www.sophiecarter.com/selling/',
    ...
    def __init__(self, *args, **kwargs):
        super(SophieCarter_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3275)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Sophie Carter Exclusive Properties'
            ...
            item['Field8'] = ''.join(e.strip() for e in response.xpath("//*[contains(@class,'la-car')]/following-sibling::span[1]/text()").getall())

            # item['Field9'] = property.get('price','').replace('$','').replace(',','').replace(' ','')
            item['Field9'] = response.xpath("//*[contains(text(),'Price')]/following-sibling::div[1]/text()").get('').strip().replace('$','').replace(',','').replace(' ','')
            item['Field12'] = f"{property.get('headline','')} {property.get('description','')}".replace('\n','').replace('\t','').replace('\r','').replace('\uf0b7','')

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.css(".embed-responsive-item img.img-orientation ::attr(src)").getall()
            images = []
            ...
            if response.css(".listing-agent"):
                ...
                if agent1_name:
                    ...
                    item['Field19'] = agent1.css(".email ::text").get('').strip().replace('E:','')
                    item['Field20'] = agent1.css(".phone ::text").get('').strip().replace('P:','')
                    item['Field21'] = agent1.css(".mobile ::text").get('').strip().replace('M:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css(".listing-agent").getall()) > 1:
                ...
                if agent2:
                    ...
                    item['Field25'] = agent2.css(".email ::text").get('').strip().replace('E:','')
                    item['Field26'] = agent2.css(".phone ::text").get('').strip().replace('P:','')
                    item['Field26A'] = agent2.css(".mobile ::text").get('').strip().replace('M:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css(".listing-agent").getall()) > 2:
                ...
                if agent3:
                    ...
                    item['Field30'] = agent3.css(".email ::text").get('').strip().replace('E:','')
                    item['Field31'] = agent3.css(".phone ::text").get('').strip().replace('P:','')
                    item['Field31A'] = agent3.css(".mobile ::text").get('').strip().replace('M:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            feature_count = 58
            for feature in response.css(".description-container .row .col-md-6"):   ##  first 4 features are ID, bed, bath etc.
