...
class ReedandCo_Sold(scrapy.Spider):
    name = "ReedandCo_Sold"

    url = "https://reedandco.co/sold-property-noosa/"
    prefix = "https://reedandco.co"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(ReedandCo_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".property-list-item a ::attr(href)").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://reedandco.co{each_url}'
        ...
        for property_url in all_links:
            property_url = f'https://reedandco.co{property_url}'
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2263)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Reed & Co'

            property_street = response.css('.address ::text').get('').strip()  # property_street
            ...
            item['Field6'] = response.css(".bed div ::text").get('').strip()
            item['Field7'] = response.css(".bath div ::text").get('').strip()
            item['Field8'] = response.css(".car div ::text").get('').strip()

            price = response.css('.display-price ::text').get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','').replace('for','')
            if price:
                item['Field9'] = price.replace('SOLD','').replace('FOR','')
            ...
            prop_images = response.css(".popup-carousel-item img ::attr(src)").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.css(".agent-wrapper"):
                agent1 = response.css(".agent-wrapper")[0]
                agent1_name = agent1.css('.name-link strong ::text').get('').strip()
                if agent1_name:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".thumb-wrapper img ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.css(".agent-wrapper").getall()) > 1:
                agent2 = response.css(".agent-wrapper")[1]
                agent2_name = agent2.css('.name-link strong ::text').get('').strip()
                if agent2:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".thumb-wrapper img ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css(".agent-wrapper").getall()) > 2:
                agent3 = response.css(".agent-wrapper")[2]
                agent3_name = agent3.css('.name-link strong ::text').get('').strip()
                if agent3:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".thumb-wrapper img ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            land_area = response.xpath('//*[contains(@class,"area")]/div/text()').get('').strip().replace('Land','').replace('is','').replace('m²','')
            if land_area:
