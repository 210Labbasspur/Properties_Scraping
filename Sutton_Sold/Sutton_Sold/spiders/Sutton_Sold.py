...
class Sutton_Sold(scrapy.Spider):
    name = "Sutton_Sold"

    url = "https://www.suttonnationwiderealty.com.au/sell/sold-properties/page/{}/"
    prefix = "https://www.suttonnationwiderealty.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Sutton_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css("a.centerimage ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3306)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Sutton Nationwide Realty'

            property_street = response.css('.street strong ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.suburb strong ::text').getall() if e.strip())  # property_suburb
            ...
            price = response.css('#property_price ::text').get('').strip().replace('Undisclosed','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price
            ...
            '''          AGENTS          '''
            if response.css(".grid-25-75"):
                agent1 = response.css(".grid-25-75")[0]
                ...
                if agent1_name:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".fit-height ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.css(".grid-25-75").getall()) > 1:
                agent2 = response.css(".grid-25-75")[1]
                ...
                if agent2:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".fit-height ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css(".grid-25-75").getall()) > 2:
                agent3 = response.css(".grid-25-75")[2]
                ...
                if agent3:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".fit-height ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
