...
class Dougherty_Rent(scrapy.Spider):
    name = "Dougherty_Rent"

    url = "https://doughertyproperty.com.au/pages/real-estate/results?widgetKey=218643&context=listings&returnAs=markup&listing_category=Residential&listing_sale_method=Lease&display_sale_method=RENT&pg={}"
    # url = "https://doughertyproperty.com.au/pages/real-estate/results?widgetKey=218643&context=listings&returnAs=markup&listing_sale_method=sale&display_sale_method=SOLD&status=sold&pg=1"
    prefix = "https://doughertyproperty.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Dougherty_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".listing-link ::attr(href)").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://doughertyproperty.com.au{each_url}'
        ...
        next_page = response.xpath("//*[contains(text(),'Load More')]/text()").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(1629)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Dougherty Property'

            property_street = response.css('.top-details  .heading  ::text').get('').strip()  # property_street
            ...
            item['Field6'] = ''.join(e.strip() for e in response.xpath("//*[contains(@class,'value-wrapper bed')]/text()").getall())
            item['Field7'] = ''.join(e.strip() for e in response.xpath("//*[contains(@class,'value-wrapper bath')]/text()").getall())
            item['Field8'] = ''.join(e.strip() for e in response.xpath("//*[contains(@class,'value-wrapper car')]/text()").getall())

            price = response.css('.price-heading ::text').get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','').replace('for','')
            if price:
                item['Field9'] = price

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.description-heading ::text').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".full-description ::text").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.xpath("//*[contains(@data-lightbox,'gallery-')]/@href").getall()
            images = []
            ...
            if response.css(".agent"):
                ...
                if agent1_name:
                    ...
                    item['Field20'] = agent1.css(".agent-mobile ::text").get('').strip().replace('tel:','')
                    item['Field21'] = agent1.css(".agent-phone ::text").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".agent-image ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.css(".agent").getall()) > 1:
                ...
                if agent2:
                    ...
                    item['Field26'] = agent2.css(".agent-mobile ::text").get('').strip().replace('tel:','')
                    item['Field26A'] = agent2.css(".agent-phone ::text").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".agent-image ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css(".card-body").getall()) > 2:
                agent3 = response.css(".card-body")[2]
                ...
                if agent3:
                    ...
                    item['Field31'] = agent3.css(".agent-mobile ::text").get('').strip().replace('tel:','')
                    item['Field31A'] = agent3.css(".agent-phone ::text").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".agent-image ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            floor_area = response.xpath("//*[contains(text(),'Internal Size')]/following-sibling::span[1]/text()").get('').strip()
            if floor_area:
