...
class Ballarat_Sold(scrapy.Spider):
    name = "Ballarat_Sold"

    url = "https://www.ballaratrealestate.com.au/sell/our-sold-results/"
    prefix = "https://www.ballaratrealestate.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Ballarat_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2560)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Ballarat Real Estate'
            ...
            item['Field6'] = ''.join(e.strip() for e in response.css(".bedroom ::text").getall())
            item['Field7'] = ''.join(e.strip() for e in response.css(".bathroom ::text").getall())
            item['Field8'] = ''.join(e.strip() for e in response.css(".carspace ::text").getall())

            price = response.css('.price__label ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            ...
            prop_images = response.xpath("//*[contains(@class,'item-image')]//img/@src").getall()
            images = []
            ...
            land = ''.join(e.strip() for e in response.css(".land-size ::text").getall())
