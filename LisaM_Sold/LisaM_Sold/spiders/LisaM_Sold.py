...
class LisaM_Sold(scrapy.Spider):
    name = 'LisaM_Sold'
    prefix = 'https://lmproperty.au'
    url = 'https://lmproperty.au/wp-admin/admin-ajax.php'
    headers = {
        ...
        'Referer': 'https://lmproperty.au/sold/',
        'X-Requested-With': 'XMLHttpRequest',
        'sec-ch-ua-platform': '"Windows"',
    }
    data = {
        'location': '', 'bedrooms': '', 'bathrooms': '', 'details_3': '', 'keyword': '', 'min': '', 'max': '',
        'listing-category': '', 'action': 'wpcasa_search_ajax_paginated', 'listing-type': '',
        'orderby': '','order': '',
        'offset': '1',
        'listing-category': 'sold',
    }
    # data = [
    #     ('location', ''),        ('bedrooms', ''),        ('bathrooms', ''),        ('details_3', ''),        ('keyword', ''),
    #     ('min', ''),        ('max', ''),        ('listing-category', ''),        ('action', 'wpcasa_search_ajax_paginated'),
    #     ('listing-type', ''),        ('orderby', ''),        ('order', ''),
    #     ('offset', '1'),
    #     ('listing-category', 'sold'),
    # ]
    ...
    def parse(self, response):
        ...
        selector = Selector(text=data['listingsHtml'])
        for property in selector.xpath("//*[contains(@href,'https://lmproperty.au/listing')]"):
            property_no += 1
        ...
        total_properties = data['found_posts']
        if property_no < total_properties:
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'Lisa Macklin'

            property_add = response.css('#property-details h1 ::text').getall()  # full_adddress
            ...
            item['Field6'] = response.xpath("//span[contains(text(),'Bed')]/following-sibling::span[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//span[contains(text(),'Bath')]/following-sibling::span[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//span[contains(text(),'Car')]/following-sibling::span[1]/text()").get('').strip()

            sold_price = response.css(".listing-price-value ::text").get('').strip().replace('Sold','').replace('-','')
            if re.search(r'\d', sold_price):
                item['Field9'] = (sold_price.strip()).replace('$','')   # sold_price

            description = response.css(".single-listing-content ::text").getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess = response.css(".listing-featured-frame ::attr(data-src)").getall()
            new_img_urls = []
            ...
            if response.css(".agent-card"):
                ...
                agent1_name = agent1.css('h2 ::text').get('').strip()
                if agent1:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field18'] = agent1.css(".agent-position ::text").get('').strip()  # agent_full_name_1
                    agent1_phone = agent1.xpath(".//*[contains(@href,'tel:')]/@href").get('').strip().replace('tel:','')
                    item['Field20'] = agent1_phone.strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images_style = agent1.css(".agent-card-image ::attr(style)").get('').strip()
                ...
                if len(response.css(".agent-card").getall()) > 1:
                    ...
                    agent2_name = agent2.css('h2 ::text').get('').strip()
                    if agent2:
                        item['Field23'] = agent2_name.strip()
                        item['Field24'] = agent2.css(".agent-position ::text").get('').strip()  # agent_full_name_1
                        agent2_phone = agent2.xpath(".//*[contains(@href,'tel:')]/@href").get('').strip().replace('tel:', '')
                        item['Field26'] = agent2_phone.strip()
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images_style = agent2.css(".agent-card-image ::attr(style)").get('').strip()
                        Agent_Images = re.search(r'url\((.*?)\)', Agent_Images_style).group(1)
                ...
                if len(response.css(".agent-card").getall()) > 2:
                    ...
                    agent3_name = agent3.css('h2 ::text').get('').strip()
                    if agent3:
                        item['Field28'] = agent3_name.strip()
                        item['Field29'] = agent3.css(".agent-position ::text").get('').strip()  # agent_full_name_1
                        agent3_phone = agent3.xpath(".//*[contains(@href,'tel:')]/@href").get('').strip().replace('tel:', '')
                        item['Field31'] = agent3_phone.strip()
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images_style = agent3.css(".agent-card-image ::attr(style)").get('').strip()
                        Agent_Images = re.search(r'url\((.*?)\)', Agent_Images_style).group(1)
            ...
            feature_count = 58
            for feature in response.xpath("//h2[contains(text(),'Features')]/following-sibling::ul[1]/li"):
