...
class Wyndham_Sold(scrapy.Spider):
    name = "Wyndham_Sold"

    url = "https://wyndhamre.com.au/search/?show_in_sale=true&sold=1&sstyle=grid&pageno=1"
    prefix = "https://wyndhamre.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Wyndham_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".property-info a ::attr(href)").getall()
        links = []
        ...
        next_page = response.xpath("//*[contains(@class,'fa fa-chevron-right')]/parent::a/@href").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3030)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Wyndham Real Estate'

            property_street = response.css('.xxxxxxxxxxxxxx ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.page-title ::text').getall() if e.strip())  # property_suburb
            property_address = f"{property_street} {property_suburb}"
            item['Field5'] = property_address  # full_adddress

            item['Field6'] = response.css(".beds strong ::text").get('').strip()
            item['Field7'] = response.css(".baths strong ::text").get('').strip()
            item['Field8'] = response.css(".parking strong ::text").get('').strip()

            price = response.css('.price span ::text').get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price
            ...
            prop_images = response.css(".item #d_image ::attr(src)").getall()
            images = []
            ...
            if response.css(".agent"):
                ...
                if agent1_name:
                    ...
                    item['Field19'] = agent1.xpath(".//*[contains(@class,'fa fa-envelope')]/parent::a/text()").get('').strip().replace('mailto:','')
                    item['Field20'] = agent1.xpath(".//*[contains(@class,'fa fa-phone')]/parent::li/text()").get('').strip().replace('mailto:','')
                    item['Field21'] = agent1.xpath(".//*[contains(@class,'fa fa-mobile')]/parent::li/text()").get('').strip().replace('mailto:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".col-md-2 img ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.css(".agent").getall()) > 1:
                ...
                if agent2:
                    ...
                    item['Field25'] = agent2.xpath(".//*[contains(@class,'fa fa-envelope')]/parent::a/text()").get('').strip().replace('mailto:','')
                    item['Field26'] = agent2.xpath(".//*[contains(@class,'fa fa-phone')]/parent::li/text()").get('').strip().replace('mailto:','')
                    item['Field26A'] = agent2.xpath(".//*[contains(@class,'fa fa-mobile')]/parent::li/text()").get('').strip().replace('mailto:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".col-md-2 img ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css(".agent").getall()) > 2:
                ...
                if agent3:
                    ...
                    item['Field30'] = agent3.xpath(".//*[contains(@class,'fa fa-envelope')]/parent::a/text()").get('').strip().replace('mailto:','')
                    item['Field31'] = agent3.xpath(".//*[contains(@class,'fa fa-phone')]/parent::li/text()").get('').strip().replace('mailto:','')
                    item['Field31A'] = agent3.xpath(".//*[contains(@class,'fa fa-mobile')]/parent::li/text()").get('').strip().replace('mailto:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".col-md-2 img ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            land_area = response.xpath('//*[contains(text(),"Area")]/strong/text()').get('').strip().replace('Land','').replace('is','').replace('m','')
            if land_area:
