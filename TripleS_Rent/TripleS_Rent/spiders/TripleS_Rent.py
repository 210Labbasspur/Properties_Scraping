...
class TripleS_Rent(scrapy.Spider):
    name = 'TripleS_Rent'
    prefix = 'https://www.triplesrental.com.au'
    url = "https://www.triplesrental.com.au/tenants/home-for-rent"
    headers = {
        'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        'cache-control': 'max-age=0',
        # 'cookie': 'VID=7601; _gid=GA1.3.1806152356.1714076935; PROTOCID=9p5fu09ionkpnh10lpcf9b7u26; _gat_gtag_UA_100084845_60=1; arp_scroll_position=4061; _ga_1MDKS5YN5L=GS1.1.1714131349.6.1.1714131369.0.0.0; _ga_WTFV1VQVK5=GS1.1.1714131350.6.1.1714131370.0.0.0; _ga=GA1.1.820164410.1713705543',
        'priority': 'u=0, i',
    ...
    def parse(self, response):
        for property in response.css(".box_title a"):
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'Triple S Property'

            property_add = response.css('.span8 .box_sub_title ::text').getall()  # full_adddress
            ...
            item['Field6'] = response.xpath("//*[contains(text(),'BED')]/following-sibling::div[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(text(),'BATH')]/following-sibling::div[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(text(),'CAR')]/following-sibling::div[1]/text()").get('').strip()

            # sold_price = response.css('h3 .muted ::text').get('').strip().replace(' ','')
            # if sold_price:
            #     item['Field9'] = (sold_price.strip()).replace('$','')   # sold_price

            description = response.css('#tabs-1 pre ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ').replace('\n', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            # Agent_Imagess = response.css(".bg-cover link ::attr(href)").getall()
            Agent_Imagess = response.xpath("//img[contains(@data-large,'https://agentboxcdn.com.au/')]/@alt").getall()
            new_img_urls = []
            ...
            '''          AGENTS          '''
            if response.css(".prop_contact .span4"):
                agent1 = response.css(".prop_contact .span4")[0]
                agent1_name = agent1.css('.prop_contact_title ::text').get('').strip()
                if agent1:
                    ...
                    agent1_mobile = agent1.xpath(".//*[contains(@class,'fa-mobile')]/parent::div[1]/following-sibling::div[1]/text()").get('').strip()
                    item['Field20'] = agent1_mobile.strip()
                    agent1_phone = agent1.xpath(".//*[contains(@class,'fa-phone')]/parent::div[1]/following-sibling::div[1]/text()").get('').strip()
                    item['Field21'] = agent1_phone.strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images = self.prefix + agent1.css(".prop_contact_pic img ::attr(src)").get('').strip()
                ...
                if len(response.css(".prop_contact .span4").getall()) > 1:
                    agent2 = response.css(".prop_contact .span4")[1]
                    agent2_name = agent2.css('.prop_contact_title ::text').get('').strip()
                    if agent2:
                        ...
                        agent2_mobile = agent2.xpath(".//*[contains(@class,'fa-mobile')]/parent::div[1]/following-sibling::div[1]/text()").get('').strip()
                        item['Field26'] = agent2_mobile.strip()
                        agent2_phone = agent2.xpath(".//*[contains(@class,'fa-phone')]/parent::div[1]/following-sibling::div[1]/text()").get('').strip()
                        item['Field26A'] = agent2_phone.strip()
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = self.prefix + agent2.css(".prop_contact_pic img ::attr(src)").get('').strip()
                ...
                if len(response.css(".prop_contact .span4").getall()) > 2:
                    agent3 = response.css(".prop_contact .span4")[2]
                    agent3_name = agent3.css('.prop_contact_title ::text').get('').strip()
                    if agent3:
                        ...
                        agent3_mobile = agent3.xpath(".//*[contains(@class,'fa-mobile')]/parent::div[1]/following-sibling::div[1]/text()").get('').strip()
                        item['Field31'] = agent3_mobile.strip()
                        agent3_phone = agent3.xpath(".//*[contains(@class,'fa-phone')]/parent::div[1]/following-sibling::div[1]/text()").get('').strip()
                        item['Field31A'] = agent3_phone.strip()
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = self.prefix + agent3.css(".prop_contact_pic img ::attr(src)").get('').strip()
            ...
            # item['Field35'] = response.xpath("//*[contains(text(),'Property ID')]/following-sibling::div[1]/text()").get('').strip().replace('Property ID:','')
            ...
            # feature_count = 58
            # features = response.xpath("//h4[contains(text(),'Property Features')]/following-sibling::ul[1]/li")
            # for feature in features:
            #     if 'Air Condition' in feature.css('span::text').get('').strip():
            #         item['Field52'] = ' '.join(element.strip() for element in feature.css('span::text').getall())  # description
            #     else:
            #         item[f'Field{feature_count}'] = ' '.join(element.strip() for element in feature.css('span::text').getall())  # description
            #         feature_count += 1
