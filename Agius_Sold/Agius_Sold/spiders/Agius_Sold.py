...
class Agius_Sold(scrapy.Spider):
    name = 'Agius_Sold'
    prefix = 'https://www.agiuspropertygroup.com.au'
    url = 'https://www.agiuspropertygroup.com.au/wp-admin/admin-ajax.php'
    headers = {
        'accept': '*/*',
        'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        'content-type': 'application/x-www-form-urlencoded; charset=UTF-8',
        # 'cookie': '_ga=GA1.1.1547038515.1714475128; _ga_28B9Y1VT02=GS1.1.1714682808.5.1.1714682878.0.0.0; arp_scroll_position=2526',
        'origin': 'https://www.agiuspropertygroup.com.au',
        'priority': 'u=1, i',
        # 'referer': 'https://www.agiuspropertygroup.com.au/sell/residential-sold/',
    ...
    data = {
        'atts[list]': 'sold',
        'atts[multilist]': '',        'atts[layout]': '',        'atts[per_page]': '20',        'atts[sur_suburbs]': '0',
        ...
        'atts[center_latlng]': '',        'atts[sur_suburbs_radius]': '10',        'atts[max_page]': 'false',        'atts[map_zoom]': '11',
        'atts[row_col_class]': 'row row-cols-xl-3 row-cols-lg-2 row-cols-1',        'atts[map]': 'map_canvas',        'atts[map_load_all_marker]': '0',
        'atts[map_callback]': 'ap_realty.searchResultsMapCallback',        'atts[map_attribute_cluster]': '0',
        'atts[map_is_visible]': '1',        'atts[property_type_column]': '1,2,3',        'atts[content_first]': '0',        'atts[auth]': '0',
        'atts[auth_type]': 'content',        'atts[auth_message]': 'Sorry, You Are Not Allowed to Access This Page',
        'atts[include_private_listing]': '0',        'atts[map_attribute_loadCallback]': 'ap_realty.searchResultsMapCallback',
        'atts[type]': 'ResidentialSale',        'atts[load_more_automatic]': 'true',
        'atts[sort]': 'sold_date_desc',        'per_page': '20',        'load_more': 'true',        'selectorMap': '#map_canvas',
        'selector': '#ap-listing-search-results',        'sort': 'sold_date_desc',        'action': 'property_search_results',
        'current_page': '1',
    }

    count = 1
    db = 'Agius'
    collection = 'Agius_Sold'
    ...
    def parse(self, response):
        ...
        for property in data['data']['props']:
            ...
            mini_data['car'] = int(property.get('garage')) + int(property.get('carport'))+ int(property.get('off_street_park'))
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'Agius Property Group'
            ...
            Agent_Imagess = response.xpath("//*[contains(@class,'embed-responsive embed-responsive-3by2')]/picture/source/img/@src").getall()
            new_img_urls = []
            ...
            if response.css(".listing-agent"):
                ...
                if agent1:
                    ...
                    agent1_phone = agent1.xpath(".//*[contains(@class,'las la-mobile')]/parent::a[contains(@href,'tel:')]/@href").get('').strip().replace('tel:','')  # description
                    item['Field20'] = agent1_phone.strip()  # description
                    agent1_mobile = agent1.xpath(".//*[contains(@class,'las la-phone')]/parent::a[contains(@href,'tel:')]/@href").get('').strip().replace('tel:','')  # description
                    item['Field21'] = agent1_mobile.strip()  # description
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                ...
                if len(response.css(".listing-agent").getall()) > 1:
                    ...
                    if agent2:
                        ...
                        agent2_phone = agent2.xpath(".//*[contains(@class,'las la-mobile')]/parent::a[contains(@href,'tel:')]/@href").get('').strip().replace('tel:', '')  # description
                        item['Field26'] = agent2_phone.strip()  # description
                        agent2_mobile = agent2.xpath(".//*[contains(@class,'las la-phone')]/parent::a[contains(@href,'tel:')]/@href").get('').strip().replace('tel:', '')  # description
                        item['Field26A'] = agent2_mobile.strip()  # description
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                ...
                if len(response.css(".listing-agent").getall()) > 2:
                    ...
                    if agent3:
                        ...
                        agent3_phone = agent3.xpath(".//*[contains(@class,'las la-mobile')]/parent::a[contains(@href,'tel:')]/@href").get('').strip().replace('tel:', '')  # description
                        item['Field31'] = agent3_phone.strip()  # description
                        agent3_mobile = agent3.xpath(".//*[contains(@class,'las la-phone')]/parent::a[contains(@href,'tel:')]/@href").get('').strip().replace('tel:', '')  # description
                        item['Field31A'] = agent3_mobile.strip()  # description
                        '''         Uploading Images on Wasabi S3 Bucket            '''
