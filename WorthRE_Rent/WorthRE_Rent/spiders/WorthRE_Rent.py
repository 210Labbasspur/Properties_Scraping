...
class WorthRE_Rent(scrapy.Spider):
    name = "WorthRE_Rent"

    url = "https://worthrealestate.com.au/properties-for-rent/"
    prefix = "https://worthrealestate.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(WorthRE_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.xpath("//*[contains(@class,'location')]/a/@href").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://worthrealestate.com.au{each_url}'
        ...
        for property_url in all_links:
            property_url = f'https://worthrealestate.com.au{property_url}'
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3258)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Worth Real Estate'

            property_street = response.css('.xxxxxxxxxxxxxx ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.pull-left .address ::text').getall() if e.strip())  # property_suburb
            property_address = f"{property_street} {property_suburb}"
            item['Field5'] = property_address  # full_adddress

            item['Field6'] = ''.join(e.strip() for e in response.xpath("//*[contains(text(),'Bedroom')]/parent::li[1]/text()").getall())
            item['Field7'] = ''.join(e.strip() for e in response.xpath("//*[contains(text(),'Bathroom')]/parent::li[1]/text()").getall())
            item['Field8'] = ''.join(e.strip() for e in response.xpath("//*[contains(text(),'Garage')]/parent::li[1]/text()").getall())

            price = ''.join(e.strip() for e in response.xpath("//*[contains(text(),'Price')]/parent::li[1]/text()").getall())
            price = price.replace('Price','').replace('Sold','').replace('for','').replace('$','').replace(':','').replace(',','').replace(' ','')
            if price:
                item['Field9'] = price.replace('Offers','').replace('Over','')
            ...
            item['Field33'] = response.url.rstrip('/').split('/')[-1]
            item['Field35'] = ''.join(e.strip() for e in response.xpath("//*[contains(text(),'Property Type')]/parent::li[1]/text()").getall())
