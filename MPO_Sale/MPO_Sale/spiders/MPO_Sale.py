...
class MPO_Sale(scrapy.Spider):
    name = "MPO_Sale"

    url = 'https://www.mpo.net.au/forsale'
    ...
    def __init__(self, *args, **kwargs):
        super(MPO_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        json_text = response.xpath("//*[contains(text(),'commitToListingsStore')]/text()").get('').strip()
        # Regex pattern to extract the content between "/**/commitToSoldListingsStore({" and "});"
        pattern = r'\/\*\*\/commitToListingsStore\(\{(.*?)\}\);'
        ...
        if match:
            ...
            for each_url in data.get('data',[]):
                ...
                each_url = f'https://www.mpo.net.au/forsale/{url_slug}'
            ...
            for property in data.get('data',[]):
                ...
                property_url = f'https://www.mpo.net.au/forsale/{url_slug}'
