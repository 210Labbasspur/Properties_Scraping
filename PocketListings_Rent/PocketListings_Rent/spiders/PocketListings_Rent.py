...
class PocketListings_Rent(scrapy.Spider):
    name = "PocketListings_Rent"

    url = "https://pocketlistings.com.au/properties-for-lease/"
    prefix = "https://pocketlistings.com.au"
    headers = {
    ...
    count = 1
    db = 'PocketListings'
    collection = 'PocketListings_Rent'
    bucket_prefix = f'PL_{collection}'

    local_file_path = '/img'

    def __init__(self, *args, **kwargs):
        super(PocketListings_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".epl-archive-entry-image a::attr(href)").getall()
        links = []
        ...
        for property in response.css('.epl-archive-entry-image a'):
        ...
        next_page = response.css(".alignright a ::attr(href)").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2903)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Pocket Listings'
            property_street = response.css('.item-street ::text').get('').strip()  # full_adddress
            property_suburb = ' '.join(e.strip() for e in response.css('.entry-title .entry-title-sub ::text').getall() if e.strip())  # full_adddress
            property_address = f"{property_street} {property_suburb}"
            item['Field5'] = property_address  # full_adddress

            item['Field6'] = response.xpath("//*[contains(@title,'Bedrooms')]/following-sibling::div[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@title,'Bathrooms')]/following-sibling::div[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@title,'Parking Spaces')]/following-sibling::div[1]/text()").get('').strip()

            price = response.css('.pricing .page-price ::text').get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price
            ...
            prop_images = response.css(".gallery-icon a ::attr(href)").getall()
            images = []
            ...
            if response.css(".epl-author-box-container"):
                ...
                if agent1_name:
                    ...
                    item['Field18'] = agent1.css('.author-position .position ::text').get('').strip()
                    item['Field19'] =  agent1.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:','')
                    item['Field20'] = agent1.css(".author-contact .mobile ::text").get('').strip().replace('tel:','')
                    item['Field21'] = agent1.css(".author-contact-office-phone .office-phone ::text").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css("img.avatar ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.css(".epl-author-box-container").getall()) > 1:
                ...
                if agent2:
                    ...
                    item['Field24'] = agent2.css('.author-position .position ::text').get('').strip()
                    item['Field25'] = agent2.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:', '')
                    item['Field26'] = agent2.css(".author-contact .mobile ::text").get('').strip().replace('tel:','')
                    item['Field26A'] = agent2.css(".author-contact-office-phone .office-phone ::text").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css("img.avatar ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css(".epl-author-box-container").getall()) > 2:
                ...
                if agent3:
                    ...
                    item['Field29'] = agent3.css('.author-position .position ::text').get('').strip()
                    item['Field30'] = agent3.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:', '')
                    item['Field31'] = agent3.css(".author-contact .mobile ::text").get('').strip().replace('tel:','')
                    item['Field31A'] = agent3.css(".author-contact-office-phone .office-phone ::text").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css("img.avatar ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
