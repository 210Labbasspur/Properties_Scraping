...
class BlackLP_Rent(scrapy.Spider):
    name = 'BlackLP_Rent'
    prefix = 'http://www.blacklp.com.au'
    url = "http://www.blacklp.com.au/properties-for-lease.php"
    headers = {
        'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'Accept-Language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        'Connection': 'keep-alive',
        # 'Cookie': 'PROTOCID=7c27d5db0anrjc33ndl2ov6rc5; __utma=161235950.886737410.1711451312.1711451312.1711451312.1; __utmc=161235950; __utmz=161235950.1711451312.1.1.utmcsr=(direct)|utmccn=(direct)|utmcmd=(none); arp_scroll_position=0; PROTOCID=dhp0s0ga2b7rn9efiuvt0p8vm0',
        # 'Referer': 'http://www.blacklp.com.au/sold-properties.php',
        'Upgrade-Insecure-Requests': '1',
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/123.0.0.0 Safari/537.36'
    }
    count = 1
    db = 'BlackLP'
    collection = 'BlackLP_Rent'
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            property_add = response.css('.med-7 ::text').getall()  # full_adddress
            ...
            item['Field6'] = response.xpath("//*[contains(@alt,'bedrooms')]/preceding-sibling::span[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@alt,'bathrooms')]/preceding-sibling::span[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@alt,'parking')]/preceding-sibling::span[1]/text()").get('').strip()
            ...
            description = response.css('.med-9 h2::text , #contentContainer p ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess = response.css('img.fill ::attr(src)').getall()
            new_img_urls = []
            ...
            agent1 = response.css(".thumbItem:nth-child(1)")
            ...
            if agent1_name:
                ...
                item['Field19'] = agent1.css('h4+ .oneline a ::text').get('').strip()
                agent1_phone = agent1.css('.oneline+ .oneline a ::text').get('').strip()
                item['Field20'] = agent1.css('.oneline+ .oneline a ::text').get('').strip()
                '''         Uploading Images on Wasabi S3 Bucket            '''
                Agent_Imagess = agent1.css(".thumbItem img ::attr(src)").getall()
                Agent_Imagesss = []
                for url in Agent_Imagess:
            ...
            agent2 = response.css(".thumbItem+ .thumbItem")
            agent2_name = agent2.css('h4 a ::text').get('').strip()
            if agent2:
                item['Field23'] = agent2_name.strip()
                item['Field25'] = agent2.css('h4+ .oneline a ::text').get('').strip()
                agent2_phone = agent2.css('.oneline+ .oneline a ::text').get('').strip()
