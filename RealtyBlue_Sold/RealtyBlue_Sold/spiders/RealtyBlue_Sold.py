...
class RealtyBlue_Sold(scrapy.Spider):
    name = "RealtyBlue_Sold"
    url = ('https://www.realtyblue.com.au/wp-admin/admin-ajax.php?action=get_posts&query%5Bpost_type%5D%5Bvalue%5D=listings&query%5Bcount%5D%5Bvalue%5D=20&query%5Borderby%5D%5Bvalue%5D=meta_value&query%5Bmeta_key%5D%5Bvalue%5D=saleDate&query%5Bsold%5D%5Bvalue%5D=1&query%5Bsold%5D%5Btype%5D=equal&query%5BsaleOrRental%5D%5Bvalue%5D=Sale&query%5BsaleOrRental%5D%5Btype%5D=equal&query%5Bdoing_wp_cron%5D%5Bvalue%5D=1738764589.7464940547943115234375&query%5Bdoing_wp_cron%5D%5Btype%5D=equal&query'
           '%5Bpaged%5D%5Bvalue%5D={}')
    prefix = "https://www.realtyblue.com.au"
    headers = {
    ...
    count = 1
    db = 'RealtyBlue'
    collection = 'RealtyBlue_Sold'
    bucket_prefix = f'Images6'

    local_file_path = '/img'

    def __init__(self, *args, **kwargs):
        super(RealtyBlue_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2253)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Realty Blue'
