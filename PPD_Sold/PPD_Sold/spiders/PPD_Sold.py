...
class PPD_Sold(scrapy.Spider):
    name = 'PPD_Sold'
    prefix = 'https://www.ppdre.com.au'
    url = ("https://www.ppdre.com.au/wp-admin/admin-ajax.php?id=properties_search&post_id=285&slug=sold&canonical_url=https%3A%2F%2Fwww.ppdre.com.au%2Fproperties%2Fsold%2F&posts_per_page=80&"
           "page={}"
           "&offset=0&post_type=properties&repeater=default&seo_start_page=1&preloaded=false&preloaded_amount=0&taxonomy=listing-type:listing-status&taxonomy_terms=sale:sold&taxonomy_operator=IN:IN&meta_key=sold-date&order=DESC&orderby=meta_value&custom_args=kw:;suburbs:;streets:;features-bedrooms:;features-bathrooms:;features-carports:;price:;rent:;layout:sold;&action=alm_get_posts&query_type=standard")
    headers = {
        'accept': 'application/json, text/plain, */*',
        'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        # 'cookie': '_ga=GA1.1.1419337615.1713442551; _ga=GA1.4.1419337615.1713442551; _gid=GA1.4.367569228.1713442551; arp_scroll_position=700; _ga_83S9T5E3QS=GS1.1.1713448546.2.1.1713450406.43.0.0; _gat=1',
        'priority': 'u=1, i',
        'referer': 'https://www.ppdre.com.au/properties/sold/',
    ...
    def start_requests(self):
        ...
        yield scrapy.Request(url=self.url.format(page_no), callback=self.parse, headers=self.headers,
                             meta={'page_no':page_no,'post_no':post_no})

    def parse(self, response):
        post_no = response.meta['post_no']
        data = json.loads(response.text)
        selector = Selector(text=data['html'])
        link_url = selector.css('div.item.clearfix a::attr(href)').getall()
        for property in link_url:
            post_no += 1
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'PPD Real Estate'

            property_add = response.css('#single__properties .header span ::text').getall()  # full_adddress
            ...
            item['Field6'] = response.css(".bedrooms .value ::text").get('').strip()
            item['Field7'] = response.css(".bathroom .value ::text").get('').strip()
            item['Field8'] = response.css(".carports .value ::text").get('').strip()

            sold_price = response.css('.price ::text').get('').strip().replace(' ','')
            if sold_price:
                item['Field9'] = (sold_price.strip()).replace('$','')   # sold_price

            description = response.css('#single__properties .content--wrap p ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess_style = response.xpath("//*[contains(@data-remodal-target,'slideshow-modal')]/div[contains(@role,'img')]/@style").getall()
            pattern = r'url\((.*?)\)'
            new_img_urls = []
            for string in Agent_Imagess_style:
                match = re.search(pattern, string)
                if match:
                    new_img_urls.append(match.group(1))
            ...
            agent1_name = agent1.css('h3 ::text').get('').strip()
            if agent1:
                ...
                item['Field19'] =  agent1.css(".email ::text").get('').strip()
                agent1_phone = agent1.xpath(".//*[contains(text(),'P:')]/following-sibling::a[1]/text()").get('').strip()
                item['Field20'] = agent1_phone.strip()
                agent1_mobile = agent1.xpath(".//*[contains(text(),'M:')]/following-sibling::a[1]/text()").get('').strip()
                item['Field21'] = agent1_mobile.strip()
                '''         Uploading Images on Wasabi S3 Bucket            '''
                Agent_Images_style = agent1.css(".photo ::attr(style)").get('').strip()
                Agent_Images = Agent_Images_style.split('(')[-1].split(')')[0]
            ...
            if len(response.css(".agent").getall()) > 1:
                ...
                agent2_name = agent2.css('h3 ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.css('.position ::text').get('').strip()
                    item['Field25'] = agent2.css(".email ::text").get('').strip()
                    agent2_phone = agent2.xpath(".//*[contains(text(),'P:')]/following-sibling::a[1]/text()").get('').strip()
                    item['Field26'] = agent2_phone.strip()
                    agent2_mobile = agent2.xpath(".//*[contains(text(),'M:')]/following-sibling::a[1]/text()").get('').strip()
                    item['Field26A'] = agent2_mobile.strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images_style = agent2.css(".photo ::attr(style)").get('').strip()
                    Agent_Images = Agent_Images_style.split('(')[-1].split(')')[0]
            ...
            if len(response.css(".agent").getall()) == 3:
                ...
                agent3_name = agent3.css('h3 ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field29'] = agent3.css('.position ::text').get('').strip()
                    item['Field30'] = agent3.css(".email ::text").get('').strip()
                    agent3_phone = agent3.xpath(".//*[contains(text(),'P:')]/following-sibling::a[1]/text()").get('').strip()
                    item['Field31'] = agent3_phone.strip()
                    agent3_mobile = agent3.xpath(".//*[contains(text(),'M:')]/following-sibling::a[1]/text()").get('').strip()
                    item['Field31A'] = agent3_mobile.strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images_style = agent3.css(".photo ::attr(style)").get('').strip()
                    Agent_Images = Agent_Images_style.split('(')[-1].split(')')[0]
