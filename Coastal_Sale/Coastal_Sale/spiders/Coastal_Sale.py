...
class Coastal_Sale(scrapy.Spider):
    name = "Coastal_Sale"
    url = 'https://www.coastal.com.au/wp-json/api/listings/all?priceRange=&landArea=&floorArea=&limit=12&type=property&status=current&address=&paged={}&bed=&bath=&car=&sort='
    prefix = "https://www.coastal.com.au"
    headers = {
            'accept': '*/*',
    ...
    def __init__(self, *args, **kwargs):
        super(Coastal_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            ...
            price = property.get('propertyPricing',{}).get('value','').replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            ...
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".listing-single__description ::text").getall())  # description
            ...
            if response.css(".agent-item"):
                ...
                if agent1_name:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".agent-image ::attr(style)").get('').strip().replace('background-image:url(','').replace(');','').replace('background-position:50% 0%','').replace('background-position:80% 0%','').replace('\n','').replace('\t','').replace('\r','')
            ...
            if len(response.css(".agent-item").getall()) > 1:
                ...
                if agent2:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".agent-image ::attr(style)").get('').strip().replace('background-image:url(','').replace(');','').replace('background-position:50% 0%','').replace('\n','').replace('\t','').replace('\r','').replace('background-position:80% 0%','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css(".agent-item").getall()) > 2:
                ...
                if agent3:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".agent-image ::attr(style)").get('').strip().replace('background-image:url(','').replace(');','').replace('background-position:50% 0%','').replace('\n','').replace('\t','').replace('\r','').replace('background-position:80% 0%','')
                    images_string = f'{agent_image}' if agent_image else ""
