...
class NoakesNickolas_Sold(scrapy.Spider):
    name = "NoakesNickolas_Sold"

    url = "https://noakesnickolas.com.au/category/sold/?status=Sold&property-type=All&price=&order=NEWEST+TO+OLDEST&price-range-select-min=0&price-range-select=10000000"
    prefix = "https://noakesnickolas.com.au"
    headers = {
    ...
    count = 1
    db = 'NoakesNickolas'
    collection = 'NoakesNickolas_Sold'
    ...
    def __init__(self, *args, **kwargs):
        super(NoakesNickolas_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".listing-item .listings-wrap a ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2938)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Noakes Nickolas'

            property_street = response.css('.mb-lg-1 ::text').get('').strip()  # property_street
            ...
            item['Field6'] = response.xpath("//*[contains(@src,'bed.svg')]/parent::li[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@src,'bath.svg')]/parent::li[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@src,'car.svg')]/parent::li[1]/text()").get('').strip()

            price = response.css('.col-lg-8  span ::text').get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price
            ...
            '''          AGENTS          '''
            if response.css(".col-md-6"):
                agent1 = response.css(".col-md-6")[0]
                ...
                if agent1_name:
                    ...
                    item['Field18'] = agent1.css('.xxxxxxxxxxx ::text').get('').strip()
                    item['Field19'] =  agent1.xpath(".//h5[contains(text(),'@')]/text()").get('').strip().replace('mailto:','')
                    item['Field20'] = agent1.css(".mb-1 ::text").get('').strip().replace('tel:','')
                    item['Field21'] = agent1.css(".mb-2 ::text").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".full-width ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.css(".col-md-6").getall()) > 1:
                agent2 = response.css(".col-md-6")[1]
                agent2_name = agent2.css('h3 ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.css('.xxxxxxxxxxx ::text').get('').strip()
                    item['Field25'] = agent2.xpath(".//h5[contains(text(),'@')]/text()").get('').strip().replace('mailto:','')
                    item['Field26'] = agent2.css(".mb-1 ::text").get('').strip().replace('tel:','')
                    item['Field26A'] = agent2.css(".mb-2 ::text").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".full-width ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css(".col-md-6").getall()) > 2:
                agent3 = response.css(".col-md-6")[2]
                agent3_name = agent3.css('h3 ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field29'] = agent3.css('.xxxxxxxxxxx ::text').get('').strip()
                    item['Field30'] = agent3.xpath(".//h5[contains(text(),'@')]/text()").get('').strip().replace('mailto:','')
                    item['Field31'] = agent3.css(".mb-1 ::text").get('').strip().replace('tel:','')
                    item['Field31A'] = agent3.css(".mb-2 ::text").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".full-width ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            land_area = response.xpath('//*[contains(text(),"Land")]/parent::td/following-sibling::td/p/text()').get('').strip().replace('Land','').replace('is','').replace('m²','')
            if land_area:
