...
class Time_Rent(scrapy.Spider):
    name = 'Time_Rent'
    prefix = 'https://www.timerealty.com.au'
    pre_url = 'https://www.timerealty.com.au/rent/properties-for-rent/'
    url = ("https://www.timerealty.com.au/wp-content/plugins/zoorealty/display/elements/load_listings.php?status=1,4&list=lease&order=&order_direction=&"
           "property_type=&suburb=&keywords=&price_min=&price_max=&bedrooms=&bathrooms=&carspaces=&sur_suburbs=&page={}")
    headers = {
        'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        'cache-control': 'max-age=0',
        # 'cookie': '_ga=GA1.1.1262540435.1713705368; PHPSESSID=pfiojmprqj1hooj4qmk3j1i6ka; _ga_BL9TP4TG01=GS1.1.1713727803.2.0.1713727803.0.0.0; arp_scroll_position=564; PHPSESSID=pfiojmprqj1hooj4qmk3j1i6ka',
        'priority': 'u=0, i',
    ...
    count = 1
    db = 'Time'
    collection = 'Time_Rent'
    bucket_prefix = f'D_{collection}'

    def start_requests(self):
        yield scrapy.Request(url=self.pre_url, callback=self.parse, headers=self.headers)

    def parse(self, response):
        total_properties = response.xpath("//div[contains(text(),'Now showing')]/text()").get('').strip()
        total_properties = int(total_properties.replace('Now','').replace('showing','').replace('properties','').replace(' ',''))
        print('Total Properties are : ', total_properties)
        property_no = 0
        page_no = 1
        yield scrapy.Request(url=self.url.format(page_no), callback=self.listing_parse, headers=self.headers,
                             meta={'page_no':page_no, 'total_properties':total_properties,'property_no':property_no})
    ...
    def listing_parse(self, response):
        ...
        if property_no < total_properties:
            ...
            yield scrapy.Request(url=self.url.format(page_no), callback=self.listing_parse, headers=self.headers,
                                 meta={'page_no': page_no, 'total_properties': total_properties,
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field6'] = response.css(".bed-gold ::text").get('').strip()
            item['Field7'] = response.css(".bath-gold ::text").get('').strip()
            item['Field8'] = response.css(".car-gold ::text").get('').strip()
            ...
            if response.css("#property-agents .width-100-sm"):
                ...
                if agent1:
                    ...
                    agent1_mobile = agent1.css(".mobile .color-secondary-hover ::text").get('').strip()
                ...
                if len(response.css("#property-agents .width-100-sm").getall()) > 1:
                    ...
                    if agent2:
                        ...
                        agent2_mobile = agent2.css(".mobile .color-secondary-hover ::text").get('').strip()
                ...
                if len(response.css("#property-agents .width-100-sm").getall()) == 3:
                    ...
                    if agent3:
                        ...
                        agent3_mobile = agent3.css(".mobile .color-secondary-hover ::text").get('').strip()
            ...
            land_area = response.xpath("//*[contains(text(),'Land size')]/text()").get('').strip().replace('Land','').replace('size','')
            if land_area:
                item['Field36'] = land_area.replace('m2','').replace(':','').replace('(approx.)','').replace(' ','')   # land_area
