...
class Geelong_Sale(scrapy.Spider):
    name = "Geelong_Sale"

    url = "https://www.geelongrealestate.co/property-category/for-sale/"
    prefix = "https://www.geelongrealestate.co"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Geelong_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        # all_links = response.css(".thumbnail a ::attr(href)").getall()
        # links = []
        # for each_url in all_links:
        #     links.append(each_url)
        #     self.links.append(each_url)
        all_links = response.css(".row .bg-primary")
        links = []
        for each_url in all_links:
            if each_url.css('span::text').get('').strip() == 'SOLD':
                pass
            else:
                each_url = each_url.css(".thumbnail a ::attr(href)").getall()
                links.append(each_url)
                self.links.append(each_url)
        ...
        for property_url in all_links:
            if property_url.css('span::text').get('').strip() == 'SOLD':
                pass
            else:
                property_url = property_url.css(".thumbnail a ::attr(href)").getall()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2987)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Geelong Real Estate Co'

            property_street = response.css('.street ::text').get('').strip().replace('\n','').replace('  ','')  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.city ::text').getall() if e.strip())  # property_suburb
            property_address = f"{property_street} {property_suburb}".replace('\n','').replace('  ','')
            item['Field5'] = property_address  # full_adddress

            item['Field6'] = response.xpath("//*[contains(@alt,'Bed')]/parent::span[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@alt,'bath')]/parent::span[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@alt,'Car')]/parent::span[1]/text()").get('').strip()

            price = response.css('.price ::text').get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.Text__Typography-sc-1103tao-0 ::text').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".property-description__content ::text").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.xpath("//*[contains(@data-fancybox,'gallery')]/@href").getall()
            images = []
            ...
            if response.css(".agent-profile"):
                ...
                if agent1_name:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".attachment-thumbnail ::attr(data-src)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.css(".agent-profile").getall()) > 1:
                ...
                if agent2:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".attachment-thumbnail ::attr(data-src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css(".agent-profile").getall()) > 2:
                ...
                if agent3:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".attachment-thumbnail ::attr(data-src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            land_area = response.xpath('//*[contains(@alt,"Area")]/parent::span[1]/text()').get('').strip().replace('Land','').replace('is','').replace('m²','')
            if land_area:
