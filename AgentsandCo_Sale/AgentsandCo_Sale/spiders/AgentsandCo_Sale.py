...
class AgentsandCo_Sale(scrapy.Spider):
    name = "AgentsandCo_Sale"
    url = 'https://www.agentsandco.com.au/wp-admin/admin-ajax.php'
    prefix = "https://www.agentsandco.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(AgentsandCo_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2891)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Agents & Co'
            ...
            prop_images = response.css(".item-image img.img-orientation ::attr(src)").getall()
            images = []
            ...
            if response.css(".agent-photo-wrapper"):
                ...
                if agent1_name:
                    ...
                    item['Field20'] = agent1.css(".agent-info-item-phone a ::attr(href)").get('').strip().replace('tel:','')
                    item['Field21'] = agent1.css(".agent-info-item-mobile a ::attr(href)").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css(".agent-photo-wrapper").getall()) > 1:
                ...
                if agent2:
                    ...
                    item['Field26'] = agent2.css(".agent-info-item-phone a ::attr(href)").get('').strip().replace('tel:','')
                    item['Field26A'] = agent2.css(".agent-info-item-mobile a ::attr(href)").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css(".agent-photo-wrapper").getall()) > 2:
                ...
                if agent3:
                    ...
                    item['Field31'] = agent3.css(".agent-info-item-phone a ::attr(href)").get('').strip().replace('tel:','')
                    item['Field31A'] = agent3.css(".agent-info-item-mobile a ::attr(href)").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
