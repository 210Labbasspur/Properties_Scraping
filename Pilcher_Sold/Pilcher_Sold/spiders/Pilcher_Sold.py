...
class Pilcher_Sold(scrapy.Spider):
    name = 'Pilcher_Sold'
    prefix = 'https://www.pilcher-residential.com'
    url = "https://www.pilcher-residential.com/recent-sales"
    headers = {
    ...
    def parse(self, response):
        ...
        next_page = response.xpath("//*[contains(text(),'VIEW MORE')]/@href").get('').strip()
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            property_add = response.css('h1.normal ::text').getall()  # full_adddress
            ...
            item['Field6'] = response.xpath("//*[contains(text(),'Bed')]/preceding-sibling::div[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(text(),'Bath')]/preceding-sibling::div[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(text(),'Car')]/preceding-sibling::div[1]/text()").get('').strip()

            sold_price = response.css('div.muted ::text').get('').strip().replace('Sold','').replace(' ','')
            ...
            Agent_Imagess = response.css("#photo-slideshow link ::attr(href)").getall()
            new_img_urls = []
            ...
            land_area = ''.join(e.strip() for e in response.css(".kvgroup .dm-sans ::text").getall())
            if land_area:
