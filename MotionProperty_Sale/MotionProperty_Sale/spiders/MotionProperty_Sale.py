...
class MotionProperty_Sale(scrapy.Spider):
    name = "MotionProperty_Sale"

    url = "https://motionproperty.com.au/property/?_status=current"
    prefix = "https://motionproperty.com.au"
    headers = {
    ...
    count = 1
    db = 'MotionProperty'
    collection = 'MotionProperty_Sale'
    ...
    def __init__(self, *args, **kwargs):
        super(MotionProperty_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css("#brxe-props-archive ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2972)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Motion Property'

            property_street = response.css('.xxxxxxxxxxxxxx ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('#brxe-fvvinr h1 ::text').getall() if e.strip())  # property_suburb
            property_address = f"{property_street} {property_suburb}".replace('\n','').replace('\t','').replace('\r','')
            item['Field5'] = property_address  # full_adddress

            item['Field6'] = response.xpath("//*[contains(@class,'fas fa-bed')]/following-sibling::*[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'fas fa-bath')]/following-sibling::*[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'fas fa-car')]/following-sibling::*[1]/text()").get('').strip()

            price = response.css('#brxe-ufhvqq h2 ::text').get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('#brxe-ukmvud h3 ::text').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css("#brxe-vwcqzj p ::text").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.xpath("//img[contains(@class,'css-filter size-large')]/@src").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.css("#brxe-eflcvr"):
                agent1 = response.css("#brxe-eflcvr")[0]
                agent1_name = agent1.css('.profile--card--name ::attr(href)').get('').strip()
                if agent1_name:
                    agent1_name = agent1_name.rstrip('/').split('/')[-1].replace('-', ' ')
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field19'] = agent1.xpath(".//a[contains(@href,'@')]/@href").get('').strip().replace('tel:','')
                    item['Field20'] = agent1.xpath(".//a[contains(@href,'tel:')]/@href").get('').strip().replace('tel:','')
                    phone_nos = agent1.xpath(".//a[contains(@href,'tel:0')]/@href").getall()
                    if len(phone_nos) > 1:
                        item['Field21'] = phone_nos[-1].strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".size-large ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.css("#brxe-eflcvr").getall()) > 1:
                agent2 = response.css("#brxe-eflcvr")[1]
                agent2_name = agent2.css('.profile--card--name ::attr(href)').get('').strip()
                if agent2:
                    agent2_name = agent2_name.rstrip('/').split('/')[-1].replace('-', ' ')
                    item['Field23'] = agent2_name.strip()
                    item['Field25'] = agent2.xpath(".//a[contains(@href,'@')]/@href").get('').strip().replace('tel:','')
                    item['Field26'] = agent2.xpath(".//a[contains(@href,'tel:')]/@href").get('').strip().replace('tel:','')
                    phone_nos = agent2.xpath(".//a[contains(@href,'tel:0')]/@href").getall()
                    if len(phone_nos) > 1:
                        item['Field26A'] = phone_nos[-1].strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".size-large ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css("#brxe-eflcvr").getall()) > 2:
                agent3 = response.css("#brxe-eflcvr")[2]
                agent3_name = agent3.css('.profile--card--name ::attr(href)').get('').strip()
                if agent3:
                    agent3_name = agent3_name.rstrip('/').split('/')[-1].replace('-', ' ')
                    item['Field28'] = agent3_name.strip()
                    item['Field30'] = agent3.xpath(".//a[contains(@href,'@')]/@href").get('').strip().replace('tel:','')
                    item['Field31'] = agent3.xpath(".//a[contains(@href,'tel:')]/@href").get('').strip().replace('tel:','')
                    phone_nos = agent3.xpath(".//a[contains(@href,'tel:0')]/@href").getall()
                    if len(phone_nos) > 1:
                        item['Field31A'] = phone_nos[-1].strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".size-large ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
