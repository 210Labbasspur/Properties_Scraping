...
class StantonTaylor_Sale(scrapy.Spider):
    name = 'StantonTaylor_Sale'
    prefix = 'https://www.stantonandtaylor.com.au'
    url = "https://www.stantonandtaylor.com.au/data/results/?listing_sale_method=Sale&listing_status=sale&status=sale&pg={}"
    headers = {
        'accept': '*/*',
        'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        'cookie': 'AMP_MKTG_16a5c84b5b=JTdCJTdE; _WHEELS_AUTHENTICITY=ULaO7W4MM5kryr%2BzTPlA11FbUqf6JZ8bhiVCfqZIzWVykakVnL9X80rZr6dYm9YvRNvcmXBlQUghzNggSW8hkniB0hXOj0EOgzuyMZEmdoPPcEd9h7%2FstSld9OWWymy4luh4XmPzOTxTPsAv6cYLcA%3D%3D; FLASH=%7B%7D; AMP_16a5c84b5b=JTdCJTIyZGV2aWNlSWQlMjIlM0ElMjIyZmE3Nzc2Yi0yOTJiLTRmNWItYTI4ZC1kNjQ1OWYwNzc0YzQlMjIlMkMlMjJzZXNzaW9uSWQlMjIlM0ExNzE4NTM1OTA3NTA5JTJDJTIyb3B0T3V0JTIyJTNBZmFsc2UlMkMlMjJsYXN0RXZlbnRUaW1lJTIyJTNBMTcxODUzNjEwODk0MSU3RA==; arp_scroll_position=990',
        'priority': 'u=0, i',
        'referer': 'https://www.stantonandtaylor.com.au/sold',
    ...
    def start_requests(self):
        ...
        yield scrapy.Request(url=self.url.format(page_no), callback=self.parse, headers=self.headers,
                             meta={'LINKS':LINKS,'DB_sale_matching_url':DB_sale_matching_url,'page_no':page_no})
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'Stanton Taylor Real Estate'

            property_add = response.css('.col-address ::text').getall()  # full_adddress
            ...
            sold_price = response.xpath("//*[contains(text(),'Price')]/following-sibling::span[1]/text()").get('').strip()
            if re.search(r'\d', sold_price):
                item['Field9'] = (sold_price.strip()).replace('Sold for','').replace('$','')   # sold_price
            ...
            Agent_Imagess = response.css(".slide-image ::attr(src)").getall()
            new_img_urls = []
            ...
            if response.css(".agent"):
                ...
                if agent1_name:
                    ...
                    item['Field20'] = agent1_phone.strip().replace('Call','')
                    phone_nos =  agent1.xpath(".//a[contains(@href,'tel:')]/@href").getall()
                    if len(phone_nos) > 1:
                        agent1_mobile = phone_nos[-1].strip()
                        item['Field21'] = agent1_mobile.strip().replace('tel:','').replace('Call','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            land_area = response.xpath("//*[contains(text(),'Land')]/following-sibling::span[1]/text()").get('').strip().replace('Property ID:','')
            if land_area:
                item['Field36'] = land_area.replace('m','').replace(' ','')   # land_area

            feature_count = 58
            for feature in response.css(".grid-row li"):
