...
class DiMez_Sold(scrapy.Spider):
    name = "DiMez_Sold"

    url = "https://www.dimezrealestate.com.au/recently-sold-page-1"
    prefix = "https://www.dimezrealestate.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(DiMez_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".col-lg-5 a ::attr(href)").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://www.dimezrealestate.com.au{each_url.replace("..","")}'
        ...
        for property_url in all_links:
            property_url = f'https://www.dimezrealestate.com.au{property_url.replace("..","")}'
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(1625)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Di Mez Real Estate'

            property_street = response.css('.xxxxxxxxxxxxxx ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.Property-address ::text').getall() if e.strip())  # property_suburb
            property_address = f"{property_street} {property_suburb}"
            item['Field5'] = property_address  # full_adddress

            item['Field6'] = response.xpath("//*[contains(@src,'bed-icon1')]/following-sibling::span[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@src,'bathroom_p')]/following-sibling::span[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@src,'car-icon1')]/following-sibling::span[1]/text()").get('').strip()

            price = response.css('.xxxxxxxxxx ::text').get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.listing_heading ::text').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".mt-4 p ::text").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.css("img.item ::attr(src)").getall()
            images = []
            ...
            '''          AGENTS          '''
            item['Field15'] = 'Dianne'  # agent_first_name_1
            item['Field16'] = 'Meznaric'  # agent_surname_name_1
            item['Field17'] = 'Dianne Meznaric'  # agent_full_name_1
            item['Field18'] = 'Principal'  #
            item['Field19'] =  'dianne@dimezrealestate.com.au'
            item['Field20'] = '02 4631 2333'
            agent_image = 'https://admin.commercialpremises.com.au/userimgs/m2560/agents/20210626161516-1.jpg'
            ...
            item['Field23'] = 'Kelly Oliver'
            item['Field24'] = 'Office Manager'
            item['Field25'] = 'Kelly@dimezrealestate.com.au'
            item['Field26'] = '0431 003 695'
            '''         Uploading Images on Wasabi S3 Bucket            '''
            agent_image = 'https://admin.commercialpremises.com.au/userimgs/m2560/agents/20230124140106-1.jpg'
            images_string = f'{agent_image}' if agent_image else ""
            ...
            item['Field28'] = 'Sarah OHara'
            item['Field29'] = 'Property Junior'
            item['Field30'] = 'propertymanagement@dimezrealestate.com.au'
            '''         Uploading Images on Wasabi S3 Bucket            '''
            agent_image = 'https://admin.commercialpremises.com.au/userimgs/m2560/agents/20230223161142-1.jpg'
            images_string = f'{agent_image}' if agent_image else ""
            ...
            item['Field35'] = response.xpath('//*[contains(text(),"Property Type:")]/following-sibling::span/text()').get('').strip()
