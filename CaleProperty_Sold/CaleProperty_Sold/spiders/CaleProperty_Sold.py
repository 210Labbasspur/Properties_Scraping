...
class CaleProperty_Sold(scrapy.Spider):
    name = "CaleProperty_Sold"

    url = "https://www.caleproperty.com.au/selling/recent-sales?&status=current&disposalMethod=sold&orderBy=sold&page=1"
    prefix = "https://www.caleproperty.com.au"
    headers = {
    ...
    count = 1
    db = 'CaleProperty'
    collection = 'CaleProperty_Sold'
    bucket_prefix = f'CP_{collection}'

    local_file_path = '/img'

    def __init__(self, *args, **kwargs):
        super(CaleProperty_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".propertyListThumb a::attr(href)").getall()
        links = []
        for each_url in all_links:
            each_url = f'https:{each_url}'  ### https missing on website
        ...
        for property in response.css('.propertyListThumb'):
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(1554)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Cale Property'
            property_street = response.css('.detailStreet ::text').get('').strip()  # full_adddress
            property_suburb = response.css('.detailSuburb ::text').get('').strip()  # full_adddress
            property_address = f"{property_street}, {property_suburb}"
            item['Field5'] = property_address  # full_adddress

            item['Field6'] = response.xpath('//*[contains(text(),"Bedrooms")]/parent::td[1]/following-sibling::td[1]/text()').get('').strip()
            item['Field7'] = response.xpath('//*[contains(text(),"Bathrooms")]/parent::td[1]/following-sibling::td[1]/text()').get('').strip()
            item['Field8'] = response.xpath('//*[contains(text(),"Garage")]/parent::td[1]/following-sibling::td[1]/text()').get('').strip()

            price = response.css('.detailPrice ::text').get('').strip().replace('Sold for','').replace(' ','')
            if price:
                item['Field9'] = price

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.detailHeading ::text, .detailIntro ::text').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.xpath("//*[contains(@class,'detailIntro')]/following-sibling::p[1]/text()").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.css('.slides .item ::attr(style)').getall()
            images = []
            ...
            if response.css(".carousel-inner .item"):
                ...
                if agent1_name:
                    ...
                    agent1_phone = agent1.css("tr a.callNumber ::text").get('').strip().replace('tel:','')
                    item['Field20'] = agent1_phone.strip()
                    phone_nos =  agent1.css("tr a.callNumber ::text").getall()
                    if len(phone_nos) > 1:
                        agent1_mobile = phone_nos[-1].strip()
                        item['Field21'] = agent1_mobile.strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".agentIMG img ::attr(src)").get('').strip()
                    images_string = f'https:{agent_image}' if agent_image else ""
            ...
            if len(response.css(".carousel-inner .item").getall()) > 1:
                ...
                if agent2:
                    ...
                    agent2_phone = agent2.css("tr a.callNumber ::text").get('').strip().replace('tel:','')
                    item['Field26'] = agent2_phone.strip()
                    phone_nos =  agent2.css("tr a.callNumber ::text").getall()
                    if len(phone_nos) > 1:
                        agent2_mobile = phone_nos[-1].strip()
                        item['Field26A'] = agent2_mobile.strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".agentIMG img ::attr(src)").get('').strip()
                    images_string = f'https:{agent_image}' if agent_image else ""
            ...
            if len(response.css(".carousel-inner .item").getall()) > 2:
                ...
                if agent3:
                    ...
                    agent3_phone = agent3.css("tr a.callNumber ::text").get('').strip().replace('tel:','')
                    item['Field31'] = agent3_phone.strip()
                    phone_nos =  agent3.css("tr a.callNumber ::text").getall()
                    if len(phone_nos) > 1:
                        agent3_mobile = phone_nos[-1].strip()
                        item['Field31A'] = agent3_mobile.strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".agentIMG img ::attr(src)").get('').strip()
                    images_string = f'https:{agent_image}' if agent_image else ""
            ...
            item['Field33'] = response.xpath('//*[contains(text(),"Property ID")]/parent::td[1]/following-sibling::td[1]/text()').get('').strip()
            # item['Field35'] = response.xpath("//label[contains(text(),'property_type')]/following-sibling::div/text()").get('').strip()   # external_property_id

            land_area = response.xpath('//*[contains(text(),"Land Size")]/parent::td[1]/following-sibling::td[1]/text()').get('').strip()
            if land_area:
                item['Field36'] = land_area.replace('Square Mtr approx','').replace('.','')   # land_area
