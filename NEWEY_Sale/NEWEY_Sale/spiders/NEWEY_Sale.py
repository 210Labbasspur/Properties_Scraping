...
class NEWEY_Sale(scrapy.Spider):
    name = 'NEWEY_Sale'
    prefix = 'https://www.newey.com.au'
    url = "https://www.newey.com.au/properties-for-sale/"
    headers = {
    ...
    count = 1
    db = 'NEWEY'
    collection = 'NEWEY_Sale'
    bucket_prefix = f'D_{collection}'

    def start_requests(self):
        LINKS = []
        DB_sale_matching_url = self.read_complete_data_base()
        yield scrapy.Request(url=self.url, callback=self.parse, headers=self.headers,
                             meta={'LINKS':LINKS,'DB_sale_matching_url':DB_sale_matching_url})

    def parse(self, response):
        LINKS = response.meta['LINKS']
        DB_sale_matching_url = response.meta['DB_sale_matching_url']
        all_details_links = []
        for url in response.css(".wpsight-listing-image a"):
            all_details_links.append(url.css('::attr(href)').get('').strip())

        links = []
        for each_url in all_details_links:
            links.append(each_url)
            LINKS.append(each_url)

        for each_db_detail_page_url in DB_sale_matching_url:
            ...
            elif each_db_detail_page_url not in LINKS:
                new_data = {
                    "Field102": "REMOVED",
                    "Field104": each_db_detail_page_url,
                    "Field3": datetime.now().strftime("%d %b, %Y")
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            sold_price = response.css('div.alignleft ::text').get('').strip().replace('SOLD OFF-MARKET','').replace(' ','')
            if re.search(r'\d', sold_price):
                item['Field9'] = (sold_price.strip()).replace('SOLD','').replace('$','')   # sold_price

            description = response.css('.wpsight-listing-description p ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess = response.css(".image img ::attr(src)").getall()
            new_img_urls = []
            ...
            '''          AGENTS          '''
            if response.xpath("//*[contains(@class,'wpsight-listing-agent clearfix')]"):
                agent1 = response.xpath("//*[contains(@class,'wpsight-listing-agent clearfix')]")[0]
                agent1_name = agent1.css('.wpsight-listing-agent-name ::text').get('').strip()
                if agent1_name:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field18'] = agent1.css(".wpsight-listing-agent-company ::text").get('').strip()
                    item['Field19'] =  agent1.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:','')
                    agent1_phone = agent1.xpath(".//*[contains(text(),'M.')]/text()").get('').strip().replace('M.','')
                    item['Field20'] = agent1_phone.strip()
                    item['Field21'] = agent1.xpath(".//*[contains(text(),'P.')]/text()").get('').strip().replace('P.','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images = agent1.css(".wpsight-list-agent-image img ::attr(src)").get('').strip()
                ...
                if len(response.xpath("//*[contains(@class,'wpsight-listing-agent clearfix')]").getall()) > 1:
                    agent2 = response.xpath("//*[contains(@class,'wpsight-listing-agent clearfix')]")[1]
                    agent2_name = agent2.css('.wpsight-listing-agent-name ::text').get('').strip()
                    if agent2:
                        item['Field23'] = agent2_name.strip()
                        item['Field24'] = agent2.css(".wpsight-listing-agent-company ::text").get('').strip()
                        item['Field25'] = agent2.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:', '')
                        agent2_phone = agent2.xpath(".//*[contains(text(),'M.')]/text()").get('').strip().replace('M.','')
                        item['Field26'] = agent2_phone.strip()
                        item['Field26A'] = agent2.xpath(".//*[contains(text(),'P.')]/text()").get('').strip().replace('P.', '')
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent2.css(".wpsight-list-agent-image img ::attr(src)").get('').strip()
                ...
                if len(response.xpath("//*[contains(@class,'wpsight-listing-agent clearfix')]").getall()) > 2:
                    agent3 = response.xpath("//*[contains(@class,'wpsight-listing-agent clearfix')]")[2]
                    agent3_name = agent3.css('.wpsight-listing-agent-name ::text').get('').strip()
                    if agent3:
                        item['Field28'] = agent3_name.strip()
                        item['Field29'] = agent3.css(".wpsight-listing-agent-company ::text").get('').strip()
                        item['Field30'] = agent3.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:', '')
                        agent3_phone = agent3.xpath(".//*[contains(text(),'M.')]/text()").get('').strip().replace('M.','')
                        item['Field31'] = agent3_phone.strip()
                        item['Field31A'] = agent3.xpath(".//*[contains(text(),'P.')]/text()").get('').strip().replace('P.', '')
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent3.css(".wpsight-list-agent-image img ::attr(src)").get('').strip()
            ...
            item['Field33'] = response.css(".wpsight-listing-id ::text").get('').strip()
            # item['Field35'] = response.xpath("//*[contains(@class,'property-type')]/text()").get('').strip()
            # land_area = ''.join(e.strip() for e in response.css("#quickinfo .land-size ::text").getall())
            # if land_area:
    ...
    '''         NEW FUNCTIONS AFTER IMPLEMENTING ACTIVE LOGIC                     '''
    def read_complete_data_base(self):
        connection_string = 'mongodb://localhost:27017'
        conn = pymongo.MongoClient(connection_string)
        db = conn[self.db]
        collection = db[self.collection]
