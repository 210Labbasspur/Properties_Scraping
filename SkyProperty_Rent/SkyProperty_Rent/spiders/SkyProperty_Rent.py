...
class SkyProperty_Rent(scrapy.Spider):
    name = "SkyProperty_Rent"

    url = "https://www.skyproperty.com.au/buying/properties-for-lease/"
    prefix = "https://www.skyproperty.com.au"
    headers = {
    ...
    count = 1
    db = 'SkyProperty'
    collection = 'SkyProperty_Rent'
    bucket_prefix = f'SkyProperty_{collection}'

    local_file_path = '/img'

    def __init__(self, *args, **kwargs):
        super(SkyProperty_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2942)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Sky Property'
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'las la-bed')]/following-sibling::span[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'las la-bath')]/following-sibling::span[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'las la-car')]/following-sibling::span[1]/text()").get('').strip()
            ...
            prop_images = response.xpath("//*[contains(@class,'popup-gallery slider-image')]/div/picture/source/img/@src").getall()
            images = []
