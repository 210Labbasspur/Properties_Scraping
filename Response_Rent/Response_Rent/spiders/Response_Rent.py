...
class Response_Rent(scrapy.Spider):
    name = 'Response_Rent'
    prefix = 'https://responsere.com.au'
    url = "https://responsere.com.au/properties-for-lease"
    headers = {
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'Response'

            property_add = response.css(".propertyPage-address ::text").getall()  # full_adddress
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'icons-bed')]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'icons-bath')]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'icons-car')]/text()").get('').strip()

            sold_price = response.css('.propertyPage-priceText ::text').get('').strip().replace('SOLD OFF-MARKET','').replace(' ','')
            ...
            Agent_Imagess = response.css("#slideshow link ::attr(href)").getall()
            new_img_urls = []
            ...
            feature_count = 58
            for feature in response.css(".check span:nth-child(1)"):
