...
class Cassidy_Rent(scrapy.Spider):
    name = 'Cassidy_Rent'
    prefix = 'https://cassidyre.com.au'
    url = "https://cassidyre.com.au/properties-for-lease"
    headers = {
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            item = dict()
            item['Field2'] = '1576'
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')
            item['Field4'] = 'Cassidy Real Estate'

            property_add = response.css('h1 span ::text').getall()
            property_address = ' '.join(element.strip().replace('\n', ' ').replace('\t','') for element in property_add)
            item['Field5'] = property_address.strip()

            item['Field6'] = response.xpath("//span[contains(text(),'bed')]/text()").get('').strip().replace('bed','').replace('s','')
            item['Field7'] = response.xpath("//span[contains(text(),'bath')]/text()").get('').strip().replace('bath','').replace('s','')
            item['Field8'] = response.xpath("//span[contains(text(),'car')]/text()").get('').strip().replace('car','').replace('s','')

            # sold_price = response.css('.muted ::text').get('').strip().replace('SOLD OFF-MARKET','').replace(' ','')
            # if re.search(r'\d', sold_price):
            #     item['Field9'] = (sold_price.strip()).replace('SOLD','').replace('$','')

            description = response.css('.propertyPage-property-description p ::text').getall()
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess = response.css(".carouselloop-items img.block ::attr(src)").getall()
            new_img_urls = []
            ...
            if response.css(".agentTile"):
                ...
                if len(response.css(".agentTile").getall()) > 1:
                    agent3 = response.css(".agentTile")[1]
                    agent3_name = agent2.css('.agentTile-name a ::text').get('').strip()
                    if agent3:
            ...
            p_type = response.css(".propertyPage-property-icons span::text").getall()
            item['Field35'] = p_type[-1].strip()

            # land_area = response.xpath("//*[contains(text(),'Land Size')]/following-sibling::div[1]/text()").get('').strip()
            # if land_area:
            #     item['Field36'] = land_area.replace('sqm','').replace(' ','')
