...
class Hood_Sold(scrapy.Spider):
    name = "Hood_Sold"

    url = "https://hoodestateagents.com.au/ajaxPublish?_s_=Results&_f_=ForSale-Search-V2&n=9&s=D&y=Settled&page={}"
    prefix = "https://hoodestateagents.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Hood_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".listing-title a ::attr(href)").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://hoodestateagents.com.au{each_url}'
        ...
        for property_url in all_links:
            property_url = f'https://hoodestateagents.com.au{property_url}'
        ...
        next_page = response.xpath("//a[contains(text(),'Next')]/@onclick").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3233)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Hood Estate Agents'

            property_street = response.css('.listing-location ::text').get('').strip()  # property_street
            ...
            item['Field6'] = response.xpath("//*[contains(text(),'Bed')]/parent::li[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(text(),'Bath')]/parent::li[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(text(),'Parking')]/parent::li[1]/text()").get('').strip()

            price = response.css('.listing-price ::text').get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.listing-title ::text').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".row-cols-1 .col p ::text").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.css(".ls-bg ::attr(src)").getall()
            images = []
            for img in prop_images:
                img = img.replace('background-image:url(','').replace(");","")
                images.append(f'https://hoodestateagents.com.au{img}' if img else "")
            ...
            '''          AGENTS          '''
            if response.css(".media"):
                agent1 = response.css(".media")[0]
                ...
                if agent1_name:
                    ...
                    item['Field20'] = agent1.css("span ::text").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".rounded-circle ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'https://hoodestateagents.com.au{agent_image}' if agent_image else ""
                    # item['Field22'] = images_string
                    # print('Agent1 image is :',images_string)
                    random_number = random.randint(1, 10000000000)
                    item['Field22'] = ",".join(self.uploaded(images_string, random_number))

            if len(response.css(".media").getall()) > 1:
                agent2 = response.css(".media")[1]
                ...
                if agent2:
                    ...
                    item['Field26'] = agent2.css("span ::text").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".rounded-circle ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'https://hoodestateagents.com.au{agent_image}' if agent_image else ""
            ...
            if len(response.css(".media").getall()) > 2:
                agent3 = response.css(".media")[2]
                ...
                if agent3:
                    ...
                    item['Field31'] = agent3.css("span ::text").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".rounded-circle ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'https://hoodestateagents.com.au{agent_image}' if agent_image else ""
            ...
            item['Field33'] = response.xpath("//*[contains(text(),'Property ID :')]/following-sibling::td[1]/text()").get('').strip()
            item['Field35'] = response.xpath("//*[contains(text(),'Category')]/following-sibling::td[1]/text()").get('').strip()

            land_area = response.xpath("//*[contains(text(),'Area')]/parent::li[1]/text()").get('').strip().replace('m','')
            if land_area:
            ...
            #
            feature_count = 58
            for feature in response.xpath("//*[contains(text(),'Property Features')]/following-sibling::ul/li"):   ##  first 4 features are ID, bed, bath etc.
                if 'condition' in ''.join(e.strip().lower() for e in feature.css('::text').getall()):
                    item['Field52'] = ''.join(e.strip() for e in feature.css('::text').getall())
                else:
                    item[f'Field{feature_count}'] = ''.join(e.strip().lower() for e in feature.css('::text').getall())
                    feature_count += 1
