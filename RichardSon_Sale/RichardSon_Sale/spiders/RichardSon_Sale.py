...
class RichardSon_Sale(scrapy.Spider):
    name = "RichardSon_Sale"

    url = "https://www.richardsonpg.com.au/buying/properties-for-sale/"
    prefix = "https://www.richardsonpg.com.au"
    headers = {
    ...
    count = 1
    db = 'RichardSon'
    collection = 'RichardSon_Sale'
    bucket_prefix = f'RSPG_{collection}'

    local_file_path = '/img'

    def __init__(self, *args, **kwargs):
        super(RichardSon_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3026)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Richardson Property Group'
            ...
            if response.xpath("//*[contains(text(),'Garage')]/following-sibling::div[1]/text()").get('').strip():
               item['Field8'] = response.xpath("//*[contains(text(),'Garage')]/following-sibling::div[1]/text()").get('').strip()
            elif  response.xpath("//*[contains(text(),'Carport')]/following-sibling::div[1]/text()").get('').strip():
               item['Field8'] = response.xpath("//*[contains(text(),'Carport')]/following-sibling::div[1]/text()").get('').strip()
            elif  response.xpath("//*[contains(text(),'Car Space')]/following-sibling::div[1]/text()").get('').strip():
               item['Field8'] = response.xpath("//*[contains(text(),'Car Space')]/following-sibling::div[1]/text()").get('').strip()

            price = response.css('.suburb-price ::text').get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = (price.strip()).replace('SOLD','').replace('$','')   # sold_price

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.col-md-7 > .sub-title ::text').getall())  # description
            ...
            if response.css(".mb-20"):
                ...
                if agent1_name:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".img-default-used-size-large ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.css(".mb-20").getall()) > 1:
                ...
                if agent2:
                    ...
                    item['Field26A'] = agent2.css(".phone a ::text").get('').strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".img-default-used-size-large ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css(".mb-20").getall()) > 2:
                ...
                if agent3:
                    ...
                    item['Field31A'] = agent3.css(".phone a ::text").get('').strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".img-default-used-size-large ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
