...
class PremiereRE_Rent(scrapy.Spider):
    name = "PremiereRE_Rent"

    url = "https://www.premiererealestate.com.au/pages/real-estate/results?widgetKey=80768&context=listings&returnAs=markup&listing_sale_method=lease&listing_category=&status=rent&listing_suburb_search=&listing_price_from=&listing_price_to=&listing_bedrooms=&listing_bathrooms=&surrounds=&staff_id=&pg=1"
    prefix = "https://www.premiererealestate.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(PremiereRE_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".listing-wrapper a ::attr(href)").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://www.premiererealestate.com.au{each_url}'
        ...
        for property_url in all_links:
            property_url = f'https://www.premiererealestate.com.au{property_url}'
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3177)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Premiere Real Estate'
