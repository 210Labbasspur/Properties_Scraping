...
class OttoCapital_Sale(scrapy.Spider):
    name = "OttoCapital_Sale"
    url = 'https://www.ottocapital.com.au/wp-admin/admin-ajax.php'
    prefix = "https://www.ottocapital.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(OttoCapital_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3017)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Otto Capital'
            ...
            item['Field8'] = ''.join(e.strip() for e in response.xpath("//*[contains(@class,'la-car')]/parent::*/text()").getall())

            # item['Field9'] = property.get('price','').replace('$','').replace(',','').replace(' ','')
            item['Field9'] = response.css('.property-price ::text').get('').strip().replace('$','').replace(',','').replace(' ','')
            ...
            if response.css(".listing-agent"):
                ...
                agent1_name = agent1.css('p.mb-0 ::text').get('').strip()
                if agent1_name:
            ...
            if len(response.css(".listing-agent").getall()) > 1:
                ...
                agent2_name = agent2.css('p.mb-0 ::text').get('').strip()
                if agent2:
            ...
            if len(response.css(".listing-agent").getall()) > 2:
                ...
                agent3_name = agent3.css('p.mb-0 ::text').get('').strip()
                if agent3:
