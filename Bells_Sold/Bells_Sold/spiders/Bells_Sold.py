...
class Bells_Sold(scrapy.Spider):
    name = "Bells_Sold"

    url = "https://www.bellsrealestate.com.au/data/results/?listing_sale_method=Sale&listing_status=sold&status=sold&gallery=1&pg={}"
    prefix = "https://www.bellsrealestate.com.au"
    headers = {
    'accept': '*/*',
    'accept-language': 'en-US,en;q=0.9',
    'priority': 'u=0, i',
    'referer': 'https://www.bellsrealestate.com.au/sold',
    ...
    # 'cookie': '_WHEELS_AUTHENTICITY=ULaO7W4MM5kryr%2BzTPlA1zbKI0DUK6cC9%2Bps5xbo%2BYaD7YDYahdk464kTtEM%2FM4ofnGUHWXbVevuTT%2FFEzWBwiKsLa49DW5qqJVC94K1K0TOBURfjzE%2FHEd1TU2F8iRwluh4XmPzOTxTPsAv6cYLcA%3D%3D; FLASH=%7B%7D',
}

    count = 1
    db = 'Bells'
    collection = 'Bells_Sold'
    ...
    def __init__(self, *args, **kwargs):
        super(Bells_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        ...
        if selector.css(".listing-link"):
            all_links = selector.css(".listing-link ::attr(href)").getall()
            links = []
            for each_url in all_links:
                each_url = f'https://www.bellsrealestate.com.au{each_url}'
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2755)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Bells Real Estate'

            property_street = response.css('.xxxxxxxxxxxxxx ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.listing-address h1 ::text').getall() if e.strip())  # property_suburb
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'fa-bed')]/preceding-sibling::*[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'fa-bath')]/preceding-sibling::*[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'fa-car')]/preceding-sibling::*[1]/text()").get('').strip()

            price = response.css('.side-content div ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.listing-heading ::text').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".listing-content p ::text").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.xpath("//*[contains(@class,'slide-image')]/@src").getall()
            images = []
            ...
            if response.css(".staff-member"):
                ...
                if agent1_name:
                    ...
                    item['Field18'] = agent1.css('.agent-pos ::text').get('').strip()
            ...
            if len(response.css(".staff-member").getall()) > 1:
                ...
                if agent2:
                    ...
                    item['Field24'] = agent2.css('.agent-pos ::text').get('').strip()
            ...
            land_area = response.xpath('//*[contains(text(),"Land Size ")]/text()').get('').strip().replace('Land','').replace('Size','').replace('is','').replace('m','')
            if land_area:
            ...
            #
            feature_count = 58
            for feature in response.css(".listing-links li"):   ##  first 4 features are ID, bed, bath etc.
