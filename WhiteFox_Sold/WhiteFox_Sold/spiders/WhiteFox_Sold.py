...
class WhiteFox_Sold(scrapy.Spider):
    name = "WhiteFox_Sold"

    url = "https://www.whitefoxrealestate.com.au/sold/"
    # url = "https://www.whitefoxrealestate.com.au/buy/"
    prefix = "https://www.whitefoxrealestate.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(WhiteFox_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def start_requests(self):
        ...
        try:
            ...
            while True:
                ...
                try:
                    load_more_button = driver.find_element(By.XPATH,'//div[@class="property-blocks__load-more" and contains(text(), "Load More")]')
                    if load_more_button.is_displayed():
                        load_more_button.click()
                        time.sleep(2)  # Wait for new content to load after clicking
                except:
                    pass  # If "Load More" button is not found, continue scrolling
        ...
        '''    FARHAN'S LOGIC    '''
        all_links = response.css("a.property-card__inner ::attr(href)").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://www.whitefoxrealestate.com.au{each_url}'
        ...
        for property_url in all_links:
            property_url = f'https://www.whitefoxrealestate.com.au{property_url}'
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2384)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'White Fox Real Estate'

            property_street = response.css('.xxxxxxxxxxxxxxxx ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.sp__intro h1 ::text').getall() if e.strip())  # property_suburb
            property_address = f"{property_street} {property_suburb}"
            item['Field5'] = property_address  # full_adddress

            for bbc in response.css('.sp__stats p'):
                bbc_check = ''.join(bbc.css('::text').getall())
                if 'Bed' in bbc_check:
                    item['Field6'] = bbc_check.replace('Bed','').replace('s','').replace(' ','')
                if 'Bath' in bbc_check:
                    item['Field7'] = bbc_check.replace('Bath','').replace('s','').replace(' ','')
                if 'Car' in bbc_check:
                    item['Field8'] = bbc_check.replace('Car','').replace('s','').replace(' ','')

            price = ''.join(e.strip() for e in response.css('.sp__price p ::text').getall()).replace('Sold','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price
            ...
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".sp__copy p ::text").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.xpath("//*[@class='slick-slide']//img/@src").getall()
            images = []
            images.append(response.xpath("//*[@class='slick-slide slick-active slick-current']//img/@src").get('').strip())
            ...
            '''          AGENTS          '''
            if response.css(".sp__agent"):
                agent1 = response.css(".sp__agent")[0]
                agent1_name = ' '.join(e.strip() for e in agent1.css('h5 a ::text').getall())
                if agent1_name:
                    ...
                    images_string = f'https://www.whitefoxrealestate.com.au{agent_image}' if agent_image else ""
                    # item['Field22'] = images_string
                    # print('Agent1 image is :',images_string)
                    random_number = random.randint(1, 10000000000)
                    item['Field22'] = ",".join(self.uploaded(images_string, random_number))

            if len(response.css(".sp__agent").getall()) > 1:
                agent2 = response.css(".sp__agent")[1]
                agent2_name = ' '.join(e.strip() for e in agent2.css('h5 a ::text').getall())
                if agent2:
            ...
            if len(response.css(".sp__agent").getall()) > 2:
                agent3 = response.css(".sp__agent")[2]
                agent3_name = ' '.join(e.strip() for e in agent3.css('h5 a ::text').getall())
                if agent3:
