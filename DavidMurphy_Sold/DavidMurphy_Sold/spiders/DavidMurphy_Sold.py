...
class DavidMurphy_Sold(scrapy.Spider):
    name = "DavidMurphy_Sold"

    url = "https://www.davidmurphy.com.au/recent-sales"
    prefix = "https://www.davidmurphy.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(DavidMurphy_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".propertyTile-anchor ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2918)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'David Murphy'

            property_street = response.css('.streetAddress ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.suburb ::text').getall() if e.strip())  # property_suburb
            property_address = f"{property_street} {property_suburb}"
            item['Field5'] = property_address  # full_adddress

            item['Field6'] = response.xpath("//i[contains(@class,'icon-bed')]/parent::span[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//i[contains(@class,'icon-bath')]/parent::span[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//i[contains(@class,'icon-car')]/parent::span[1]/text()").get('').strip()

            price = response.css('.muted ::text').get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price
            ...
            if response.css(".agentTile"):
                ...
                if agent1_name:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".agentTile-image ::attr(data-bg-src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'https://www.davidmurphy.com.au{agent_image}' if agent_image else ""
            ...
            if len(response.css(".agentTile").getall()) > 1:
                ...
                if agent2:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".agentTile-image ::attr(data-bg-src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'https://www.davidmurphy.com.au{agent_image}' if agent_image else ""
            ...
            if len(response.css(".agentTile").getall()) > 2:
                ...
                if agent3:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".agentTile-image ::attr(data-bg-src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'https://www.davidmurphy.com.au{agent_image}' if agent_image else ""
            ...
            item['Field35'] = response.css('.counter span ::text').get('').strip()
            ...
            floor_area = response.xpath('//*[contains(text(),"Floor Area")]/following-sibling::span[1]/text()').get('').strip().replace('Land','').replace('is','').replace('m²','')
            if floor_area:
            ...
            feature_count = 58
            for feature in response.css(".kvList li"):   ##  first 4 features are ID, bed, bath etc.
