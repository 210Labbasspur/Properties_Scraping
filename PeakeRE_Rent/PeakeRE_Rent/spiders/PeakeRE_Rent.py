...
class PeakeRE_Rent(scrapy.Spider):
    name = "PeakeRE_Rent"

    url = "https://peakere.com.au/lease/"
    prefix = "https://peakere.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(PeakeRE_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def start_requests(self):
        ...
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".card--property a ::attr(href)").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://peakere.com.au{each_url}'
        ...
        for property in response.css(".card--property"):
            price = property.css('.card__price ::text').get('').strip()
            property_url = f'https://peakere.com.au{property.css("a ::attr(href)").get("").strip()}'
            print(self.count,' # Property URL :', property_url)
            self.count += 1
            yield scrapy.Request(property_url, headers=self.headers, callback=self.detail_parse, meta={'price':price})
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3176)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Peake Real Estate'

            property_street = response.css('.xxxxxxxxxxxxx ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.property-template__popup-details h4 ::text').getall() if e.strip())  # property_suburb
            property_address = f"{property_street} {property_suburb}".replace('\n','').replace('\r','').replace('\t','')
            item['Field5'] = property_address.replace('  ','')  # full_adddress

            for bbc in response.css('.property-template__popup-details span'):
                bbc = ''.join(e.strip() for e in bbc.css("::text").getall())
                if 'bed' in bbc.lower():
                    item['Field6'] = bbc.strip().lower().replace("bed","")
                if 'bath' in bbc.lower():
                    item['Field7'] = bbc.strip().lower().replace("bath","")
                if 'car' in bbc.lower():
                    item['Field8'] = bbc.strip().lower().replace("car","")
                if 'm2' in bbc.lower():
                    item['Field36'] = bbc.strip().lower().replace("m2","")
            ...
            '''          AGENTS          '''
            # if response.css(".property-content__agents .card--agent"):
            #     agent1 = response.css(".property-content__agents .card--agent")[0]
            ...
            #         item['Field18'] = agent1.css('.card__role ::text').get('').strip()
            ...
            #         '''         Uploading Images on Wasabi S3 Bucket            '''
            #         agent_image = agent1.css(".card__image img ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            #
            # if len(response.css(".property-content__agents .card--agent").getall()) > 1:
            #     agent2 = response.css(".property-content__agents .card--agent")[1]
            #     agent2_name = agent2.css('h4 ::text').get('').strip()
            #     if agent2:
            #         item['Field23'] = agent2_name.strip()
            #         item['Field24'] = agent2.css('.card__role ::text').get('').strip()
            ...
            #         '''         Uploading Images on Wasabi S3 Bucket            '''
            #         agent_image = agent2.css(".card__image img ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
            #         images_string = f'{agent_image}' if agent_image else ""
            ...
            #
            # if len(response.css(".property-content__agents .card--agent").getall()) > 2:
            #     agent3 = response.css(".property-content__agents .card--agent")[2]
            #     agent3_name = agent3.css('h4 ::text').get('').strip()
            #     if agent3:
            #         item['Field28'] = agent3_name.strip()
            #         item['Field29'] = agent3.css('.card__role ::text').get('').strip()
            ...
            #         '''         Uploading Images on Wasabi S3 Bucket            '''
            #         agent_image = agent3.css(".card__image img ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
            #         images_string = f'{agent_image}' if agent_image else ""
            ...
            # land_area = response.xpath('//*[contains(@alt,"Land Size")]/following-sibling::span/text()').get('').strip().replace('Land','').replace('is','').replace('m²','')
            # if land_area:
            ...
            #
            feature_count = 58
            for feature in response.xpath("//*[contains(text(),'Key Features')]/following-sibling::ul/li"):   ##  first 4 features are ID, bed, bath etc.
