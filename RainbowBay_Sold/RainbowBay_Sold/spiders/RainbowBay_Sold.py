...
class RainbowBay_Sold(scrapy.Spider):
    name = "RainbowBay_Sold"

    url = 'https://www.rainbowbayrealty.com.au/sold?&status=current&disposalMethod=sold&orderBy=sold&page=1'
    prefix = "https://www.rainbowbayrealty.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(RainbowBay_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css("#propertyListWrapper .col-lg-4 a ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2248)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Rainbow Bay Realty Property Consultants'

            property_street = response.css('.addressHeading ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.suburbHeading span ::text').getall() if e.strip())  # property_suburb
            property_address = f"{property_street} {property_suburb}"
            item['Field5'] = property_address  # full_adddress

            item['Field6'] = response.xpath("//*[contains(@d,'M72.6,41.3V18.1c0-1.4-1.2-')]/ancestor::span[1]/following-sibling::*[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@d,'4,50.3c0,3.3,2,6.2,4.9,7.6l-')]/ancestor::span[1]/following-sibling::*[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@d,'M84.3,30.1c-0.5')]/ancestor::span[1]/following-sibling::*[1]/text()").get('').strip()

            price = response.css('.leftContentWrapper h2 ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            ...
            prop_images = response.xpath("//*[contains(@data-fancybox-group,'gallery')]/@href").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.css("#agentSlider .carousel-inner"):
                agent1 = response.css("#agentSlider .carousel-inner")[0]
            ...
            if len(response.css("#agentSlider .carousel-inner").getall()) > 1:
                agent2 = response.css("#agentSlider .carousel-inner")[1]
            ...
            if len(response.css("#agentSlider .carousel-inner").getall()) > 2:
                agent3 = response.css("#agentSlider .carousel-inner")[2]
            ...
            land_area = response.xpath('//*[contains(text(),"Land Size")]/ancestor::span[1]/following-sibling::*[1]/section[1]/text()').get('').strip().replace('Land','').replace('is','').replace('m²','')
            if land_area:
            ...
            #
            feature_count = 58
            for feature in response.css(".showFeatureList section b"):   ##  first 4 features are ID, bed, bath etc.
