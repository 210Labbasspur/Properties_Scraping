...
class ReMax_Rent(scrapy.Spider):
    name = 'ReMax_Rent'
    prefix = 'https://www.propertyspecialists.com.au'
    url = "https://www.propertyspecialists.com.au/renting/properties-for-rent/"
    headers = {
    ...
    def parse(self, response):
        for property in response.xpath("//*[contains(@class,'image notsold')]"):
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            item = dict()
            item['Field2'] = '1111'     # Keep_it_as_it_is
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')   # date_data_scanned

            address = response.css('.address ::text').getall()
            property_address = ', '.join(element.strip() for element in address)
            ...
            if response.xpath("//*[contains(text(),'Land Size')]/following-sibling::span/text()").get('').strip():
            ...
            elif response.xpath("//*[contains(text(),'Building Size')]/following-sibling::span/text()").get('').strip():
                land_area = response.xpath("//*[contains(text(),'Land Size')]/following-sibling::span/text()").get('').strip()
                item['Field36'] = land_area.replace(' m2','')   # land_area
