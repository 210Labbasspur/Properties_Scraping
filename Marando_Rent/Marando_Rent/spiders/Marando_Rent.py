...
class Marando_Rent(scrapy.Spider):
    name = 'Marando_Rent'
    prefix = 'http://www.marandorealestatesw.com.au'
    url = "https://www.marandorealestatesw.com.au/page/rental-listings"
    headers = {
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            if agent1_name:
                ...
                '''         Uploading Images on Wasabi S3 Bucket            '''
                Agent_Imagess = response.xpath(
                    "//*[contains(@class,'agent_details')][1]/preceding-sibling::img[1]/@src").getall()
                Agent_Imagesss = []
            ...
            if agent2:
                ...
                '''         Uploading Images on Wasabi S3 Bucket            '''
                Agent_Imagess = response.xpath(
                    "//*[contains(@class,'agent_details')][2]/preceding-sibling::img[1]/@src").getall()
                Agent_Imagesss = []
                for url in Agent_Imagess:
                    Agent_Imagesss.append(self.prefix + url)
                Agent_Images = ','.join(Agent_Imagesss[:1])
            ...
            item['Field33'] = response.xpath("//*[contains(text(),'Property ID:')]/text()").get('').strip().replace(
                'Property ID:', '')  # external_property_id
