...
class ArthurRE_Sold(scrapy.Spider):
    name = 'ArthurRE_Sold'
    prefix = 'https://arthurrealestate.com.au'
    url = "https://arthurrealestate.com.au/recently-sold-properties/page/1/?pagination_id=1"
    headers = {
    ...
    def parse(self, response):
        for property in response.css("a.epl-more-link"):
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'Arthur Real Estate'
            ...
            if re.search(r'\d', sold_price):
                item['Field9'] = (sold_price.strip()).replace('for','').replace('$','')   # sold_price
            ...
            Agent_Imagess = response.xpath("//*[contains(@u,'image')]/@src").getall()
            new_img_urls = []
            ...
            item['Field35'] = response.css(".property-category ::text").get('').strip().replace(',','').replace('sale','')
            land_area = response.xpath("//*[contains(text(),'Land is')]/text()").get('').strip().replace('Land is','')
            if land_area:
                item['Field36'] = land_area.replace('m²','').replace(' ','')   # land_area

            # feature_count = 58
            # for feature in response.css(".epl-tax-feature a"):
