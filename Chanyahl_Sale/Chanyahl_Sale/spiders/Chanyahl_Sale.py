...
class Chanyahl_Sale(scrapy.Spider):
    name = 'Chanyahl_Sale'
    prefix = 'https://chanyahl.com.au'
    url = "https://chanyahl.com.au/wp-json/api/listings/all?priceRange=&limit=12&type=property&status=current&address=&paged={}&bed=&bath=&car=&sort=&homeLand=&landProperty=&ruralLifestyle=&networkProperties="
    headers = {
        'accept': '*/*',
        'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        # 'cookie': 'selectedSiteID=192; afl_wc_utm_192_cookie_expiry=90; afl_wc_utm_192_sess_visit=1714346113; afl_wc_utm_192_sess_landing=https%3A%2F%2Fchanyahl.com.au%2Frecent-sales%2F; afl_wc_utm_192_utm_1st_url=https%3A%2F%2Fchanyahl.com.au%2Frecent-sales%2F%3Futm_source%3Ddirect%26utm_medium%3Dnone; afl_wc_utm_192_utm_1st_visit=1714346113; afl_wc_utm_192_utm_url=https%3A%2F%2Fchanyahl.com.au%2Frecent-sales%2F%3Futm_source%3Ddirect%26utm_medium%3Dnone; afl_wc_utm_192_utm_visit=1714346113; afl_wc_utm_192_main=%7B%22updated_ts%22%3A1714346157%7D; arp_scroll_position=100; cf_chl_3=00dd0326d1a1fec; cf_chl_rc_m=13; rest-api-token=6f4bf7edb8; _gid=GA1.3.374641711.1714349851; _ga_JVW86Z9FG7=GS1.1.1714349843.1.1.1714349852.0.0.0; _ga=GA1.3.1004767950.1714349844; rest-api-token=6f4bf7edb8; selectedSiteID=192',
        'pagefrom': 'archive',
        'priority': 'u=1, i',
        # 'referer': 'https://chanyahl.com.au/recent-sales/?pageno=2',
    ...
    def parse(self, response):
        ...
        for property in data['results']:
            property_no += 1
            property_url = property.get('link')
            ...
            mini_data['sold_price'] = property.get('propertyPricing').get('value').replace('$','')
            ...
            elif 'm' in property.get('landAreaUnit'):
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            # item['Field9'] = mini_data['sold_price'].replace('$','')

            description = response.css('#fws_5c9c6ac25d1fe p ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            new_img_urls = []
            for url in full_data['propertyListImages']:
            ...
            '''          AGENTS          '''
            if response.css(".listing-agents-wrapper .author-box-wrapper"):
                agent1 = response.css(".listing-agents-wrapper .author-box-wrapper")[0]
                agent1_name = agent1.css('.font-bold a ::text').get('').strip()
                if agent1:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field18'] = agent1.css('.author-position ::text').get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images_style = agent1.css(".author-profile-image ::attr(style)").get('').strip()
                ...
                if len(response.css(".listing-agents-wrapper .author-box-wrapper").getall()) > 1:
                    agent2 = response.css(".listing-agents-wrapper .author-box-wrapper")[1]
                    agent2_name = agent2.css('.font-bold a ::text').get('').strip()
                    if agent2:
                        item['Field23'] = agent2_name.strip()
                        item['Field24'] = agent2.css('.author-position ::text').get('').strip()
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images_style = agent2.css(".author-profile-image ::attr(style)").get('').strip()
                        Agent_Images = re.search(r'url\((.*?)\)', Agent_Images_style).group(1)
                ...
                if len(response.css(".listing-agents-wrapper .author-box-wrapper").getall()) > 2:
                    agent3 = response.css(".listing-agents-wrapper .author-box-wrapper")[2]
                    agent3_name = agent3.css('.font-bold a ::text').get('').strip()
                    if agent3:
                        item['Field28'] = agent3_name.strip()
                        item['Field29'] = agent3.css('.author-position ::text').get('').strip()
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images_style = agent3.css(".author-profile-image ::attr(style)").get('').strip()
                        Agent_Images = re.search(r'url\((.*?)\)', Agent_Images_style).group(1)
            ...
            item['Field35'] = full_data['propertyCategory']
