...
class Bennett_Rent(scrapy.Spider):
    name = 'Bennett_Rent'
    prefix = 'https://www.bennettproperty.com.au'
    url = "https://www.bennettproperty.com.au/wp-admin/admin-ajax.php"
    headers = {
        'accept': '*/*',
        'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        'content-type': 'application/x-www-form-urlencoded; charset=UTF-8',
        # 'cookie': '_gid=GA1.3.1153846141.1713560580; _ga=GA1.3.896166064.1713300981; _ga_T9SNFCQX3F=GS1.1.1713560579.2.1.1713561829.60.0.0; arp_scroll_position=2440',
        'origin': 'https://www.bennettproperty.com.au',
        'priority': 'u=1, i',
        # 'referer': 'https://www.bennettproperty.com.au/selling/recent-sales/',
    ...
    data = {
        'atts[list]': 'lease',        'atts[multilist]': '',        'atts[layout]': '',
        'atts[template]': 'Shortcode.SearchResults.SearchResults',        'atts[selector_listings]': 'ap-listing-search-results',
        'atts[ajax_template]': 'Ajax.SearchResults',        'atts[load_more]': 'true',
        'atts[hide_search_form]': '0',        'atts[row_col_class]': 'row row-cols-xl-3 row-cols-lg-2 row-cols-1',
        'atts[sur_suburbs]': '0',        'atts[center_latlng]': '',
        'atts[sur_suburbs_radius]': '10',        'atts[max_page]': 'false',
        'atts[map_zoom]': '11',        'atts[map]': 'map_canvas',        'atts[map_load_all_marker]': '0',
        'atts[map_callback]': 'ap_realty.searchResultsMapCallback',        'atts[map_attribute_cluster]': '0',
        'atts[map_is_visible]': '1',        'atts[property_type_column]': '1,2,3',        'atts[content_first]': '0',
        'atts[auth]': '0',        'atts[auth_type]': 'content',        'atts[per_page]': '12',
        ...
        'per_page': '12',        'load_more': 'true',        'selectorMap': '#map_canvas',
    ...
    def parse(self, response):
        ...
        for property in data['data']['props']:
            ...
            mini_data['car'] = property.get('carport')
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            Agent_Imagess = response.css(".item-image a::attr(href)").getall()
            new_img_urls = []
            ...
            if response.css(".listing-agent"):
                ...
                if agent1:
                    ...
                    agent1_phone = agent1.xpath(".//*[contains(@class,'las la-phone')]/parent::a[contains(@href,'tel:')]/@href").get('').strip().replace('tel:','')  # description
                    item['Field20'] = agent1_phone.strip()  # description
                    agent1_mobile = agent1.xpath(".//*[contains(@class,'las la-mobile')]/parent::a[contains(@href,'tel:')]/@href").get('').strip().replace('tel:','')  # description
                ...
                if len(response.css(".listing-agent").getall()) > 1:
                    ...
                    if agent2:
                        ...
                        agent2_phone = agent2.xpath(".//*[contains(@class,'las la-phone')]/parent::a[contains(@href,'tel:')]/@href").get('').strip().replace('tel:', '')  # description
                        item['Field26'] = agent2_phone.strip()  # description
                        agent2_mobile = agent2.xpath(".//*[contains(@class,'las la-mobile')]/parent::a[contains(@href,'tel:')]/@href").get('').strip().replace('tel:', '')  # description
                ...
                if len(response.css(".listing-agent").getall()) == 3:
                    agent3 = response.css(".listing-agent")[2]
                    agent3_name = agent3.css('.agent-name ::text').get('').strip()
                    if agent3:
                        item['Field28'] = agent3_name.strip()
                        agent3_phone = agent3.xpath(".//*[contains(@class,'las la-phone')]/parent::a[contains(@href,'tel:')]/@href").get('').strip().replace('tel:', '')  # description
                        item['Field31'] = agent3_phone.strip()  # description
                        agent3_mobile = agent3.xpath(".//*[contains(@class,'las la-mobile')]/parent::a[contains(@href,'tel:')]/@href").get('').strip().replace('tel:', '')  # description
