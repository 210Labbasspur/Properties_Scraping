...
class Bellarine_Sale(scrapy.Spider):
    name = "Bellarine_Sale"

    url = "https://www.bellarineproperty.com.au/buyer/properties-for-sale/"
    prefix = "https://www.bellarineproperty.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Bellarine_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".property-image ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2603)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Bellarine Property'

            property_street = response.css('.xxxxxxxxxxxxxx ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.property-title ::text').getall() if e.strip())  # property_suburb
            property_address = f"{property_street} {property_suburb}"
            item['Field5'] = property_address  # full_adddress

            item['Field6'] = response.xpath("//*[contains(@class,'las la-bed')]/following-sibling::*[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'las la-bath')]/following-sibling::*[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'las la-car')]/following-sibling::*[1]/text()").get('').strip()

            price = response.css('.description .price ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.sub-title ::text').getall())  # description
            ...
            prop_images = response.xpath("//*[contains(@class,'popup-gallery slider-image')]/@href").getall()
            images = []
            ...
            if response.css(".listing-agent"):
                ...
                if agent1_name:
                    ...
                    item['Field18'] = agent1.css('.agent-role ::text').get('').strip()
            ...
            if len(response.css(".listing-agent").getall()) > 1:
                ...
                if agent2:
                    ...
                    item['Field24'] = agent2.css('.agent-role ::text').get('').strip()
            ...
            if len(response.css(".listing-agent").getall()) > 2:
                ...
                if agent3:
                    ...
                    item['Field29'] = agent3.css('.agent-role ::text').get('').strip()
            ...
            land = response.xpath("//*[contains(@class,'las la-expand')]/following-sibling::*[1]/text()").get('').strip()
            land_area = land.replace('Land','').replace('is','').replace('m²','').replace('sqm','')
            if land_area:
