...
class Barnett_Rent(scrapy.Spider):
    name = "Barnett_Rent"

    url = "https://barnettproperty.com.au/search/?show_in_rental=true&sold=0"
    prefix = "https://barnettproperty.com.au"
    headers = {
    ...
    count = 1
    db = 'Barnett'
    collection = 'Barnett_Rent'
    bucket_prefix = f'WEA_{collection}'

    local_file_path = '/img'

    def __init__(self, *args, **kwargs):
        super(Barnett_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".image-wrapper a ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2979)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Barnett Real Estate'

            property_street = response.css('.address h3 ::text').get('').strip()  # property_street
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'attribute attr-bed')]/span[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'attribute attr-bath')]/span[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'attribute attr-car')]/span[1]/text()").get('').strip()

            price = response.css('.price h3 ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = (price.strip()).replace('SOLD','').replace('$','')   # sold_price
            ...
            '''          AGENTS          '''
            if response.css(".agents .col-md-6"):
                agent1 = response.css(".agents .col-md-6")[0]
                ...
                if agent1_name:
                    ...
                    item['Field18'] = agent1.css('.agent-name small ::text').get('').strip()
                    item['Field19'] =  agent1.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:','')
                    item['Field20'] = agent1.xpath("//*[contains(@class,'fa-phone')]/following-sibling::a/text()").get('').strip()
                    item['Field21'] = agent1.xpath("//*[contains(@class,'fa-mobile')]/following-sibling::a/text()").get('').strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".lazy-image ::attr(data-src)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.css(".agents .col-md-6").getall()) > 1:
                agent2 = response.css(".agents .col-md-6")[1]
                agent2_name = agent2.css('.agent-name ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.css('.agent-name small ::text').get('').strip()
                    item['Field25'] = agent2.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:', '')
                    item['Field26'] = agent2.xpath("//*[contains(@class,'fa-phone')]/following-sibling::a/text()").get('').strip()
                    item['Field26A'] = agent2.xpath("//*[contains(@class,'fa-mobile')]/following-sibling::a/text()").get('').strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".lazy-image ::attr(data-src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css(".agents .col-md-6").getall()) > 2:
                agent3 = response.css(".agents .col-md-6")[2]
                agent3_name = agent3.css('.agent-name ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field29'] = agent3.css('.agent-name small ::text').get('').strip()
                    item['Field30'] = agent3.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:', '')
                    item['Field31'] = agent3.xpath("//*[contains(@class,'fa-phone')]/following-sibling::a/text()").get('').strip()
                    item['Field31A'] = agent3.xpath("//*[contains(@class,'fa-mobile')]/following-sibling::a/text()").get('').strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".lazy-image ::attr(data-src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            item['Field35'] = response.css(".address p ::text").get('').strip()
            land_area = response.xpath("//*[contains(@class,'attribute attr-land')]/span[1]/text()").get('').strip()
            if land_area:
                item['Field36'] = land_area.replace('m','').replace(' ','')   # land_area

            # floor_area = response.xpath("//*[contains(text(),'Floor Size')]/following-sibling::div[1]/text()").get('').strip()
            # if floor_area:
            ...
            feature_count = 58
            for feature in response.css(".features-wrapper .feature"):   ##  first 4 features are ID, bed, bath etc.
