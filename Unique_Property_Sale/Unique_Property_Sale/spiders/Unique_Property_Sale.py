...
class Unique_Property_Sale(scrapy.Spider):
    name = 'Unique_Property_Sale'
    prefix = 'https://www.unique-property.com'
    url = "https://www.unique-property.com/buying/properties-for-sale/"
    headers = {
        'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'Accept-Language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        'Cache-Control': 'max-age=0',
        'Connection': 'keep-alive',
        # 'Cookie': '_ga=GA1.1.1390818424.1711187472; _ga_6EQ6WL2YWS=GS1.1.1711187472.1.1.1711187599.0.0.0; arp_scroll_position=2216',
    ...
    count = 1
    db = 'Unique_Property'
    collection = 'Unique_Property_Sale'
    bucket_prefix = 'D_Unique_Property_Sale'
    ...
    def parse(self, response):
        for property in response.xpath("//*[contains(@class,'container p-0 bg-black-5')]"):
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            property_address = response.xpath("//label[contains(text(),'Location')]/following-sibling::div/text()").get('').strip()   # full_adddress
            item['Field5'] = response.xpath("//label[contains(text(),'Location')]/following-sibling::div/text()").get('').strip()   # full_adddress
            ...
            car = response.xpath("//label[contains(text(),'Parking')]/following-sibling::div/text()").get('').strip()
            if car:
            ...
            new_img_urls = response.xpath("//*[contains(@class,'img-orientation img-orientation-landscape img-default-used-size-medium')]/@src").getall()
            ...
            agent1 = response.xpath("//*[contains(@class,'bg-black-50 p-3 d-flex flex-grow-1 flex-column')][1]")
            ...
            if agent1_name:
                ...
                item['Field20'] = (agent1.xpath(".//*[contains(@class,'mobile mb-0')]/a/@href").get('').strip()).replace('tel:','')   # agent_mobile_1
                '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            agent2 = response.xpath("//*[contains(@class,'bg-black-50 p-3 d-flex flex-grow-1 flex-column')][2]")
            if agent2:
                item['Field23'] = agent2.css('.agent-name ::text').get('').strip()   # agent_full_name_2
                item['Field26'] = (agent2.xpath(".//*[contains(@class,'mobile mb-0')]/a/@href").get('').strip()).replace('tel:','')   # agent_mobile_1
                '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            agent3 = response.xpath("//*[contains(@class,'bg-black-50 p-3 d-flex flex-grow-1 flex-column')][3]")
            if agent3:
                item['Field28'] = agent3.css('.agent-name ::text').get('').strip()   # agent_full_name_3
                item['Field31'] = (agent3.xpath(".//*[contains(@class,'mobile mb-0')]/a/@href").get('').strip()).replace('tel:','')   # agent_mobile_1
                '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            item['Field102'] = response.xpath("//label[contains(text(),'Contract')]/following-sibling::div/text()").get('').strip()   # external_property_id
