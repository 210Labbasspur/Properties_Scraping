...
class FoxRealEstate_Rent(scrapy.Spider):
    name = "FoxRealEstate_Rent"
    url = 'https://www.foxrealestate.com.au/wp-admin/admin-ajax.php'
    prefix = "https://www.foxrealestate.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(FoxRealEstate_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2071)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Fox Real Estate'
            ...
            item['Field9'] = response.css('.price ::text').get('').strip().replace('$','').replace(',','').replace(' ','')
