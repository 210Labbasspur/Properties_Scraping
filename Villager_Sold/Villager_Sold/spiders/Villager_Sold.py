...
class Villager_Sold(scrapy.Spider):
    name = 'Villager_Sold'
    prefix = 'https://villagerproperty.com.au'
    url = 'https://villagerproperty.com.au/wp-admin/admin-ajax.php'
    headers = {
        ...
        # 'Referer': 'https://villagerproperty.com.au/buy/sold/',
    ...
    data = {
        'view_number': '22236',
        'page': '1',
        'sort[wpv_orderby]': '',        'sort[wpv_order]': '',
        'sort[wpv_sort_orderby]': '',        'sort[wpv_sort_order]': '',
        'sort[wpv_sort_orderby_as]': '',        'sort[wpv_sort_orderby_second]': '',
        'sort[wpv_sort_order_second]': '',        'id': '22236',
        'environment[wpv_aux_current_post_id]': '10604',        'environment[wpv_aux_parent_post_id]': '10604',
        'environment[archive][type]': '',        'environment[archive][name]': '',
        'expect': 'full',        'action': 'wpv_get_view_query_results',        'wpv_view_widget_id': '0',
    }
    count = 1
    db = 'Villager'
    collection = 'Villager_Sold'
    ...
    def parse(self, response):
        ...
        selector = Selector(text=data['data']['full'])
        for property in selector.css('a.masonry-link'):
        ...
        '''       Pagination     '''
        next_page = selector.css('a.js-wpv-pagination-next-link ::attr(href)').get('').strip()
        if next_page:
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'Villager Property'

            property_add = response.css('.property-heading3 a ::text').getall()  # full_adddress
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'bedrooms')]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'bathrooms')]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'garages')]/text()").get('').strip()

            sold_price = response.css('.sale-price ::text').get('').strip().replace(' ','')
            if sold_price:
                item['Field9'] = (sold_price.strip()).replace('$','')   # sold_price

            description = response.css('#readmore-section ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess = response.css(".slides li a ::attr(href)").getall()
            new_img_urls = []
            ...
            '''          AGENTS          '''
            if response.css(".agent-list-container"):
                agent1 = response.css(".agent-list-container")[0]
                agent1_name = agent1.css('.agent-list-title a ::text').get('').strip()
                if agent1:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images = agent1.css("img ::attr(data-lazy-src)").get('').strip()
                ...
                if len(response.css(".agent-list-container").getall()) > 1:
                    agent2 = response.css(".agent-list-container")[1]
                    agent2_name = agent2.css('.agent-list-title a ::text').get('').strip()
                    if agent2:
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent2.css("img ::attr(data-lazy-src)").get('').strip()
                ...
                if len(response.css(".agent-list-container").getall()) > 2:
                    agent3 = response.css(".agent-list-container")[2]
                    agent3_name = agent3.css('.agent-list-title a ::text').get('').strip()
                    if agent3:
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent3.css("img ::attr(data-lazy-src)").get('').strip()
