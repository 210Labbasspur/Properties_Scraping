...
class ThreeRealty_Rent(scrapy.Spider):
    name = 'ThreeRealty_Rent'
    prefix = 'https://www.3realty.com.au'
    url = "https://www.3realty.com.au/lease/properties-for-lease/"
    headers = {
        'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        'cache-control': 'max-age=0',
        # 'cookie': '_ga=GA1.1.464605258.1713705162; _ga_MX4H14PVHP=GS1.1.1713815343.2.0.1713815343.0.0.0',
        'priority': 'u=0, i',
    ...
    def parse(self, response):
        for property in response.css(".img-container"):
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = '3 Realty'

            property_add = response.css('.property-address-title ::text').getall()  # full_adddress
            ...
            # item['Field6'] = response.css(".listing-mid-section__information .bedroom ::text").get('').strip()
            item['Field6'] = ''.join(element.strip() for element in response.css(".listing-mid-section__information .bedroom ::text").getall())
            item['Field7'] = ''.join(element.strip() for element in response.css(".listing-mid-section__information .bathroom ::text").getall())
            item['Field8'] = ''.join(element.strip() for element in response.css(".listing-mid-section__information .carspace ::text").getall())

            # sold_price = response.css('.price__label ::text').get('').strip().replace(' ','')
            # if sold_price:
            #     item['Field9'] = (sold_price.strip()).replace('$','')   # sold_price

            description = response.css('.detail-description ::text, #description .sub-title ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess = response.xpath("//*[contains(@class,'item-image')]/div/a/picture/source/img/@src").getall()
            new_img_urls = []
            ...
            if response.css(".listing-agent"):
                ...
                agent1_name = agent1.css('.mb-20 ::text').get('').strip()
                if agent1:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    agent1_mobile = agent1.css(".agent-info-item-mobile a ::attr(href)").get('').strip().replace('tel:','')  # description
                    item['Field20'] = agent1_mobile.strip()  # description
                    agent1_phone = agent1.css(".agent-info-item-phone a ::attr(href)").get('').strip().replace('tel:','')  # description
                    item['Field21'] = agent1_phone.strip()  # description
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                ...
                if len(response.css(".listing-agent").getall()) > 1:
                    ...
                    agent2_name = agent2.css('.mb-20 ::text').get('').strip()
                    if agent2:
                        item['Field23'] = agent2_name.strip()
                        agent2_mobile = agent2.css(".agent-info-item-mobile a ::attr(href)").get('').strip().replace('tel:','')  # description
                        item['Field26'] = agent2_mobile.strip()  # description
                        agent2_phone = agent2.css(".agent-info-item-phone a ::attr(href)").get('').strip().replace('tel:','')  # description
                        item['Field26A'] = agent2_phone.strip()  # description
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                ...
                if len(response.css(".listing-agent").getall()) == 3:
                    ...
                    agent3_name = agent3.css('.mb-20 ::text').get('').strip()
                    if agent3:
                        item['Field28'] = agent3_name.strip()
                        agent3_mobile = agent3.css(".agent-info-item-mobile a ::attr(href)").get('').strip().replace('tel:','')  # description
                        item['Field31'] = agent3_mobile.strip()  # description
                        agent3_phone = agent3.css(".agent-info-item-phone a ::attr(href)").get('').strip().replace('tel:','')  # description
                        item['Field31A'] = agent3_phone.strip()  # description
                        '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            item['Field35'] = response.css(".property-type ::text").get('').strip()
            ...
            #     item['Field36'] = response.xpath("//*[contains(text(),'Building Size')]/following-sibling::div[1]/text()").get('').strip().replace('sqm','').replace(' ','')   # land_area
            #
            # feature_count = 58
