...
class ReMaxPS_Rent(scrapy.Spider):
    name = "ReMaxPS_Rent"

    url = "https://www.remaxpropertysales.com.au/renting/residential-rentals"
    prefix = "https://www.remaxpropertysales.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(ReMaxPS_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".fw-lighter ::attr(href)").getall()
        links = []
        ...
        next_page = response.xpath("//*[contains(@class,'current')]/following-sibling::li[1]/a/@href").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2890)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'ReMax Property Sales'

            property_street = response.css('.col-md-8 h3 ::text').get('').strip()  # property_street
            ...
            price = response.css('.m-0 ::text').get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','').replace('for','')
            if price:
                item['Field9'] = price

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('#description_1 h3 ::text').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css("#description_1 p ::text").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.xpath("//a[contains(@data-fancybox,'panama_slider_gallery')]/img/@src").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.css(".align-items-end"):
                agent1 = response.css(".align-items-end")[0]
                ...
                if agent1_name:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".w-100 ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.css(".align-items-end").getall()) > 1:
                agent2 = response.css(".align-items-end")[1]
                ...
                if agent2:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".w-100 ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'https:{agent_image}' if agent_image else ""
            ...
            if len(response.css(".agentTile").getall()) > 2:
                ...
                if agent3:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".w-100 ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'https:{agent_image}' if agent_image else ""
            ...
            item['Field35'] = response.xpath("//*[contains(text(),'Type:')]/text()").get('').strip().replace('Type','').replace(':','')
            ...
            #
            feature_count = 58
            for feature in response.xpath("//*[contains(@class,'col float-md-start mb-3')]/li"):   ##  first 4 features are ID, bed, bath etc.
