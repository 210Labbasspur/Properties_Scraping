...
class Hunters_Sold(scrapy.Spider):
    name = 'Hunters_Sold'
    prefix = 'https://www.huntersagency.com.au'
    url = 'https://www.huntersagency.com.au/wp-content/plugins/zoorealty/display/pages/ajax/fetch_pages.php'
    headers = {
        ...
        'Accept': '*/*',
        # 'Referer': 'https://www.huntersagency.com.au/sell/sold/',
    ...
    data = {
        'page': '0',
        'dlist': 'sale',
        'dsuburb': '',        'office_id': '',        'keywords': '',        'price_min': '',
        'price_max': '',        'bedrooms': '',        'bathrooms': '',        'carspaces': '',
        'order': 'created_at',        'order_direction': 'desc',        'exclude_id': '',
        'include_non_commercial_lease_id': '',        'days_limit': '',        'status': '2',
    }

    count = 1
    db = 'Hunters'
    collection = 'Hunters_Sold'
    ...
    def parse(self, response):
        ...
        for property in response.xpath("//*[contains(@class,'cycle-next')]/following-sibling::div[1]/"
                                       "a[contains(@class,'centerimage')]"):
            property_no += 1
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            sold_price = response.css('.price+ .price .items ::text').get('').strip().replace('Sold','').replace('-','')
            if re.search(r'\d', sold_price):
                item['Field9'] = (sold_price.strip()).replace('$','')   # sold_price

            description = response.css('#property-description ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess = response.css(".slide_photo_ref ::attr(href)").getall()
            new_img_urls = []
            ...
            '''          AGENTS          '''
            if response.css("#contact-agents .alt"):
                agent1 = response.css("#contact-agents .alt")[0]
                ...
                if agent1:
                    ...
                    agent1_phone = agent1.css(".mobile ::text").get('').strip().replace('M:','')
                ...
                if len(response.css("#contact-agents .alt").getall()) > 1:
                    agent2 = response.css("#contact-agents .alt")[1]
                ...
                if len(response.css("#contact-agents .alt").getall()) > 2:
                    agent3 = response.css("#contact-agents .alt")[2]
