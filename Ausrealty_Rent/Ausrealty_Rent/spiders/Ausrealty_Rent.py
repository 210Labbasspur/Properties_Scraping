...
class Ausrealty_Rent(scrapy.Spider):
    name = 'Ausrealty_Rent'
    prefix = 'https://ausrealty.com.au'
    url = 'https://ausrealty.com.au/wp-admin/admin-ajax.php'
    headers = {
        'accept': '*/*',
        'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        'content-type': 'application/x-www-form-urlencoded; charset=UTF-8',
        # 'cookie': 'intercom-id-pziemwt6=1e3d37db-fa13-4421-86b5-6265ed20f9f6; intercom-session-pziemwt6=; intercom-device-id-pziemwt6=a1c19fce-82bb-44fd-b9a9-868e85a6d416; _hjSessionUser_2526813=eyJpZCI6ImU4NTY4MDBhLWMyMmQtNTAzNS04YThiLWRkNzE5YmEzYWRhZSIsImNyZWF0ZWQiOjE3MTM1NTUwODg0NTQsImV4aXN0aW5nIjp0cnVlfQ==; epl_wp_session=7a19456af735983c348a51d7f2dcaaf5%7C%7C1713675621%7C%7C1713673821; _hjSession_2526813=eyJpZCI6IjA2Njk2YWE4LTc0NzQtNDg1OS04NGQ5LThhZTIxNTczNTUzOCIsImMiOjE3MTM2MzI0Mjc1MDYsInMiOjEsInIiOjEsInNiIjowLCJzciI6MCwic2UiOjAsImZzIjowLCJzcCI6MH0=; PHPSESSID=hgvlqei3hiouenm1aoj9khuc7g; arp_scroll_position=300',
        'origin': 'https://ausrealty.com.au',
        'priority': 'u=1, i',
        'referer': 'https://ausrealty.com.au/result/?property_address=&post_type1=buy',
    ...
    data = {
        'page': '1',
        'getval1[post_type1]': 'rental',
        'getval1[property_address]': '',        'getval1[property_type]': '',        'getval1[property_price_from]': '',
        'getval1[property_price_to]': '',        'getval1[property_bedrooms_min]': '',        'getval1[property_bathrooms]': '',
        'getval1[property_carport]': '',        'getval1[property_location]': '',        'getval1[subnearby]': '',
        'getval1[sortby]': '',        'getval1[sortDateby]': '',        'getval1[inspectionDay]': '',        'action': 'search-property-result-grid-view',
    }
    count = 1
    db = 'Ausrealty'
    collection = 'Ausrealty_Rent'
    ...
    def parse(self, response):
        ...
        for property in response.css('.item-info a'):
            property_no += 1
        ...
        total_results = int(response.xpath("//*[contains(@class,'result-item col-lg-4')]/@data-total-posts").get('').strip())
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'Ausrealty'

            property_add = response.css('.property-title h5.text-uppercase ::text').getall()  # full_adddress
            ...
            bed_bath_car = response.css('.property-plans .property-details-inner p ::text').get('').strip()
            beds, baths, cars = [int(part[:-1]) if part.endswith(('B', 'C')) else None for part in bed_bath_car.split()]
            item['Field6'] = beds
            item['Field7'] = baths
            item['Field8'] = cars
            ...
            description = response.css('.property-details-center ::text , .property-details-inner ul ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess = response.css(".NewSuperSlickSliderImg ::attr(src)").getall()
            new_img_urls = []
            ...
            '''          AGENTS          '''
            if response.css(".col-lg-5.mx-auto"):
                agent1 = response.css(".col-lg-5.mx-auto")[0]
                agent1_name = agent1.css('h5.text-uppercase ::text').get('').strip()
                if agent1:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images = agent1.css(".img-fluid ::attr(src)").get('').strip()
                ...
                if len(response.css(".col-lg-5.mx-auto").getall()) > 1:
                    agent2 = response.css(".col-lg-5.mx-auto")[1]
                    agent2_name = agent2.css('h5.text-uppercase ::text').get('').strip()
                    if agent2:
                        item['Field23'] = agent2_name.strip()
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent2.css(".img-fluid ::attr(src)").get('').strip()
                ...
                if len(response.css(".col-lg-5.mx-auto").getall()) == 3:
                    agent3 = response.css(".col-lg-5.mx-auto")[2]
                    agent3_name = agent3.css('h5.text-uppercase ::text').get('').strip()
                    if agent3:
                        item['Field28'] = agent3_name.strip()
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent3.css(".img-fluid ::attr(src)").get('').strip()
