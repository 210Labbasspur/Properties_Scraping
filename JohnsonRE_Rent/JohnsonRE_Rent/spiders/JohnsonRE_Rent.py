...
class JohnsonRE_Rent(scrapy.Spider):
    name = "JohnsonRE_Rent"

    url = "https://www.johnsonre.com.au/rental-properties?203695f3_page=1"
    prefix = "https://www.johnsonre.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(JohnsonRE_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".listinglinkblock ::attr(href)").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://www.johnsonre.com.au{each_url}'
        ...
        next_page = response.css(".nextbutton ::attr(href)").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2124)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Johnson Real Estate'

            property_street = response.xpath("//*[contains(@class,'h2 navy')]/text()").get('').strip()  # property_street
            ...
            bbc = response.css('.listingdetails')
            item['Field6'] = bbc.xpath(".//*[contains(@src,'bed.svg')]/following-sibling::*[1]/text()").get('').strip()
            item['Field7'] = bbc.xpath(".//*[contains(@src,'bathtub.svg')]/following-sibling::*[1]/text()").get('').strip()
            item['Field8'] = bbc.xpath(".//*[contains(@src,'car.svg')]/following-sibling::*[1]/text()").get('').strip()

            price = response.xpath("//*[@class='h5 orange']/text()").get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            ...
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".listinghero .p ::text").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            images_script = response.xpath("//*[contains(text(),'Function to load the Vite build dynamically')]/text()").get('').strip()
            prop_images = re.findall(r'https://[^\s,]+\.jpg', images_script)
            images = []
            for img in prop_images[1:]:
            ...
            land_area = response.xpath('//*[contains(@class,"sqmcont")]/div[1]/text()').get('').strip().replace('Land','').replace('is','').replace('m²','')
            if land_area:
