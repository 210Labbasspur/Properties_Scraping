...
class PropertyProvidore_Rent(scrapy.Spider):
    name = "PropertyProvidore_Rent"

    url = "https://propertyprovidore.com.au/listings?page=1&saleOrRental=Rental&rental_period=week&status=available_under_contract&viewType=gallery&sortby=dateListed-desc"
    prefix = "https://propertyprovidore.com.au"
    headers = {
    ...
    count = 1
    db = 'PropertyProvidore'
    collection = 'PropertyProvidore_Rent'
    ...
    def __init__(self, *args, **kwargs):
        super(PropertyProvidore_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.xpath("//*[contains(@class,'v2-w-full v2-relative v2-bg-light-grey')]/a/@href").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://propertyprovidore.com.au{each_url}'
        ...
        for property_url in all_links:
            property_url = f'https://propertyprovidore.com.au{property_url}'
        ...
        next_page = response.xpath("//a[contains(text(),'Next')]/@href").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3304)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Brendan Weathrill Property Providore'

            property_street = response.xpath("//*[contains(@data-testid,'address')]/text()").get('').strip()  # property_street
            ...
            item['Field6'] = response.xpath("//p[contains(text(),' Bed')]/text()").get('').strip().replace("Bed",'')
            item['Field7'] = response.xpath("//p[contains(text(),' Bath')]/text()").get('').strip().replace("Bath",'')
            item['Field8'] = response.xpath("//p[contains(text(),' Car')]/text()").get('').strip().replace("Car",'')

            price = response.xpath("//*[contains(@data-testid,'listing-price')]/text()").get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','').replace('ContactUs','')
            if price:
                item['Field9'] = price

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.xpath("//*[contains(@role,'text-listing-marketing-title')]/text()").getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.xpath("//*[contains(@class,'v2-prose v2-max-w-none v2-text-body siteloft-ucg v2-text-text-grey')]/p/text()").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.xpath("//*[contains(@property,'og:image')]/@content").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.xpath("//*[contains(@data-testid,'agent-details')]/div"):
                agent1 = response.xpath("//*[contains(@data-testid,'agent-details')]/div")[0]
                agent1_name = agent1.xpath("//*[contains(@class,'v2-text-body-bold v2-text-dark v2-capitalize hover:v2-underline')]/text()").get('').strip()
                if agent1_name:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field18'] = agent1.xpath("//*[contains(@class,'v2-text-body-small v2-text-text-grey v2-capitalize')]/text()").get('').strip()
                    item['Field19'] = agent1.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:','')
                    item['Field20'] = agent1.xpath(".//a[contains(@href,'tel:') and contains(@class,'v2-text-body v2-truncate')]/text()").get('').strip().replace('tel:','')
                    phone_nos = agent1.xpath(".//a[contains(@href,'tel:') and contains(@class,'v2-text-body v2-truncate')]/text()").getall()
                    if len(phone_nos) > 1:
            ...
            if len(response.xpath("//*[contains(@data-testid,'agent-details')]/div").getall()) > 2:
                agent2 = response.xpath("//*[contains(@data-testid,'agent-details')]/div")[1]
                agent2_name = agent2.xpath("//*[contains(@class,'v2-text-body-bold v2-text-dark v2-capitalize hover:v2-underline')]/text()").get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.xpath("//*[contains(@class,'v2-text-body-small v2-text-text-grey v2-capitalize')]/text()").get('').strip()
                    item['Field25'] = agent2.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:', '')
                    item['Field26'] = agent2.xpath(".//a[contains(@href,'tel:') and contains(@class,'v2-text-body v2-truncate')]/text()").get('').strip().replace('tel:','')
                    phone_nos = agent2.xpath(".//a[contains(@href,'tel:') and contains(@class,'v2-text-body v2-truncate')]/text()").getall()
                    if len(phone_nos) > 1:
            ...
            if len(response.xpath("//*[contains(@data-testid,'agent-details')]/div").getall()) > 4:
                agent3 = response.xpath("//*[contains(@data-testid,'agent-details')]/div")[2]
                agent3_name = agent3.xpath("//*[contains(@class,'v2-text-body-bold v2-text-dark v2-capitalize hover:v2-underline')]/text()").get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field29'] = agent3.xpath("//*[contains(@class,'v2-text-body-small v2-text-text-grey v2-capitalize')]/text()").get('').strip()
                    item['Field30'] = agent3.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:', '')
                    item['Field31'] = agent3.xpath(".//a[contains(@href,'tel:') and contains(@class,'v2-text-body v2-truncate')]/text()").get('').strip().replace('tel:','')
                    phone_nos = agent3.xpath(".//a[contains(@href,'tel:') and contains(@class,'v2-text-body v2-truncate')]/text()").getall()
                    if len(phone_nos) > 1:
            ...
            land_area = response.xpath('//*[contains(text(),"Land Area")]/following-sibling::span[1]/text()').get('').strip().replace('Land','').replace('is','').replace('m²','')
            if land_area:
                if 'Acres' in land_area:
                    pass
                else:
                    item['Field36'] = land_area.replace('m²','').replace(' ','')   # land_area
            floor_area = response.xpath('//*[contains(text(),"Build Area")]/following-sibling::span[1]/text()').get('').strip().replace('Land','').replace('is','').replace('m²','')
            if floor_area:
                if 'Acres' in floor_area:
                    pass
                else:
                    item['Field37'] = floor_area.replace('m²','').replace(' ','')   # land_area

            feature_count = 58
            for feature in response.xpath("//*[contains(@data-testid,'property-feature-item')]/p"):   ##  first 4 features are ID, bed, bath etc.
