...
class CommunityFirst_Sold(scrapy.Spider):
    name = "CommunityFirst_Sold"

    url = "https://communityfirstrealestate.com.au/sell/page/1/?pagination_id=1&instance_id=1"
    prefix = "https://communityfirstrealestate.com.au"
    headers = {
    ...
    count = 1
    db = 'CommunityFirst'
    collection = 'CommunityFirst_Sold'
    ...
    def __init__(self, *args, **kwargs):
        super(CommunityFirst_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(1617)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Community First Real Estate'
            ...
            price = response.css('.page-price ::text').get('').strip().replace('Sold','').replace('Contact','').replace('Agent','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price
            ...
            if response.css(".agentRow"):
                ...
                agent1_name = agent1.css('.agentNameHeading ::text').get('').strip()
                if agent1_name:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field19'] =  agent1.css(".agentEmail ::attr(data-email)").get('').strip().replace('[at]','@')
                    item['Field20'] = agent1.xpath(".//a[contains(@href,'tel:')]/@href").get('').strip().replace('tel:', '')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".property-agent ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'https://communityfirstrealestate.com.au{agent_image}' if agent_image else ""
            ...
            if len(response.css(".agentTile").getall()) > 1:
                ...
                agent2_name = agent2.css('.agentNameHeading ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field25'] = agent2.css(".agentEmail ::attr(data-email)").get('').strip().replace('[at]','@')
                    item['Field26'] = agent2.xpath(".//a[contains(@href,'tel:')]/@href").get('').strip().replace('tel:', '')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".property-agent ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'https://communityfirstrealestate.com.au{agent_image}' if agent_image else ""
            ...
            if len(response.css(".agentTile").getall()) > 2:
                ...
                agent3_name = agent3.css('.agentNameHeading ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field30'] = agent3.css(".agentEmail ::attr(data-email)").get('').strip().replace('[at]','@')
                    item['Field31'] = agent3.xpath(".//a[contains(@href,'tel:')]/@href").get('').strip().replace('tel:', '')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".property-agent ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'https://communityfirstrealestate.com.au{agent_image}' if agent_image else ""
