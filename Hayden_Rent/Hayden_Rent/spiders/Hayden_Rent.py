...
class Hayden_Rent(scrapy.Spider):
    name = 'Hayden_Rent'
    prefix = 'https://www.hayden.com.au'
    url = "https://www.hayden.com.au/rent/properties-for-rent/"
    headers = {
    ...
    def parse(self, response):
        for property in response.css(".overview a"):
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field6'] = response.css(".bedrooms .num ::text").get('').strip()
            item['Field7'] = response.css(".bathrooms .num ::text").get('').strip()
            item['Field8'] = response.css(".carspaces .num ::text").get('').strip()

            # sold_price = response.css('#property_price ::text').get('').strip().replace('SOLD OFF-MARKET','').replace(' ','')
            # if re.search(r'\d', sold_price):
            #     item['Field9'] = (sold_price.strip()).replace('SOLD','').replace('$','')   # sold_price

            description = response.css('.copy ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess = response.css(".slider-nav .img img ::attr(src)").getall()
            new_img_urls = []
            ...
            if response.css(".agent_info"):
                ...
                if agent1_name:
                    ...
                    agent1_phone = agent1.css(".agent_mobile a ::text").get('').strip()
                    item['Field20'] = agent1_phone.strip()
                    agent1_mobile = agent1.css(".agent_phone a ::text").get('').strip()
                    item['Field21'] = agent1_mobile.strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images = agent1.css(".img-agent ::attr(src)").get('').strip()
                ...
                if len(response.css(".agent_info").getall()) > 1:
                    ...
                    if agent2:
                        ...
                        agent2_phone = agent2.css(".agent_mobile a ::text").get('').strip()
                        item['Field26'] = agent2_phone.strip()
                        agent2_mobile = agent2.css(".agent_phone a ::text").get('').strip()
                        item['Field26A'] = agent2_mobile.strip()
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent2.css(".img-agent ::attr(src)").get('').strip()
                ...
                if len(response.css(".agent_info").getall()) > 2:
                    ...
                    if agent3:
                        ...
                        agent3_phone = agent3.css(".agent_mobile a ::text").get('').strip()
                        item['Field31'] = agent3_phone.strip()
                        agent3_mobile = agent3.css(".agent_phone a ::text").get('').strip()
                        item['Field31A'] = agent3_mobile.strip()
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent3.css(".img-agent ::attr(src)").get('').strip()
            ...
            land_area = response.css(".land-size .num ::text").get('').strip()
            if land_area:
