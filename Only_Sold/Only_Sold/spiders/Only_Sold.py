...
class Only_Sold(scrapy.Spider):
    name = "Only_Sold"

    url = "https://onlyestateagents.com.au/sold"
    prefix = "https://onlyestateagents.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Only_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        ...
        for each_url in all_links:
            each_url = f'https://onlyestateagents.com.au{each_url}'
        ...
        for property_url in all_links:
            property_url = f'https://onlyestateagents.com.au{property_url}'
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3173)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Only Estate Agents'
            ...
            price = response.css('.pp_info_header_label_blck ::text').get('').strip().replace('Contact Agent','').replace('Sold','').replace('$','').replace('SOLD','').replace(',','')
            if price:
                item['Field9'] = price
            ...
            if response.css(".pp_info_agent_blck"):
                ...
                if agent1_name:
                    ...
                    if match:
                        ...
                        images_string = f'https://onlyestateagents.com.au{agent_image}' if agent_image else ""
