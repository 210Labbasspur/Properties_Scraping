...
class Sublime_Sold(scrapy.Spider):
    name = 'Sublime_Sold'
    prefix = 'https://www.sublimeproperty.com.au'
    url = "https://www.sublimeproperty.com.au/residential/recent-sales/page/1/"
    headers = {
        'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        'cache-control': 'max-age=0',
        # 'cookie': 'PHPSESSID=5m82qf8nih4upudq56dsllfqun; PHPSESSID=5m82qf8nih4upudq56dsllfqun',
        'priority': 'u=0, i',
        # 'referer': 'https://www.sublimeproperty.com.au/residential/recent-sales/',
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'Sublime Property Agents'
            ...
            item['Field8'] = response.xpath("//*[contains(@class,'carspaces')]/text()").get('').strip()

            sold_price = response.xpath("//strong[contains(text(),'Price')]/parent::p/text()").get('').strip().replace(' ','')
            if re.search(r'\d', sold_price):
                item['Field9'] = (sold_price.strip()).replace('$','')   # sold_price

            description = response.css('.headline+ p ::text').getall()  # description
            ...
            if response.css("#property-enquiry .agent"):
                ...
                if agent1:
                    ...
                    agent1_phone = agent1.xpath(".//strong[contains(text(),'m')]/parent::p/text()").get('').strip()
                    item['Field20'] = agent1_phone.strip()
                    agent1_mobile = agent1.xpath(".//strong[contains(text(),'p')]/parent::p/text()").get('').strip()
                ...
                if len(response.css("#property-enquiry .agent").getall()) > 1:
                    ...
                    if agent2:
                        ...
                        agent2_phone = agent2.xpath(".//strong[contains(text(),'m')]/parent::p/text()").get('').strip()
                        item['Field26'] = agent2_phone.strip()
                        agent2_mobile = agent2.xpath(".//strong[contains(text(),'p')]/parent::p/text()").get('').strip()
                ...
                if len(response.css("#property-enquiry .agent").getall()) > 2:
                    ...
                    if agent3:
                        ...
                        agent3_phone = agent3.xpath(".//strong[contains(text(),'m')]/parent::p/text()").get('').strip()
                        item['Field31'] = agent3_phone.strip()
                        agent3_mobile = agent3.xpath(".//strong[contains(text(),'p')]/parent::p/text()").get('').strip()
            ...
            item['Field35'] = response.xpath("//*[contains(text(),'Type')]/following-sibling::span[1]/text()").get('').strip()
