...
class DeBrennan_Sold(scrapy.Spider):
    name = "DeBrennan_Sold"

    url = "https://www.debrennan.com.au/sold/page/{}/"
    prefix = "https://www.debrennan.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(DeBrennan_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".image-url ::attr(href)").getall()
        links = []
        ...
        next_page = response.css(".load-more a ::attr(href)").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2919)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'De Brennan Property'

            property_street = response.css('.street ::text').get('').strip()  # property_street
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'icon-bed')]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'icon-bath')]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'icon-car')]/text()").get('').strip()
            ...
            if response.css(".agent"):
                ...
                if agent1_name:
                    ...
                    item['Field20'] = agent1.xpath("//*[contains(text(),'Mobile')]/parent::dt/following-sibling::dd[1]/span/a/text()").get('').strip().replace('tel:','')
                    item['Field21'] = agent1.xpath("//*[contains(text(),'Phone')]/parent::dt/following-sibling::dd[1]/span/a/text()").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".img-wrap img ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.css(".agent").getall()) > 1:
                ...
                if agent2:
                    ...
                    item['Field26'] = agent2.xpath("//*[contains(text(),'Mobile')]/parent::dt/following-sibling::dd[1]/span/a/text()").get('').strip().replace('tel:','')
                    item['Field26A'] = agent2.xpath("//*[contains(text(),'Phone')]/parent::dt/following-sibling::dd[1]/span/a/text()").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".img-wrap img ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css(".agent").getall()) > 2:
                ...
                if agent3:
                    ...
                    item['Field31'] = agent3.xpath("//*[contains(text(),'Mobile')]/parent::dt/following-sibling::dd[1]/span/a/text()").get('').strip().replace('tel:','')
                    item['Field31A'] = agent3.xpath("//*[contains(text(),'Phone')]/parent::dt/following-sibling::dd[1]/span/a/text()").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".img-wrap img ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
