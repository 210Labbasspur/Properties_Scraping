...
class Henzells_Rent(scrapy.Spider):
    name = "Henzells_Rent"

    url = "https://henzells.com.au/renting/available-rentals"
    prefix = "https://henzells.com.au"
    headers = {
    ...
    count = 1
    db = 'Henzells'
    collection = 'Henzells_Rent'
    # bucket_prefix = f'P_{collection}'
    bucket_prefix = f'Images2'

    local_file_path = '/img'

    def __init__(self, *args, **kwargs):
        super(Henzells_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        links = []
        for each_url in response.css(".propertyItem"):
            each_url = each_url.css('a::attr(href)').get('').strip()
        ...
        for property_url in response.css(".propertyItem"):
            price = property_url.css('.price ::text').get('').strip().replace('OFFERS OVER','').replace('INTEREST OVER','')
            property_url = property_url.css('a::attr(href)').get('').strip()
            print(self.count,' # Property URL :', property_url)
            self.count += 1
            yield response.follow(property_url, headers=self.headers, callback=self.detail_parse, meta={'price':price})
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2094)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Henzells Agency'

            property_street = response.css('.listing-address ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('xxxxxxxxxxxxxxxxxxxx ::text').getall() if e.strip())  # property_suburb
            property_address = f"{property_street} {property_suburb}"
            item['Field5'] = property_address  # full_adddress

            item['Field6'] = response.xpath("//*[contains(@src,'svg/bed-icon')]/following-sibling::*[1]/text()").get('').strip().replace('BED','')
            item['Field7'] = response.xpath("//*[contains(@src,'svg/bath-icon')]/following-sibling::*[1]/text()").get('').strip().replace('BATH','')
            item['Field8'] = response.xpath("//*[contains(@src,'svg/car-icon')]/following-sibling::*[1]/text()").get('').strip().replace('CAR','')

            price = response.meta['price'].replace('Sold','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price.replace('Contact','').replace('Agent','')

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.col-lg-9 h1 ::text').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.xpath("//*[contains(@class,'col-lg-9 col-md-12 mt-5')]/p/text()").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.css(".ldGallery img ::attr(data-lazy)").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.css(".youragent"):
                agent1 = response.css(".youragent")[0]
                agent1_name = agent1.css('h6 ::text').get('').strip()
                if agent1_name:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field18'] = agent1.css('.lspace-3 ::text').get('').strip()
                    ...
                    phones = agent1.xpath(".//a[contains(@href,'tel:')]/@href").getall()
                    if len(phones) > 1:
                        item['Field21'] = phones[-1].strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = response.xpath(f"//*[contains(text(),'{agent1_name}')]/ancestor::ul[1]/preceding-sibling::a[1]/img/@src").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'https://henzells.com.au{agent_image}' if agent_image else ""
                    # item['Field22'] = images_string
                    # print('Agent1 image is :',images_string)
                    random_number = random.randint(1, 10000000000)
                    item['Field22'] = ",".join(self.uploaded(images_string, random_number))

            if len(response.css(".youragent").getall()) > 1:
                agent2 = response.css(".youragent")[1]
                agent2_name = agent2.css('h6 ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.css('.lspace-3 ::text').get('').strip()
                    ...
                    phones = agent2.xpath(".//a[contains(@href,'tel:')]/@href").getall()
                    if len(phones) > 1:
                        item['Field26A'] = phones[-1].strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = response.xpath(f"//*[contains(text(),'{agent2_name}')]/ancestor::ul[1]/preceding-sibling::a[1]/img/@src").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'https://henzells.com.au{agent_image}' if agent_image else ""
            ...
            if len(response.css(".youragent").getall()) > 2:
                agent3 = response.css(".youragent")[2]
                agent3_name = agent3.css('h6 ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field29'] = agent3.css('.lspace-3 ::text').get('').strip()
                    ...
                    phones = agent3.xpath(".//a[contains(@href,'tel:')]/@href").getall()
                    if len(phones) > 1:
                        item['Field31A'] = phones[-1].strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = response.xpath(f"//*[contains(text(),'{agent3_name}')]/ancestor::ul[1]/preceding-sibling::a[1]/img/@src").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'https://henzells.com.au{agent_image}' if agent_image else ""
            ...
            item['Field33'] = response.url.split(',')[-1]
            # item['Field33'] = response.css('.ididididididid ::text').get('').strip()
            # bbc = response.css("xxxxxxxxxxxxxxxxxxxx ::text").getall()
            # item['Field35'] = bbc[-1].strip() if bbc else ''

            land_area = response.xpath('//*[contains(text(),"Land Size")]/*/text()').get('').strip().replace('Land','').replace('is','').replace('m²','')
            if land_area:
                if 'Acres' in land_area:
                    pass
                else:
                    item['Field36'] = land_area.replace('m2','').replace(' ','')   # land_area
            floor_area = response.xpath('//*[contains(text(),"Floor Size")]/*/text()').get('').strip().replace('Land','').replace('is','').replace('m²','')
            if floor_area:
                if 'Acres' in floor_area:
                    pass
                else:
                    item['Field37'] = floor_area.replace('m2','').replace(' ','')   # land_area

            feature_count = 58
            for feature in response.css(".featureItems li")[3:]:   ##  first 4 features are ID, bed, bath etc.
