...
class HugoAlexander_Sale(scrapy.Spider):
    name = "HugoAlexander_Sale"

    url = "https://hugoalexander.com.au/properties-for-sale/"
    prefix = "https://hugoalexander.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(HugoAlexander_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3234)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Hugo Alexander Property Group'
            ...
            '''          AGENTS          '''
            if response.css("div.agent-details"):
                agent1 = response.css("div.agent-details")[0]
                ...
                if agent1_name:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".thumbnail ::attr(data-lzl-src)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.css("div.agent-details").getall()) > 1:
                agent2 = response.css("div.agent-details")[1]
                ...
                if agent2:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".thumbnail ::attr(data-lzl-src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css(".agentTile").getall()) > 2:
                ...
                if agent3:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".thumbnail ::attr(data-lzl-src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
