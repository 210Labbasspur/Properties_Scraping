...
class Milestone_Sale(scrapy.Spider):
    name = "Milestone_Sale"

    url = "https://www.milestonere.com.au/browse-all-properties/"
    prefix = "https://www.milestonere.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Milestone_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.xpath("//*[@class='property-image']/@href").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3171)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Milestone Real Estate'

            property_street = response.css('title ::text').get('').strip().replace('- Milestone Real Estate','')  # property_street
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'fa fa-bed')]/parent::span[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'fa fa-bath')]/parent::span[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'fa fa-car')]/parent::span[1]/text()").get('').strip()

            price = response.css('.xxxxxxxxxx ::text').get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.property-dtls-right h4 ::text').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".cmspage p ::text").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.css("#prop_main .item img ::attr(src)").getall()
            images = []
            ...
            if response.css(".agent-item"):
                ...
                if agent1_name:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".agent-img img ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.css(".agent-item").getall()) > 1:
                ...
                if agent2:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".agent-img img ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css(".agent-item").getall()) > 2:
                ...
                if agent3:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".agent-img img ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            land_area = response.xpath('//*[contains(text(),"Square metres")]/text()').get('').strip().replace('Land','').replace('is','').replace('Square metres','')
            if land_area:
