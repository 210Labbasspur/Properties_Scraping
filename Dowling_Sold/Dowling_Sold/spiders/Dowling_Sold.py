...
class Dowling_Sold(scrapy.Spider):
    name = 'Dowling_Sold'
    prefix = 'https://dowlingpropertygroup.com.au'
    url = "https://dowlingpropertygroup.com.au/recent-sales/"
    headers = {
        'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        'cache-control': 'max-age=0',
        # 'cookie': 'AMP_MKTG_16a5c84b5b=JTdCJTdE; _gid=GA1.3.549052095.1713560626; epl_wp_session=ff4fdf0659256df57dde45736c43bc3a%7C%7C1713655352%7C%7C1713653552; _gat_gtag_UA_155872398_1=1; _ga_SB99SVCDRW=GS1.1.1713612156.4.1.1713612168.0.0.0; _ga=GA1.1.713788620.1713301080; AMP_16a5c84b5b=JTdCJTIyZGV2aWNlSWQlMjIlM0ElMjI1NWMwMzUwMy1iNjA4LTQ3ZjItYTAzOS05MjkwYjI2MjViOTQlMjIlMkMlMjJzZXNzaW9uSWQlMjIlM0ExNzEzNjEyMTU2ODQ1JTJDJTIyb3B0T3V0JTIyJTNBZmFsc2UlMkMlMjJsYXN0RXZlbnRUaW1lJTIyJTNBMTcxMzYxMjE3MDIyMSU3RA==; arp_scroll_position=500; epl_wp_session=ff4fdf0659256df57dde45736c43bc3a%7C%7C1713655352%7C%7C1713653552',
        'priority': 'u=0, i',
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'Dowling Property Group'

            property_add = response.css('.entry-title ::text').getall()  # full_adddress
            ...
            description = response.css('.tab-content ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess = response.css("#epl-slider-slides a ::attr(href)").getall()
            new_img_urls = []
            ...
            '''          AGENTS          '''
            agent1 = response.css("#epl-box")[0]
            agent1_name = agent1.css('.epl-author-title ::text').get('').strip()
            if agent1:
                ...
                '''         Uploading Images on Wasabi S3 Bucket            '''
                Agent_Images = agent1.css(".size-medium ::attr(src)").get('').strip()
            ...
            if len(response.css("#epl-box").getall()) > 1:
                agent2 = response.css("#epl-box")[1]
                agent2_name = agent2.css('.epl-author-title ::text').get('').strip()
                if agent2:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images = agent2.css(".size-medium ::attr(src)").get('').strip()
            ...
            if len(response.css("#epl-box").getall()) == 3:
                agent3 = response.css("#epl-box")[2]
                agent3_name = agent3.css('.epl-author-title ::text').get('').strip()
                if agent3:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images = agent3.css(".size-medium ::attr(src)").get('').strip()
