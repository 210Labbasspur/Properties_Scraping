...
class GaryPeer_Rent(scrapy.Spider):
    name = "GaryPeer_Rent"

    url = "https://www.garypeer.com.au/rent/search-listings?page={}&listing_type=2&add_status=1"
    prefix = "https://www.garypeer.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(GaryPeer_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".tile__item-content_inner a ::attr(href)").getall()
        links = []
        ...
        next_page = response.css('.j-load-next-page ::attr(href)').get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3158)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'GaryPeer'

            property_street = response.css('.display ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.listing-card h3 ::text').getall() if e.strip())  # property_suburb
            ...
            item['Field8'] = response.css(".parking ::text").get('').strip()

            price = response.css('.listing-details p ::text').get('').strip().replace('Price undisclosed','').replace('Sold','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price
            ...
            '''          AGENTS          '''
            if response.css(".-persons .col-xs-12"):
                agent1 = response.css(".-persons .col-xs-12")[0]
                ...
                if agent1_name:
                    ...
                    item['Field18'] = agent1.css('.tile__item-content_inner p ::text').get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".carousel-cell ::attr(style)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.css(".-persons .col-xs-12").getall()) > 1:
                agent2 = response.css(".-persons .col-xs-12")[1]
                agent2_name = agent2.css('.h3 ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.css('.tile__item-content_inner p ::text').get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".carousel-cell ::attr(style)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css(".-persons .col-xs-12").getall()) > 2:
                agent3 = response.css(".-persons .col-xs-12")[2]
                agent3_name = agent3.css('.h3 ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field29'] = agent3.css('.tile__item-content_inner p ::text').get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".carousel-cell ::attr(style)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            land_area = response.css(".land ::text").get('').strip().replace('Land','').replace('is','').replace('m','')
            if land_area:
