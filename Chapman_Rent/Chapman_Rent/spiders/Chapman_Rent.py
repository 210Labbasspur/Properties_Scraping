...
class Chapman_Rent(scrapy.Spider):
    name = 'Chapman_Rent'
    prefix = 'https://www.chapmanrealestate.com.au'
    url = "https://www.chapmanrealestate.com.au/search-results/?list=lease&keywords="
    headers = {
        'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        'cache-control': 'max-age=0',
        # 'cookie': '_ga=GA1.3.1766551002.1713442828; _gid=GA1.3.125837934.1713560598; _ga_67P859NBLX=GS1.3.1713617686.3.1.1713617722.24.0.0; arp_scroll_position=400',
        'priority': 'u=0, i',
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'Chapman Real Estate'
            ...
            item['Field6'] = response.css(".bedroom ::text").get('').strip().replace('Bed','').replace(' ','').replace(',','')
            item['Field7'] = response.css(".bathroom ::text").get('').strip().replace('Bath','').replace(' ','').replace(',','')
            item['Field8'] = response.css(".carspace ::text").get('').strip().replace('Car','').replace(' ','').replace(',','')
            ...
            Agent_Imagess = response.xpath("//*[contains(@class,'popup-gallery slider-image')]/picture/source/img/@src").getall()
            new_img_urls = []
            ...
            land_area = response.xpath("//*[contains(text(),'Land Size')]/parent::div[1]/text()").getall()
            land_area = ''.join(element.strip() for element in land_area)
            if land_area:
