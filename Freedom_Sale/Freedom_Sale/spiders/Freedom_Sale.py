...
class Freedom_Sale(scrapy.Spider):
    name = "Freedom_Sale"

    url = "https://www.freedomproperty.com.au/ForSale?includeSurrounds=true&excludeContracts=false&p={}&s="
    prefix = "https://www.freedomproperty.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Freedom_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".followButtonContainer ::attr(data-parent-id)").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://www.freedomproperty.com.au/PropertyProfile?id={each_url}&mode=property'
        ...
        for property in all_links:
            property_url = f'https://www.freedomproperty.com.au/PropertyProfile?id={property}&mode=property'
        ...
        next_page = response.xpath("//*[@class='paginationNumber selected']/following-sibling::a[1]/@onclick").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2072)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Freedom Property'

            property_street = response.css('.mt-small .green-text ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.xpath("//*[contains(@class,'mt-small inline-block')]/text()").getall() if e.strip())  # property_suburb
            property_address = f"{property_street} {property_suburb}"
            item['Field5'] = property_address  # full_adddress

            item['Field6'] = response.xpath("//*[contains(@class,'bedText icon bed')]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'bathText icon bath')]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'carText icon car')]/text()").get('').strip()

            price = response.css('.ms-xsmall ::text').get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price.replace('Contact','').replace('Agent','')

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('h2.mt-medium ::text').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css("#bodyExpander ::text").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.xpath("//*[contains(@class,'slideshow')]/@data-image-urls").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.css(".contactAgentBox .content"):
                agent1 = response.css(".contactAgentBox .content")[0]
                agent1_name = ' '.join(e.strip() for e in agent1.css('.name ::text').getall())
                if agent1_name:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".agentImage ::attr(style)").get('').strip().replace("background-image: url('","").replace("')","")
            ...
            if len(response.css(".contactAgentBox .content").getall()) > 1:
                agent2 = response.css(".contactAgentBox .content")[1]
                agent2_name = ' '.join(e.strip() for e in agent2.css('.name ::text').getall())
                if agent2:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".agentImage ::attr(style)").get('').strip().replace("background-image: url('","").replace("')","")
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css(".contactAgentBox .content").getall()) > 2:
                agent3 = response.css(".contactAgentBox .content")[2]
                agent3_name = ' '.join(e.strip() for e in agent3.css('.name ::text').getall())
                if agent3:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".agentImage ::attr(style)").get('').strip().replace("background-image: url('","").replace("')","")
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            item['Field33'] = response.meta['property']
            # bbc = response.css(".propertyPage-property-icons ::text").getall()
            # item['Field35'] = bbc[-1].strip() if bbc else ''

            land_area = response.xpath('//*[contains(text(),"Land Size")]/following-sibling::*[1]/text()').get('').strip().replace('Land','').replace('is','').replace('\n','')
            if land_area:
                if 'Acres' in land_area:
                    pass
                else:
                    item['Field36'] = land_area.replace('m','').replace(' ','')   # land_area
            floor_area = response.xpath('//*[contains(text(),"Floor Size")]/following-sibling::*[1]/text()').get('').strip().replace('Land','').replace('is','').replace('\n','')
            if floor_area:
            ...
            feature_count = 58
            for feature in response.css(".featureList li"):   ##  first 4 features are ID, bed, bath etc.
