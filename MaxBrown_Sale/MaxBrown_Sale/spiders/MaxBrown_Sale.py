...
class MaxBrown_Sale(scrapy.Spider):
    ...
    def __init__(self, *args, **kwargs):
        super(MaxBrown_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2994)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Max Brown Real Estate'
            ...
            item['Field9'] = response.css('.listing-info-price ::text').get('').strip().replace('$','').replace(',','').replace('Contact Agent','')
