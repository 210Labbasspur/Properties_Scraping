...
class ThePropertyHub_Rent(scrapy.Spider):
    name = "ThePropertyHub_Rent"

    url = "https://www.thepropertyhub.com.au/commercial-for-lease/"
    prefix = "https://www.thepropertyhub.com.au"
    headers = {
    ...
    count = 1
    db = 'ThePropertyHub'
    collection = 'ThePropertyHub_Rent'
    ...
    def __init__(self, *args, **kwargs):
        super(ThePropertyHub_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3343)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'The Property Hub'
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'bedrooms')]/text()").get('').strip().replace('bed','')
            item['Field7'] = response.xpath("//*[contains(@class,'bathrooms')]/text()").get('').strip().replace('bath','')
            item['Field8'] = response.xpath("//*[contains(@class,'parking')]/text()").get('').strip().replace('Parking Spaces','')

            price = response.css('.page-price ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.property-title ::text').getall())  # description
            ...
            prop_images = response.xpath("//*[contains(@u,'image')]/@src2").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.xpath("//*[contains(@class,'epl-tab-wrapper-right')]"):
                agent1 = response.xpath("//*[contains(@class,'epl-tab-wrapper-right')]")[0]
                agent1_name = agent1.xpath('.//*[contains(@class,"agent-name-epl")]/h3/text()').get('').strip()
                if agent1_name:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field18'] = agent1.css('.agent-name-epl span ::text').get('').strip()
            ...
            if len(response.xpath("//*[contains(@class,'epl-tab-wrapper-right')]").getall()) > 1:
                agent2 = response.xpath("//*[contains(@class,'epl-tab-wrapper-right')]")[1]
                agent2_name = agent2.xpath('.//*[contains(@class,"agent-name-epl")]/h3/text()').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.css('.agent-name-epl span ::text').get('').strip()
            ...
            if len(response.xpath("//*[contains(@class,'epl-tab-wrapper-right')]").getall()) > 2:
                agent3 = response.xpath("//*[contains(@class,'epl-tab-wrapper-right')]")[2]
                agent3_name = agent3.xpath('.//*[contains(@class,"agent-name-epl")]/h3/text()').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field29'] = agent3.css('.agent-name-epl span ::text').get('').strip()
            ...
            feature_count = 58
            for feature in response.css(".epl-property-features li")[4:]:   ##  first 4 features are ID, bed, bath etc.
