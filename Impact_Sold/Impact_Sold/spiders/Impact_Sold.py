...
class Impact_Sold(scrapy.Spider):
    name = "Impact_Sold"

    url = "https://www.impactrealtygroup.com.au/sold-properties/page/1/?pagination_id=sort_sold"
    prefix = "https://www.impactrealtygroup.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Impact_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2109)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Impact Realty Group'

            property_street = response.css('.item-street ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.entry-title-sub span ::text').getall() if e.strip())  # property_suburb
            ...
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".read-more-wrapper ::text").getall())  # description
            ...
            if response.css("div.epl-author-box-outer-wrapper"):
                ...
                if agent1_name:
                    ...
                    item['Field20'] = agent1.css('.mobile ::text').get('').strip()
                    item['Field21'] = agent1.css('.office-phone ::text').get('').strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css(".epl-author-box-outer-wrapper").getall()) > 1:
                ...
                if agent2:
                    ...
                    item['Field26'] = agent2.css('.mobile ::text').get('').strip()
                    item['Field26A'] = agent2.css('.office-phone ::text').get('').strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css(".epl-author-box-outer-wrapper").getall()) > 2:
                ...
                if agent3:
                    ...
                    item['Field31'] = agent3.css('.mobile ::text').get('').strip()
                    item['Field31A'] = agent3.css('.office-phone ::text').get('').strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            for bbc in response.xpath("//*[contains(@class,'fal fa-vector-square')]/parent::*[1]/text()").getall():
                if 'm' in bbc:
                    item['Field36'] = bbc.strip().replace('m','').replace('\t','').replace('\n','').replace('\r','')
