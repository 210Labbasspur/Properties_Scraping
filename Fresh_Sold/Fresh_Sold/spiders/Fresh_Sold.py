...
class Fresh_Sold(scrapy.Spider):
    name = 'Fresh_Sold'
    prefix = 'https://listing.freshpropertyagents.com.au'
    url = "https://listing.freshpropertyagents.com.au/page/1/?action=epl_search&post_type=property&property_status=sold"
    headers = {
        'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        # 'cookie': 'epl_wp_session=aad259e8090ee3e9cb0bee8656f4215b%7C%7C1714388932%7C%7C1714387132; epl_wp_session=aad259e8090ee3e9cb0bee8656f4215b%7C%7C1714438257%7C%7C1714436457',
        'priority': 'u=0, i',
        # 'referer': 'https://listing.freshpropertyagents.com.au/?action=epl_search&post_type=property&property_status=sold',
    ...
    def parse(self, response):
        for property in response.css(".entry-title a"):
        ...
        next_page = response.xpath("//*[contains(text(),'Next Page')]/@href").get('').strip()
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'Fresh Property Agents'

            property_add = response.css('h1.entry-title ::text').getall()  # full_adddress
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'icon beds')]/span[1]/text()").get('').strip()
            ...
            description = response.css('.tab-content p ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess = response.css(".wp-post-image ::attr(src)").getall()
            new_img_urls = []
            ...
            '''          AGENTS          '''
            if response.css("#epl-box3 .epl-author-box-outer-wrapper"):
                agent1 = response.css("#epl-box3 .epl-author-box-outer-wrapper")[0]
                ...
                if agent1:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images = agent1.css(".avatar ::attr(src)").get('').strip()
                ...
                if len(response.css("#epl-box3 .epl-author-box-outer-wrapper").getall()) > 1:
                    agent2 = response.css("#epl-box3 .epl-author-box-outer-wrapper")[1]
                    ...
                    if agent2:
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent2.css(".avatar ::attr(src)").get('').strip()
                ...
                if len(response.css("#epl-box3 .epl-author-box-outer-wrapper").getall()) > 2:
                    agent3 = response.css("#epl-box3 .epl-author-box-outer-wrapper")[2]
                    ...
                    if agent3:
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent3.css(".avatar ::attr(src)").get('').strip()
            ...
            # item['Field33'] = response.xpath("//*[contains(text(),'Property ID')]/following-sibling::div[1]/text()").get('').strip()
            item['Field35'] = response.xpath("//*[contains(@class,'property-category')]/text()").get('').strip()
            land_area = response.xpath("//*[contains(@class,'land-size')]/text()").get('').strip().replace('Land is','')
            if land_area:
