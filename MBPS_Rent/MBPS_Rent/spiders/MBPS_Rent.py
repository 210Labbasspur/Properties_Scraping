...
class MBPS_Rent(scrapy.Spider):
    name = "MBPS_Rent"

    url = 'https://www.mainbeachpropertysales.com/scripts/content-load.php'
    prefix = "https://www.mainbeachpropertysales.com"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(MBPS_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def start_requests(self):
        ...
        payload['row'] = str(offset)
        yield scrapy.FormRequest(self.url, method='POST', formdata=payload, headers=self.headers, callback=self.parse, meta={'offset':offset})

    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.xpath("//*[contains(@class,'box-property-image')]/a/@href").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3267)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Main Beach Property Services '

            property_street = response.css('.column h1 ::text').get('').strip()  # property_street
            ...
            item['Field6'] = response.xpath("//*[contains(@alt,'Bed')]/parent::*[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@alt,'Bath')]/parent::*[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@alt,'Car')]/parent::*[1]/text()").get('').strip()

            price = response.css('.span-2-of-3 h4 ::text').get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price.replace('Contact','').replace('Agent','').replace('WillSellAt','')
            ...
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.xpath("//*[contains(@class,'span-2-of-2')]//p[contains(@align,'justify')]/text()").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.xpath("//*[contains(@class,'gallery')]/@style").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.xpath("//*[contains(text(),'Agent Details')]/following-sibling::div[1]//div[@class='box-property']"):
                agent1 = response.xpath("//*[contains(text(),'Agent Details')]/following-sibling::div[1]//div[@class='box-property']")[0]
                agent1_name = agent1.css('.box-property-price a ::text').get('').strip()
                if agent1_name:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field18'] = agent1.css('.box-property-title ::text').get('').strip()
                    item['Field19'] = agent1.xpath(".//a[contains(text(),'@')]/text()").get('').strip()
                    ...
                    if len(phone_nos) > 1:
                        item['Field21'] = phone_nos[-1].strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.xpath(".//*[contains(@style,'/photographs/')]/@style").get('').strip().replace('position:relative;width:100%;height:100%;background-image:url(','').replace(');background-size:cover;background-position:center bottom;','')
                    images_string = f'https://www.mainbeachpropertysales.com{agent_image}' if agent_image else ""
                    # item['Field22'] = images_string
                    # print('Agent1 image is :',images_string)
                    random_number = random.randint(1, 10000000000)
                    item['Field22'] = ",".join(self.uploaded(images_string, random_number))

            if len(response.xpath("//*[contains(text(),'Agent Details')]/following-sibling::div[1]//div[@class='box-property']").getall()) > 1:
                agent2 = response.xpath("//*[contains(text(),'Agent Details')]/following-sibling::div[1]//div[@class='box-property']")[1]
                agent2_name = agent2.css('.box-property-price a ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.css('.box-property-title ::text').get('').strip()
                    item['Field25'] = agent2.xpath(".//a[contains(text(),'@')]/text()").get('').strip()
                    ...
                    if len(phone_nos) > 1:
                        item['Field26A'] = phone_nos[-1].strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.xpath(".//*[contains(@style,'/photographs/')]/@style").get('').strip().replace('position:relative;width:100%;height:100%;background-image:url(','').replace(');background-size:cover;background-position:center bottom;','')
                    images_string = f'https://www.mainbeachpropertysales.com{agent_image}' if agent_image else ""
            ...
            if len(response.xpath("//*[contains(text(),'Agent Details')]/following-sibling::div[1]//div[@class='box-property']").getall()) > 2:
                agent3 = response.xpath("//*[contains(text(),'Agent Details')]/following-sibling::div[1]//div[@class='box-property']")[2]
                agent3_name = agent3.css('.box-property-price a ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field29'] = agent3.css('.box-property-title ::text').get('').strip()
                    item['Field30'] = agent3.xpath(".//a[contains(text(),'@')]/text()").get('').strip()
                    ...
                    if len(phone_nos) > 1:
                        item['Field31A'] = phone_nos[-1].strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.xpath(".//*[contains(@style,'/photographs/')]/@style").get('').strip().replace('position:relative;width:100%;height:100%;background-image:url(','').replace(');background-size:cover;background-position:center bottom;','')
                    images_string = f'https://www.mainbeachpropertysales.com{agent_image}' if agent_image else ""
            ...
            land_area = response.xpath('//*[contains(text(),"Building Size")]/following-sibling::*[1]/text()').get('').strip().replace('Land','').replace('is','').replace('m²','')
            if land_area:
                if 'Acres' in land_area:
                    pass
                else:
                    item['Field36'] = land_area.replace('squareMeter','').replace(' ','')   # land_area
            floor_area = response.xpath('//*[contains(text(),"Floor Size")]/following-sibling::*[1]/text()').get('').strip().replace('Land','').replace('is','').replace('m²','')
            if floor_area:
            ...
            feature_count = 58
            for feature in response.xpath("//*[contains(text(),' Features')]/following-sibling::*[1]//tr/td[1]")[2:]:   ##  first 4 features are ID, bed, bath etc.
