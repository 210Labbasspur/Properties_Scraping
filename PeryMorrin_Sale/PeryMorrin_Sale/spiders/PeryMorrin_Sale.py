###########             PeryMorrin_Sale         ## 'https://perymorrinproperty.com/for-sale/'
...
class PeryMorrin_Sale(scrapy.Spider):
    name = "PeryMorrin_Sale"

    url = 'https://perymorrinproperty.com/for-sale/'
    prefix = "https://perymorrinproperty.com"
    headers = {
    ...
    count = 1
    db = 'PeryMorrin'
    collection = 'PeryMorrin_Sale'
    bucket_prefix = f'PM_{collection}'

    local_file_path = '/img'

    def __init__(self, *args, **kwargs):
        super(PeryMorrin_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".wpsight-listing-image a::attr(href)").getall()
        links = []
        ...
        for property in response.css('.wpsight-listing-image a'):
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2901)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Pery Morrin Property'
            property_street = response.css('.address ::text').get('').strip()  # full_adddress
            property_suburb = ''  # full_adddress
            property_address = f"{property_street} {property_suburb}"
            item['Field5'] = property_address  # full_adddress

            item['Field6'] = response.xpath("//*[contains(text(),'Beds:')]/following-sibling::span[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(text(),'Baths:')]/following-sibling::span[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(text(),'Car Park:')]/following-sibling::span[1]/text()").get('').strip()

            price = response.css('.listing-price-value ::text').get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.widget_listing_description .widget-title ::text').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".wpsight-listing-description ::text").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.css("span.image img ::attr(src)").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.css("div.wpsight-listing-agent-info"):
                agent1 = response.css("div.wpsight-listing-agent-info")[0]
                agent1_name = agent1.css('.agent-name ::text').get('').strip()
                if agent1_name:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field18'] = agent1.css('.wpsight-list-agent-company ::text').get('').strip()  # agent_full_name_1
                    item['Field19'] =  agent1.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:','')
                    item['Field20'] = agent1.css(".wpsight-listing-agent-phone ::text").get('').strip().replace('tel:','')
                    item['Field21'] = agent1.css(".wpsight-listing-agent-mobile ::text").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    if 'marcia' in agent1_name.lower():
                        item['Field22'] = "https://s3.ap-southeast-1.wasabisys.com/PM_PeryMorrin_Sold_1/5741694040_1.jpg"
                    else:
                        item['Field22'] = 'https://s3.ap-southeast-1.wasabisys.com/PM_PeryMorrin_Sold_1/7177271294_1.jpg'

            if len(response.css("div.wpsight-listing-agent-info").getall()) > 1:
                agent2 = response.css("div.wpsight-listing-agent-info")[1]
                agent2_name = agent2.css('.agent-name ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.css('.wpsight-list-agent-company ::text').get('').strip()  # agent_full_name_1
                    item['Field25'] = agent2.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:', '')
                    item['Field26'] = agent2.css(".wpsight-listing-agent-phone ::text").get('').strip().replace('tel:','')
                    item['Field26A'] = agent2.css(".wpsight-listing-agent-mobile ::text").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    if 'marcia' in agent2_name.lower():
                        item['Field27'] = "https://s3.ap-southeast-1.wasabisys.com/PM_PeryMorrin_Sold_1/5741694040_1.jpg"
                    else:
                        item['Field27'] = 'https://s3.ap-southeast-1.wasabisys.com/PM_PeryMorrin_Sold_1/7177271294_1.jpg'

            if len(response.css("div.wpsight-listing-agent-info").getall()) > 2:
                agent3 = response.css("div.wpsight-listing-agent-info")[2]
                agent3_name = agent3.css('.agent-name ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field29'] = agent3.css('.wpsight-list-agent-company ::text').get('').strip()  # agent_full_name_1
                    item['Field30'] = agent3.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:', '')
                    item['Field31'] = agent3.css(".wpsight-listing-agent-phone ::text").get('').strip().replace('tel:','')
                    item['Field31A'] = agent3.css(".wpsight-listing-agent-mobile ::text").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    if 'marcia' in agent3_name.lower():
                        item['Field32'] = "https://s3.ap-southeast-1.wasabisys.com/PM_PeryMorrin_Sold_1/5741694040_1.jpg"
                    else:
                        item['Field32'] = 'https://s3.ap-southeast-1.wasabisys.com/PM_PeryMorrin_Sold_1/7177271294_1.jpg'
            ...
            feature_count = 58
            for feature in response.css(".listing-term-wrap  .listing-term"):   ##  first 4 features are ID, bed, bath etc.
                if 'aircondition' in feature.css('::text').get('').strip():
