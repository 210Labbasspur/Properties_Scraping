...
class PropertyPlace_Sold(scrapy.Spider):
    name = "PropertyPlace_Sold"

    url = "https://propertyplace.agency/real-estate/sold-properties/"
    prefix = "https://propertyplace.agency"
    headers = {
    ...
    count = 1
    db = 'PropertyPlace'
    collection = 'PropertyPlace_Sold'
    bucket_prefix = f'PP_{collection}'

    local_file_path = '/img'

    def __init__(self, *args, **kwargs):
        super(PropertyPlace_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".epl-row__link ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2975)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Property Place Agency'

            property_street = response.css('.item-street ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.address .entry-title-sub ::text').getall() if e.strip())  # property_suburb
            ...
            '''          AGENTS          '''
            if response.css("#agents .epl-author-box-container"):
                agent1 = response.css("#agents .epl-author-box-container")[0]
                ...
                if agent1_name:
                    ...
                    item['Field20'] = agent1.css(".mobile a ::text").get('').strip()
                    item['Field21'] = agent1.css(".office-phone a ::text").get('').strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".attachment-large ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.css("#agents .epl-author-box-container").getall()) > 1:
                agent2 = response.css("#agents .epl-author-box-container")[1]
                ...
                if agent2:
                    ...
                    item['Field26'] = agent2.css(".mobile a ::text").get('').strip()
                    item['Field26A'] = agent2.css(".office-phone a ::text").get('').strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".attachment-large ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css("#agents .epl-author-box-container").getall()) > 2:
                agent3 = response.css("#agents .epl-author-box-container")[2]
                ...
                if agent3:
                    ...
                    item['Field31'] = agent3.css(".mobile a ::text").get('').strip()
                    item['Field31A'] = agent3.css(".office-phone a ::text").get('').strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".attachment-large ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            land_area = response.xpath("//*[contains(@class,'land-size')]/text()").get('').strip().replace('Land','').replace(':','')
            if land_area:
