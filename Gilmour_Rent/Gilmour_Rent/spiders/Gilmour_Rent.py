...
class Gilmour_Rent(scrapy.Spider):
    name = 'Gilmour_Rent'
    prefix = 'https://www.gilmour.com.au'
    url = "https://www.gilmour.com.au/renting/properties-for-lease/"
    headers = {
        'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'Accept-Language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        'Cache-Control': 'max-age=0',
        'Connection': 'keep-alive',
        # 'Cookie': '_ga=GA1.1.1730955262.1711812584; arp_scroll_position=3100; _ga_E9TJDXNV51=GS1.1.1711812583.1.1.1711812639.0.0.0',
        # 'Referer': 'https://www.gilmour.com.au/selling/recent-sales/',
    ...
    def parse(self, response):
        for property in response.css(".listing-headline a"):#[:2]:
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field6'] = response.css('.la-bed+ .value ::text').get('').strip()
            item['Field7'] = response.css('.la-bath+ .value ::text').get('').strip()
            item['Field8'] = response.css('.la-car+ .value ::text').get('').strip()
            ...
            Agent_Imagess = response.css('.img-default-used-size-large ::attr(src)').getall()
            new_img_urls = []
            ...
            if agent1:
                ...
                item['Field19'] = agent1.css('.email .color-black ::text').get('').strip()
                agent1_phone = agent1.css('.phone .color-black ::text').get('')
                item['Field20'] = agent1_phone.strip()
                item['Field21'] = agent1.css('.mobile .color-black ::text').get('')
                '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css(".mb-20").getall()) > 1:
                ...
                if agent2:
                    ...
                    item['Field25'] = agent2.css('.email .color-black ::text').get('').strip()
                    agent2_phone = agent2.css('.phone .color-black ::text').get('')
                    item['Field26'] = agent2_phone.strip()
                    item['Field26A'] = agent2.css('.mobile .color-black ::text').get('')
            ...
            item['Field35'] = response.xpath("//label[contains(text(),'Type')]/following-sibling::div/text()").get('').strip().replace(': ','')   # external_property_id

            land_area = response.xpath("//label[contains(text(),'Building Size')]/following-sibling::div/text()").get('').strip().replace(': ','')   # external_property_id
            if land_area:
