...
class NoelJones_Sold(scrapy.Spider):
    name = "NoelJones_Sold"

    url = "https://noeljones.com.au/sell/?property_type=&listing_type=sale%2Cboth&per_page=12&property_status=sold&surrounding=&agent_id=&archive=1&order_by=sold&pg={}&1737527040243=&1737527044060=&1737527050562"
    prefix = "https://noeljones.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(NoelJones_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".listing-card-link ::attr(href)").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://noeljones.com.au{each_url}'
        ...
        for property_url in all_links:
            property_url = f'https://noeljones.com.au{property_url}'
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3196)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Noel Jones'
            ...
            item['Field6'] = response.xpath("//*[contains(@src,'4zNSAzMi41NCI')]/parent::div[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@src,'Ny44MSA0MC4wNyI')]/parent::div[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@src,'0OC44MyAzMy43MiI')]/parent::div[1]/text()").get('').strip()
            ...
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".listing-description p ::text").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.xpath("//*[@class='swiper-lazy']/@data-bgset").getall()
            images = []
            for img in prop_images:
                first_url = img.split(',')[0]                      # Split by ',' and take the first part
                cleaned_url = first_url.split()[0]                    # Remove any trailing width information
                img = cleaned_url.replace('1920w','').replace(' ','').replace('background-image:url(','').replace(");","")
            ...
            if response.css(".listing-agent"):
                ...
                agent1_name = agent1.css('.listing-agent-name ::text').get('').strip()
                if agent1_name:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field18'] = agent1.css('.listing-agent-title ::text').get('').strip()
                    item['Field19'] =  agent1.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:','')
                    item['Field20'] = agent1.css(".listing-agent-mobile a ::attr(href)").get('').strip().replace('tel:','')
                    item['Field21'] = agent1.xpath(".//a[contains(@href,'tel:')]/@href").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".listing-agent-thumb ::attr(data-bgset)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.css(".listing-agent").getall()) > 1:
                ...
                agent2_name = agent2.css('.listing-agent-name ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.css('.listing-agent-title ::text').get('').strip()
                    item['Field25'] = agent2.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:','')
                    item['Field26'] = agent2.css(".listing-agent-mobile a ::attr(href)").get('').strip().replace('tel:','')
                    item['Field26A'] = agent2.xpath(".//a[contains(@href,'tel:')]/@href").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".listing-agent-thumb ::attr(data-bgset)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css(".listing-agent").getall()) > 2:
                ...
                agent3_name = agent3.css('.listing-agent-name ::text').get('').strip()
                if agent3:
                    ...
                    item['Field31'] = agent3.css(".listing-agent-mobile a ::attr(href)").get('').strip().replace('tel:','')
                    item['Field31A'] = agent3.xpath(".//a[contains(@href,'tel:')]/@href").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".listing-agent-thumb ::attr(data-bgset)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            land_area = response.xpath('//*[contains(text(),"Land size")]/text()').get('').strip().replace('Land size','').replace(':','').replace('m','')
            if land_area:
            ...
            #
            feature_count = 58
            for feature in response.css(".listing-features .cell p"):   ##  first 4 features are ID, bed, bath etc.
