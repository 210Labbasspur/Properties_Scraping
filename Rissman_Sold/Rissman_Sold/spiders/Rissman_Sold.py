...
class Rissman_Sold(scrapy.Spider):
    name = "Rissman_Sold"

    url = "https://rissmanproperty.com.au/sold-properties/"
    prefix = "https://rissmanproperty.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Rissman_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css("h3.entry-title a ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3250)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Rissman Property'
            ...
            item['Field6'] = response.css(".epl-icon-container-bed .icon-value ::text").get('').strip()
            item['Field7'] = response.css(".epl-icon-container-bath .icon-value ::text").get('').strip()
            item['Field8'] = response.css(".epl-icon-container-car .icon-value ::text").get('').strip()
            ...
            desc = response.xpath("//*[contains(@class,'fl-module fl-module-pp-heading fl-node-njr9wqy1f6vi')]")
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in desc.css("::text").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.css(".epl-blog-image img ::attr(src)").getall()
            images = []
