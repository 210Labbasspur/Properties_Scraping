...
class JrLanding_Sold(scrapy.Spider):
    name = 'JrLanding_Sold'
    prefix = 'https://www.jrlanding.com.au'
    url = "https://www.jrlanding.com.au/properties/sold/?order_by=sold_date&order=desc&sstyle=grid-small&pageno=1"
    headers = {
        'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        # 'cookie': 'PHPSESSID=udonvvdnd53jmq1o6jjrdo4shv; arp_scroll_position=1700',
        'priority': 'u=0, i',
        # 'referer': 'https://www.jrlanding.com.au/properties/sold/?order_by=sold_date&order=desc&sstyle=grid-small',
    ...
    def parse(self, response):
        for property in response.css(".image-wrapper a"):
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'JrLanding Green Square'

            property_add = response.css('.address h3 ::text').getall()  # full_adddress
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'attr-bed')]/span[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'attr-bath')]/span[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'attr-car')]/span[1]/text()").get('').strip()

            sold_price = response.css('.price h3 ::text').get('').strip().replace('Sold for','')
            if re.search(r'\d', sold_price):
                item['Field9'] = (sold_price.strip()).replace('$','') .replace(' ','')   # sold_price

            description = response.css('div.property-description  ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess = response.css(".property-gallery ::attr(href)").getall()
            new_img_urls = []
            ...
            if response.css(".agents .col-md-6"):
                ...
                if agent1:
                    ...
                    agent1_phone = agent1.css(".fa-mobile+ a ::text").get('').strip()
                    item['Field20'] = agent1_phone.strip()
                    agent1_mobile = agent1.css(".fa-phone+ a ::text").get('').strip()
                    item['Field21'] = agent1_mobile.strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images = agent1.css(".lazy-image ::attr(data-src)").get('').strip()
                ...
                if len(response.css(".agents .col-md-6").getall()) > 1:
                    ...
                    if agent2:
                        ...
                        agent2_phone = agent2.css(".fa-mobile+ a ::text").get('').strip()
                        item['Field26'] = agent2_phone.strip()
                        agent2_mobile = agent2.css(".fa-phone+ a ::text").get('').strip()
                        item['Field26A'] = agent2_mobile.strip()
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent2.css(".lazy-image ::attr(data-src)").get('').strip()
                ...
                if len(response.css(".agents .col-md-6").getall()) > 2:
                    ...
                    if agent3:
                        ...
                        agent3_phone = agent3.css(".fa-mobile+ a ::text").get('').strip()
                        item['Field31'] = agent3_phone.strip()
                        agent3_mobile = agent3.css(".fa-phone+ a ::text").get('').strip()
                        item['Field31A'] = agent3_mobile.strip()
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent3.css(".lazy-image ::attr(data-src)").get('').strip()
