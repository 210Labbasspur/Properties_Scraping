...
class Sanders_Sold(scrapy.Spider):
    name = 'Sanders_Sold'
    prefix = 'https://www.sanders.com.au'
    url = "https://www.sanders.com.au/buy/recent-sales/?pg={}&sort=&agent="
    headers = {
    ...
    def parse(self, response):
        for property in response.css(".img-loading a"):
            property_url = 'https://www.sanders.com.au/buy/recent-sales' + property.css('::attr(href)').get('').strip().replace('.','')
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            property_add = response.css('.page-profile--header .brand ::text, .page-profile--header h4 ::text').getall()  # full_adddress
            ...
            description = response.css('.panel-preview--closed ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess = response.css(".item img ::attr(src)").getall()
            new_img_urls = []
