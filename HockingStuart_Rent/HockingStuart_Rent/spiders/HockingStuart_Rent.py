...
class HockingStuart_Rent(scrapy.Spider):
    name = 'HockingStuart_Rent'
    prefix = 'https://hockingstuart.com.au'
    url = "https://hockingstuart.com.au/listings?propertyType=residential&sort=newold&searchStatus=rent&surr=1&state=all"
    headers = {
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'feature bed')]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'feature bath')]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'feature car')]/text()").get('').strip()

            # sold_price = response.css('.price ::text').get('').strip().replace('SOLD OFF-MARKET','').replace(' ','')
            # if re.search(r'\d', sold_price):
            #     item['Field9'] = (sold_price.strip()).replace('SOLD','').replace('$','')   # sold_price

            description = response.css('div.summary ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess = response.xpath("//div[contains(@class,'hero-item')]/@data-swiper-image").getall()
            new_img_urls = []
            ...
            if response.css(".agent"):
                ...
                if agent1_name:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images = self.prefix + agent1.css(".image ::attr(data-background)").get('').strip()
                ...
                if len(response.css(".agent").getall()) > 1:
                    ...
                    if agent2:
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = self.prefix + agent2.css(".image ::attr(data-background)").get('').strip()
                ...
                if len(response.css(".agent").getall()) > 2:
                    ...
                    if agent3:
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = self.prefix + agent3.css(".image ::attr(data-background)").get('').strip()
