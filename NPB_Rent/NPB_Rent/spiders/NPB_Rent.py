...
class NPB_Rent(scrapy.Spider):
    name = 'NPB_Rent'
    prefix = 'https://www.npbre.com'
    url = "https://www.npbre.com/renting/available-properties"
    headers = {
    ...
    def parse(self, response):
        ...
        all_details_links = []
        for url in response.xpath("//*[contains(@class,'renet_listing_item_parent portrait')]/a[1]"):
            all_details_links.append('https:' + url.css('::attr(href)').get('').strip())
        ...
        '''  #######   Original Code   #######  '''
        for property in response.xpath("//*[contains(@class,'renet_listing_item_parent portrait')]/a[1]"):
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'Nationwide Property Brokers'
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'fas fa-bed')]/parent::h5[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'fas fa-bath')]/parent::h5[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'fas fa-car')]/parent::h5[1]/text()").get('').strip()

            sold_price = response.css('.renet_txt_listing_price ::text').get('').strip().replace('Sold for','').replace(' ','')
            if re.search(r'\d', sold_price):
                item['Field9'] = (sold_price.strip()).replace(' ','').replace('$','')   # sold_price

            description = response.css('.row .row .row ::text').getall()  # description
            ...
            for url in Agent_Imagess:
                if url.strip():
                    image = re.search(r'url\((.*?)\)', url.strip()).group(1)
                    new_img_urls.append('https:' + image.replace('"',''))
            ...
            '''          AGENTS          '''
            if response.css(".showAgentImage .item"):
                ...
                if agent1_name:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images_style = agent1.css(".agentImage ::attr(style)").get('').strip()
                ...
                if len(response.css(".showAgentImage .item").getall()) > 1:
                    ...
                    if agent2:
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images_style = agent2.css(".agentImage ::attr(style)").get('').strip()
                        Agent_Images = re.search(r'url\((.*?)\)', Agent_Images_style).group(1)
                ...
                if len(response.css(".showAgentImage .item").getall()) > 2:
                    ...
                    if agent3:
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images_style = agent3.css(".agentImage ::attr(style)").get('').strip()
                        Agent_Images = re.search(r'url\((.*?)\)', Agent_Images_style).group(1)
                        Agent_Images = re.search(r'url\((.*?)\)', Agent_Images_style).group(1)
            ...
            land_area = response.xpath("//div[contains(text(),'sqm approx')]/text()").get('').strip()
            if land_area:
                item['Field36'] = land_area.replace('sqm approx','').replace('.','').replace(' ','')   # land_area

            feature_count = 58
            for feature in response.xpath("//*[contains(@class,'fa fa-check')]/parent::div[1]/following-sibling::div[1]"):
