...
class TheModernHouse_Sale(scrapy.Spider):
    name = 'TheModernHouse_Sale'
    prefix = 'https://www.themodernhouse.com'
    url = "https://www.themodernhouse.com/sales-list/page/{}/"
    headers = {
    ...
    count = 1
    db = 'TheModernHouse'
    collection = 'TheModernHouse_Sale'
    ...
    def start_requests(self):
        ...
        yield scrapy.Request(url=self.url.format(page_no), callback=self.parse, headers=self.headers,
                             meta={'page_no':page_no,'LINKS':LINKS,'DB_sale_matching_url':DB_sale_matching_url})

    def parse(self, response):
        LINKS = response.meta['LINKS']
        DB_sale_matching_url = response.meta['DB_sale_matching_url']
        all_details_links = []
        for url in response.css("a.listing-item"):
            url = self.prefix + url.css('::attr(href)').get('').strip()
        ...
        '''  #######   Original Code   #######  '''
        for property in response.css("a.listing-item"):
        ...
        next_page = response.css("a.listing-item")
        if next_page:
            page_no = response.meta['page_no'] + 1
            yield scrapy.Request(url=self.url.format(page_no), callback=self.parse, headers=self.headers,
                                 meta={'LINKS': LINKS,'page_no': page_no, 'DB_sale_matching_url': DB_sale_matching_url})
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'The Modern House'

            property_add = response.css('.property__top__name ::text').getall()  # full_adddress
            ...
            # item['Field6'] = response.xpath("//*[contains(text(),'BED')]/preceding-sibling::span[1]/text()").get('').strip()
            # item['Field7'] = response.xpath("//*[contains(text(),'BATH')]/preceding-sibling::span[1]/text()").get('').strip()
            # item['Field8'] = response.xpath("//*[contains(text(),'CAR')]/preceding-sibling::span[1]/text()").get('').strip()
            #
            # sold_price = response.css('.feature-list+ .panel-content--block p+ p ::text').get('').strip().replace('$','').replace(' ','')
            # if re.search(r'\d', sold_price):
            #     item['Field9'] = (sold_price.strip()).replace('SOLD','').replace('$','')   # sold_price

            description = response.css('.property__text__paragraphs p ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess = response.css(".js-propertyGalleryTrigger ::attr(data-lazy)").getall()
            new_img_urls = []
            ...
            '''          AGENTS          '''
            # if response.css(".agent-profile"):
            #     agent1 = response.css(".agent-profile")[0]
            #     agent1_name = agent1.css('.sub-heading ::text').get('').strip()
            #     if agent1_name:
            ...
            #
            #     if len(response.css(".agent-profile").getall()) > 1:
            #         agent2 = response.css(".agent-profile")[1]
            #         agent2_name = agent2.css('.sub-heading ::text').get('').strip()
            #         if agent2:
            ...
            #
            #     if len(response.css(".agent-profile").getall()) > 2:
            #         agent3 = response.css(".agent-profile")[2]
            #         agent3_name = agent3.css('.sub-heading ::text').get('').strip()
            #         if agent3:
