...
class RomicMoore_Rent(scrapy.Spider):
    name = "RomicMoore_Rent"

    url = 'https://www.romicmoore.com.au/wp-admin/admin-ajax.php'
    prefix = "https://www.romicmoore.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(RomicMoore_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        ...
        all_links = []
        links = []
        for each_url in data.get('data',{}).get('props',[]):
        ...
        for property in data.get('data', {}).get('props', []):
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(1850)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Romic Moore'

            address = property.get('address','')   # full_adddress
            postcode = property.get('postcode','')   # full_adddress
            ...
            item['Field9'] = property.get('price','')  # price
            item['Field12'] = property.get('description','').replace('\n','').replace('\t','').replace('\r','')  # price

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.css(".item-image img ::attr(src)").getall()
            images = []
            ...
            if response.css(".listing-agent"):
                ...
                agent1_name = agent1.css('.d-block p ::text').get('').strip()
                if agent1_name:
                    ...
                    item['Field20'] = agent1.css('.phone a ::text').get('').strip()
                    item['Field21'] = agent1.css('.mobile a ::text').get('').strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css(".listing-agent").getall()) > 1:
                ...
                agent2_name = agent2.css('.d-block p ::text').get('').strip()
                if agent2:
                    ...
                    item['Field26'] = agent2.css('.phone a ::text').get('').strip()
                    item['Field26A'] = agent2.css('.mobile a ::text').get('').strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css(".listing-agent").getall()) > 2:
                ...
                agent3_name = agent3.css('.d-block p ::text').get('').strip()
                if agent3:
                    ...
                    item['Field31'] = agent3.css('.phone a ::text').get('').strip()
                    item['Field31A'] = agent3.css('.mobile a ::text').get('').strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
