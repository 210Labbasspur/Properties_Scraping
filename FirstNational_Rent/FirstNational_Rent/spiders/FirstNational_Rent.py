...
class FirstNational_Rent(scrapy.Spider):
    name = 'FirstNational_Rent'
    prefix = 'https://www.clk.com.au'
    url = "https://www.clk.com.au/pages/real-estate/results?widgetKey=191129&context=listings&returnAs=markup&listing_category=Residential&listing_sale_method=lease&display_sale_method=RENT&pg={}"
    headers = {
    ...
    def parse(self, response):
        for property in response.css(".listing-card a"):
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'First National Real Estate (Charles L. King & Co)'

            property_add = response.css('.top .heading ::text').getall()  # full_adddress
            ...
            item['Field6'] = ''.join(element.strip() for element in response.xpath("//*[contains(@class,'bedrooms')]/text()").getall())
            item['Field7'] = ''.join(element.strip() for element in response.xpath("//*[contains(@class,'bathroom')]/text()").getall())
            item['Field8'] = ''.join(element.strip() for element in response.xpath("//*[contains(@class,'carspace')]/text()").getall())

            # sold_price = response.css('.price ::text').get('').strip().replace('Sold','').replace(' ','')
            # if re.search(r'\d', sold_price):
            #     item['Field9'] = (sold_price.strip()).replace('Sold','').replace('$','')   # sold_price

            description = response.css('.body ::text').getall()  # description
            ...
            if response.css(".agent"):
                ...
                if agent1_name:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images = agent1.css(".agent-image ::attr(src)").get('').strip()
                ...
                if len(response.css(".agent").getall()) > 1:
                    ...
                    if agent2:
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent2.css(".agent-image ::attr(src)").get('').strip()
                ...
                if len(response.css(".agent").getall()) > 2:
                    ...
                    if agent3:
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent3.css(".agent-image ::attr(src)").get('').strip()
            ...
            item['Field35'] = response.xpath("//*[contains(text(),'Property Type')]/following-sibling::span[1]/text()").get('').strip().replace('Property ID:','')

            land_area = response.xpath("//*[contains(text(),'Land Area')]/following-sibling::span[1]/text()").get('').strip().replace('Property ID:','')
            if land_area:
