...
class ChisholmGamon_Rent(scrapy.Spider):
    name = "ChisholmGamon_Rent"

    url = "https://chisholmgamon.com.au/real-estate-for-rent/rental-search?page={}"
    prefix = "https://chisholmgamon.com.au"
    headers = {
    ...
    count = 1
    db = 'ChisholmGamon'
    collection = 'ChisholmGamon_Rent'
    # bucket_prefix = f'CG_{collection}'
    bucket_prefix = f'Images1'

    local_file_path = '/img'

    def __init__(self, *args, **kwargs):
        super(ChisholmGamon_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        ...
        all_links = []
        if data:
            for property in data.get('props',{}).get('pageProps',{}).get('pageData',{}).get('results',{}).get('listings',[]):
                property_slug = property.get('url','')
                property_url = f'https://chisholmgamon.com.au{property_slug}' if property_slug else ""
                all_links.append(property_url)

        # all_links = response.css("a.latest-right-detail ::attr(href)").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://chisholmgamon.com.au/{each_url}'
        ...
        for property in data.get('props',{}).get('pageProps',{}).get('pageData',{}).get('results',{}).get('listings',[]):
            ...
            self.count += 1
            # yield response.follow(property_url, headers=self.headers, callback=self.detail_parse)     ### Not implementable here
            self.detail_parse(property)

        next_page = response.xpath("//*[contains(text(),'→')]").get('').strip()
    ...
    #     detail_page_url = response.url
        ...
        if detail_page_url not in database_sale_matching_url:
            ...
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2983)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Chisholm Gamon'

            address_subNumber = property.get('address_subNumber','')
            address_lotNumber = property.get('address_lotNumber','')
            address_streetNumber = property.get('address_streetNumber','')
            address_street = property.get('address_street','')
            address_suburb = property.get('address_suburb','')
            property_address = f"{address_subNumber} {address_lotNumber} {address_streetNumber} {address_street} {address_suburb}"
            item['Field5'] = property_address  # full_adddress

            item['Field6'] = property.get('bedrooms','')
            item['Field7'] = property.get('bathrooms','')
            item['Field8'] = property.get('parking','')

            item['Field9'] = property.get('price','')
            item['Field12'] = property.get('description','')

            '''         Uploading Images on Wasabi S3 Bucket            '''
            images = []
            for img in property.get('images',[]):
                img = img.get('image',{}).get('large','')
            ...
            if property.get('agents', []):
                agent1_name = property['agents'][0]['title']
                first_name1, last_name1 = agent1_name.split(maxsplit=1)
                item['Field15'] = first_name1.strip()  # agent_first_name_1
                item['Field16'] = last_name1.strip()  # agent_surname_name_1

            index = 17
            for i, rows in enumerate(property.get('agents', []), start=1):
                item[f'Field{index}'] =rows.get("title")
                item[f'Field{index + 1}'] = rows.get("job_title")
                item[f'Field{index + 2}'] = rows.get("email")
                item[f'Field{index + 3}'] = rows.get("phone")
                item[f'Field{index + 4}'] = ''
                if i == 1:
                    agent_image = rows.get("profile_image_thumbnail",{}).get('url','')
                    images_string = f'{agent_image}' if agent_image else ""
                    # item[f'Field{index + 5}'] = images_string
                    # print(f'Field{index + 5} image is :',images_string)
                    random_number = random.randint(1, 10000000000)
                    item[f'Field{index + 5}'] = ",".join(self.uploaded(images_string, random_number))
                    index += 6
                else:
                    item[f'Field{index + 3}A'] = ''
                    agent_image = rows.get("profile_image_thumbnail",{}).get('url','')
                    images_string = f'{agent_image}' if agent_image else ""
                    # item[f'Field{index + 4}'] = images_string
                    # print(f'Field{index + 4} image is :',images_string)
                    random_number = random.randint(1, 10000000000)
                    item[f'Field{index + 4}'] = ",".join(self.uploaded(images_string, random_number))
                    index += 5

            '''          AGENTS          -      Not implementable here      '''
            # if property.get('agents', []):
            ...
            #         item['Field20'] = agent1.css(".agentTile-contact-mobile a ::text").get('').strip().replace('tel:','')
            #         item['Field21'] = agent1.css(".agentTile-contact-phone a ::text").get('').strip().replace('tel:','')
            #         '''         Uploading Images on Wasabi S3 Bucket            '''
            #         agent_image = agent1.css(".agentTile-image ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            #         item['Field26'] = agent2.css(".agentTile-contact-mobile a ::text").get('').strip().replace('tel:','')
            #         item['Field26A'] = agent2.css(".agentTile-contact-phone a ::text").get('').strip().replace('tel:','')
            #         '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            #         item['Field31'] = agent3.css(".agentTile-contact-mobile a ::text").get('').strip().replace('tel:','')
            #         item['Field31A'] = agent3.css(".agentTile-contact-phone a ::text").get('').strip().replace('tel:','')
            #         '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            item['Field36'] = property.get('buildingDetails_area','')
