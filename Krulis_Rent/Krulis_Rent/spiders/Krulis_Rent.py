...
class Krulis_Rent(scrapy.Spider):
    name = 'Krulis_Rent'
    prefix = 'https://krulis.com.au'
    url = "https://krulis.com.au/?action=epl_search&post_type=rental&property_address="
    headers = {
        'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        'cache-control': 'max-age=0',
        # 'cookie': 'epl_wp_session=42568e4b4dd6f0c4276e20902c01afb5%7C%7C1711860088%7C%7C1711858288; _ga=GA1.1.945583069.1711816890; preferredView=grid; _ga_NJSM38C3RS=GS1.1.1711816889.1.1.1711816970.0.0.0; arp_scroll_position=600; epl_wp_session=42568e4b4dd6f0c4276e20902c01afb5%7C%7C1711860088%7C%7C1711858288',
        # 'referer': 'https://krulis.com.au/properties-sold/',
    ...
    def parse(self, response):
        for property in response.css(".property-address a"):#[:2]:
        ...
        '''    PAGINATION    '''
        if response.css('.nextpostslink'):
            next_page_url = response.css('.nextpostslink ::attr(href)').get('').strip()
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            property_add = response.css('.tab-address span ::text').getall()  # full_adddress
            ...
            item['Field6'] = response.css('.kicon-bedroom+ .attr-text ::text').get('').strip().replace(' Bedrooms','')
            item['Field7'] = response.css('.kicon-bathroom+ .attr-text ::text').get('').strip().replace(' Bathrooms','')
            item['Field8'] = response.css('.kicon-parkingspace+ .attr-text ::text').get('').strip().replace(' Car Spaces','')
            ...
            description = response.css('.epl-section-description p ::text , .entry-title ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess = response.css('#imageGallery li img:nth-child(1) ::attr(data-src)').getall()
            new_img_urls = []
            ...
            if agent1:
                ...
                item['Field19'] = agent1.css('.email-author ::text').get('').strip()
                agent1_phone = agent1.css('.office-phone ::text').get('')
                item['Field20'] = agent1_phone.strip()
                item['Field21'] = agent1.css('.mobile ::text').get('')
                '''         Uploading Images on Wasabi S3 Bucket            '''
                Agent_Imagess = agent1.css(".author-image img ::attr(data-src)").getall()
                Agent_Imagesss = []
                for url in Agent_Imagess:
            ...
            if len(response.xpath("//*[contains(@class,'epl-author-box-outer-wrapper author-box-outer-wrapper epl-clearfix')]").getall()) > 1:
                ...
                if agent2:
                    ...
                    item['Field25'] = agent2.css('.email-author ::text').get('').strip()
                    agent2_phone = agent2.css('.office-phone ::text').get('')
                    item['Field26'] = agent2_phone.strip()
                    item['Field26A'] = agent2.css('.mobile ::text').get('')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Imagess = agent2.css(".author-image img ::attr(data-src)").getall()
                    Agent_Imagesss = []
                    for url in Agent_Imagess:
                        Agent_Imagesss.append(url)
                    Agent_Images = ','.join(Agent_Imagesss[:1])
