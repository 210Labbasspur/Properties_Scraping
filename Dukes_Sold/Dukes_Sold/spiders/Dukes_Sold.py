...
class Dukes_Sold(scrapy.Spider):
    name = 'Dukes_Sold'
    prefix = 'https://dukesestateagents.com.au'
    # url = "https://dukesestateagents.com.au/ajax-search-listings?page=1&type=Sold"
    url = "https://dukesestateagents.com.au/ajax-search-listings?page={}&type=Sold"
    headers = {
        'accept': '*/*',
        'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        # 'cookie': '_gs=2ca485ae17d74ca1c30d76ad26f79f913640592e; arp_scroll_position=824',
        'priority': 'u=1, i',
        # 'referer': 'https://dukesestateagents.com.au/listings?type=Sold',
    ...
    def parse(self, response):
        for property in response.css(".propWidget-2"):
        ...
        if response.xpath("//*[contains(text(),'Load More')]/@href").get('').strip():
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            property_add = response.css('.address a ::text').getall()  # full_adddress
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'fa fa-bed')]/following-sibling::div[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'fa fa-bath')]/following-sibling::div[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'fa fa-car')]/following-sibling::div[1]/text()").get('').strip()

            sold_price = ''.join(element.strip().replace('Price:','') for element in response.css(".ld_price_tag ::text").getall())  # description
            if sold_price:
                item['Field9'] = (sold_price.strip()).replace('$','').replace(' ','')   # sold_price

            description = response.css('.listing-detail-description ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess = response.css(".pgwslideshow img ::attr(data-large-src)").getall()
            new_img_urls = []
            ...
            index_of_equal = (response.url).rindex("=")
            id = response.url[index_of_equal + 1:]
            ...
            '''          AGENTS          '''
            if response.css(".listing-agent-wrapper"):
                agent1 = response.css(".listing-agent-wrapper")[0]
                agent1_name = agent1.css('.agentName a ::text').get('').strip()
                if agent1:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    agent1_phone = agent1.css(".agentPhone a ::text").get('').strip()
                    item['Field20'] = agent1_phone.strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images_style = agent1.css(".agent-bg-img-review ::attr(style)").get('').strip()
                ...
                if len(response.css(".listing-agent-wrapper").getall()) > 1:
                    agent2 = response.css(".listing-agent-wrapper")[1]
                    agent2_name = agent2.css('.agentName a ::text').get('').strip()
                    if agent2:
                        item['Field23'] = agent2_name.strip()
                        agent2_phone = agent2.css(".agentPhone a ::text").get('').strip()
                        item['Field26'] = agent2_phone.strip()
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images_style = agent2.css(".agent-bg-img-review ::attr(style)").get('').strip()
                        Agent_Images = re.search(r'url\((.*?)\)', Agent_Images_style).group(1)
                ...
                if len(response.css(".listing-agent-wrapper").getall()) == 3:
                    agent3 = response.css(".listing-agent-wrapper")[2]
                    agent3_name = agent3.css('.agentName a ::text').get('').strip()
                    if agent3:
                        item['Field28'] = agent3_name.strip()
                        agent3_phone = agent3.css(".agentPhone a ::text").get('').strip()
                        item['Field31'] = agent3_phone.strip()
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images_style = agent3.css(".agent-bg-img-review ::attr(style)").get('').strip()
                        Agent_Images = re.search(r'url\((.*?)\)', Agent_Images_style).group(1)
            ...
            land_area = response.xpath("//*[contains(@class,'fa fa-map')]/following-sibling::div[1]/text()").get('').strip()

            if land_area:
                item['Field36'] = land_area.replace('Sqm','').replace(' ','')   # land_area

            feature_count = 58
            for feature in response.css(".amItem"):
