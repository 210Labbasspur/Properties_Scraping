...
class CampbellJones_Sold(scrapy.Spider):
    name = 'CampbellJones_Sold'
    prefix = 'https://www.cjpbowral.com.au'
    url = "https://www.cjpbowral.com.au/recently-sold"
    headers = {
    ...
    def parse(self, response):
        for property in response.css("a.listing-img-container"):
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'Campbell Jones Property'
            ...
            item['Field6'] = response.xpath("//*[contains(text(),'Bedrooms')]/span[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(text(),'Bathrooms')]/span[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(text(),'Car Spaces')]/span[1]/text()").get('').strip()

            sold_price = response.css('.property-price ::text').get('').strip().replace('SOLD','').replace(' ','')
            ...
            Agent_Imagess = response.css(".property-slider a ::attr(href)").getall()
            new_img_urls = []
            ...
            '''          AGENTS          '''
            if response.css(".agent-title"):
                agent1 = response.css(".agent-title")[0]
                ...
                if agent1_name:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images = agent1.css(".agent-photo img ::attr(src)").get('').strip()
                ...
                if len(response.css(".agent-title").getall()) > 1:
                    ...
                    if agent2:
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent2.css(".agent-photo img ::attr(src)").get('').strip()
                ...
                if len(response.css(".agent-title").getall()) > 2:
                    agent3 = response.css(".agent-title")[2]
                    ...
                    if agent3:
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent3.css(".agent-photo img ::attr(src)").get('').strip()
            ...
            item['Field35'] = response.css(".sub-price ::text").get('').strip()

            land_area = response.xpath("//*[contains(text(),'Land Size')]/span[1]/text()").get('').strip()
            if land_area:
