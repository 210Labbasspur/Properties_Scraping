...
class MurrayKennedy_Sale(scrapy.Spider):
    name = 'MurrayKennedy_Sale'
    prefix = 'https://murraykennedyrealestate.com.au'
    url = "https://murraykennedyrealestate.com.au/listings?saleOrRental=Sale&status=available_under_contract&sortby=dateListed-desc&type=Residential"
    headers = {
    ...
    def parse(self, response):
        for property in response.css(".v2-items-start.v2-gap-y-1\.5 a"):
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'Murray Kennedy Real Estate'

            property_add = response.xpath("//*[contains(@data-testid,'address')]/text()").getall()  # full_adddress
            ...
            item['Field6'] = response.xpath("//*[contains(@data-testid,'stats-bed')]/p[1]/text()").get('').strip().replace('Bed','')
            item['Field7'] = response.xpath("//*[contains(@data-testid,'stats-bath')]/p[1]/text()").get('').strip().replace('Bath','')
            item['Field8'] = response.xpath("//*[contains(@data-testid,'stats-car')]/p[1]/text()").get('').strip().replace('Car','')

            # sold_price = response.xpath("//*[contains(@data-testid,'listing-price')]/text()").get('').strip().replace('$','')
            # if re.search(r'\d', sold_price):
            #     item['Field9'] = (sold_price.strip()).replace('SOLD','').replace('$','')   # sold_price

            description = response.xpath("//*[contains(@class,'v2-prose v2-max-w-none v2-text-body siteloft-ucg v2-text-text-grey')]/p[1]/text()").getall()
            ...
            '''          AGENTS          '''
            if response.css("#sidebar-item-wrapper .v2-gap-5"):
                agent1 = response.css("#sidebar-item-wrapper .v2-gap-5")[0]
                agent1_name = agent1.css('.hover\:v2-underline ::text').get('').strip()
                if agent1:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images = agent1.xpath(".//*[contains(@alt,'Staff image')]/@src").get('').strip()
                ...
                if len(response.css("#sidebar-item-wrapper .v2-gap-5")) > 2:
                    agent2 = response.css("#sidebar-item-wrapper .v2-gap-5")[2]
                    agent2_name = agent2.css('.hover\:v2-underline ::text').get('').strip()
                    if agent2:
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent2.xpath(".//*[contains(@alt,'Staff image')]/@src").get('').strip()
                ...
                if len(response.css("#sidebar-item-wrapper .v2-gap-5")) > 4:
                    agent3 = response.css("#sidebar-item-wrapper .v2-gap-5")[4]
                    agent3_name = agent3.css('.hover\:v2-underline ::text').get('').strip()
                    if agent3:
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent3.xpath(".//*[contains(@alt,'Staff image')]/@src").get('').strip()
