...
class ClarkeCo_Rent(scrapy.Spider):
    name = 'ClarkeCo_Rent'
    prefix = 'https://presence.realestate'
    # url = 'https://www.clarkeandco.com.au/wp-json/api/listings/all?status=current&type=rental&paged=1&priceRange=&isProject=false&limit=8&author=&bed=0&bath=0&sort='
    url = 'https://www.clarkeandco.com.au/wp-json/api/listings/all?status=current&type=rental&paged={}&priceRange=&isProject=false&limit=8&author=&bed=0&bath=0&sort='
    headers = {
        'accept': '*/*',
        'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        # 'cookie': '_gid=GA1.3.166773514.1713640302; _ga_MR9TJ2E7L1=GS1.1.1713644995.2.0.1713644996.0.0.0; _ga=GA1.3.1751017295.1713640300; _gat_gtag_UA_64680522_25=1',
        'priority': 'u=1, i',
        # 'referer': 'https://www.clarkeandco.com.au/properties-for-sale/sold/?pageno=2',
    ...
    def parse(self, response):
        ...
        for property in data['results']:
            ...
            # mini_data['sold_price'] = property.get('propertyPricing').get('propertyPrice').replace('Sold Price','').replace('SOLD','')
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'Clarke & Co Estate Agents'
            ...
            Agent_Imagess = response.css(".img-wrapper img::attr(src)").getall()
            new_img_urls = []
            ...
            if response.css(".agent-container"):
                ...
                if agent1:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images_style = agent1.css(".agent-profile ::attr(style)").get('').strip()
                ...
                if len(response.css(".agent-container").getall()) > 1:
                    ...
                    if agent2:
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images_style = agent2.css(".agent-profile ::attr(style)").get('').strip()
                        Agent_Images = re.search(r"url\((.*?)\)", Agent_Images_style).group(1)
                ...
                if len(response.css(".agent-container").getall()) == 3:
                    ...
                    if agent3:
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images_style = agent3.css(".agent-profile ::attr(style)").get('').strip()
                        Agent_Images = re.search(r"url\((.*?)\)", Agent_Images_style).group(1)
