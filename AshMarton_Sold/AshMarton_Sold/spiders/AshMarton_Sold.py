...
class AshMarton_Sold(scrapy.Spider):
    name = "AshMarton_Sold"

    url = 'https://ashmarton.com.au/wp-admin/admin-ajax.php'
    prefix = "https://ashmarton.com.au"
    headers = {
    ...
    data = {
        'action': 'load_more_sold_property',
        'offset': '10',
        'per_page': '5',
        'sorting': '',
    }
    ...
    def __init__(self, *args, **kwargs):
        super(AshMarton_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.xpath("//*[contains(@href,'https://ashmarton.com.au/property/')]/@href").getall()
        links = []
        ...
        if response.xpath("//*[contains(@href,'https://ashmarton.com.au/property/')]/@href"):
            print("offset is :", offset)
            payload = deepcopy(self.data)
            payload['offset'] = str(offset)
            yield scrapy.FormRequest(self.url, method='POST', formdata=payload, headers=self.headers,
                                     callback=self.parse, meta={'offset': offset})
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2597)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Ash Marton'

            property_street = response.css('.prppt-name-det h1 ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.addres-pdeta ::text').getall() if e.strip())  # property_suburb
            ...
            price = response.css('.price-range-detail ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            ...
            '''          AGENTS          '''
            if response.css(".thumb-team"):
                agent1 = response.css(".thumb-team")[0]
                ...
                if agent1_name:
                    ...
                    item['Field18'] = agent1.css('h6 a ::text').get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css("source  ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.css(".thumb-team").getall()) > 1:
                agent2 = response.css(".thumb-team")[1]
                agent2_name = agent2.css('h4 a ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.css('h6 a ::text').get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css("source  ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css(".thumb-team").getall()) > 2:
                agent3 = response.css(".thumb-team")[2]
                agent3_name = agent3.css('h4 a ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field29'] = agent3.css('h6 a ::text').get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css("source  ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            type_land = response.css('.hos-dmns ::text').get('').strip()
            item['Field35'] = type_land.split('|')[0]
