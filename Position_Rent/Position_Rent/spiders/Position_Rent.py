...
class Position_Rent(scrapy.Spider):
    name = "Position_Rent"

    url = "https://www.positionproperty.com.au/renting/property-search"
    prefix = "https://www.positionproperty.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Position_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".inner a.btn-primary ::attr(href)").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://www.positionproperty.com.au{each_url}'
        ...
        for property_url in all_links:
            property_url = f'https://www.positionproperty.com.au{property_url}'
        ...
        next_page = response.xpath("//*[contains(@aria-label,'Next')]/@href").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2222)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Position Property'

            property_street = response.css('.page-title .col-md-12 ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.xxxxxxxxxxxxxxxxxx ::text').getall() if e.strip())  # property_suburb
            property_address = f"{property_street} {property_suburb}"
            item['Field5'] = property_address  # full_adddress

            item['Field6'] = response.xpath("//img[contains(@class,'bed')]/following-sibling::*[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//img[contains(@class,'bath')]/following-sibling::*[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//img[contains(@class,'car')]/following-sibling::*[1]/text()").get('').strip()

            price = response.css('p.price ::text').get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','')
            ...
            '''          AGENTS          '''
            if response.css(".staff"):
                agent1 = response.css(".staff")[0]
                agent1_name = agent1.css('p span ::text').get('').strip()
                if agent1_name:
            ...
            if len(response.css(".staff").getall()) > 1:
                agent2 = response.css(".staff")[1]
                agent2_name = agent2.css('p span ::text').get('').strip()
                if agent2:
            ...
            if len(response.css(".staff").getall()) > 2:
                agent3 = response.css(".staff")[2]
                agent3_name = agent3.css('p span ::text').get('').strip()
                if agent3:
