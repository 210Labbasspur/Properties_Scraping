...
class RandR_Rent(scrapy.Spider):
    name = 'RandR_Rent'
    prefix = 'https://www.randrproperty.com.au'
    url = "https://www.randrproperty.com.au/browse-properties-rental/"
    headers = {
    ...
    def parse(self, response):
        ...
        all_details_links = []
        for url in response.css(".epl-archive-entry-image a"):
        ...
        next_page = response.css('a.nextpostslink ::attr(href)').get('').strip()
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'R & R Property'

            property_add = response.css('.epl-clearfix .entry-title ::text').getall()  # full_adddress
            ...
            item['Field6'] = response.xpath("//*[contains(@title,'Bedroom')]/following-sibling::div[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@title,'Bathroom')]/following-sibling::div[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@title,'Parking Space')]/following-sibling::div[1]/text()").get('').strip()

            sold_price = response.css('span.page-price ::text').get('').strip().replace('Sold','').replace(' ','')
            if re.search(r'\d', sold_price):
                item['Field9'] = (sold_price.strip()).replace('Sold','').replace('$','')   # sold_price

            description = response.css('.entry-content p ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess = response.xpath("//*[contains(@u,'image')]/@src2").getall()
            new_img_urls = []
            ...
            '''          AGENTS          '''
            if response.css("div.epl-author-box-outer-wrapper"):
                agent1 = response.css("div.epl-author-box-outer-wrapper")[0]
                ...
                if agent1_name:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images = agent1.css("img ::attr(data-src)").get('').strip()
                ...
                if len(response.css("div.epl-author-box-outer-wrapper").getall()) > 1:
                    agent2 = response.css("div.epl-author-box-outer-wrapper")[1]
                    ...
                    if agent2:
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent2.css("img ::attr(data-src)").get('').strip()
                ...
                if len(response.css("div.epl-author-box-outer-wrapper").getall()) > 2:
                    agent3 = response.css("div.epl-author-box-outer-wrapper")[2]
                    ...
                    if agent3:
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent3.css("img ::attr(data-src)").get('').strip()
            ...
            land_area = response.xpath("//*[contains(@class,'icon land-size')]/span[1]/text()").get('').strip()
            if land_area:
                item['Field36'] = land_area.replace('m²','').replace(' ','')   # land_area

            feature_count = 58
            for feature in response.css(".epl-property-features li"):
