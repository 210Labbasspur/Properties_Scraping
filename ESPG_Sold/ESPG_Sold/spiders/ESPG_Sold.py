...
class ESPG_Sold(scrapy.Spider):
    name = 'ESPG_Sold'
    prefix = 'https://www.espg.com.au'
    url = "https://www.espg.com.au/Sell/Recent-Sales/Results.aspx?sort=&indexpage=1"
    headers = {
        'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        # 'cookie': 'CMSPreferredCulture=en-AU; __utmz=189452607.1714476092.1.1.utmcsr=(direct)|utmccn=(direct)|utmcmd=(none); ASP.NET_SessionId=n3vjalqa1uevsr55osjdny55; __utma=189452607.626745852.1714476092.1714476092.1714597177.2; __utmc=189452607; __utmt=1; __utmb=189452607.2.10.1714597177; arp_scroll_position=2289',
        'priority': 'u=0, i',
        # 'referer': 'https://www.espg.com.au/Sell/Recent-Sales/Results.aspx',
    ...
    def parse(self, response):
        for property in response.css(".view-link"):
        ...
        next_page = response.css('.NextPage::attr(href)').get('').strip()
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'Eastern Suburbs Property Group'

            property_add = response.css('.listing h1 ::text').getall()  # full_adddress
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'bed')]/strong[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'bath')]/strong[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'car')]/strong[1]/text()").get('').strip()

            sold_price = response.css('.ld-right h3 ::text').get('').strip().replace(' ','')
            if re.search(r'\d', sold_price):
                item['Field9'] = (sold_price.strip()).replace('$','')   # sold_price

            description = response.css('.ld-right p ::text, .catch-phrase ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess = response.css(".image-container a::attr(href)").getall()
            new_img_urls = []
            ...
            '''          AGENTS          '''
            if response.css("div.member"):
                agent1 = response.css("div.member")[0]
                ...
                if agent1:
                    ...
                    agent1_phone = agent1.css(".name+ p ::text").get('').strip()
                ...
                if len(response.css("div.member").getall()) > 1:
                    agent2 = response.css("div.member")[1]
                    ...
                    if agent2:
                        ...
                        agent2_phone = agent2.css(".name+ p ::text").get('').strip()
                ...
                if len(response.css("div.member").getall()) > 2:
                    agent3 = response.css("div.member")[2]
                    ...
                    if agent3:
                        ...
                        agent3_phone = agent3.css(".name+ p ::text").get('').strip()
