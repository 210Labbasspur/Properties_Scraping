...
class Harwood_Sale(scrapy.Spider):
    name = "Harwood_Sale"

    url = "https://www.harwoodproperty.com.au/buy/properties-for-sale/"
    prefix = "https://www.harwoodproperty.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Harwood_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2923)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Harwood Property'

            property_street = response.css('.property-address-title ::text').get('').strip()  # property_street
            ...
            item['Field6'] = ''.join(e.strip() for e in response.css("#quickinfo .bedroom ::text").getall())
            item['Field7'] = ''.join(e.strip() for e in response.css("#quickinfo .bathroom ::text").getall())
            item['Field8'] = ''.join(e.strip() for e in response.css("#quickinfo .carspace ::text").getall())

            price = response.css('.price__label ::text').get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('#description .sub-title ::text').getall())  # description
            ...
            prop_images = response.xpath("//img[contains(@class,'img-default-used-size-hd')]/@src").getall()
            images = []
            ...
            land_area = response.css("#quickinfo .land-size ::text").get('').strip().replace('Land','').replace('is','').replace('m²','')
            if land_area:
