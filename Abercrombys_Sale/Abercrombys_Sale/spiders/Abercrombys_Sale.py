...
class Abercrombys_Sale(scrapy.Spider):
    name = "Abercrombys_Sale"
    url = ('https://www.abercrombys.com.au/wp-admin/admin-ajax.php?action=ppd_get_filtered_properties&filters%5Bsort%5D=high-price&filters%5BlistingType%5D=For%20Sale&filters%5Bsuburbs%5D=&filters%5BpropertyTypes%5D=&filters%5BminPrice%5D=&filters%5BmaxPrice%5D=&filters%5Bbedrooms%5D=&filters%5Bbathrooms%5D=&filters%5BisCoastal%5D=false&filters'
           '%5Bpage%5D={}')
    prefix = "https://www.abercrombys.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Abercrombys_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        ...
        selector = Selector(text=data.get('html',''))
        if selector.css('.listing a'):
            links = []
            all_links = selector.css('.listing a ::attr(href)').getall()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2562)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Abercrombys'

            property_street = response.css('.h3 ::text').get('').strip()  # property_street
            ...
            item['Field6'] = response.xpath("//*[contains(@d,'M18.5348 5.78831V0H1.86518V5')]/parent::*[1]/preceding-sibling::span[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@d,'M15.9195 2.87904H14.3155V2')]/parent::*[1]/parent::*[1]/preceding-sibling::span[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@d,'M26.4 7.26373L26.3715 ')]/parent::*[1]/parent::*[1]/preceding-sibling::span[1]/text()").get('').strip()

            # price = response.css('#property__meta p ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            ...
            if response.css(".agent"):
                ...
                agent1_name = agent1.css('p ::text').get('').strip()
                if agent1_name:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field18'] = agent1.css('em ::text').get('').strip()
            ...
            if len(response.css(".agent").getall()) > 1:
                ...
                agent2_name = agent2.css('p ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.css('em ::text').get('').strip()
            ...
            if len(response.css(".agent").getall()) > 2:
                ...
                agent3_name = agent3.css('p ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field29'] = agent3.css('em ::text').get('').strip()
            ...
            land = response.xpath("//*[contains(@d,'M5.34022 0V7.64648H21.986V0H5')]/parent::*[1]/following-sibling::span[1]/text()").get('').strip()
            land_area = land.replace('Land','').replace('is','').replace('m2','')
            if land_area:
