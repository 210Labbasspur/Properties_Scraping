...
class AgencyHQ_Sale(scrapy.Spider):
    name = "AgencyHQ_Sale"

    url = "https://agencyhq.net.au/properties-for-sale"
    prefix = "https://agencyhq.net.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(AgencyHQ_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3011)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Agency HQ'

            property_street = response.css('.xxxxxxxxxxxxxx ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.propertyPage-address ::text').getall() if e.strip())  # property_suburb
            property_address = f"{property_street} {property_suburb}"
            item['Field5'] = property_address  # full_adddress

            item['Field6'] = response.css(".icons-bed ::text").get('').strip()
            item['Field7'] = response.css(".icons-bath ::text").get('').strip()
            item['Field8'] = response.css(".icons-car ::text").get('').strip()

            price = response.css('.propertyPage-priceText ::text').get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.propertyPage-title ::text').getall())  # description
            ...
            if response.css(".agentTile"):
                ...
                if agent1_name:
                    ...
                    images_string = f'https://agencyhq.net.au{agent_image}' if agent_image else ""
            ...
            land_area = response.xpath('//*[contains(text(),"Land area")]/following-sibling::span/text()').get('').strip().replace('Land','').replace('is','').replace('m²','')
            if land_area:
                if 'Acres' in land_area:
                    pass
                else:
                    item['Field36'] = land_area.replace('sqm','').replace(' ','')   # land_area
            floor_area = response.xpath('//*[contains(text(),"Floor area")]/following-sibling::span/text()').get('').strip().replace('Land','').replace('is','').replace('m²','')
            if floor_area:
            ...
            feature_count = 58
            for feature in response.css(".kvlist li span:nth-child(1)"):   ##  first 4 features are ID, bed, bath etc.
