...
class OlsenRomano_Rent(scrapy.Spider):
    name = 'OlsenRomano_Rent'
    prefix = 'https://www.olsenromano.com.au'
    url = ("https://www.olsenromano.com.au/wp-content/themes/ap-olsen/zoorealty/display/elements/load_listings.php?post_id=&status"
           "=1,4&list=lease&order=&order_direction=&property_type=&keywords=&price_min=&price_max=&bedrooms=&page={}")
    headers = {
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'Olsen Romano'

            property_add = response.css('h2.address ::text').getall()  # full_adddress
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'icon-bedrooms')]/parent::span[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'icon-bathrooms')]/parent::span[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'icon-carspaces')]/parent::span[1]/text()").get('').strip()

            # sold_price = response.css('.price__label ::text').get('').strip().replace('SOLD OFF-MARKET','').replace(' ','')
            # if re.search(r'\d', sold_price):
            #     item['Field9'] = (sold_price.strip()).replace('SOLD','').replace('$','')   # sold_price

            description = response.css('#property-details .left.borderbox ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess = response.xpath("//*[contains(@class,'fit-height')]/@src").getall()
            new_img_urls = []
            ...
            '''          AGENTS          '''
            if response.css("#property-agents .agent"):
                agent1 = response.css("#property-agents .agent")[0]
                ...
                if agent1_name:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images = agent1.css(".fit-width ::attr(src)").get('').strip()
                ...
                if len(response.css("#property-agents .agent").getall()) > 1:
                    agent2 = response.css("#property-agents .agent")[1]
                    ...
                    if agent2:
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent2.css(".fit-width ::attr(src)").get('').strip()
                ...
                if len(response.css("#property-agents .agent").getall()) > 2:
                    agent3 = response.css("#property-agents .agent")[2]
                    ...
                    if agent3:
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent3.css(".fit-width ::attr(src)").get('').strip()
