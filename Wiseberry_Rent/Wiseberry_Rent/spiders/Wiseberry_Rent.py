...
class Wiseberry_Rent(scrapy.Spider):
    name = "Wiseberry_Rent"

    url = "https://wiseberry.com.au/rent?page=rent?&page={}&limit=9"
    prefix = "https://wiseberry.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Wiseberry_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".contents ::attr(href)").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://wiseberry.com.au{each_url}'
        ...
        for property_url in all_links:
            property_url = f'https://wiseberry.com.au{property_url}'
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2889)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Wiseberry Real Estate'

            property_street = ' '.join(e.strip() for e in response.xpath("//*[contains(@class,'text-center mt-[2px] text-gray-500 dark:text-white lg:text-xl')]/text()").getall() if e.strip())  # property_suburb
            property_suburb = ' '.join(e.strip() for e in response.xpath("//*[contains(@class,'text-wiseberry_red text-center')]/text()").getall() if e.strip())  # property_suburb
            property_address = f"{property_street} {property_suburb}"
            item['Field5'] = property_address  # full_adddress

            item['Field6'] = response.xpath("//*[contains(@src,'bed-icon')]/parent::span[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@src,'bath-icon')]/parent::span[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@src,'car-icon')]/parent::span[1]/text()").get('').strip()

            price = response.xpath("//*[contains(@class,'flex justify-center items-center my-2 font-normal lg:text-xl')]/text()").get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','').replace('for','')
            if price:
                item['Field9'] = price.replace('ContactAgent','')
            ...
            '''          AGENTS          '''
            if response.css(".space-x-5"):
                agent1 = response.css(".space-x-5")[0]
                agent1_name = ' '.join(e.strip() for e in agent1.css('.text-xl ::text').getall() if e.strip())
                if agent1_name:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field18'] = agent1.css('.text-wiseberry_red ::text').get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css("img.shadow-lg ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'https://wiseberry.com.au{agent_image}' if agent_image else ""
                    # item['Field22'] = images_string
                    # print('Agent1 image is :',images_string)
                    random_number = random.randint(1, 10000000000)
                    item['Field22'] = ",".join(self.uploaded(images_string, random_number))

            if len(response.css(".space-x-5").getall()) > 1:
                agent2 = response.css(".space-x-5")[1]
                agent2_name = ' '.join(e.strip() for e in agent2.css('.text-xl ::text').getall() if e.strip())
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.css('.text-wiseberry_red ::text').get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css("img.shadow-lg ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'https://wiseberry.com.au{agent_image}' if agent_image else ""
            ...
            if len(response.css(".space-x-5").getall()) > 2:
                agent3 = response.css(".space-x-5")[2]
                agent3_name = ' '.join(e.strip() for e in agent3.css('.text-xl ::text').getall() if e.strip())
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field29'] = agent3.css('.text-wiseberry_red ::text').get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css("img.shadow-lg ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'https://wiseberry.com.au{agent_image}' if agent_image else ""
