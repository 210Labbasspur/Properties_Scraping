...
class Broadhurst_Sold(scrapy.Spider):
    name = "Broadhurst_Sold"

    url = "https://www.broadhurstproperty.com.au/sold/page/1/?pagination_id=sort_sold&instance_id=sort_sold"
    prefix = "https://www.broadhurstproperty.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Broadhurst_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".epl-featured-image a ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            ...
            item['Field4'] = 'Broadhurst Property'

            property_street = response.css('.col-sm-7 .item-street ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.col-sm-7 .entry-title-sub ::text').getall() if e.strip())  # property_suburb
            property_address = f"{property_street} {property_suburb}"
            item['Field5'] = property_address  # full_adddress

            item['Field6'] = response.xpath("//*[contains(@class,'col-sm-7')]//*[contains(@class,'beds')]/span[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'col-sm-7')]//*[contains(@class,'bath')]/span[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'col-sm-7')]//*[contains(@class,'parking')]/span[1]/text()").get('').strip()

            price = response.css('.page-price ::text').get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.property-heading ::text').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".tab-content p ::text").getall())  # description
            ...
            '''          AGENTS          '''
            if response.css(".sidebar_wrap .epl-author-box-container"):
                agent1 = response.css(".sidebar_wrap .epl-author-box-container")[0]
            ...
            if len(response.css(".sidebar_wrap .epl-author-box-container").getall()) > 1:
                agent2 = response.css(".sidebar_wrap .epl-author-box-container")[1]
            ...
            if len(response.css(".sidebar_wrap .epl-author-box-container").getall()) > 2:
                agent3 = response.css(".sidebar_wrap .epl-author-box-container")[2]
            ...
            land_area = response.css('.col-sm-7 .property-land_area ::text').get('').strip().replace('Land','').replace('is','').replace('m²','')
            if 'sqm' in response.css('.col-sm-7 .property_land_area_unit ::text').get('').strip():
