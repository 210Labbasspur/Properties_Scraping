...
class Shead_Rent(scrapy.Spider):
    name = 'Shead_Rent'
    prefix = 'https://www.shead.com.au'
    url = "https://www.shead.com.au/properties-for-lease"
    headers = {
        'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        'cache-control': 'max-age=0',
        # 'cookie': 'PHPSESSID=5057848741486758d17d42285d0385f7; _ga=GA1.1.232631381.1712610780; _ga_1S89C8DHYM=GS1.1.1712610779.1.1.1712610934.0.0.0; arp_scroll_position=0',
    ...
    def parse(self, response):
        for property in response.css('.propertyTile-caption'):#[:2]:
            property_url = property.css('a ::attr(href)').get('').strip()
            print(self.count, property_url)
            self.count += 1
            sold_price = property.css('.propertyTile-pricetext ::text').get('').strip()
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            property_add = response.css('.propertyPage-property-address span ::text').getall()  # full_adddress
            ...
            description = response.css('.propertyPage-property-description ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess = response.css('img.block ::attr(src)').getall()
            new_img_urls = []
            ...
            if agent1:
                ...
                '''         Uploading Images on Wasabi S3 Bucket            '''
                Agent_Images = self.prefix + agent1.css(".agentAvatar-image img ::attr(src)").get('').strip()
            ...
            if len(response.css(".agentAvatar").getall()) > 1:
                ...
                if agent2:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images = self.prefix + agent2.css(".agentAvatar-image img ::attr(src)").get('').strip()
            ...
            if len(response.css(".agentAvatar").getall()) == 3:
                ...
                if agent3:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images = self.prefix + agent3.css(".agentAvatar-image img ::attr(src)").get('').strip()
