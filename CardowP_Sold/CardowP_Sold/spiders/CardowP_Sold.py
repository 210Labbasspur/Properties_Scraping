...
class CardowP_Sold(scrapy.Spider):
    name = 'CardowP_Sold'
    prefix = 'https://www.cardowpartners.com.au'
    url = "https://www.cardowpartners.com.au/sell/sold-properties/1/"
    headers = {
    ...
    def parse(self, response):
        ...
        next_page = response.css('a.next_page_link ::attr(href)').get('').strip()
        if next_page:
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            sold_price = response.css('.section p.price ::text').get('').strip().replace('SOLD','').replace(' ','')
            ...
            Agent_Imagess = response.css("#lightgallery .photos ::attr(href)").getall()
            new_img_urls = []
            ...
            '''          AGENTS          '''
            if response.css(".agent_item"):
                agent1 = response.css(".agent_item")[0]
                ...
                if len(response.css(".agent_item").getall()) > 1:
                    agent2 = response.css(".agent_item")[1]
                ...
                if len(response.css(".agent_item").getall()) > 2:
                    agent3 = response.css(".agent_item")[2]
            ...
            land_area = response.css("p.land_size ::text").get('').strip().replace('Landsize','').replace(':','')
            if 'm2' in land_area:
                item['Field36'] = land_area.replace('m2','').replace(' ','')   # land_area

            feature_count = 58
            for feature in response.css("#property_features .block_content span"):
