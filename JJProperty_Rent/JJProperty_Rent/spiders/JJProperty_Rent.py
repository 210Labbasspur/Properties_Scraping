...
class JJProperty_Rent(scrapy.Spider):
    name = "JJProperty_Rent"

    url = "https://www.jjproperty.com.au/pages/real-estate/results?widgetKey=266319&context=listings&returnAs=markup&listing_sale_method=lease&display_sale_method=Rent&status=lease&pg=1"
    prefix = "https://www.jjproperty.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(JJProperty_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        ...
        for each_url in all_links:
            each_url = f'https://www.jjproperty.com.au{each_url}'
        ...
        for property_url in all_links:
            property_url = f'https://www.jjproperty.com.au{property_url}'
        ...
        next_page = response.css(".load-listings div ::attr(hx-get)").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3240)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'JJ Property'
            ...
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".about div ::text").getall())  # description
            ...
            feature_count = 58
            for feature in response.css(".features li"):   ##  first 4 features are ID, bed, bath etc.
