...
'''
        for_sale = []
        for all_p in response.xpath("//*[contains(@class,'property type-property')]"):
            if all_p.xpath(".//*[contains(@class,'status-sticker sold')]"):
                # print(count,'# This is Leased : ',all_p.css('.epl-archive-entry-image a::attr(href)').get('').strip())
                # count += 1
                pass
            else:
                # print('This is for Lease : ',all_p.css('.epl-archive-entry-image a::attr(href)').get('').strip())
                for_sale.append(all_p.css('.epl-archive-entry-image a::attr(href)').get('').strip())
'''
##################          Roberts_Sale
...
class Roberts_Sale(scrapy.Spider):
    name = "Roberts_Sale"

    url = "https://robertsre.com.au/tasmanian-property/"
    prefix = "https://robertsre.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Roberts_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def start_requests(self):
        yield scrapy.Request(url=self.url, headers=self.headers, callback=self.pre_parse)

    def pre_parse(self, response):
        count = 1
        for area in response.css('.fusion-column-anchor')[:-1]:    ##  This -1 slicing is important because last is commercial - Not required
        # for area in response.css('.fusion-column-anchor')[:1]:    ##  This is for testing
            area_url = area.css('::attr(href)').get('').strip()
            print(count,' # Area URL is : ', area_url)
            count += 1
            yield response.follow(area_url, headers=self.headers, callback=self.parse)
    ...
    def parse(self, response):
        ...
        for all_p in response.xpath("//*[contains(@class,'property type-property')]"):
            if all_p.xpath(".//*[contains(@class,'status-sticker sold')]"):
                # print(count,'# This is Sold : ',all_p.css('.epl-archive-entry-image a::attr(href)').get('').strip())
                # count += 1
                pass
            else:
                # print('This is for sale : ',all_p.css('.epl-archive-entry-image a::attr(href)').get('').strip())
                for_sale.append(all_p.css('.epl-archive-entry-image a::attr(href)').get('').strip())

        # all_links = response.xpath("//*[contains(@class,'status-sticker sold')]/ancestor::a[1]/@href").getall()
        all_links = for_sale
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2888)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Roberts Real Estate'

            property_street = response.css('.rec-contents .item-street ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.rec-contents .entry-title-sub ::text').getall() if e.strip())  # property_suburb
            property_address = f"{property_street} {property_suburb}"
            item['Field5'] = property_address  # full_adddress

            item['Field6'] = response.css(".rec-contents .bedroom .rec-icon-value ::text").get('').strip()
            item['Field7'] = response.css(".rec-contents .bathroom .rec-icon-value ::text").get('').strip()
            item['Field8'] = response.css(".rec-contents .parking .rec-icon-value ::text").get('').strip()

            price = response.css('#content .page-price ::text').get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.epl-section-description .entry-title ::text').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".epl-section-description .epl-entry-content ::text").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.xpath("//*[contains(@class,'epl-slick-item-image')]/@data-lazy").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.css(".epl-author-box-outer-wrapper"):
                agent1 = response.css(".epl-author-box-outer-wrapper")[0]
                ...
                if agent1_name:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".attachment-full ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.css(".epl-author-box-outer-wrapper").getall()) > 1:
                agent2 = response.css(".epl-author-box-outer-wrapper")[1]
                ...
                if agent2:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".attachment-full ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css(".epl-author-box-outer-wrapper").getall()) > 2:
                agent3 = response.css(".epl-author-box-outer-wrapper")[2]
                ...
                if agent3:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".attachment-full ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            floor_area = response.xpath('//*[contains(@class,"building-size")]/text()').get('').strip().replace('Floor Area','').replace('is','').replace('m²','')
            if floor_area:
