...
class Denovans_Sale(scrapy.Spider):
    name = "Denovans_Sale"
    url = 'https://denovans.com.au/wp-json/api/listings/all?suburbsSurrounding=&priceRange=&category=&type=property&status=current&paged={}&limit=16&address=&bed=&bath=&car=&sort='
    prefix = "https://denovans.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Denovans_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        ...
        print(f"Current-offset = {offset} || Total-Records = {total_records}")
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3221)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Denovans'

            streetNumber = property.get('address',{}).get('streetNumber','')
            streetAddress = property.get('address',{}).get('streetAddress','')
            suburb = property.get('address',{}).get('suburb','')
            state = property.get('address',{}).get('state','')
            postcode = property.get('address',{}).get('postcode','')
            item['Field5'] = f"{streetNumber} {streetAddress} {suburb} {state} {postcode}"

            item['Field6'] = property.get('propertyBed','')
            item['Field7'] = property.get('propertyBath','')
            item['Field8'] = property.get('propertyParking','')

            item['Field9'] = property.get('propertyPricing',{}).get('value','').replace('$','').replace(',','').replace(' ','')
            # item['Field9'] = response.css('.price__label ::text').get('').strip().replace('$','').replace(',','').replace(' ','')
            item['Field12'] = ' '.join(e.strip() for e in response.css('.listing-single__content-propertydesc-inner p ::text').getall())

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = property.get('propertyImage',{}).get('listImg',[])
            images = []
            ...
            '''          AGENTS          '''
            if response.css(".contact-agent-desktop .listing-single__contact-agents-item"):
                agent1 = response.css(".contact-agent-desktop .listing-single__contact-agents-item")[0]
                agent1_name = agent1.css('.listing-single__contact-agents-name a ::text').get('').strip()
                if agent1_name:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css("img ::attr(data-lazy-src)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.css(".contact-agent-desktop .listing-single__contact-agents-item").getall()) > 1:
                agent2 = response.css(".contact-agent-desktop .listing-single__contact-agents-item")[1]
                agent2_name = agent2.css('.listing-single__contact-agents-name a ::text').get('').strip()
                if agent2:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css("img ::attr(data-lazy-src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css(".contact-agent-desktop .listing-single__contact-agents-item").getall()) > 2:
                agent3 = response.css(".contact-agent-desktop .listing-single__contact-agents-item")[2]
                agent3_name = agent3.css('.listing-single__contact-agents-name a ::text').get('').strip()
                if agent3:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css("img ::attr(data-lazy-src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
