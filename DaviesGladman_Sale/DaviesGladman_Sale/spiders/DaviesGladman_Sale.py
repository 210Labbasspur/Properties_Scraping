...
class DaviesGladman_Sale(scrapy.Spider):
    name = "DaviesGladman_Sale"

    url = "https://daviesgladman.com.au/for-sale"
    prefix = "https://daviesgladman.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(DaviesGladman_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".sqs-block-button-element--medium")
        links = []
        for each_url in all_links:
            if 'sold' in each_url.css("::text").get('').strip().lower():
                pass
            else:
                each_url = each_url.css("::attr(href)").get('').strip()
                each_url = f'https://daviesgladman.com.au{each_url}'
        ...
        for property_url in all_links:
            if 'sold' in property_url.css("::text").get('').strip().lower():
                pass
            else:
                property_url = property_url.css("::attr(href)").get('').strip()
                property_url = f'https://daviesgladman.com.au{property_url}'
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2984)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Davies Gladman Properties'

            property_street = response.css('.sqs-html-content h2 ::text').get('').strip()  # property_street
            ...
            data_src = response.css(".sqs-image-shape-container-element img ::attr(data-src)").get('').strip()
            parts = data_src.split('/')[-1].split('.')[0]  # Get the last segment before ".png"
            values = parts.split('-')
            item['Field6'] = values[0] if len(values) > 1 else ''
            item['Field7'] = values[1] if len(values) > 1 else ''
            item['Field8'] = values[2].replace('+%281%29','') if len(values) > 2 else ''

            price = response.css('.xxxxxxxxxxx ::text').get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price
            ...
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".sqs-block-content .sqs-html-content ::text").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.css(".Index-gallery-item-image img ::attr(data-src)").getall()
            images = []
