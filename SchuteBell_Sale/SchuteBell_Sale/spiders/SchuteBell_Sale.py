...
class SchuteBell_Sale(scrapy.Spider):
    name = 'SchuteBell_Sale'
    prefix = 'https://www.schutebell.com.au'
    url = "https://www.schutebell.com.au/properties/residential/"
    headers = {
        'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        'cache-control': 'max-age=0',
        # 'cookie': 'PHPSESSID=mmrbdamrebgjbtlbl7knf85qs9; __utmz=228449695.1712910629.1.1.utmcsr=(direct)|utmccn=(direct)|utmcmd=(none); __utmc=228449695; PHPSESSID=mmrbdamrebgjbtlbl7knf85qs9; __utma=228449695.1765696602.1712910629.1713129070.1713131916.5; __utmt=1; __utmb=228449695.2.10.1713131916; arp_scroll_position=213; PHPSESSID=mmrbdamrebgjbtlbl7knf85qs9',
    ...
    def parse(self, response):
        for property in response.css(".description a"):
        ...
        if 'page' in response.css(".next_page_link ::attr(href)").get('').strip():
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'Schute Bell Badgery Lumby'

            property_add = response.css('.property_headline ::text, .suburb .value ::text').getall()  # full_adddress
            ...
            item['Field7'] = response.css(".bathrooms .value ::text").get('').strip()
            item['Field8'] = response.css(".carspaces .value ::text").get('').strip()

            # sold_price = response.css('.price .value ::text').get('').strip().replace('SOLD','')
            # if sold_price:
            #     item['Field9'] = (sold_price.strip()).replace('$','')   # sold_price

            description = response.css('#realty_widget_property_description ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess = response.css("#slide_photo ::attr(src) , #scroll-content a ::attr(href)").getall()
            new_img_urls = []
            ...
            if agent1:
                ...
                agent1_phone = agent1.css(".agent_phone span ::text").get('').strip()
                item['Field20'] = agent1_phone.strip()
                '''         Uploading Images on Wasabi S3 Bucket            '''
                Agent_Images = agent1.css(".agent_photo img ::attr(src)").get('').strip()
            ...
            if len(response.css("#agent_").getall()) > 1:
                agent2 = response.css("#agent_")[1]
                agent2_name = agent2.css('h4 a ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    agent2_phone = agent2.css(".agent_phone span ::text").get('').strip()
                    item['Field26'] = agent2_phone.strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images = agent2.css(".agent_photo img ::attr(src)").get('').strip()
            ...
            if len(response.css("#agent_").getall()) == 3:
                agent3 = response.css("#agent_")[2]
                ...
                if agent3:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images = agent3.css(".agent_photo img ::attr(src)").get('').strip()
            ...
            land_area = response.css(".side_land_size .value span ::text").get('').strip()
            if land_area:
