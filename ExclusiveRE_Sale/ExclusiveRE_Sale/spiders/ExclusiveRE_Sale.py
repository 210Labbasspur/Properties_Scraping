...
class ExclusiveRE_Sale(scrapy.Spider):
    name = 'ExclusiveRE_Sale'
    prefix = 'https://www.exclusivere.com.au'
    url = "https://www.exclusivere.com.au/buy"
    headers = {
        'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        'cache-control': 'max-age=0',
        # 'cookie': '_ga=GA1.1.392366256.1711712208; AMP_MKTG_16a5c84b5b=JTdCJTdE; arp_scroll_position=1100; _eagle_session=--45db9a0280f86449511593584d37aa9cf16b84da; AMP_16a5c84b5b=JTdCJTIyZGV2aWNlSWQlMjIlM0ElMjI0ZTVmMWEzYi1kZjgwLTRjMWQtYWNiZS1hZTAyMmQ4ZmMxZGUlMjIlMkMlMjJzZXNzaW9uSWQlMjIlM0ExNzExNzEyMjEyMDQwJTJDJTIyb3B0T3V0JTIyJTNBZmFsc2UlMkMlMjJsYXN0RXZlbnRUaW1lJTIyJTNBMTcxMTcxMjMxMjQ1OCU3RA==; _ga_73JQ6M410Y=GS1.1.1711712208.1.1.1711712320.0.0.0; _eagle_session=--83d0ab0438c26680f96b0f165323c76b82910d1a',
        # 'referer': 'https://www.exclusivere.com.au/recently-sold',
    ...
    def start_requests(self):
        ...
        yield scrapy.Request(url='https://www.mintpropertyagents.com.au/commercial', headers=self.headers, callback=self.parse)

    def parse(self, response):
        for property in response.css("h4 a"):#[:2]:
        ...
        if response.xpath("//a[contains(text(),'Next')]"):
            next_page_url = response.xpath("//a[contains(text(),'Next')]//@href").get('').strip()
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            property_add = response.css('#titlebar h2 ::text').getall()  # full_adddress
            ...
            item['Field6'] = response.xpath("//span[contains(text(),' Beds')]/text()").get('').strip().replace(' Beds ','')
            item['Field7'] = response.xpath("//span[contains(text(),' Baths')]/text()").get('').strip().replace(' Baths','')
            item['Field8'] = response.xpath("//span[contains(text(),' Car Spaces')]/text()").get('').strip().replace(' Car Spaces','')
            ...
            description = response.css('.show-more p ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess = response.css('.fullwidth-property-slider-nav img::attr(src)').getall()
            new_img_urls = []
            ...
            agent1 = response.css(".agent-title")
            ...
            if agent1:
                ...
                '''         Uploading Images on Wasabi S3 Bucket            '''
                Agent_Imagess = agent1.css(".agent-photo img ::attr(src)").getall()
                Agent_Imagesss = []
                for url in Agent_Imagess:
            ...
            if len(response.css(".agent-title").getall()) > 1:
                agent2 = response.css(".agent-title")[1]
                ...
                if agent2:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Imagess = agent2.css(".agent-photo img ::attr(src)").getall()
                    Agent_Imagesss = []
                    for url in Agent_Imagess:
                        Agent_Imagesss.append(url)
                    Agent_Images = ','.join(Agent_Imagesss[:1])
            ...
            land_area = response.xpath("//span[contains(text(),'Square metres')]/text()").get('').strip()
            if land_area:
