...
class Dalton_Rent(scrapy.Spider):
    name = 'Dalton_Rent'
    prefix = 'https://www.daltonpartners.net.au'
    url = 'https://www.daltonpartners.com.au/wp-admin/admin-ajax.php'
    headers = {
        'accept': '*/*',
        'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        'content-type': 'application/x-www-form-urlencoded; charset=UTF-8',
        # 'cookie': 'arp_scroll_position=2083',
        'origin': 'https://www.daltonpartners.com.au',
        'priority': 'u=1, i',
        # 'referer': 'https://www.daltonpartners.com.au/sell/sold-residential/',
    ...
    data = {
        ...
        'atts[type]': 'ResidentialLease,lease',
        'atts[property_type]': 'Apartment,Block of Units,Development,House,Industrial,Land,Offices,Semi Detached,Studio,Terrace,Townhouse,Unit,Villa',
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            Agent_Imagess = response.xpath("//*[contains(@class,'embed-responsive embed-responsive-16by9')]/div/picture/source"
                                           "/img[contains(@class,'img-default-used-size-hd')]/@src").getall()
            new_img_urls = []
            ...
            if response.css(".listing-agent"):
                ...
                if agent1:
                    ...
                    item['Field18'] = agent1.xpath(".//*[contains(@class,'email mb-0')]/text()").get('').strip().replace('E:','')  # description
                    agent1_phone = agent1.xpath(".//*[contains(@class,'mobile mb-0')]/text()").get('').strip().replace('M:','')  # description
                    item['Field20'] = agent1_phone.strip()  # description
                    agent1_mobile = agent1.xpath(".//*[contains(@class,'phone mb-0')]/text()").get('').strip().replace('P:','')  # description
                ...
                if len(response.css(".listing-agent").getall()) > 1:
                    ...
                    if agent2:
                        ...
                        item['Field25'] = agent2.xpath(".//*[contains(@class,'email mb-0')]/text()").get('').strip().replace('E:', '')  # description
                        agent2_phone = agent2.xpath(".//*[contains(@class,'mobile mb-0')]/text()").get('').strip().replace('M:', '')  # description
                        item['Field26'] = agent2_phone.strip()  # description
                        agent2_mobile = agent2.xpath(".//*[contains(@class,'phone mb-0')]/text()").get('').strip().replace('P:', '')  # description
                ...
                if len(response.css(".listing-agent").getall()) > 2:
                    ...
                    if agent3:
                        ...
                        item['Field30'] = agent3.xpath(".//*[contains(@class,'email mb-0')]/text()").get('').strip().replace('E:', '')  # description
                        agent3_phone = agent3.xpath(".//*[contains(@class,'mobile mb-0')]/text()").get('').strip().replace('M:', '')  # description
                        item['Field31'] = agent3_phone.strip()  # description
                        agent3_mobile = agent3.xpath(".//*[contains(@class,'phone mb-0')]/text()").get('').strip().replace('P:', '')  # description
