...
class TRGW_Sold(scrapy.Spider):
    name = "TRGW_Sold"

    url = "https://trgw.com.au/listings?saleOrRental=Sale&minprice=0&rental_period=week&status=sold&viewType=gallery&sortby=saleDate&page=1"
    prefix = "https://trgw.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(TRGW_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".v2-items-start a ::attr(href)").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://trgw.com.au{each_url}'
        ...
        for property_url in all_links:
            property_url = f'https://trgw.com.au{property_url}'
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(1897)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'The Realty Group'

            property_street = response.xpath("//*[contains(@data-testid,'address')]/text()").get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.xxxxxxxxxx ::text').getall() if e.strip())  # property_suburb
            property_address = f"{property_street} {property_suburb}"
            item['Field5'] = property_address  # full_adddress

            item['Field6'] = response.xpath("//*[contains(text(),' Bed')]/text()").get('').strip().replace('Bed','').replace('s','')
            item['Field7'] = response.xpath("//*[contains(text(),' Bath')]/text()").get('').strip().replace('Bath','').replace('s','')
            item['Field8'] = response.xpath("//*[contains(text(),' Car')]/text()").get('').strip().replace('Car','').replace('s','')

            price = response.xpath("//*[contains(@data-testid,'listing-price')]/text()").get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price
            ...
            prop_images = response.css("img.v2-hidden ::attr(src)").getall()
            images = []
            ...
            '''          AGENTS          '''
            item['Field15'] = 'The Realty Group'  # agent_first_name_1
            item['Field16'] = 'Group'  # agent_first_name_1
            item['Field17'] = 'The Realty Group'  # agent_first_name_1
            item['Field19'] = 'mark@trgw.com.au'  # agent_first_name_1
            item['Field20'] = '02 4677 7177'  # agent_first_name_1
