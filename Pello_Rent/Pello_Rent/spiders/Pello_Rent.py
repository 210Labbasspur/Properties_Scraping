...
class Pello_Rent(scrapy.Spider):
    name = 'Pello_Rent'
    prefix = 'https://pello.com.au'
    url = "https://pello.com.au/properties-for-lease"
    headers = {
    ...
    def parse(self, response):
        ...
        all_details_links = []
        for url in response.css("a.propertyTile-anchor"):
            url = url.css('::attr(href)').get('').strip()
        ...
        '''  #######   Original Code   #######  '''
        for property in response.css("a.propertyTile-anchor"):
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            property_add = response.xpath("//*[contains(@itemprop,'addressLocality')]/span/text()").getall()  # full_adddress
            ...
            bbc = response.css('.row .icons ::text').getall()
            print('BBC is : ', bbc)
            bbc_code = response.css('.row .icons')
            for check in bbc:
                if 'Bed' in check.strip():
                    item['Field6'] = check.strip().replace('Bed','')
                if 'Bath' in check.strip():
                    item['Field7'] = check.strip().replace('Bath','')
                if 'Car' in check.strip():
                    input_string = check.strip().replace('Car','')
                    numbers = ''.join([char for char in input_string if char.isdigit()])
                    item['Field8'] = numbers

            sold_price = response.css('.text-bold+ div:nth-child(2) ::text').get('').strip().replace('SOLD OFF-MARKET','').replace(' ','')
            if re.search(r'\d', sold_price):
                item['Field9'] = (sold_price.strip()).replace('SOLD','').replace('$','')   # sold_price

            description = response.css('.show-more-content p ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess = response.xpath("//*[contains(@itemprop,'url')]/@href").getall()
            new_img_urls = []
            ...
            '''          AGENTS          '''
            if response.css(".desktop-info .agentTile"):
                agent1 = response.css(".desktop-info .agentTile")[0]
                agent1_name = agent1.css('.agentTile-name ::text').get('').strip()
                if agent1_name:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field18'] = agent1.css(".agentTile-title ::text").get('').strip()  # agent_full_name_1
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images = self.prefix + agent1.css(".agentTile-image ::attr(data-bg-src)").get('').strip()
                ...
                if len(response.css(".desktop-info .agentTile").getall()) > 1:
                    agent2 = response.css(".desktop-info .agentTile")[1]
                    agent2_name = agent2.css('.agentTile-name ::text').get('').strip()
                    if agent2:
                        item['Field23'] = agent2_name.strip()
                        item['Field24'] = agent2.css(".agentTile-title ::text").get('').strip()  # agent_full_name_1
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = self.prefix + agent2.css(".agentTile-image ::attr(data-bg-src)").get('').strip()
                ...
                if len(response.css(".desktop-info .agentTile").getall()) > 2:
                    agent3 = response.css(".desktop-info .agentTile")[2]
                    agent3_name = agent3.css('.agentTile-name ::text').get('').strip()
                    if agent3:
                        item['Field28'] = agent3_name.strip()
                        item['Field29'] = agent3.css(".agentTile-title ::text").get('').strip()  # agent_full_name_1
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = self.prefix + agent3.css(".agentTile-image ::attr(data-bg-src)").get('').strip()
            ...
            land_area = response.xpath("//*[contains(text(),'Land area approx')]/following-sibling::div[1]/text()").get('').strip()
            if land_area:
                item['Field36'] = land_area.replace('sqm','').replace(' ','')   # land_area

            feature_count = 58
            for feature in response.css(".float-left"):
