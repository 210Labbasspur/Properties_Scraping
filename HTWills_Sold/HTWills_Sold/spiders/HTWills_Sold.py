...
class HTWills_Sold(scrapy.Spider):
    name = 'HTWills_Sold'
    prefix = 'https://www.htwills.com.au/'
    url = "https://www.htwills.com.au/listings/ressale/p/1&status=sold"
    headers = {
        'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        # 'cookie': 'PHPSESSID=df33f1647872081509055becf9af923b; _gcl_au=1.1.1113624786.1714476081; _gid=GA1.3.1840237643.1714600934; _gat=1; _gat_secondTracker=1; _gat_UA-212127156-1=1; _ga_THQS6QP53G=GS1.1.1714600935.3.0.1714600935.60.0.0; _ga=GA1.1.749130938.1714476080; _ga_N72C9D992H=GS1.3.1714600935.2.0.1714600935.0.0.0; arp_scroll_position=2152',
        'priority': 'u=0, i',
        # 'referer': 'https://www.htwills.com.au/listings/sold',
    ...
    def parse(self, response):
        for property in response.css(".desc a"):
        ...
        next_page = response.xpath("//*[contains(@class,'active')]/parent::li[1]/following-sibling::li[1]/a/@href").get('').strip()
        if next_page:
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'HTWills Real Estate St George'
            ...
            item['Field6'] = response.xpath("//*[contains(@alt,'bed')]/following-sibling::span[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@alt,'bath')]/following-sibling::span[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@alt,'car')]/following-sibling::span[1]/text()").get('').strip()
            ...
            description = response.css('#property-content p ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess = response.css("#property-gallery a ::attr(href)").getall()
            new_img_urls = []
            ...
            '''          AGENTS          '''
            if response.css("section#side-agent-profile"):
                agent1 = response.css("section#side-agent-profile")[0]
                ...
                if agent1:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images = self.prefix + agent1.css(".agent-photo img ::attr(src)").get('').strip()
                ...
                if len(response.css("section#side-agent-profile").getall()) > 1:
                    agent2 = response.css("section#side-agent-profile")[1]
                    agent2_name = agent2.css('h3 ::text').get('').strip()
                    if agent2:
                        item['Field23'] = agent2_name.strip()
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = self.prefix + agent2.css(".agent-photo img ::attr(src)").get('').strip()
                ...
                if len(response.css("section#side-agent-profile").getall()) > 2:
                    agent3 = response.css("section#side-agent-profile")[2]
                    agent3_name = agent3.css('h3 ::text').get('').strip()
                    if agent3:
                        item['Field28'] = agent3_name.strip()
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = self.prefix + agent3.css(".agent-photo img ::attr(src)").get('').strip()
