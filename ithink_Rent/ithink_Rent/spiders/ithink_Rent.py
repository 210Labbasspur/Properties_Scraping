...
class ithink_Rent(scrapy.Spider):
    name = "ithink_Rent"

    url = "https://www.ithinkproperty.com.au/rent/houses-for-rent/"
    prefix = "https://www.ithinkproperty.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(ithink_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        # all_links = response.css("a.rec-cover-link ::attr(href)").getall()
        all_links = response.xpath("//*[contains(@class,'rec-cover-link')]/@href").getall()
        links = []
        ...
        next_page = response.xpath("//*[contains(@class,'nextpostslink')]/@href").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3328)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'ithink Property'
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'bedroom')]/span[@class='rec-icon-value']/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'bathroom')]/span[@class='rec-icon-value']/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'parking')]/span[@class='rec-icon-value']/text()").get('').strip()
            ...
            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('h3.entry-title ::text').getall())  # description
            ...
            prop_images = response.xpath("//*[contains(@class,'frame slick-slide')]/img/@data-lazy").getall()
            images = []
