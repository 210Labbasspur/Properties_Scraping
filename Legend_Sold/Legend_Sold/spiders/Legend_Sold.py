...
class Legend_Sold(scrapy.Spider):
    name = 'Legend_Sold'
    prefix = 'https://www.legendrealestate.com.au'
    url = "https://www.legendrealestate.com.au/recently-sold"
    headers = {
        'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        'cache-control': 'max-age=0',
        # 'cookie': '_gid=GA1.3.1163080908.1714475725; _ga_KSDYRLENQD=GS1.1.1714516496.3.0.1714516496.0.0.0; _ga=GA1.1.1948430999.1714475724; arp_scroll_position=0; _eagle_session=--1b52d80efa9db0ef5595c97fa576bf1bd90a13e3; _eagle_session=--de99f4e12de513e8a2d9ccb5470c5e3133214316',
        'priority': 'u=0, i',
    ...
    def parse(self, response):
        for property in response.css(".overlay"):
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'Legend Committed Realty'

            property_add = response.css('h2.margin0  ::text').getall()  # full_adddress
            ...
            item['Field6'] = response.xpath("//*[contains(text(),'Bedroom')]/following-sibling::span[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(text(),'Bathroom')]/following-sibling::span[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(text(),'Car Space')]/following-sibling::span[1]/text()").get('').strip()

            sold_price = response.css('.italic ::text').get('').strip().replace(' ','')
            if re.search(r'\d', sold_price):
                item['Field9'] = (sold_price.strip()).replace('$','')   # sold_price

            description = response.css('.clearfix.padding30 p ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess = response.css(".cycle-slideshow img ::attr(src)").getall()
            new_img_urls = []
            ...
            '''          AGENTS          '''
            if response.xpath("//h3[contains(text(),'Contact Agent')]/following-sibling::div[1]"):
                agent1 = response.xpath("//h3[contains(text(),'Contact Agent')]/following-sibling::div[1]")
                agent1_name = agent1.xpath(".//li/a[contains(@href,'agent-profile')]/text()").get('').strip()
                if agent1:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images = agent1.css(".photo ::attr(src)").get('').strip()
                ...
                if response.xpath("//h3[contains(text(),'Contact Agent')]/following-sibling::div[3]"):
                    agent2 = response.xpath("//h3[contains(text(),'Contact Agent')]/following-sibling::div[3]")
                    agent2_name = agent2.xpath(".//li/a[contains(@href,'agent-profile')]/text()").get('').strip()
                    if agent2:
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent2.css(".photo ::attr(src)").get('').strip()
                ...
                # if response.xpath("//h3[contains(text(),'Contact Agent')]/following-sibling::div[5]"):
                #     agent3 = response.xpath("//h3[contains(text(),'Contact Agent')]/following-sibling::div[5]")
                #     agent3_name = agent3.xpath(".//li/a[contains(@href,'agent-profile')]/text()").get('').strip()
                #     if agent3:
                ...
                #         '''         Uploading Images on Wasabi S3 Bucket            '''
                #         Agent_Images = agent3.css(".photo ::attr(src)").get('').strip()
            ...
            item['Field35'] = response.xpath("//*[contains(text(),'Property')]/following-sibling::span[1]/text()").get('').strip()
            land_area = response.xpath("//*[contains(text(),'Land size')]/following-sibling::span[1]/text()").get('').strip()
            if land_area:
                item['Field36'] = land_area.replace('sq metres','').replace(' ','')   # land_area
            elif response.xpath("//*[contains(text(),'Building Size')]/following-sibling::div[1]/text()").get('').strip():
                item['Field36'] = land_area.replace('sq metres','').replace(' ','')   # land_area

            feature_count = 58
            for feature in response.css(".padding30 .info-label span"):
