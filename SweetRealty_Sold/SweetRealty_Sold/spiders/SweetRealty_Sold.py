...
class SweetRealty_Sold(scrapy.Spider):
    name = "SweetRealty_Sold"

    url = "http://www.sweetrealty.com.au/sold-properties"
    prefix = "http://www.sweetrealty.com.au"
    headers = {
    ...
    count = 1
    db = 'SweetRealty'
    collection = 'SweetRealty_Sold'
    bucket_prefix = f'SR_{collection}'

    local_file_path = '/img'

    def __init__(self, *args, **kwargs):
        super(SweetRealty_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".thumbnail-img a::attr(href)").getall()
        links = []
        for each_url in all_links:
            each_url = f'http:{each_url}'
        ...
        next_page = response.css(".next a ::attr(href)").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2943)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Sweet Realty'
            property_street = response.xpath("//*[contains(@class,'bbc col-xs-12')]/preceding-sibling::div[1]/h2/text()").get('').strip()  # full_adddress
            property_suburb = ' '.join(e.strip() for e in response.css('.xxxxxxxxxxx ::text').getall() if e.strip())  # full_adddress
            property_address = f"{property_street} {property_suburb}"
            item['Field5'] = property_address  # full_adddress

            item['Field6'] = response.xpath("//*[contains(@d,'M-205.3,182.4c6.5,0,11.')]/ancestor::span[1]/following-sibling::h4[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@d,'M-121.3,188.2c0,0,0-17.3')]/ancestor::span[1]/following-sibling::h4[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@d,'M-123.2,178.2l-9.8-25.5c0')]/ancestor::span[1]/following-sibling::h4[1]/text()").get('').strip()

            price = response.css('.price ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.container h1 ::text').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".description ::text").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.css(".thumbnail-img a ::attr(href)").getall()
            images = []
            for img in prop_images:
                img = img.replace('background-image:url(','').replace(");","")
                images.append(f'http:{img}' if img else "")
            ...
            '''          AGENTS          '''
            if response.css(".carousel-inner .item"):
                agent1 = response.css(".carousel-inner .item")[0]
                agent1_name = agent1.css('.detailAgentName ::text').get('').strip()
                if agent1_name:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field18'] = agent1.css('.xxxxxxx ::text').get('').strip()
                    item['Field19'] =  agent1.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:','')
                    item['Field20'] = agent1.xpath(".//*[contains(text(),'M:')]/following-sibling::span[1]/text()").get('').strip().replace('mailto:','')
                    item['Field21'] = agent1.xpath(".//*[contains(text(),'P:')]/following-sibling::span[1]/text()").get('').strip().replace('mailto:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".agentIMG ::attr(style)").get('').strip().replace('background-image:url(','').replace(');','')
                    images_string = f'http:{agent_image}' if agent_image else ""
                    # item['Field22'] = images_string
                    # print('Agent1 image is :',images_string)
                    random_number = random.randint(1, 10000000000)
                    item['Field22'] = ",".join(self.uploaded(images_string, random_number))

            if len(response.css(".carousel-inner .item").getall()) > 1:
                agent2 = response.css(".carousel-inner .item")[1]
                agent2_name = agent2.css('.detailAgentName ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.css('.xxxxxxx ::text').get('').strip()
                    item['Field25'] = agent2.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:', '')
                    item['Field26'] = agent2.xpath(".//*[contains(text(),'M:')]/following-sibling::span[1]/text()").get('').strip().replace('mailto:','')
                    item['Field26A'] = agent2.xpath(".//*[contains(text(),'P:')]/following-sibling::span[1]/text()").get('').strip().replace('mailto:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".agentIMG ::attr(style)").get('').strip().replace('background-image:url(','').replace(');','')
                    images_string = f'http:{agent_image}' if agent_image else ""
            ...
            if len(response.css(".carousel-inner .item").getall()) > 2:
                agent3 = response.css(".carousel-inner .item")[2]
                agent3_name = agent3.css('.detailAgentName ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field29'] = agent3.css('.xxxxxxx ::text').get('').strip()
                    item['Field30'] = agent3.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:', '')
                    item['Field31'] = agent3.xpath(".//*[contains(text(),'M:')]/following-sibling::span[1]/text()").get('').strip().replace('mailto:','')
                    item['Field31A'] = agent3.xpath(".//*[contains(text(),'P:')]/following-sibling::span[1]/text()").get('').strip().replace('mailto:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".agentIMG ::attr(style)").get('').strip().replace('background-image:url(','').replace(');','')
                    images_string = f'http:{agent_image}' if agent_image else ""
            ...
            # item['Field35'] = response.css(".property-category ::text").get('').strip()   # external_property_id

            # land_area = response.xpath('//*[contains(text(),"Land Size:")]/text()').get('').strip().replace('Land','').replace('size','').replace('m','')
            # if land_area:
            ...
            feature_count = 58
            for feature in response.xpath("//*[contains(@class,'col-xs-12 col-md-6')]"):   ##  first 4 features are ID, bed, bath etc.
                if 'condition' in ''.join(e.strip().lower() for e in feature.css('::text').getall() if e.strip()):
                    item['Field52'] = ''.join(e.strip() for e in feature.css('::text').getall() if e.strip())
                else:
                    item[f'Field{feature_count}'] = ''.join(e.strip() for e in feature.css('::text').getall() if e.strip())
                    feature_count += 1
