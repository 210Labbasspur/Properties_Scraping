...
class TheBeaches_Sold(scrapy.Spider):
    name = "TheBeaches_Sold"
    url = 'https://www.thebeachesrealestate.com.au/wp-admin/admin-ajax.php'
    prefix = "https://www.thebeachesrealestate.com.au"
    headers = {
    ...
    data = {
    'action': 'apt_ajax_recent_sales',
    'params[template]': 'Shortcode.RecentSales.Output',
    'params[row_col_class]': 'row row-cols-xl-3 row-cols-lg-2 row-cols-1',
    'params[per_page]': '15',
    'params[show_thumbnail]': '1',
    'params[show_excerpt]': '0',
    'params[show_date]': '0',
    'params[exclude_current_post]': '0',
    'params[excerpt_length]': '55',
    'params[category_name]': 'house',
    'params[tag]': '',
    'params[img_quality]': 'post-thumbnail',
    'params[excerpt_more]': '',
    'params[show_pagination]': '0',
    'params[get_year]': '',
    'params[get_month]': '',
    'params[get_month_year]': '',
    'params[get_suburb]': '',
    'params[get_minprice]': '',
    'params[get_maxprice]': '',
    'query[post_type]': 'aptcasestudy',
    'query[paged]': '1',
    'query[posts_per_page]': '15',
    'query[post_status]': 'publish',
    'query[ignore_sticky_posts]': 'true',
    'query[tag]': '',
    'query[meta_key]': 'group_inspected',
    'query[orderby]': 'meta_value_num',
    'query[order]': 'ASC',
    'query[tax_query][0][taxonomy]': 'aptcasestudy_category',
    'query[tax_query][0][field]': 'slug',
    'query[tax_query][0][terms]': 'house',
    'no_more_text': 'No more posts to load',
    'selector_post': '#ap-recent-sales-wrapper',
    'paged': '2',
}

    count = 1
    db = 'TheBeaches'
    collection = 'TheBeaches_Sold'
    ...
    def __init__(self, *args, **kwargs):
        super(TheBeaches_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        ...
        links = []
        for each_url in data['data']['query']:
            each_url = each_url.get('guid').strip().replace('&#038;','&')
        ...
        for property in data['data']['query']:
            property_url = property.get('guid').strip().replace('&#038;','&')
            print(self.count,' # Property URL :', property_url)
            self.count += 1
            yield response.follow(property_url, headers=self.headers, callback=self.detail_parse, meta={'property':property}, dont_filter=True)

        next_paged = data['data']['atts']['next_paged']
        page_no = response.meta['page_no']
        print(f"Current-Page = {page_no} || next_paged = {next_paged}")
        if page_no < next_paged:
            page_no += 1
            payload = deepcopy(self.data)
    ...
    def detail_parse(self, response):
        ...
        detail_page_url = property.get('guid').strip().replace('&#038;', '&')

        database_sale_matching_url = self.read_data_base()
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(1887)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'The Beaches Real Estate'

            property_street = response.css('.title-address ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.title-suburb ::text').getall() if e.strip())  # property_suburb
            property_address = f"{property_street} {property_suburb}"
            item['Field5'] = property_address  # full_adddress

            item['Field6'] = ''.join(e.strip() for e in response.xpath("//*[contains(@class,'bedroom')]/text()").getall())
            item['Field7'] = ''.join(e.strip() for e in response.xpath("//*[contains(@class,'bathroom')]/text()").getall())
            item['Field8'] = ''.join(e.strip() for e in response.xpath("//*[contains(@class,'carspace')]/text()").getall())

            price = response.css('.title1-as-sold-price h2 ::text').get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price
            ...
            item['Field14'] = property.get('guid').strip().replace('&#038;','&')
            '''          AGENTS          '''
            if response.css(".listing-agent"):
                ...
                if agent1_name:
                    ...
                    item['Field18'] = agent1.css('.xxxxxxxx ::text').get('').strip()
                    item['Field19'] = agent1.xpath(".//a[contains(@href,'mailto:')]/text()").get('').strip().replace('mailto:','')
            ...
            if len(response.css(".listing-agent").getall()) > 1:
                ...
                if agent2:
                    ...
                    item['Field24'] = agent2.css('.xxxxxxxx ::text').get('').strip()
                    item['Field25'] = agent2.xpath(".//a[contains(@href,'mailto:')]/text()").get('').strip().replace('mailto:','')
            ...
            if len(response.css(".listing-agent").getall()) > 2:
                ...
                if agent3:
                    ...
                    item['Field29'] = agent3.css('.xxxxxxxx ::text').get('').strip()
                    item['Field30'] = agent3.xpath(".//a[contains(@href,'mailto:')]/text()").get('').strip().replace('mailto:','')
            ...
            item['Field104'] = property.get('guid').strip().replace('&#038;','&')
            print(item)
