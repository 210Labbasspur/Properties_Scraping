...
class Guildea_Sold(scrapy.Spider):
    name = 'Guildea_Sold'
    prefix = 'https://guildea.com.au'
    url = "https://guildea.com.au/sold-properties"
    headers = {
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            sold_price = response.xpath("//h4[contains(text(),'Sold')]/following-sibling::div[1]/text()").get('').strip()
            if re.search(r'\d', sold_price):
                item['Field9'] = (sold_price.strip()).replace('SOLD','').replace('$','')   # sold_price

            description = response.css('#buttonbar+ .contentRegion p ::text').getall()  # description
            ...
            if response.css(".agentTile"):
                ...
                if len(response.css(".agentTile").getall()) > 1:
                    ...
                    if agent2:
                        ...
                        item['Field25'] = agent1.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:', '')
