...
class Strud_Sale(scrapy.Spider):
    name = "Strud_Sale"

    url = "https://strudproperty.com.au/for-sale/{}/"
    prefix = "https://pacepa.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Strud_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def start_requests(self):
        yield scrapy.Request(self.url.format(1), headers=self.headers, callback=self.parse)

    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.xpath("//*[contains(@class,'eael-wrapper-link-f289a57 --eael-wrapper-link-tag')]/@href").getall()
        links = []
        ...
        next_page = response.css(".e-load-more-anchor ::attr(data-next-page)").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3341)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Strud Property'

            property_street = response.css('.elementor-heading-title ::text').get('').strip()  # property_street
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'fa-bed')]/parent::*[1]/following-sibling::*[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'fa-bath')]/parent::*[1]/following-sibling::*[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'fa-car')]/parent::*[1]/following-sibling::*[1]/text()").get('').strip()

            price = response.css('.elementor-element-6798306 .elementor-icon-list-text ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            ...
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".elementor-element-87c5975 ::text").getall())  # description
            ...
            '''          AGENTS          '''
            if response.css(".elementor-element-4c334dd .elementor-widget-icon-list"):
                agent1 = response.css(".elementor-element-4c334dd .elementor-widget-icon-list")[0]
                agent_phone = agent1.xpath(".//a[contains(@href,'tel:')]/@href").get('').strip()
                agent1_name = response.xpath(f"//*[contains(@href,'{agent_phone}')]/ancestor::*[contains(@class,'elementor-widget-icon-list')]/preceding-sibling::*[1]//h3/text()").get('').strip()
                if agent1_name:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field19'] = agent1.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:','').replace('%20','')
                    item['Field20'] = agent1.xpath(".//a[contains(@href,'tel:')]/@href").get('').strip().replace('tel:','').replace('%20','')
                    '''         Uploading Images on Wasabi S3 Bucket            --  No Images on website       '''

            if len(response.css(".elementor-element-4c334dd .elementor-widget-icon-list").getall()) > 1:
                agent2 = response.css(".elementor-element-4c334dd .elementor-widget-icon-list")[1]
                agent_phone = agent2.xpath(".//a[contains(@href,'tel:')]/@href").get('').strip()
                agent2_name = response.xpath(f"//*[contains(@href,'{agent_phone}')]/ancestor::*[contains(@class,'elementor-widget-icon-list')]/preceding-sibling::*[1]//h3/text()").get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field25'] = agent2.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:', '').replace('%20','')
                    item['Field26'] = agent2.xpath(".//a[contains(@href,'tel:')]/@href").get('').strip().replace('tel:','').replace('%20','')
                    '''         Uploading Images on Wasabi S3 Bucket            --  No Images on website       '''

            if len(response.css(".elementor-element-4c334dd .elementor-widget-icon-list").getall()) > 4:
                agent3 = response.css(".elementor-element-4c334dd .elementor-widget-icon-list")[2]
                agent_phone = agent3.xpath(".//a[contains(@href,'tel:')]/@href").get('').strip()
                agent3_name = response.xpath(f"//*[contains(@href,'{agent_phone}')]/ancestor::*[contains(@class,'elementor-widget-icon-list')]/preceding-sibling::*[1]//h3/text()").get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field30'] = agent3.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:', '').replace('%20','')
                    item['Field31'] = agent3.xpath(".//a[contains(@href,'tel:')]/@href").get('').strip().replace('tel:','').replace('%20','')
                    '''         Uploading Images on Wasabi S3 Bucket            --  No Images on website       '''
            ...
            land_area = response.css('.elementor-element-8099c90 .elementor-icon-list-text ::text').get('').strip().replace('Land','').replace('is','').replace('m²','')
            if land_area:
