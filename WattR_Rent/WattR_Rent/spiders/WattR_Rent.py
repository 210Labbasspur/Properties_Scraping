...
class WattR_Rent(scrapy.Spider):
    name = "WattR_Rent"

    url = "https://www.wattrealty.com.au/property/?buyrent=Rental&include-suburbs=1"
    prefix = "https://www.wattrealty.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(WattR_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".property-met a ::attr(href)").getall()
        links = []
        ...
        next_page = response.css(".arrow-next ::attr(href)").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3257)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Watt Realty'

            property_street = response.css('.row h4 ::text').get('').strip()  # property_street
            ...
            item['Field6'] = response.xpath("//*[@class='brief-propertydetails']//*[contains(text(),'bed')]/following-sibling::span[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[@class='brief-propertydetails']//*[contains(text(),'bath')]/following-sibling::span[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[@class='brief-propertydetails']//*[contains(text(),'car')]/following-sibling::span[1]/text()").get('').strip()

            price = response.css('.brief-propertydetails-info ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price
            ...
            '''          AGENTS          '''
            if response.css(".brief-agent-container"):
                agent1 = response.css(".brief-agent-container")[0]
                agent1_name = agent1.css('.brief-agentdetails-name ::text').get('').strip()
                if agent1_name:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field18'] = agent1.css('.brief-agentdetails-title ::text').get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".brief-agentimage ::attr(style)").get('').strip().replace("background-image: url( '","").replace("' );","")
            ...
            if len(response.css(".brief-agent-container").getall()) > 1:
                agent2 = response.css(".brief-agent-container")[1]
                agent2_name = agent2.css('.brief-agentdetails-name ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.css('.brief-agentdetails-title ::text').get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".brief-agentimage ::attr(style)").get('').strip().replace("background-image: url( '","").replace("' );","")
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css(".brief-agent-container").getall()) > 2:
                agent3 = response.css(".brief-agent-container")[2]
                agent3_name = agent3.css('.brief-agentdetails-name ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field29'] = agent3.css('.brief-agentdetails-title ::text').get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".brief-agentimage ::attr(style)").get('').strip().replace("background-image: url( '","").replace("' );","")
                    images_string = f'{agent_image}' if agent_image else ""
