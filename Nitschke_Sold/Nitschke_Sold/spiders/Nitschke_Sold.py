...
class Nitschke_Sold(scrapy.Spider):
    name = "Nitschke_Sold"

    url = "https://www.nitschke.com.au/properties-for-sale/?action=epl_search&post_type=property&property_status=sold&sortby=sold_latest/"
    prefix = "https://www.nitschke.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Nitschke_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".active a ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2456)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Nitschke'

            property_street = ''.join(e.strip() for e in response.css('.sub-heading ::text').getall())  # property_street
            property_suburb = response.css('.property-suburb ::text').get('').strip()  # property_street
            property_address = f"{property_street} {property_suburb}"
            item['Field5'] = property_address  # full_adddress

            item['Field6'] = response.xpath("//*[contains(@class,'icon-bed')]/following-sibling::div[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'icon-bath')]/following-sibling::div[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'icon-car')]/following-sibling::div[1]/text()").get('').strip()

            price = response.css('.page-price ::text').get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.property-description-title ::text').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".module ::text").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.css("#main-carousel .image ::attr(src)").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.css(".agent-card"):
                agent1 = response.css(".agent-card")[0]
                agent1_name = agent1.css('.agent-title a ::text').get('').strip()
                if agent1_name:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field18'] = agent1.css('.agent-position p ::text').get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css("img.agent-img ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.css(".agent-card").getall()) > 1:
                agent2 = response.css(".agent-card")[1]
                agent2_name = agent2.css('.agent-title a ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.css('.agent-position p ::text').get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css("img.agent-img ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css(".agent-card").getall()) > 2:
                agent3 = response.css(".agent-card")[2]
                agent3_name = agent3.css('.agent-title a ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field29'] = agent3.css('.agent-position p ::text').get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css("img.agent-img ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            land_area = response.xpath("//*[contains(@class,'icon-land')]/following-sibling::div[1]/text()").get('').strip()
            if land_area:
                if 'Acres' in land_area:
                    pass
                else:
                    item['Field36'] = land_area.replace('m','').replace(' ','')   # land_area
            floor_area = response.xpath("//*[contains(@class,'building-size')]/text()").get('').strip().replace('m²','')
            if floor_area:
                if 'Acres' in floor_area:
                    pass
                else:
                    item['Field37'] = floor_area.replace('Floor Area is ','').replace('s','').replace(' ','')   # land_area
