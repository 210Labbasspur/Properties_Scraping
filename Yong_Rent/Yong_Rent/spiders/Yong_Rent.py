...
class Yong_Rent(scrapy.Spider):
    name = 'Yong_Rent'
    prefix = 'https://www.yong.com.au'
    url = "https://www.yong.com.au/rent/rental-search"
    headers = {
        'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'Accept-Language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        'Cache-Control': 'max-age=0',
        'Connection': 'keep-alive',
        # 'Cookie': 'PHPSESSID=989ct2sbpfaieh960gi6lf4tm6; visited=yes; arp_scroll_position=900',
        # 'Referer': 'https://www.yong.com.au/buy/business',
    ...
    def parse(self, response):
        for property in response.css("#masonryGrid .listingSelector"):#[:2]:
        ...
        if response.xpath("//li[contains(@class,'next')]"):
            next_page_url = 'https:' + response.xpath("//li[contains(@class,'next')]/a/@href").get('').strip()
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            property_add = response.css('#propertyHeaderWrapper:nth-child(1) .row .pull-left ::text').getall()  # full_adddress
            ...
            beds = response.xpath("//*[contains(@class,'fa fa-bed')]/parent::li[1]/text()").getall()
            if beds:
                item['Field6'] = ' '.join(element.strip() for element in beds)
            baths = response.xpath("//*[contains(@class,'fa fa-shower')]/parent::li[1]/text()").getall()
            if baths:
                item['Field7'] = ' '.join(element.strip() for element in baths)
            cars = response.xpath("//*[contains(@class,'fa fa-car')]/parent::li[1]/text()").getall()
            if cars:
                item['Field8'] = ' '.join(element.strip() for element in cars)
            ...
            description = response.css('#propertyDescriptionWrapper p ::text, #propertyDescriptionWrapper h3 ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\uf06c', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess = response.css('.img-responsive ::attr(src)').getall()
            new_img_urls = []
            ...
            agent1 = response.xpath("//*[contains(@class,'col-xs-12 pull-left agentWrapper margin-bottom-30')]")[0]
            agent1_name =  ' '.join(element.strip() for element in agent1.css('.topSection:nth-child(1) h4 ::text').getall())  # description
            if agent1:
                first_name1, last_name1 = agent1_name.split(maxsplit=1)
                item['Field15'] = first_name1.strip()  # agent_first_name_1
                item['Field16'] = last_name1.strip()  # agent_surname_name_1
                item['Field17'] = agent1_name.strip()  # agent_full_name_1
                item['Field18'] = agent1.css('.topSection h5 ::text').get('').strip()
                ...
                '''         Uploading Images on Wasabi S3 Bucket            '''
                Agent_Imagess = agent1.css(".imgWrapper ::attr(style)").getall()
                pattern = r"url\((.*?)\);"
                matches = re.findall(pattern, Agent_Imagess[0])
                Agent_Images = None
                if matches:
                    Agent_Images = matches[0]
            ...
            if len(response.xpath("//*[contains(@class,'col-xs-12 pull-left agentWrapper margin-bottom-30')]").getall()) > 1:
                agent2 = response.xpath("//*[contains(@class,'col-xs-12 pull-left agentWrapper margin-bottom-30')]")[1]
                agent2_name = ' '.join(element.strip() for element in agent2.css('.topSection:nth-child(1) h4 ::text').getall())  # description
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.css('.topSection h5 ::text').get('').strip()
                    agent2_phone = agent2.xpath(".//a[contains(@href,'tel:')]/text()").getall()
                    item['Field26'] = agent2_phone[0].strip()
                    if len(agent2_phone) > 1:
                        item['Field26A'] = agent2_phone[1].strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Imagess = agent2.css(".imgWrapper ::attr(style)").getall()
                    pattern = r"url\((.*?)\);"
                    matches = re.findall(pattern, Agent_Imagess[0])
                    Agent_Images = None
                    if matches:
            ...
            land_area = response.xpath("//td[contains(text(),'Land Size')]/following-sibling::td/text()").get('').strip()
            if land_area:
                item['Field36'] = land_area.replace('Square Mtr','').replace(' ','').replace('approx.','')   # land_area

            feature_count = 58
            for feature in response.css("td:nth-child(1)"):
                if 'Air Conditioning' in feature.css('::text').get('').strip():
                    item['Field52'] = feature.css("::text").get('').strip()
                elif feature.css('::text').get('').strip() == 'Land Size':
                    pass
                else:
