...
class MaisonBridge_Sale(scrapy.Spider):
    name = "MaisonBridge_Sale"

    url = "https://maisonbridge.com.au/wp-admin/admin-ajax.php?action=ere_property_paging_ajax&layout=property-grid&items_amount=9&columns=3&image_size=403x268&columns_gap=col-gap-30&view_all_link=&paged={}&property_type=&property_status=for-sale&property_feature=&property_city=&property_state=&property_neighborhood=&property_label=&property_featured=&author_id=&agent_id="
    prefix = "https://maisonbridge.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(MaisonBridge_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".property-link ::attr(href)").getall()
        links = []
        ...
        next_page = response.xpath("//*[contains(@class,'next page-numbers')]/text()").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(1745)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Maison Bridge'

            property_street = response.css('.property-location a span  ::text').get('').strip()  # property_street
            ...
            item['Field6'] = response.xpath("//*[contains(text(),'Bedrooms')]/preceding-sibling::p/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(text(),'Bathrooms')]/preceding-sibling::p/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(text(),'Garage')]/preceding-sibling::p/text()").get('').strip()

            price = response.css('.property-price ::text').get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','').replace('for','').replace('Just Listed)','')
            if price:
                item['Field9'] = price

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.property-heading h2 ::text').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".ere-property-element p ::text").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.xpath("//*[contains(@data-rel,'ere_light_gallery')]/@href").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.xpath("//*[contains(@class,'agent-info')][2]"):
                agent1 = response.xpath("//*[contains(@class,'agent-info')][2]")
                agent1_name = agent1.css('.agent-heading a ::text').get('').strip()
                if agent1_name:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field18'] = agent1.css('.agent-heading span ::text').get('').strip()
                    item['Field19'] = agent1.css(".agent-email span ::text").get('').strip().replace('mailto:','')
                    item['Field20'] = agent1.css(".agent-mobile span ::text").get('').strip().replace('mailto:','')
                    item['Field21'] = agent1.css(".agent-phone span ::text").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".agent-avatar img ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if response.xpath("//*[contains(@class,'agent-info')][3]"):
                agent2 = response.xpath("//*[contains(@class,'agent-info')][3]")
                agent2_name = agent2.css('.agent-heading a ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.css('.agent-heading span ::text').get('').strip()
                    item['Field25'] = agent2.css(".agent-email span ::text").get('').strip().replace('mailto:','')
                    item['Field26'] = agent2.css(".agent-mobile span ::text").get('').strip().replace('mailto:','')
                    item['Field26A'] = agent2.css(".agent-phone span ::text").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".agent-avatar img ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if response.xpath("//*[contains(@class,'agent-info')][4]"):
                agent3 = response.xpath("//*[contains(@class,'agent-info')][4]")
                agent3_name = agent3.css('.agent-heading a ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field29'] = agent3.css('.agent-heading span ::text').get('').strip()
                    item['Field30'] = agent3.css(".agent-email span ::text").get('').strip().replace('mailto:','')
                    item['Field31'] = agent3.css(".agent-mobile span ::text").get('').strip().replace('mailto:','')
                    item['Field31A'] = agent3.css(".agent-phone span ::text").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".agent-avatar img ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            floor_area = response.xpath("//*[contains(text(),'Size')]/following-sibling::span[1]/text()").get('').strip()
            if floor_area:
