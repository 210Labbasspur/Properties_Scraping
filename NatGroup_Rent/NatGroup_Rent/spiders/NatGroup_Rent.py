...
class NatGroup_Rent(scrapy.Spider):
    name = "NatGroup_Rent"

    url = "https://www.natgroup.net/rental-properties-in-queensland"
    prefix = "https://www.natgroup.net"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(NatGroup_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".detail-box ::attr(href)").getall()
        links = []
        ...
        next_page = response.xpath("//*[contains(@rel,'next')]/@href").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2185)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Nat Group Real Estate'

            property_street = response.css('.heading-properties-3 .location ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.heading-properties-3 h2 ::text').getall() if e.strip())  # property_suburb
            property_address = f"{property_street} {property_suburb}"
            item['Field5'] = property_address  # full_adddress

            item['Field6'] = response.xpath("//*[contains(@src,'bed.jpg')]/following-sibling::span[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@src,'bath.jpg')]/following-sibling::span[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@src,'parking.jpg')]/following-sibling::span[1]/text()").get('').strip()
            ...
            '''          AGENTS          '''
            if response.css(".agent-box"):
                agent1 = response.css(".agent-box")[0]
                agent1_name = agent1.css('h5 a ::text').get('').strip()
                if agent1_name:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field19'] = agent1.xpath(".//b[contains(text(),'E')]/parent::*[1]/text()").get('').strip().replace('mailto:','')
                    item['Field20'] = agent1.xpath(".//b[contains(text(),'M')]/parent::*[1]/text()").get('').strip().replace('mailto:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css(".agent-box").getall()) > 1:
                agent2 = response.css(".agent-box")[1]
                agent2_name = agent2.css('h5 a ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field25'] = agent2.xpath(".//b[contains(text(),'E')]/parent::*[1]/text()").get('').strip().replace('mailto:','')
                    item['Field26'] = agent2.xpath(".//b[contains(text(),'M')]/parent::*[1]/text()").get('').strip().replace('mailto:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css(".agent-box").getall()) > 2:
                agent3 = response.css(".agent-box")[2]
                agent3_name = agent3.css('h5 a ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field29'] = agent3.css('.agentTile ::text').get('').strip()
                    item['Field30'] = agent3.xpath(".//b[contains(text(),'E')]/parent::*[1]/text()").get('').strip().replace('mailto:','')
                    item['Field31'] = agent3.xpath(".//b[contains(text(),'M')]/parent::*[1]/text()").get('').strip().replace('mailto:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            item['Field33'] = response.xpath("//*[contains(text(),'Property ID')]/following-sibling::*[1]/text()").get('').strip()
            item['Field35'] = response.xpath("//*[contains(text(),'Property Category')]/following-sibling::*[1]/text()").get('').strip()
            item['Field36'] = response.xpath("//*[contains(@src,'area.jpg')]/following-sibling::span[1]/text()").get('').strip()
