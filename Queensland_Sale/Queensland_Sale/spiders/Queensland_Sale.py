...
class Queensland_Sale(scrapy.Spider):
    name = "Queensland_Sale"

    url = "https://queenslandsothebysrealty.com/properties-for-sale"
    prefix = "https://queenslandsothebysrealty.com"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Queensland_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".searchResults a.block ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2244)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Queensland Sothebys Realty'

            property_street = response.css('.streetaddress ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('#addressContainer .block ::text').getall() if e.strip())  # property_suburb
            property_address = f"{property_street} {property_suburb}"
            item['Field5'] = property_address  # full_adddress

            bbc = response.xpath("//*[contains(@name,'description')]/@content").get('').strip()
            # Extract bed, bath, and car_park using regex
            bed = re.search(r'(\d+)\s*bedrooms?', bbc, re.IGNORECASE)
            bath = re.search(r'(\d+)\s*bathrooms?', bbc, re.IGNORECASE)
            car_park = re.search(r'(\d+)\s*car\s*parks?', bbc, re.IGNORECASE)
            # Return the extracted values (or None if not found)
            item['Field6'] = bed.group(1) if bed else ''
            item['Field7'] = bath.group(1) if bath else ''
            item['Field8'] = car_park.group(1) if car_park else ''

            price = response.css('.details .muted ::text').get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','').replace('Price','').replace('guide','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.container .title ::text').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".container .contentRegion ::text").getall())  # description
            ...
            for img in prop_images:
                img = img.replace('background-image:url(','').replace(");","").replace("576x324","1024x576")
            ...
            if response.css(".agentTile"):
                ...
                if agent1_name:
                    ...
                    item['Field18'] = agent1.css('small ::text').get('').strip()
                    ...
                    images_string = f'https://queenslandsothebysrealty.com{agent_image}' if agent_image else ""
            ...
            if len(response.css(".agentTile").getall()) > 1:
                ...
                if agent2:
                    ...
                    item['Field24'] = agent2.css('small ::text').get('').strip()
            ...
            if len(response.css(".agentTile").getall()) > 2:
                ...
                if agent3:
                    ...
                    item['Field29'] = agent3.css('small ::text').get('').strip()
            ...
            land_area = response.xpath('//*[contains(text(),"Land area approx")]/following-sibling::*[1]/text()').get('').strip().replace('Land','').replace('is','').replace('m²','')
            if land_area:
