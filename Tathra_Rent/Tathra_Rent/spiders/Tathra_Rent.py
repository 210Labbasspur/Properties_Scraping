...
class Tathra_Rent(scrapy.Spider):
    name = 'Tathra_Rent'
    prefix = 'https://www.tathrare.com.au'
    url = "https://www.tathrare.com.au/properties-for-rent"
    headers = {
    ...
    def parse(self, response):
        ...
        all_details_links = []
        for url in response.css("#properties_display_subpage a"):
            url = self.prefix + url.css('::attr(href)').get('').strip().replace('..','')
        ...
        next_page = response.xpath("//*[contains(@class,'active')]/following-sibling::a[1]/@href").get('').strip()
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'Tathra Real Estate'
            ...
            item['Field6'] = response.xpath("//*[contains(text(),'Bedrooms')]/following-sibling::span[1]/text()").get('').strip().replace('Property ID:','')
            item['Field7'] = response.xpath("//*[contains(text(),'Bathrooms')]/following-sibling::span[1]/text()").get('').strip().replace('Property ID:','')
            item['Field8'] = 0
            g = response.xpath("//*[contains(text(),'Garage')]/following-sibling::span[1]/text()").get('').strip()
            c = response.xpath("//*[contains(text(),'Carport')]/following-sibling::span[1]/text()").get('').strip()
            cs = response.xpath("//*[contains(text(),'Car Spaces')]/following-sibling::span[1]/text()").get('').strip()
            if g:
                item['Field8'] = item['Field8'] + int(g)
            if c:
                item['Field8'] = item['Field8'] + int(c)
            if cs:
                item['Field8'] = item['Field8'] + int(cs)

            sold_price_s = response.css('.pull-left.margin0 ::text').get('').strip()
            if re.search(r'\d', sold_price_s):
                sold_price = re.search(r'\$\d{1,3}(?:,\d{3})*', sold_price_s).group()
                item['Field9'] = (sold_price.strip()).replace('SOLD','').replace('$','')   # sold_price

            description = response.css('.desc ::text').getall()  # description
            ...
            '''          AGENTS          '''
            if response.css(".widget-title+ div"):
                agent1 = response.css(".widget-title+ div")[0]
                agent1_name = agent1.xpath(".//*[contains(@style,'color:#213567;')]/text()").get('').strip()
                if agent1_name:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images = agent1.css("img.avatar  ::attr(src)").get('').strip()
                ...
                if len(response.css(".widget-title+ div").getall()) > 1:
                    ...
                    agent2_name = agent2.xpath(".//*[contains(@style,'color:#213567;')]/text()").get('').strip()
                    if agent2:
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent2.css("img.avatar  ::attr(src)").get('').strip()
                ...
                if len(response.css(".widget-title+ div").getall()) > 2:
                    agent3 = response.css(".widget-title+ div")[2]
                    agent3_name = agent3.xpath(".//*[contains(@style,'color:#213567;')]/text()").get('').strip()
                    if agent3:
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent3.css("img.avatar  ::attr(src)").get('').strip()
            ...
            if land_area:
                item['Field36'] = land_area.replace('s²','').replace('Metre','').replace('s','').replace(' ','')   # land_area

            feature_count = 58
            for feature in response.xpath("//*[contains(text(),'yes')]/preceding-sibling::span[1]"):
                if 'Air Condition' in feature.css('::text').get('').strip():
                    item['Field52'] = feature.css('::text').get('').strip().replace(':','')
                else:
                    item[f'Field{feature_count}'] = feature.css('::text').get('').strip().replace(':','')
                    feature_count += 1
