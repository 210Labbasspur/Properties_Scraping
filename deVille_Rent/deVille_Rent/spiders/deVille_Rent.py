...
class deVille_Rent(scrapy.Spider):
    name = 'deVille_Rent'
    prefix = 'https://www.deville.net.au'
    url = "https://www.deville.net.au/renting/residential-rentals/"
    headers = {
        'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        'cache-control': 'max-age=0',
        # 'cookie': '_ga_H7WQJDRZFV=GS1.1.1712711268.1.0.1712711268.0.0.0; _ga=GA1.1.1340557515.1712711268; arp_scroll_position=400',
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'deVille Estate Agency'

            property_add = response.css('.suburb-address ::text').getall()  # full_adddress
            ...
            item['Field6'] = response.xpath("//label[contains(text(),'Bedroom')]/following-sibling::div[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//label[contains(text(),'Bathroom')]/following-sibling::div[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//label[contains(text(),'Garage')]/following-sibling::div[1]/text()").get('').strip()

            # sold_price = response.css('.suburb-price ::text').get('').strip().replace(' ','')
            # if sold_price:
            #     item['Field9'] = (sold_price.strip()).replace('$','')   # sold_price

            description = response.css('.sub-title ::text, #property-description span ::text').getall()  # description
            ...
            if agent1:
                ...
                agent1_phone = agent1.css(".phone a ::text").get('').strip()
                item['Field20'] = agent1_phone.strip()
                agent1_mobile = agent1.css(".mobile a ::text").get('').strip()
            ...
            if len(response.css(".mb-20").getall()) == 3:
                ...
                if agent3:
                    ...
                    item['Field21A'] = agent3_mobile.strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            land_area = response.xpath("//label[contains(text(),'Land Size')]/following-sibling::div[1]/text()").get('').strip()
            if land_area:
