...
class MackayCity_Sold(scrapy.Spider):
    name = "MackayCity_Sold"

    url = "https://mackaycityproperty.com.au/?action=epl_search&post_type=property&property_status=sold"
    prefix = "https://mackaycityproperty.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(MackayCity_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".epl-property-blog-entry-wrapper")
        links = []
        for each_url in all_links:
            each_url = each_url.css(".epl-archive-entry-image a ::attr(href)").get('').strip()
        ...
        for each_url in all_links:
            price = each_url.xpath(".//*[contains(@class,'page-price')]/text()").get('').strip()
            each_url = each_url.css(".epl-archive-entry-image a ::attr(href)").get('').strip()
            print(self.count,' # Property URL :', each_url)
            self.count += 1
            yield response.follow(each_url, headers=self.headers, callback=self.detail_parse, meta={'price':price})
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3337)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Mackay City Property'
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'icon bed')]/*[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'icon bath')]/*[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'icon parking')]/*[1]/text()").get('').strip()

            price = response.meta['price'].strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            ...
            prop_images.append(response.xpath("//*[contains(@property,'og:image')]/@content").get('').strip())
            images = []
