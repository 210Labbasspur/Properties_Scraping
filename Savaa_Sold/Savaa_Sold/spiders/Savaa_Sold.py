...
class Savaa_Sold(scrapy.Spider):
    name = 'Savaa_Sold'
    prefix = 'https://www.savaaproperties.com.au'
    url = "https://www.savaaproperties.com.au/sold?page=1"
    headers = {
        'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        # 'cookie': 'cf_clearance=AWlJ3GzkMTW1MJuAYSA06GDNO7aVHVjO1hrnAfQ5DZ4-1714605326-1.0.1.1-RKj6Bi6e4s1WjVkLanPUxr7IAsPZsqk0OrVRKOErE..tyxYui4PgYYTWe8oI_PRJCMM57cgvnWP2x_x8RQOxFA; arp_scroll_position=3586',
        'priority': 'u=0, i',
        'referer': 'https://www.savaaproperties.com.au/sold?page=1',
    ...
    def parse(self, response):
        for property in response.css(".clickable-div"):
            string =  property.css('::attr(onclick)').get('').strip()
            property_id = re.search(r"id=(\d+)", string).group(1) if re.search(r"id=(\d+)", string) else None
            property_url =  'https://www.savaaproperties.com.au/solddetails?id=' + property_id
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            description = response.css('.mb-3 ::text').getall()  # description
            ...
            '''          AGENTS          '''
            if response.css(".mt-0 .author-widget2"):
                agent1 = response.css(".mt-0 .author-widget2")[0]
                agent1_name = agent1.css('.author__title a ::text').get('').strip()
                if agent1:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field18'] = agent1.css('.author__meta ::text').get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images = agent1.css(".author__img ::attr(src)").get('').strip()
                ...
                if len(response.css(".mt-0 .author-widget2").getall()) > 1:
                    agent2 = response.css(".mt-0 .author-widget2")[1]
                    agent2_name = agent2.css('.author__title a ::text').get('').strip()
                    if agent2:
                        item['Field23'] = agent2_name.strip()
                        item['Field24'] = agent2.css('.author__meta ::text').get('').strip()
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent2.css(".author__img ::attr(src)").get('').strip()
                ...
                if len(response.css(".mt-0 .author-widget2").getall()) > 2:
                    agent3 = response.css(".mt-0 .author-widget2")[2]
                    agent3_name = agent3.css('.author__title a ::text').get('').strip()
                    if agent3:
                        item['Field28'] = agent3_name.strip()
                        item['Field29'] = agent3.css('.author__meta ::text').get('').strip()
                        ...
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent3.css(".author__img ::attr(src)").get('').strip()
            ...
            land_area = response.xpath("//*[contains(text(),'Area')]/following-sibling::span[1]/text()").get('').strip()
            if land_area:
