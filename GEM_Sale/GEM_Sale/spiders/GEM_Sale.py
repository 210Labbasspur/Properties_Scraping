...
class GEM_Sale(scrapy.Spider):
    ...
    def __init__(self, *args, **kwargs):
        super(GEM_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def start_requests(self):
        yield scrapy.Request('https://gempropertyco.com.au/properties-for-sale/', headers=self.headers, callback=self.parse)

    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        if response.xpath("//*[contains(@class,'elementor-heading-title elementor-size-default')]//a/@href"):
            all_links = response.xpath("//*[contains(@class,'elementor-heading-title elementor-size-default')]//a/@href").getall()
            links = []
