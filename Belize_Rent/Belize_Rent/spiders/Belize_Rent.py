...
class Belize_Rent(scrapy.Spider):
    name = "Belize_Rent"

    url = "https://www.belizeproperty.com.au/renting/residential-properties"
    prefix = "https://www.belizeproperty.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Belize_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".banner--image a ::attr(href)").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://www.belizeproperty.com.au{each_url}'
        ...
        for property_url in all_links:
            property_url = f'https://www.belizeproperty.com.au{property_url}'
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3260)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Belize Property'

            property_street = response.css('.mb-1 ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.pl-md-2 .font--secondary ::text').getall() if e.strip())  # property_suburb
            property_address = f"{property_street} {property_suburb}"
            item['Field5'] = property_address  # full_adddress

            item['Field6'] = response.xpath("//*[contains(@class,'fa-bed')]/following-sibling::span[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'fa-bath')]/following-sibling::span[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'fa-car')]/following-sibling::span[1]/text()").get('').strip()

            price = response.css('.pl-md-2 .mb-0 ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if price:
            ...
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".listing__details--content p::text").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.xpath("//*[contains(@data-fancybox,'fancyboxImageGallery')]/@href").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.xpath("//*[contains(@class,'details__contact-box')]/div"):
                agent1 = response.xpath("//*[contains(@class,'details__contact-box')]/div")[0]
                agent1_name = agent1.css('.mb-0 ::text').get('').strip()
                if agent1_name:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field18'] = agent1.css('h5.mb-2 ::text').get('').strip()
                    item['Field19'] = agent1.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:','')
                    item['Field20'] = agent1.xpath(".//*[contains(@class,'phone-number')]/@data-phone").get('').strip()
                    phone_nos = agent1.xpath(".//*[contains(@class,'phone-number')]/@data-phone").getall()
                    if len(phone_nos) > 1:
                        item['Field21'] = phone_nos[-1].strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".profile-pic img ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.xpath("//*[contains(@class,'details__contact-box')]/div").getall()) > 1:
                agent2 = response.xpath("//*[contains(@class,'details__contact-box')]/div")[1]
                agent2_name = agent2.css('.mb-0 ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.css('h5.mb-2 ::text').get('').strip()
                    item['Field25'] = agent2.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:', '')
                    item['Field26'] = agent2.xpath(".//*[contains(@class,'phone-number')]/@data-phone").get('').strip()
                    phone_nos = agent2.xpath(".//*[contains(@class,'phone-number')]/@data-phone").getall()
                    if len(phone_nos) > 1:
                        item['Field26A'] = phone_nos[-1].strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".profile-pic img ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.xpath("//*[contains(@class,'details__contact-box')]/div").getall()) > 2:
                agent3 = response.xpath("//*[contains(@class,'details__contact-box')]/div")[2]
                agent3_name = agent3.css('.mb-0 ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field29'] = agent3.css('h5.mb-2 ::text').get('').strip()
                    item['Field30'] = agent3.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:', '')
                    item['Field31'] = agent3.xpath(".//*[contains(@class,'phone-number')]/@data-phone").get('').strip()
                    phone_nos = agent3.xpath(".//*[contains(@class,'phone-number')]/@data-phone").getall()
                    if len(phone_nos) > 1:
                        item['Field31A'] = phone_nos[-1].strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".profile-pic img ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            land_area = ''.join(e.strip() for e in response.xpath('//*[contains(text(),"Land Size")]/parent::p/text()').getall())
            if land_area:
                if 'Acres' in land_area:
                    pass
                else:
                    item['Field36'] = land_area.replace('Square Mtr','').replace(' ','').replace('Land','').replace('is','').replace('m²','')   # land_area
            ...
            #
            feature_count = 58
            for feature in response.css(".mb-3 strong")[1:]:   ##  first 4 features are ID, bed, bath etc.
