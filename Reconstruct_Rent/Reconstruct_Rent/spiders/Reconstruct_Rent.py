...
class Reconstruct_Sale(scrapy.Spider):
    name = 'Reconstruct_Rent'
    prefix = 'https://www.reconstructrealestate.com.au'
    url = "https://www.reconstructrealestate.com.au/renting/properties-for-lease/"
    headers = {
        # 'Referer': 'https://www.reconstructrealestate.com.au/selling/recent-sales/',
    ...
    def parse(self, response):
        for property in response.css(".listing-headline"):
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            description = response.css('.col-md-7 .sub-title ::text , .detail-description span ::text').getall()  # description
            ...
            agent1_name = agent1.css('.color-black .mb-0 ::text').get('').strip()  # description
            if agent1:
                ...
                agent1_phone = agent1.css(".phone .color-black ::text").get('').strip()
                item['Field20'] = agent1_phone.strip()
                item['Field21'] = agent1.css(".mobile .color-black ::text").get('').strip()
                '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css(".mb-20").getall()) > 1:
                ...
                agent2_name =  agent2.css('.color-black .mb-0 ::text').get('').strip()  # description
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field25'] = agent2.css(".email .color-black ::text").get('').strip()
                    agent2_phone = agent2.css(".phone .color-black ::text").get('').strip()
                    item['Field26'] = agent2_phone.strip()
                    item['Field26A'] = agent2.css(".mobile .color-black  ::text").get('').strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
