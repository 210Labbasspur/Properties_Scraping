...
class Domain_Sale(scrapy.Spider):
    name = 'Domain_Sale'
    prefix = 'https://www.domainillawarra.com.au'
    url = "https://www.domainillawarra.com.au/for-sale/"
    headers = {
        'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'Accept-Language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        'Cache-Control': 'max-age=0',
        'Connection': 'keep-alive',
        # 'Cookie': '_gid=GA1.3.1400165743.1714077180; _ga_GKBSLLQXX5=GS1.1.1714219938.6.1.1714219989.0.0.0; _ga=GA1.3.515510434.1713705689; arp_scroll_position=2267',
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'Domain Illawarra Real Estate'
            ...
            # sold_price = response.css('.listing-price-value ::text').get('').strip().replace('Price Guide','').replace(' ','')
            # if sold_price:
            ...
            Agent_Imagess = response.xpath("//*[contains(@itemprop,'contentUrl')]/@href").getall()
            new_img_urls = []
            ...
            if response.css("div.wpsight-listing-agent"):
                ...
                if agent1:
                    ...
                    agent1_phone = agent1.xpath(".//*[contains(text(),'M.')]/a/text()").get('').strip()
                    item['Field20'] = agent1_phone.strip()
                    agent1_mobile = agent1.xpath(".//*[contains(text(),'P.')]/a/text()").get('').strip()
                    item['Field21'] = agent1_mobile.strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images = agent1.css(".image img ::attr(src)").get('').strip()
                ...
                if len(response.css("div.wpsight-listing-agent").getall()) > 1:
                    ...
                    if agent2:
                        ...
                        agent2_phone = agent2.xpath(".//*[contains(text(),'M.')]/a/text()").get('').strip()
                        item['Field26'] = agent2_phone.strip()
                        agent2_mobile = agent2.xpath(".//*[contains(text(),'P.')]/a/text()").get('').strip()
                        item['Field26A'] = agent2_mobile.strip()
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent2.css(".image img ::attr(src)").get('').strip()
                ...
                if len(response.css("div.wpsight-listing-agent").getall()) > 2:
                    ...
                    if agent3:
                        ...
                        agent3_phone = agent3.xpath(".//*[contains(text(),'M.')]/a/text()").get('').strip()
                        item['Field31'] = agent3_phone.strip()
                        agent3_mobile = agent3.xpath(".//*[contains(text(),'P.')]/a/text()").get('').strip()
                        item['Field31A'] = agent3_mobile.strip()
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent3.css(".image img ::attr(src)").get('').strip()
