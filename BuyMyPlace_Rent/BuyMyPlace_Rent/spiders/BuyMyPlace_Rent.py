...
class BuyMyPlace_Rent(scrapy.Spider):
    name = "BuyMyPlace_Rent"

    url = "https://buymyplace.com.au/lease-rent-your-property/listings/Australia/anyPropertyTypes/anyMinPrice/anyMaxPrice/anyMinBeds/anyMinBaths/surround/12/0"
    prefix = "https://buymyplace.com.au"
    headers = {
    ...
    count = 1
    db = 'BuyMyPlace'
    collection = 'BuyMyPlace_Rent'
    bucket_prefix = f'Images1'

    local_file_path = '/img'

    def __init__(self, *args, **kwargs):
        super(BuyMyPlace_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".property-item ::attr(href)").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://buymyplace.com.au/{each_url}'
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3021)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Buy My Place'

            property_street = response.css('.section-heading ::text').get('').strip()  # property_street
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'bed')]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'bath')]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'car')]/text()").get('').strip()

            price = response.css('.info-summary h4 ::text').get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.xpath('//*[contains(@class,"row property-id")]/preceding-sibling::div[1]/h4/text()').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.xpath('//*[contains(@class,"row property-id")]/following-sibling::div[1]/text()').getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.css(".carousel-item img ::attr(data-src)").getall()
            images = []
            ...
            '''          AGENTS          -          NO AGENTS ON THE WEBSITE         '''
            item['Field15'] = 'buymyplace'  # agent_first_name_1
            item['Field17'] = 'buymyplace'  # agent_full_name_1
            item['Field20'] = '+61 488 842 044'  # agent_full_name_1
            # if response.css(".card-body"):
            #     agent1 = response.css(".card-body")[0]
            ...
            #         '''         Uploading Images on Wasabi S3 Bucket            '''
            #         agent_image = agent1.css(".agent-img ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            #
            # if len(response.css(".card-body").getall()) > 1:
            #     agent2 = response.css(".card-body")[1]
            ...
            #         '''         Uploading Images on Wasabi S3 Bucket            '''
            #         agent_image = agent2.css(".agent-img ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
            #         images_string = f'{agent_image}' if agent_image else ""
            ...
            #         '''         Uploading Images on Wasabi S3 Bucket            '''
            #         agent_image = agent3.css(".agent-img ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
            #         images_string = f'{agent_image}' if agent_image else ""
            ...
            item['Field33'] = response.xpath("//*[contains(@class,'property-id')]/text()").get('').strip().replace('Property ID:','').replace(':','').replace(' ','')
            # bbc = response.css(".propertyPage-property-icons ::text").getall()
            item['Field35'] = response.xpath("//*[contains(@class,'propertyType')]/text()").get('').strip()
            land_area = response.xpath("//*[contains(@class,'area')]/text()").get('').strip()
            if land_area:
            ...
            #
            feature_count = 58
            for feature in response.css(".feature .row"):   ##  first 4 features are ID, bed, bath etc.
