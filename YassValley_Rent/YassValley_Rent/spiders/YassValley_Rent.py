...
class YassValley_Rent(scrapy.Spider):
    name = "YassValley_Rent"

    url = 'https://yassvalleyproperty.com.au/wp-content/themes/wpresidence/ajax_handler.php'
    prefix = "https://yassvalleyproperty.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(YassValley_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def start_requests(self):
        ...
        yield scrapy.Request('https://yassvalleyproperty.com.au/rent/', headers=self.headers, callback=self.parse)

    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        if response.css(".listing-unit-img-wrapper a ::attr(href)"):
            all_links = response.css(".listing-unit-img-wrapper a ::attr(href)").getall()
            links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(1926)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Yass Valley Property'

            property_street = response.css('.notice_area .entry-title ::text').get('').strip()  # property_street
            ...
            item['Field6'] = response.xpath("//li[contains(text(),' Bedroom')]/text()").get('').strip().replace('Bedroom','').replace('s','')
            item['Field7'] = response.xpath("//li[contains(text(),' Bathroom')]/text()").get('').strip().replace('Bathroom','').replace('s','')
            item['Field8'] = response.xpath("//li[contains(text(),' Garages')]/text()").get('').strip().replace('Garage','').replace('s','')

            price = ''.join(response.css('.price_area .price_label ::text').getall())
            price = price.strip().replace('Sold','').replace('$','').replace(' ','').replace(',','')
            if price:
            ...
            '''          AGENTS          '''
            if response.css(".full_width_prop .wpestate_agent_details_wrapper"):
                agent1 = response.css(".full_width_prop .wpestate_agent_details_wrapper")[0]
                ...
                if agent1_name:
                    ...
                    item['Field18'] = agent1.css('.agent_position ::text').get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".agentpict ::attr(data-bg-image)").get('').strip().replace('url(','').replace(')','')
            ...
            if len(response.css(".full_width_prop .wpestate_agent_details_wrapper").getall()) > 1:
                agent2 = response.css(".full_width_prop .wpestate_agent_details_wrapper")[1]
                agent2_name = agent2.css('h3 a ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.css('.agent_position ::text').get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".agentpict ::attr(data-bg-image)").get('').strip().replace('url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css(".full_width_prop .wpestate_agent_details_wrapper").getall()) > 2:
                agent3 = response.css(".full_width_prop .wpestate_agent_details_wrapper")[2]
                agent3_name = agent3.css('h3 a ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field29'] = agent3.css('.agent_position ::text').get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".agentpict ::attr(data-bg-image)").get('').strip().replace('url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            item['Field33'] = response.xpath('//*[contains(text(),"Property Id :")]/parent::*[1]/text()').get('').strip()
            # item['Field35'] = response.xpath('//*[contains(text(),"Property Id :")]/parent::*[1]/text()').get('').strip()

            land_area = response.xpath('//*[contains(text(),"Property Size")]/parent::*[1]/text()').get('').strip().replace('Land','').replace('is','').replace('m','')
            if land_area:
                if 'Acres' in land_area:
                    pass
                else:
                    item['Field36'] = land_area.replace('m²','').replace(' ','')   # land_area
            floor_area = response.xpath('//*[contains(text(),"Property Lot Size")]/parent::*[1]/text()').get('').strip().replace('Land','').replace('is','').replace('m','')
            if floor_area:
            ...
            feature_count = 58
            for feature in response.xpath("//*[contains(@class,'far fa-check-circle')]/parent::*[1]"):   ##  first 4 features are ID, bed, bath etc.
