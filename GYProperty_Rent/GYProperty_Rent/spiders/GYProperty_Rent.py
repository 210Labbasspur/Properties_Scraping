...
class GYProperty_Rent(scrapy.Spider):
    name = 'GYProperty_Rent'
    prefix = 'https://www.gyproperty.com.au'
    url = 'https://www.gyproperty.com.au/wp-content/themes/apb-gilbertyounan/zoorealty/display/pages/ajax/fetch_pages.php'
    headers = {
    ...
    def parse(self, response):
        ...
        all_details_links = []
        for url in response.css(".address a"):
        ...
        '''  #######   Original Code   #######  '''
        for property in response.css(".address a"):
        ...
        next_page = response.css('.address a ::attr(href)').get('').strip()
        if next_page:
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'Gilbert Younan Property'

            property_add = response.css('.listing-address ::text').getall()  # full_adddress
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'ti-bed')]/parent::div[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'ti-bath')]/parent::div[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'ti-car')]/parent::div[1]/text()").get('').strip()

            sold_price = response.css(".price .value ::text").get('').strip().replace(',','').replace(' ','')
            if re.search(r'\d', sold_price):
                item['Field9'] = (sold_price.strip()).replace('SOLD','').replace('$','')   # sold_price

            description = response.css('#full_property_description ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ').replace('[Less]', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess = response.css(".slider .slide_photo_ref ::attr(href)").getall()
            new_img_urls = []
            ...
            if response.css(".listing-agent"):
                ...
                agent1_name = agent1.css('.font-bold ::text').get('').strip()
                if agent1_name:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    # item['Field19'] =  agent1.css(".email a ::text").get('').strip()
                ...
                if len(response.css(".listing-agent").getall()) > 1:
                    ...
                    agent2_name = agent2.css('.font-bold ::text').get('').strip()
                    if agent2:
                        item['Field23'] = agent2_name.strip()
                        # item['Field25'] = agent2.css(".email a ::text").get('').strip()
                ...
                if len(response.css(".listing-agent").getall()) > 2:
                    ...
                    agent3_name = agent3.css('.font-bold ::text').get('').strip()
                    if agent3:
                        item['Field28'] = agent3_name.strip()
                        # item['Field30'] = agent3.css(".email a ::text").get('').strip()
            ...
            item['Field35'] = response.xpath("//*[contains(text(),'Property Type')]/following-sibling::div[1]/text()").get('').strip()
            land_area = response.xpath("//*[contains(text(),'Land Size')]/following-sibling::div[1]/span[1]/text()").get('').strip()
            if land_area:
