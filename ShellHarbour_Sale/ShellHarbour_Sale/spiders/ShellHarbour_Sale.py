...
class ShellHarbour_Sale(scrapy.Spider):
    name = 'ShellHarbour_Sale'
    prefix = 'https://www.shellharbourmarinarealestate.com.au'
    url = "https://www.shellharbourmarinarealestate.com.au/sales-and-property-management/for-sale/"
    headers = {
        'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'Accept-Language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
        'Cache-Control': 'max-age=0',
        'Connection': 'keep-alive',
        # 'Cookie': 'AMP_MKTG_16a5c84b5b=JTdCJTdE; AMP_16a5c84b5b=JTdCJTIyZGV2aWNlSWQlMjIlM0ElMjJkNjg0MjU2Yy1jODY5LTQ0MzUtOWE4Yi0zMTYyMDIwYmQ3NmElMjIlMkMlMjJzZXNzaW9uSWQlMjIlM0ExNzE0MDc3MjExNTgxJTJDJTIyb3B0T3V0JTIyJTNBZmFsc2UlMkMlMjJsYXN0RXZlbnRUaW1lJTIyJTNBMTcxNDA3NzIxMTU5NSU3RA==; arp_scroll_position=1858',
    ...
    def Detail_parse(self, response):
        ...
        if not DB_already_exists:
            ...
            item['Field4'] = 'Shell Harbour Marina Real Estate'
            ...
            item['Field6'] = response.xpath("//*[contains(text(),'Bed')]/following-sibling::span[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(text(),'Bath')]/following-sibling::span[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(text(),'Parking')]/following-sibling::span[1]/text()").get('').strip()

            # sold_price = response.css('.listing-price-value ::text').get('').strip().replace(' ','')
            # if sold_price:
            #     item['Field9'] = (sold_price.strip()).replace('$','')   # sold_price

            description = response.css('.wpsight-listing-section-description ::text').getall()  # description
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ') for element in description)  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            Agent_Imagess = response.xpath("//img[contains(@itemprop,'thumbnail')]/@data-src").getall()
            new_img_urls = []
            ...
            if response.xpath("//*[contains(@class,'wpsight-listing-agent clearfix')]"):
                ...
                if agent1:
                    ...
                    agent1_mobile = agent1.xpath(".//*[contains(text(),'M.')]/a/text()").get('').strip()
                    item['Field20'] = agent1_mobile.strip()
                    agent1_phone = agent1.xpath(".//*[contains(text(),'P.')]/a/text()").get('').strip()
                    item['Field21'] = agent1_phone.strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    Agent_Images = agent1.css(".agent-logo ::attr(data-src)").get('').strip()
                ...
                if len(response.xpath("//*[contains(@class,'wpsight-listing-agent clearfix')]").getall()) > 1:
                    ...
                    if agent2:
                        ...
                        agent2_mobile = agent2.xpath(".//*[contains(text(),'M.')]/a/text()").get('').strip()
                        item['Field26'] = agent2_mobile.strip()
                        agent2_phone = agent2.xpath(".//*[contains(text(),'P.')]/a/text()").get('').strip()
                        item['Field26A'] = agent2_phone.strip()
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent2.css(".agent-logo ::attr(data-src)").get('').strip()
                ...
                if len(response.xpath("//*[contains(@class,'wpsight-listing-agent clearfix')]").getall()) == 3:
                    ...
                    if agent3:
                        ...
                        agent3_mobile = agent3.xpath(".//*[contains(text(),'M.')]/a/text()").get('').strip()
                        item['Field31'] = agent3_mobile.strip()
                        agent3_phone = agent3.xpath(".//*[contains(text(),'P.')]/a/text()").get('').strip()
                        item['Field31A'] = agent3_phone.strip()
                        '''         Uploading Images on Wasabi S3 Bucket            '''
                        Agent_Images = agent3.css(".agent-logo ::attr(data-src)").get('').strip()
