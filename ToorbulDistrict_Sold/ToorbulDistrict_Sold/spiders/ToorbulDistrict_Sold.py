...
class ToorbulDistrict_Sold(scrapy.Spider):
    name = "ToorbulDistrict_Sold"

    url = "https://toorbuldistrictrealty.com.au/sold-properties/"
    prefix = "https://toorbuldistrictrealty.com.au"
    headers = {
    ...
    count = 1
    db = 'ToorbulDistrict'
    collection = 'ToorbulDistrict_Sold'
    ...
    def __init__(self, *args, **kwargs):
        super(ToorbulDistrict_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".wp-block-post-featured-image a ::attr(href)").getall()
        links = []
        ...
        next_page = response.css(".wp-block-query-pagination-next ::attr(href)").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3365)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Toorbul District Real Estate'

            property_street = response.css('.wp-block-group .wp-block-post-title ::text').get('').strip()  # property_street
            ...
            # item['Field6'] = response.xpath("//*[contains(@class,'icon-bed')]/parent::span[1]/text()").get('').strip()
            # item['Field7'] = response.xpath("//*[contains(@class,'icon-bath')]/parent::span[1]/text()").get('').strip()
            # item['Field8'] = response.xpath("//*[contains(@class,'icon-car')]/parent::span[1]/text()").get('').strip()

            price = response.css('.wp-block-post-excerpt__excerpt ::text').get('').strip()
            if re.search(r'\d', price):
                item['Field9'] = price.strip()

            description_head = ''
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.xpath('//*[contains(@class,"wp-block-group has-global-padding is-layout-constrained wp-block-group-is-layout-constrained")]//p/text()').getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}" # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.xpath("//*[contains(@class,'wp-block-uagb-image__figure')]/img/@src").getall()
            images = []
